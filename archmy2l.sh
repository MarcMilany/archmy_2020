...
#   # добавить в загрузчик grub другие os
# или раскомментить строку GRUB_DISABLE_OS_PROBER  в /etc/default/grub
  echo " Загрузчик GRUB установлен на выбранный вами диск (раздел) "
elif [[ $i_grub == 0 ]]; then
  echo ""
...
mkdir -p ~/.config/fontconfig   # Создать каталог fontconfig в ~/.config/
