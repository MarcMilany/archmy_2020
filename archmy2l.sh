...
echo -e "${YELLOW}==> Примечание! ${BOLD} *Чтобы избежать ошибки при создании RAM (mkinitcpio -p), вспомните какое именно ядро Вы выбрали ранее. И загрузочный RAM диск (начальный RAM-диск) будет создан именно с таким же ядром, иначе 'ВАЙ ВАЙ'! Эта информация необходима, потому что при создании начального RAM-диска (initrd), который используется для загрузки, вы должны указать номер (название) ядра, чтобы включить соответствующие модули для вашей аппаратной конфигурации. ${NC}"
...
echo -e "${CYAN}:: ${NC} *Здесь представлены варианты создания RAM-диска, с конкретными ядрами. "
...
if [[ $i_grub == 1 ]]; then
  ...
  pacman -Syy  # обновление баз пакмэна (pacman)
  pacman -S grub --noconfirm  # Файлы и утилиты для установки GRUB2 содержатся в пакете grub
  pacman -S --noconfirm --needed efibootmgr  # Приложение пользовательского пространства Linux для изменения диспетчера загрузки EFI ; https://github.com/rhboot/efibootmgr ; 2024-03-13 16:32 UTC
# pacman -S --noconfirm --needed grub  # GNU GR и унифицированный загрузчик (2)
  pacman -S libisoburn --noconfirm  # Интерфейс для библиотек libburn и libisofs; https://dev.lovelyhq.com/libburnia
  pacman -S dosfstools --noconfirm  # Утилиты файловой системы DOS; Для поддержки grub-mkrescue FAT FS и EFI; https://github.com/dosfstools/dosfstools
clear
...
if [[ $dual_boot  == 1 ]]; then
  ...
  pacman -S os-prober --noconfirm  # Утилита для обнаружения других ОС на наборе дисков ; https://archlinux.org/packages/extra/x86_64/os-prober/ ; https://joeyh.name/code/os-prober/ ; 24.02.2025 16:59 UTC
...
# Ядро автоподписи (Autosign Kernel)
mkdir -p /etc/pacman.d/hooks && cat << EOF > /etc/pacman.d/hooks/999-sign_kernel_for_secureboot.hook
[Trigger]
Operation = Install
Operation = Upgrade
Type = Package
...
clear
echo -e "${GREEN}
  <<< Итак, вы установили Arch — Поздравляем! ${BOLD}*Мы знаем, что он считается одним из самых сложных для установки дистрибутивов Linux, вероятно, процесс оказался проще, чем вы ожидали (хотя и затянувшимся из-за установки утилит из AUR, Если таковые были вами установлены).
${YELLOW}Установка завершена. Перезагрузите систему. ${GREEN}>>>
${NC}"
sleep 07
