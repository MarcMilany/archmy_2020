...
echo -e "${YELLOW} Примечание! ${BOLD} *Вы можете пропустить этот шаг, если сейчас ваш часовой пояс настроен правильно, или Вы не уверены в правильности выбора! "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
...
sleep 03
    ...
    grub-mkconfig -o /boot/grub/grub.cfg
Эта команда автоматически создаст файл конфигурации для вашего загрузчика.
Перезагрузите компьютер, чтобы проверить работу загрузчика.
Если у кого-то нет сжатого ядра Linux

Вы можете проверить, произошло ли это из-за первой установки, проверив пункт меню, используя
