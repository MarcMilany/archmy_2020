...
pacman -S --noconfirm --needed cronie  # Демон, запускающий указанные программы в запланированное время и связанные с ними инструменты ; https://archlinux.org/packages/extra/x86_64/cronie/ ; https://github.com/cronie-crond/cronie/ ; Обеспечивает: cron ; Конфликты: с cron ; 2024-04-09 12:08 UTC
echo ""
echo " Добавляем в автозагрузку планировщик заданий (cronie.service) "
systemctl enable cronie.service
#systemctl enable --now cronie.service  # Запускает и включает службу
#systemctl start cronie.service
# systemctl status cronie.service
#EDITOR=nano crontab -e                 # Редактируем параметр
echo ""
...
echo " Советы: Поскольку ccm требует прав sudo, рассмотрите возможность создания псевдонима для его вызова в файле ~/.bashrc или аналогичном файле. Например: alias ccm='sudo ccm' . сли в вашей локальной сети несколько компьютеров, попросите их помочь вам с компиляцией через distcc, который поддерживается в CCM. $XDG_CONFIG_HOME/clean-chroot-manager.conf - Инструкции по настройке см. здесь (https://github.com/graysky2/clean-chroot-manager). "
