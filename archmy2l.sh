...
EOF
###
echo -e "${BLUE}:: ${NC}Настройка вашего Logon при помощи /etc/issue (вставка информации о системе)"
echo " Файл /etc/issue является простым текстовым файлом, в котором можно записать определенные последовательности управляющих символов (смотрите ниже) с тем, чтобы можно было отображать системную информацию. Есть так же файл issue.net, который может быть использован при удаленной регистрации. "
echo -e "${YELLOW}=> Примечание: ${NC}Когда вы первый раз запускаете вашу новую LFS систему, экран приглашения будет приятным и запланированным. Хотя многие люди захотят иметь в их системе отображение большей информации в тексте приглашения. Это может быть достигнуто использованием файла /etc/issue."
echo " Для начала сделаем backup (копию) оригинального файла /etc/issue "
cp /etc/issue  /etc/issue.back  # Для начала сделаем его бэкап
echo " Вносим изменения в файл /etc/issue "
> /etc/issue
cat <<EOF >>/etc/issue
#
#Arch Linux \r (\l)
