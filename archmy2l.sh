...
#pacman -S --noconfirm --needed alsa-lib lib32-alsa-lib pipewire lib32-pipewire pipewire-docs pipewire-alsa pipewire-audio pipewire-pulse pipewire-roc pipewire-jack pipewire-session-manager gst-plugin-pipewire easyeffects noise-suppression-for-voice
######### alsa-lib ##########
...
############ Дополнения для Budgie ##########
#pacman -S --noconfirm --needed mutter  # (необязательно) ; Оконный менеджер и композитор для GNOME ; https://archlinux.org/packages/extra/x86_64/mutter/ ; https://gitlab.gnome.org/GNOME/mutter ; Разделенные пакеты:  mutter , mutter-devkit , mutter-docs , mutter-docs ; Обеспечивает: libmutter-16.so=0-64 ; Обратные конфликты: mutter46, mutter46 ; 2025-06-29 17:25 UTC
pacman -S --noconfirm --needed magpie-wm  # (необязательно) ; Оконный менеджер X11 и библиотека композитора Budgie, созданные на основе Mutter ; https://archlinux.org/packages/extra/x86_64/magpie-wm/ ; https://github.com/BuddiesOfBudgie/magpie ; 2024-09-27 00:10 UTC
### Magpie используется в качестве оконного менеджера Budgie Desktop. Его также можно запустить автономно, используя команду "magpie", но простой запуск Magpie предназначен только для отладки.
##########################
clear
  echo ""
