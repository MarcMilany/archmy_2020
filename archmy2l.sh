...
clear
echo -e "${MAGENTA}
  <<< Установить Num-Lock (numlockx)(для автоматического включения Num-Lock при старте иксов) >>> ${NC}"
echo ""
echo " Num-Lock enable on X (будет автоматически включать Num-Lock при старте иксов) "
echo " Цифровая клавиатура, также известная как Numeric Keypad (NumPad), содержит специальные цифровые клавиши, расположенные в правой части стандартной клавиатуры. Это удобное устройство, которое позволяет быстро и эффективно вводить числа и выполнить различные математические операции. "
echo " Однако для использования цифровой клавиатуры иногда требуется ее активация. Для этого используется клавиша, называемая «Num Lock» или «NumLk». С помощью этой клавиши можно включить или выключить функцию цифровой клавиатуры, в зависимости от ваших потребностей. "
...
echo " Включите клавишу Num Lock при загрузке - Большинство клавиатур имеют клавишу Num Lock, которая управляет переключением малой клавиатуры. Пользователи могут захотеть включить Num Lock при запуске системы. "
...
echo " Вам нужен пакет из AUR , называемый systemd-numlockontty. Как и большинство пакетов программного обеспечения, доступных через AUR, этот пакет поставляется только в виде исходного кода, который будет встроен в двоичный код на вашей локальной системе с помощью makepkg и PKGBUILD скрипта. "
...
echo " Вам нужен пакет из AUR , называемый ckbcomp. Как и большинство пакетов программного обеспечения, доступных через AUR, этот пакет поставляется только в виде исходного кода, который будет встроен в двоичный код на вашей локальной системе с помощью makepkg и PKGBUILD скрипта. Настройка консоли https://manpages.org/ckbcomp . "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
