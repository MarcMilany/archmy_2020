...
_arch_fast_install_banner_2() {
  ...
  ***************************** ИНФОРМАЦИЯ! *****************************
${NC}
${MAGENTA}==> Примечание! ${BOLD}
  <<< Chroot - Эта arch-chroot команда представляет собой расширенную версию традиционной chroot команды, разработанную специально для Arch Linux. Она позволяет пользователям изменять корневой каталог текущей работающей среды и выполнять команды в этом новом корневом каталоге. Это особенно полезно при установке или восстановлении систем Arch Linux, поскольку позволяет пользователям взаимодействовать с системой так, как будто она была загружена в обычном режиме. В отличие от chroot, arch-chroot автоматически монтирует дополнительные файловые системы, такие как /proc, /sys и /dev, что упрощает управление установкой и восстановлением системы. >>> ${NC}
${CYAN}:: ${NC}Продолжается работа скрипта: - будет проходить установка первоначально необходимого софта (пакетов), запуск необходимых служб, запись данных в конфиги (hhh.conf) по настройке системы. В процессе работы сценария (скрипта) Вам будет предложено выполнить следующие действия:
...
#clear
 echo ""
 echo " Очистка папки конфигурации пропущена "
sleep 01
elif [[ $i_rm == 1 ]]; then
clear
rm -rf /home/$username/.*
 echo ""
 echo " Очистка папки конфигурации завершена "
sleep 01
fi
...
echo ""
echo -e "${MAGENTA}:: ${BOLD}Мир состоит из шести частей света: ${NC}"
echo " Азия, Африка, Америка, Европа, Австралия и Океания, Антарктика (Антарктида с прибрежными морями и островами). "
echo " Иногда Океанию и Арктику выделяют в отдельные части света. "
...
# hwclock --show  # Просмотр времени из аппаратных часов
...
echo -e "${YELLOW}==> Примечание! ${BOLD} *В операционной системе время (часы) определяется тремя понятиями: значением времени (будь то местное время, UTC или что-то другое), часовым поясом и информацией о переходе на летнее время (DST), если это применимо. В системах присутствуют двое часов: аппаратные часы и системные часы. ${NC}"
echo -e "${MAGENTA}:: ${BOLD}Есть два основных стандарта времени: местное (локальное) время (localtime), и Всемирное координированное время (Coordinated Universal Time, UTC). Местное время зависит от текущего часового пояса, а время UTC — это глобальное время, которое одинаково для всех и не зависит от часовых поясов. UTC иногда называют гринвичским временем (GMT), хотя это немного разные вещи. *Стандарт, который будет использоваться в аппаратных часах, выбирается операционной системой. По умолчанию Windows использует местное время, macOS использует UTC, а другие UNIX и UNIX-подобные системы используют разные стандарты определения времени. ОС, использующая стандарт UTC, обычно рассматривает аппаратные часы как UTC и вносит в них поправку для установки времени ОС при загрузке в соответствии с часовым поясом. ${NC}"
echo " Домашняя страница: https://wiki.archlinux.org/title/System_time  "
echo -e "${BLUE}:: ${NC}*Программные — часы По умолчанию системное время хранится в формате UTC, но система преобразует UTC в правильное местное время на основе региона или часового пояса. *Аппаратные часы (они же часы реального времени (RTC) или часы CMOS) хранят следующие значения: год, месяц, день, час, минута и секунда. Только в прошивках UEFI 2016 года или новее есть возможность хранить часовой пояс, а также информацию о переходе на летнее время."
echo -e "${CYAN}:: ${NC}Если на машине установлено несколько операционных систем, все они будут получать текущее время от одних и тех же аппаратных часов: рекомендуется настроить их на использование UTC, чтобы избежать конфликтов. В противном случае, если аппаратные часы установлены на местное время, сразу несколько операционных систем могут попытаться скорректировать его, например, после перехода на летнее/зимнее время, что приведёт к избыточной коррекции; проблемы могут также возникнуть при перемещении между различными часовыми поясами и использовании одной из операционных систем для сброса системных/аппаратных часов. "
echo -e "${YELLOW} Примечание! ${BOLD} *Вы можете пропустить этот шаг, если сейчас ваш часовой пояс настроен правильно, или Вы не уверены в правильности выбора! ${NC}"
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - UTC — Всемирное координированное время,   2 - Localtime — местное (локальное) время,

    0 - Пропустить настройку времени: " hw_clock  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$hw_clock" =~ [^120] ]]
do
    :
...
echo " Локализация системы подразумевает главным образом три вещи: "
echo " Настройку локали и настройку клавиатуры в виртуальной консоли и графическом окружении (Xorg) "
...
# localectl set-locale ru_RU.UTF-8  # Когда локаль сгенерирована, установите её в качестве системной
sleep 01
echo ""
echo -e "${BLUE}:: ${NC}Указываем язык системы "  # Localization (Русский)
...
clear
echo ""
echo -e "${GREEN}==> ${NC}Проверим корректность загрузки установленных микрокодов? "
echo -e "${YELLOW}==> Примечание! ${BOLD} *Микрокод — это код для процессора (его прошивка), поставляемая Intel или AMD.
        Ядро Linux может обновлять прошивку процессора без обновления BIOS во время загрузки.
        Микрокод процессора хранится в ОЗУ, а ядро обновляет микрокод во время каждой загрузки. ${NC}"
echo -e "${MAGENTA}=> ${NC}Если таковые (микрокод-ы: amd-ucode; intel-ucode) были установлены! "
echo -e "${BLUE}:: ${NC}Если микрокод был успешно загружен, Вы увидите несколько сообщений об этом "
echo -e "${CYAN}:: ${NC}Вы увидите вывод после команды и наверняка вас попросят обновить микрокод "
echo -e "${YELLOW} Примечание! ${BOLD} *Будьте внимательны! Вы можете пропустить это действие, Но лучше НЕ пропускать обновление! Процесс установки был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. ${NC}"
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
...
if [[ $x_ucode == 0 ]]; then
 echo ""
 echo " Проверка установленных микрокодов пропущена "
sleep 01
elif [[ $x_ucode == 1 ]]; then
 echo ""
 echo " Выполним проверку корректности загрузки установленных микрокодов "
### Команда используется для отображения сообщений ядра, связанных с обновлениями микрокода ЦП во время процесса загрузки.
dmesg | grep -E microcode  # https://wiki.archlinux.org/title/Microcode
sleep 03
fi
...
echo -e "${YELLOW} Примечание! ${BOLD} *Будьте внимательны! Без этих обновлений Вы можете наблюдать ложные падения или неожиданные зависания системы, которые может быть сложно отследить. Процесс установки был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. ${NC}"
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
...
clear
echo -e "${MAGENTA}
  <<< Редактирование системного файла mkinitcpio.conf для файловой системы Btrfs в Archlinux >>> ${NC}"
echo -e "${YELLOW}==> Примечание! ${BOLD} *Перед созданием загрузочного RAM диска (начальный RAM-диск), Если вы установили файловую систему Btrfs (B-tree File System) — нужно внести некоторые изменения в /etc/mkinitcpio.conf, в разделе HOOKS=, Уберём хук 'fsck' из конфига, т.к. для Btrfs он не требуется, иначе создание загрузочного RAM диск (начальный RAM-диск) для выбранного вами ядра может привести к ошибке (т.е. установка Archlinux НЕ Будет выполнена). При генерации initramfs mkinitcpio будет ругаться на отсутствие fsck.btrfs — это нормальное явление. ${NC}"
echo -e "${YELLOW}==> Примечание! ${BOLD} *В случае, Если вами, БЫЛА установлена другая файловая система для вашего системного раздела(ов), например: Ext4 , XFS , F2FS , JFS , вы можете просто пропустить действие выполнения сценария скрипта Введя на клавиатуре: "0". ${NC}"
#clear
echo ""
echo -e "${GREEN}==> ${NC}Выполнить редактирование файла mkinitcpio.conf — для Btrfs (B-tree File System)?"
echo -e "${YELLOW}==> Примечание! ${BOLD} *Некоторые хуки, которые могут быть нужны для вашей системы, вроде lvm2, mdadm_udev и encrypt, НЕ включены по умолчанию. Смотрите раздел #HOOKS, чтобы узнать, как их включить и настроить. Массив HOOKS наиболее важный в файле настроек. Хуки — это небольшие скрипты, которые описывают, что будет добавлено к образу, а также дополнительные действия, выполняемые при загрузке системы. Хуки указываются по имени и выполняются в порядке, заданном в массиве HOOKS. ${NC}"
echo -e "${MAGENTA}:: ${BOLD}/etc/mkinitcpio.conf — основной файл настроек mkinitcpio. Поддерживаются drop-in файлы, например /etc/mkinitcpio.conf.d/myhooks.conf (но они игнорируются при запуске с опцией -c и/или использовании предустановки, содержащей ALL_config). Кроме того, в каталоге /etc/mkinitcpio.d располагаются файлы предустановок (например /etc/mkinitcpio.d/linux.preset). ${NC}"
echo " Домашняя страница: https://wiki.archlinux.org/title/Mkinitcpio. "
echo -e "${BLUE}:: ${NC}Функции: Можно изменять семь переменных в файле настроек (подробнее в документации mkinitcpio.conf(5) § VARIABLES): *MODULES — Модули ядра, которые будут загружены до выполнения хуков. *BINARIES — Дополнительные исполняемые файлы, которые необходимо включить в initramfs. *FILES — Дополнительные файлы, которые необходимо включить в initramfs. *HOOKS — Хуки — скрипты, выполняемые в initramfs. *COMPRESSION — Тип используемого сжатия initramfs. *COMPRESSION_OPTIONS — Дополнительные опции, передаваемые программе, указанной в переменной COMPRESSION. Использование этого параметра не рекомендуется. mkinitcpio будет обрабатывать особые требования к инструментам сжатия (например --check=crc32 для xz), и неправильное использование может привести к невозможности запуска системы. *MODULES_DECOMPRESS — Разжимать ли загружаемые модули ядра и файлы прошивок перед их записью в initramfs или сохранять их в исходном сжатом виде. "
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Да редактировать файла mkinitcpio.conf — для Btrfs,

    2 - Да редактировать файла mkinitcpio.conf — для Btrfs (если у вас более 1 диска BTRFS),

    0 - НЕТ - Пропустить действие: " in_mkinitcpio  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_mkinitcpio" =~ [^120] ]]
do
    :
done
if [[ $in_mkinitcpio == 0 ]]; then
#HOOKS=(base udev autodetect microcode modconf kms keyboard keymap consolefont block filesystems fsck)  # Мой хук из Archlinux
# HOOKS=(base udev autodetect modconf block filesystems keyboard keymap)
# HOOKS=(base udev autodetect microcode modconf kms keyboard keymap consolefont block resume filesystems fsck systemd)
#echo 'HOOKS=(base udev autodetect microcode modconf kms keyboard keymap consolefont block resume filesystems fsck systemd)' >> /etc/mkinitcpio.conf  # plymouth - добавить, если установлен plymouth
 echo ""
 echo " Редактирование системного файла mkinitcpio.conf пропущено "
sleep 01
elif [[ $in_mkinitcpio == 1 ]]; then
 echo ""
 echo " Редактирование системного файла mkinitcpio.conf "
 echo " Уберём хук fsck из конфига, т.к. для Btrfs он не требуется "
# HOOKS=(base udev autodetect modconf block filesystems keyboard keymap)
# HOOKS=(base udev autodetect microcode modconf kms keyboard keymap consolefont block resume filesystems systemd)
cat << EOF >> /etc/mkinitcpio.conf
HOOKS=(base udev autodetect microcode modconf kms keyboard keymap consolefont block resume filesystems systemd)
EOF
#echo 'HOOKS=(base udev autodetect microcode modconf kms keyboard keymap consolefont block resume filesystems systemd)' >> /etc/mkinitcpio.conf  # plymouth btrfs fsck
# echo 'HOOKS=(base udev autodetect microcode modconf kms keyboard keymap consolefont block resume filesystems fsck systemd)' >> /etc/mkinitcpio.conf  # plymouth btrfs fsck
# sed -i 's/^HOOKS/HOOKS=(base udev autodetect microcode modconf kms keyboard keymap consolefont block resume filesystems systemd)/' mkinitcpio.conf
 echo ""
 echo " Редактирование системного файла mkinitcpio.conf выполнено "
sleep 01
elif [[ $in_mkinitcpio == 2 ]]; then
 echo ""
 echo " Редактирование системного файла mkinitcpio.conf "
 echo " Уберём хук fsck из конфига, т.к. для Btrfs он не требуется "
 echo " Добавим хук btrfs, если у вас более 1 диска с BTRFS — НЕ раздела! "
# если у вас более 1 диска BTRFS
#echo 'HOOKS=(base udev autodetect microcode modconf kms keyboard keymap consolefont block resume btrfs filesystems systemd)' >> /etc/mkinitcpio.conf  # plymouth btrfs fsck
# echo 'HOOKS=(base udev autodetect microcode plymouth modconf kms keyboard keymap consolefont block resume btrfs filesystems fsck systemd)' >> /etc/mkinitcpio.conf
cat << EOF >> /etc/mkinitcpio.conf
HOOKS=(base udev autodetect microcode modconf kms keyboard keymap consolefont block resume btrfs filesystems systemd)
EOF
# sed -i 's/^HOOKS/HOOKS=(base udev autodetect microcode modconf kms keyboard keymap consolefont block resume btrfs filesystems systemd)/' mkinitcpio.conf
 echo ""
 echo " Редактирование системного файла mkinitcpio.conf выполнено "
sleep 01
fi
...
clear
echo -e "${MAGENTA}
  <<< Создание загрузочного образа RAM диска (начальный RAM-диск) для Archlinux >>> ${NC}"
echo -e "${YELLOW}==> Примечание! ${BOLD} *Сгенерируем образ initramfs, используя предустановку, предоставленную пакетом ядра, используйте параметр -p/--preset с именем нужной предустановки. ${NC}"
#clear
echo ""
echo -e "${GREEN}==> ${NC}Создадим загрузочный RAM диск (начальный RAM-диск)"
echo -e "${YELLOW}==> Примечание! ${BOLD} *Чтобы избежать ошибки при создании RAM (mkinitcpio -p), вспомните какое именно ядро Вы выбрали ранее. И загрузочный RAM диск (начальный RAM-диск) будет создан именно с таким же ядром, иначе 'ВАЙ ВАЙ'! Эта информация необходима, потому что при создании начального RAM-диска (initrd), который используется для загрузки, вы должны указать название ядра, чтобы включить соответствующие модули для вашей аппаратной конфигурации. ${NC}"
echo -e "${MAGENTA}:: ${BOLD}В Arch Linux mkinitcpio — это Bash скрипт используемый для создания начального загрузочного диска системы (https://en.wikipedia.org/wiki/ru:initrd). mkinitcpio является модульным инструментом для построения initramfs CPIO образа, предлагая много преимуществ по сравнению с альтернативными методами. Предоставляет много возможностей для настройки из командной строки ядра без необходимости пересборки образа. ${NC}"
echo " Домашняя страница: https://wiki.archlinux.org/title/Kernel_module ; (https://wiki.archlinux.org/title/Mkinitcpio ; https://man.archlinux.org/man/initrd.4.ru). "
echo -e "${BLUE}:: ${NC}Начальный ram-диск по сути представляет собой очень маленькую среду (раннее пользовательское пространство, early userspace), которая загружает различные модули ядра и настраивает необходимые вещи перед передачей управления процессу init. Это позволяет, например, использовать зашифрованную корневую файловую систему или разместить её на программном RAID-массиве. mkinitcpio легко расширяется с помощью настраиваемых хуков, имеет автоопределение во время работы системы и многие другие функции. *Модули ядра — это отдельные кусочки кода, которые могут быть загружены в ядро и выгружены из ядра по мере необходимости. Они расширяют функциональность ядра без необходимости перезагрузки системы. Для автоматической пересборки модулей после установки новой версии ядра используется DKMS. *Здесь представлены варианты создания RAM-диска, с конкретными ядрами. "
echo -e "${YELLOW} Примечание! ${BOLD} *Будьте внимательны! Процесс создания (сборки) был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. ${NC}"
echo -e "${GREEN}==> ${BOLD}Вот ядро (это номер версии ядра), Которое установлено в live-системе Archlinux: ${NC}"
echo ""
uname -r  # Чтобы узнать текущую версию вашего ядра ; Команда: uname (Unix name) — это системная утилита, которая выводит различную информацию о системе и ядре Linux. Аргумент -r: Этот флаг говорит uname вывести только номер релиза (версии) ядра.
# uname -a  # Подробная информация о системе ; Если вам нужна более подробная информация, используйте параметр -a . Этот вывод включает: Название системы (Linux). Имя хоста. Версию ядра. Архитектуру процессора (x86_64). Дату сборки ядра.
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
...
mkinitcpio -p linux
# mkinitcpio -P linux  # - при ошибке!
 echo ""
 echo " Создание загрузочный RAM диск — для ядра (linux) выполнено "
sleep 01
elif [[ $x_ram == 2 ]]; then
clear
 echo ""
...
# mkinitcpio -P linux-hardened  # - при ошибке!
 echo ""
 echo " Создание загрузочный RAM диск — для ядра (linux-hardened) выполнено "
sleep 01
elif [[ $x_ram == 3 ]]; then
clear
 echo ""
...
# mkinitcpio -P linux-lts  # - при ошибке!
 echo ""
 echo " Создание загрузочный RAM диск — для ядра (linux-lts) выполнено "
sleep 01
elif [[ $x_ram == 4 ]]; then
clear
 echo ""
...
# mkinitcpio -P linux-zen  # - при ошибке!
 echo ""
 echo " Создание загрузочный RAM диск — для ядра (linux-zen) выполнено "
sleep 01
elif [[ $x_ram == 0 ]]; then
clear
 echo ""
...
### mkinitcpio (Русский):
# https://wiki.archlinux.org/title/Mkinitcpio_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)
# https://man.archlinux.org/man/initrd.4.ru
### Для получения списка всех доступных хуков:
# $ mkinitcpio -L
# Используйте опцию -H/--hookhelp для вывода информации о конкретном хуке:
# $ mkinitcpio -H udev
...
# Чтобы узнать версию ядра в Arch Linux, откройте терминал и введите команду uname -r. Эта команда выведет номер версии ядра вашей системы, например, 6.5.8-arch1-1.
...
# Чтобы узнать, какие модули ядра загружены в настоящий момент: $ lsmod
...
echo " Пароли позволяют следующее: Строчные буквы алфавита (a, b, c и т. д.) ; Заглавные буквы алфавита (A, B, C и т. д.) ;
       Числа (0, 1, 2 и т. д.) ; Специальные символы (@, %, ! и т. д.) для более подробной информации посетите страницу: https://help.ubuntu.com/community/StrongPasswords "
...
### Файл passwdдоступен для чтения всем, поэтому хранение паролей (хешированных или иным образом) в нём небезопасно.
############################
clear
echo -e "${MAGENTA}
  <<< Установка GRUB (Bootloader) — Загрузчик операционной системы — системное программное обеспечение, обеспечивающее загрузку операционной системы непосредственно после включения компьютера и начальной загрузки для ArchLinux. >>> ${NC}"
echo -e "${YELLOW}==> Примечание! ${BOLD} *Если у вас Прошивка UEFI и вам нужен GRUB для UEFI, или вы хотите установить rEFInd - UEFI-загрузчик, они будут представлены далее в сценарии (скрипте), здесь просто пропустите действие установки загрузчика, в запросе на установку загрузчика — Введя на клавиатуре: "0". ${NC}"
echo ""
echo -e "${GREEN}==> ${NC}Установить (bootloader) загрузчик GRUB2 — для BIOS-версии?"
echo -e "${YELLOW}==> Примечание! ${BOLD} *Коротко говоря, загрузчик — это первая программа, которая запускается при запуске компьютера. Он отвечает за загрузку и передачу управления ядру Linux. Ядро, в свою очередь, инициализирует остальную часть операционной системы. *Предупреждение! GRUB Legacy больше не поддерживается в основной ветке разработки и официально не поддерживается в Arch. ${NC}"
echo -e "${MAGENTA}:: ${BOLD}GRUB2 для BIOS — это версия загрузчика, которая взаимодействует с традиционными системами на основе BIOS, используя файлы, такие как boot.img и core.img, для управления процессом загрузки операционной системы напрямую, в отличие от версии GRUB2 для UEFI, которая использует стандартные API UEFI. Эта версия позволяет отображать меню выбора ОС, загружать ядра и модули, а также поддерживает множество операционных систем, включая Linux, FreeBSD и Windows. ${NC}"
echo " Домашняя страница: https://wiki.archlinux.org/title/GRUB ; (https://wiki.archlinux.org/title/Arch_boot_process#Boot_loader ; https://wiki.archlinux.org/title/GRUB_Legacy). "
echo -e "${BLUE}:: ${NC}GRUB2 — для BIOS-версии, можно установить не только для таблицы разделов MS-DOS, а также для таблицы разделов и GPT — GUID Partition Table (Таблица разделов GUID) — современный стандарт разметки жестких дисков, пришедший на смену более старой системе MBR. *Для этого перед загрузочным расделом BIOS создаём раздел обьёмом от 8 МБ до 50 МБ (в зависимости от ваших возможностей). Далее на этом (созданном) разделе устанавливаем флаг 'grub_bios' (Bios boot), а на разделе самого загрузчика GRUB — для BIOS (/boot), он будет распознавать системой как раздел EFI , установить флаги 'boot' и 'esp' (Efy system). И вы сможете познать все 'прелести' файловой системы Btrfs (B-tree File System)."
echo -e "${CYAN}:: ${NC}В сценарии скрипта есть несколько вариантов: *1 — Установка полноценной BIOS-версии загрузчика GRUB(legacy), тогда укажите "1". Файлы загрузчика будут установлены в каталог /boot. Код GRUB (boot.img) будет встроен в начальный сектор, а загрузочный образ core.img в просвет перед первым разделом MBR, или BIOS boot partition для GPT. *2 — Если нужно установить BIOS-версию загрузчика из-под системы, загруженной в режиме UEFI, тогда укажите "2". В этом варианте требуется принудительно задать программе установки нужную сборку GRUB — предложиn выбрать ядро x64 или i686 (Если вы не используете UEFI). "
echo -e "${YELLOW}==> Примечание! ${NC}Вы можете пропустить этот шаг, Если у вас Прошивка UEFI и вам нужет GRUB для UEFI, или вы хотите установить rEFInd - UEFI-загрузчик, также если у вас уже имеется BOOT раздел от другой (предыдущей) системы gnu-linux, с установленным на нём GRUB. "
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Установить GRUB — для BIOS-версии,   2 - Установить GRUB — BIOS-версию --target=i386-pc,

    0 - Нет пропустить установку: " i_grub  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$i_grub" =~ [^120] ]]
do
    :
...
if [[ $i_grub == 1 ]]; then
 echo ""
 echo " Установка утилиты (пакета) GRUB — для BIOS-версии "
pacman -Syy  # обновление баз пакмэна (pacman)
pacman -S grub --noconfirm  # Унифицированный загрузчик GNU GRand (2) ; Файлы и утилиты для установки GRUB2 содержатся в пакете grub ; https://archlinux.org/packages/core/x86_64/grub/ ; https://www.gnu.org/software/grub/ ; Обеспечивает: grub-bios, grub-common, grub-efi-x86_64, grub-emu ; Заменяет: grub-bios, grub-common, grub-efi-x86_64, grub-emu ; Конфликты: с  grub-bios, grub-common, grub-efi-x86_64, grub-emu, grub-legacy ; 2025-09-06 07:34 UTC
...
############ Зависимости ##############
pacman -S --noconfirm --needed dosfstools  # (необязательно) — для поддержки FAT FS и EFI в grub-mkrescue ; Утилиты файловой системы DOS; Для поддержки grub-mkrescue FAT FS и EFI ; https://archlinux.org/packages/core/x86_64/dosfstools/ ; https://github.com/dosfstools/dosfstools ; 2024-08-25 13:37 UTC
pacman -S --noconfirm --needed libisoburn  # (необязательно) — предоставляет xorriso для создания образа ISO для восстановления grub с помощью grub-mkrescue ; Интерфейс для библиотек libburn и libisofs ; https://archlinux.org/packages/extra/x86_64/libisoburn/ ; https://dev.lovelyhq.com/libburnia ; 2023-06-08 17:59 UTC
# pacman -S --noconfirm --needed libusb  # (необязательно) — для поддержки USB grub-emu ; Библиотека, обеспечивающая общий доступ к USB-устройствам ; https://archlinux.org/packages/core/x86_64/libusb/ ; https://libusb.info/ ; Обеспечивает: libusb-1.0.so=0-64 ; 2025-06-05 07:28 UTC
pacman -S --noconfirm --needed efibootmgr  # (необязательно) — для поддержки EFI при установке grub ; Приложение пользовательского пространства Linux для изменения диспетчера загрузки EFI ; https://github.com/rhboot/efibootmgr ; 2024-03-13 16:32 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
sleep 01
clear
echo ""
echo -e "${CYAN}:: ${NC}Определения архитектуры процессора, имени хоста системы и версии ядра "
echo ""
...
echo " Чтобы подтвердить действия ввода, нажмите кнопку 'Ввод' ("Enter") "
read -p " => Укажите диск куда установить GRUB (sda/sdb например: sda или sdb) : " x_cfd  # To confirm the input actions, click 'Enter' ; #
...
grub-mkconfig -o /boot/grub/grub.cfg   # Эта команда автоматически создаст файл конфигурации для вашего загрузчика.
# cat /boot/grub/grub.cfg | grep menuentry  # Вы можете проверить, произошло ли это из-за первой установки, проверив пункт меню
sleep 03
elif [[ $i_grub == 2 ]]; then
 echo ""
 echo " Установка утилиты (пакета) GRUB — BIOS-версию --target=i386-pc "
pacman -Syy  # обновление баз пакмэна (pacman)
pacman -S grub --noconfirm  # Унифицированный загрузчик GNU GRand (2) ; Файлы и утилиты для установки GRUB2 содержатся в пакете grub ; https://archlinux.org/packages/core/x86_64/grub/ ; https://www.gnu.org/software/grub/ ; Обеспечивает: grub-bios, grub-common, grub-efi-x86_64, grub-emu ; Заменяет: grub-bios, grub-common, grub-efi-x86_64, grub-emu ; Конфликты: с  grub-bios, grub-common, grub-efi-x86_64, grub-emu, grub-legacy ; 2025-09-06 07:34 UTC
# pacman -S --noconfirm --needed grub  # GNU GR и унифицированный загрузчик (2)
  echo ""
...
echo ""
echo " Операция установки загрузчика GRUB пропущена "
sleep 01
fi

clear
echo -e "${MAGENTA}
  <<< Установка UEFI-GRUB (Bootloader) и rEFInd - UEFI-загрузчик. Загрузчики операционной системы — системное программное обеспечение, обеспечивающее загрузку операционной системы непосредственно после включения компьютера и начальной загрузки для ArchLinux. >>> ${NC}"
echo -e "${YELLOW}==> Примечание! ${BOLD} *Вы можете пропустить действие установкки загрузчика: Если у вас Прошивка для BIOS-версии и GRUB уже установлен, а также если у вас уже имеется BOOT раздел от другой (предыдущей) системы gnu-linux, с установленным на нём GRUB. *Здесь просто пропустите действие установки загрузчика, в запросе на установку загрузчика — Введя на клавиатуре: "0". ${NC}"
echo ""
echo -e "${GREEN}==> ${NC}Установить UEFI-GRUB (Bootloader) или rEFInd - UEFI-загрузчик?"
echo -e "${YELLOW}==> Предупреждение! ${BOLD} *Файловые системы могут получить новые функции, ещё не поддерживаемые GRUB, что делает их непригодными для использования, /boot если только не отключить несовместимые функции. Обычно этого можно избежать, используя отдельный раздел /boot с универсально поддерживаемой файловой системой, например, FAT32 . ${NC}"
echo -e "${MAGENTA}:: ${BOLD}UEFI ( Unified Extensible Firmware Interface) — это интерфейс между операционными системами и микропрограммами. Он предоставляет стандартную среду для загрузки операционной системы и запуска предзагрузочных приложений. Он отличается от метода загрузки с использованием MBR-кода , который использовался в устаревших системах BIOS . ${NC}"
echo -e "${MAGENTA}:: ${BOLD}rEFInd —  это менеджер загрузки UEFI , способный запускать ядра в качестве загрузочных заглушек EFI . Он является ответвлением более не поддерживаемого rEFIt и устраняет множество проблем, связанных с загрузкой на устройствах, отличных от Mac, через UEFI. Он разработан с учётом платформенной независимости и упрощает загрузку нескольких операционных систем. ${NC}"
echo " Домашняя страница: https://wiki.archlinux.org/title/GRUB ; (https://wiki.archlinux.org/title/Arch_boot_process#Boot_loader ; https://wiki.archlinux.org/title/Unified_Extensible_Firmware_Interface ; https://wiki.archlinux.org/title/REFInd). "
echo -e "${BLUE}:: ${NC}*Поддерживаемые файловые системы: GRUB поддерживает множество файловых систем , в частности FAT32 , ext4 , Btrfs и XFS . *Поддержка безопасной загрузки: GRUB полностью поддерживает безопасную загрузку с использованием как ключей CA, так и прокладок, однако команда установки различается в зависимости от того, что вы собираетесь использовать. "
echo -e "${CYAN}:: ${NC}В сценарии скрипта есть несколько вариантов: *1 — Установка полноценной версии загрузчика UEFI-GRUB, тогда укажите "1". Файлы загрузчика будут установлены в каталог /boot. Код GRUB (boot.img) будет встроен в начальный сектор, а загрузочный образ core.img в просвет перед первым разделом MBR, или boot partition для GPT. *2 — Если нужно можно установить rEFInd - UEFI-загрузчик, способный запускать ядра EFISTUB. rEFInd наследует поддержку файловых систем из прошивки (как минимум FAT12, FAT16 и FAT32). Кроме того, он загружает все драйверы UEFI, размещённые в каталогах drivers и drivers_x64 подкаталогах своего установочного каталога на ESP. "
echo -e "$GREEN} Совет! ${BOLD} *По умолчанию refind-install устанавливает лишь драйвер для той файловой системы, где находится ядро. Драйверы дополнительных файловых систем нужно устанавливать вручную копированием из /usr/share/refind/drivers_x64/ в esp/EFI/refind/drivers_x64/, или Вы можете установить все драйвера с опцией --alldrivers. Это полезно для загрузочных USB-устройств. После установки файлов rEFInd в ESP убедитесь, что rEFInd создал файл refind_linux.conf, содержащий параметры ядра в том же каталоге, где и Ваше ядро. Этот конфигурационный файл не будет создан, если Вы использовали опцию --usedefault - запустите mkrlconf с правами root, чтобы создать его.${NC}"
echo -e "${YELLOW} Примечание! ${BOLD} *Будьте внимательны! Процесс установки был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. ${NC}"
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Установить UEFI-GRUB,    2 - Менеджер загрузки EFI - rEFInd пакет (refind),
...
if [[ $i_grub == 1 ]]; then
  echo ""
  echo " Установка утилиты (пакета) GRUB — для UEFI-GRUB-версии "
pacman -Syy  # обновление баз пакмэна (pacman)
pacman -S grub --noconfirm  # Унифицированный загрузчик GNU GRand (2) ; Файлы и утилиты для установки GRUB2 содержатся в пакете grub ; https://archlinux.org/packages/core/x86_64/grub/ ; https://www.gnu.org/software/grub/ ; Обеспечивает: grub-bios, grub-common, grub-efi-x86_64, grub-emu ; Заменяет: grub-bios, grub-common, grub-efi-x86_64, grub-emu ; Конфликты: с  grub-bios, grub-common, grub-efi-x86_64, grub-emu, grub-legacy ; 2025-09-06 07:34 UTC
# pacman -S --noconfirm --needed grub  # GNU GR и унифицированный загрузчик (2)
  echo ""
...
lsblk -f  # Команда lsblk выводит список всех блочных устройств
#echo ""
#echo -e "${CYAN}:: ${NC}Определение активного EFI-раздела "
#echo ""
#efibootmgr
echo ""
echo " В chroot-окружении В качестве цели должен быть указан физический диск, а не раздел для ESP "
echo " ESP обозначает точку монтирования системного раздела EFI "
echo -e "${YELLOW}=> Примечание! ${BOLD}/dev/sdX - диск (не раздел), на котором должен быть установлен GRUB. ${NC}"
echo ""
# Если вы используете LVM для вашего /boot, вы можете установить GRUB на нескольких физических дисках.
echo " Чтобы подтвердить действия ввода, нажмите кнопку 'Ввод' ("Enter") "
read -p " => Укажите диск куда установить GRUB (sda/sdb например sda или sdb) : " x_cfd  # To confirm the input actions, click 'Enter'
# grub-install /dev/$x_cfd  # Записываем загрузчик в MBR (Master Boot Record) нашего внутреннего накопителя (sda; sdb; sdc; sdd)
# grub-install --target=x86_64-efi --efi-directory=/boot --bootloader-id=GRUB /dev/$x_cfd  # Записываем загрузчик в MBR (Master Boot Record) нашего внутреннего накопителя (sda; sdb; sdc; sdd)
# Устанавливаем непосредственно сам загрузчик
# grub-install --target=x86_64-efi --efi-directory=/boot/efi --bootloader-id=grub /dev/$x_cfd
...
###########  Или так ###############
# grub-install --target=x86_64-efi --efi-directory=/boot/efi --bootloader-id=GRUB --no-nvram --removable /dev/$x_cfd  # Записываем загрузчик в MBR (Master Boot Record) нашего внутреннего накопителя (sda; sdb; sdc; sdd)
grub-install --target=x86_64-efi --efi-directory=/boot/efi --bootloader-id=grub --no-nvram --removable /dev/$x_cfd
# echo 'GRUB_DISABLE_OS_PROBER=false' >> /etc/default/grub
sed -i 's/#GRUB_DISABLE_OS_PROBER=false/GRUB_DISABLE_OS_PROBER=false/' /etc/default/grub
#echo " Создадим каталог локали (locale) для GRUB "
#mkdir -p /boot/grub/locale
#echo " Настройка локали (locale) для GRUB "
### нет необходимости вручную копировать файлы *.mo — «grub-install» автоматически помещает их в /boot/grub/locale/.
#cp /usr/share/locale/ru/LC_MESSAGES/grub.mo /boot/grub/locale/ru.gmo  # помог такой костыль
# cp /boot/grub/locale/ru@quot.mo /boot/grub/locale/ru.mo   # решило мою проблему
# cp /usr/share/locale/ru\@quot/LC_MESSAGES/grub.mo /boot/grub/locale/ru.mo
#cp /boot/grub/locale/en@quot.mo /boot/grub/locale/en.mo   # решило мою проблему
...
sleep 03
 ...
 echo ""
 echo " Установка утилиты (пакета) rEFInd — Менеджер загрузки EFI "
pacman -Syy  # обновление баз пакмэна (pacman)
pacman -S refind --noconfirm  # Менеджер загрузки EFI ; https://archlinux.org/packages/extra/x86_64/refind/ ; https://www.rodsbooks.com/refind/ ; 2024-04-07 18:54 UTC
### refind-install — установка rEFInd в ESP и создание записи NVRAM
# https://wiki.archlinux.org/title/REFInd
# https://man.archlinux.org/man/refind-install.8.en
echo ""
echo " Установка утилит (пакетов) выполнена "
sleep 01
clear
...
echo " В chroot-окружении В качестве цели должен быть указан физический диск, а не раздел для ESP "
echo " ESP обозначает точку монтирования системного раздела EFI "
echo " Вы также можете установить rEFInd в стандартный/запасной путь загрузки esp/EFI/BOOT/bootx64.efi. Это полезно для загрузочных USB-устройств или в системах с проблемами, связанными с изменениями в NVRAM, произведенными efibootmgr "
echo " Когда refind-install запускается в chroot (например, в live-системе при установке Arch Linux), /boot/refind_linux.conf заполняется параметрами ядра из live-системы, а не из той, на которую он установлен. Отредактируйте /boot/refind_linux.conf и убедитесь, что параметры ядра в нем верны для вашей системы. Файл конфигурации rEFInd находится по пути /boot/efi/EFI/refind/refind.conf . "
echo -e "${YELLOW}=> Примечание! ${BOLD}Где /dev/sdX - ваш системный раздел EFI (устройство, а не точка его монтирования), на котором должен быть установлен rEFInd. ${NC}"
  echo ""
# Если вы используете LVM для вашего /boot, вы можете установить GRUB на нескольких физических дисках.
  echo " Чтобы подтвердить действия ввода, нажмите кнопку 'Ввод' ("Enter") "
  read -p " => Укажите куда установить rEFInd (sda/sdb например sda или sdb) : " x_cfd  # To confirm the input actions, click 'Enter'
...
# refind-install  # Этот скрипт постарается найти и примонтировать Ваш ESP, скопировать файлы rEFInd в esp/EFI/refind/, и применить efibootmgr, чтобы сделать rEFInd загрузчиком EFI по умолчанию .
...
# refind-install --usedefault /dev/sdXY  # Где /dev/sdXY - ваш системный раздел EFI (устройство, а не точка его монтирования).
refind-install --usedefault /dev/$x_cfd
# Откройте его для редактирования:
# sudo nano /boot/efi/EFI/refind/refind.conf
# grub-mkconfig -o /boot/grub/grub.cfg
# https://man.archlinux.org/man/refind-install.8.en
 echo ""
 echo " Загрузчик rEFInd установлен на выбранный вами системный раздел EFI "
 echo " Отредактируйте файл конфигурации rEFInd refind.conf "
sleep 03
echo ""
echo -e "${BLUE}:: ${NC}Обновляем grub.cfg (Сгенерируем grub.cfg)"
grub-mkconfig -o /boot/grub/grub.cfg   # Эта команда автоматически создаст файл конфигурации для вашего загрузчика.
...
echo -e "${YELLOW} Примечание! ${BOLD} *Для двойной загрузки Arch Linux с другой системой Linux, Windows, установить другой Linux без загрузчика, вам необходимо установить утилиту os-prober, необходимую для обнаружения других операционных систем. ${NC}"
echo -e "${YELLOW}=> ${NC}*И обновить загрузчик Arch Linux, чтобы иметь возможность загружать новую ОС. "
echo -e "${YELLOW} Примечание! ${BOLD} *Будьте внимательны! Процесс установки был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. ${NC}"
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
...
pacman -S --noconfirm --needed os-prober  # Утилита для обнаружения других ОС на наборе дисков ; https://archlinux.org/packages/extra/x86_64/os-prober/ ; https://joeyh.name/code/os-prober/ ; 24.02.2025 16:59 UTC
# pacman -S os-prober --noconfirm  # Утилита для обнаружения других ОС на наборе дисков
pacman -S --noconfirm --needed mtools  # Коллекция утилит для доступа к дискам MS-DOS ; https://archlinux.org/packages/extra/x86_64/mtools/ ; https://www.gnu.org/software/mtools/ ; 2025-06-16 11:09 UTC
pacman -S --noconfirm --needed fuse  # (Filesystem in Userspace, FUSE) — это механизм для Unix-подобных операционных систем, позволяющий непривилегированным пользователям создавать собственные файловые системы без редактирования кода ядра. Это достигается путём запуска кода файловой системы в пространстве пользователя (user space), в то время как модуль ядра FUSE обеспечивает только «мост» к реальным интерфейсам ядра.
echo ""
echo -e "${BLUE}:: ${NC}Обновляем grub.cfg (Сгенерируем grub.cfg)"
grub-mkconfig -o /boot/grub/grub.cfg   # Эта команда автоматически создаст файл конфигурации для вашего загрузчика.
# cat /boot/grub/grub.cfg | grep menuentry  # Вы можете проверить, произошло ли это из-за первой установки, проверив пункт меню
...
echo -e "${YELLOW}==> Примечание! ${BOLD} *Если у Вас есть Wi-fi модуль и вы сейчас его не используете, но будете использовать в будущем. *Или вы подключены через Wi-fi сеть, то эти утилиты (пакеты) обязательно установите! ${NC}"
...
if [[ $i_wifi  == 1 ]]; then
 echo ""
 echo " Установка утилит (пакетов) для Wi-fi сети "
...
pacman -S --noconfirm --needed wpa_supplicant  # Утилита, обеспечивающая согласование ключей для беспроводных сетей WPA ; https://archlinux.org/packages/core/x86_64/wpa_supplicant/ ; https://w1.fi/wpa_supplicant/ ; 2025-01-28 21:45 UTC
pacman -S --noconfirm --needed iw  # Утилита конфигурации CLI на базе nl80211 для беспроводных устройств ; https://archlinux.org/packages/core/x86_64/iw/ ; https://wireless.wiki.kernel.org/en/users/documentation/iw ; 2025-09-04 13:23 UTC
pacman -S --noconfirm --needed wireless_tools  # Инструменты, позволяющие управлять беспроводными расширениями ; https://archlinux.org/packages/extra/x86_64/wireless_tools/ ; https://hewlettpackard.github.io/wireless-tools/Tools.html ; 2024-06-08 20:09 UTC
pacman -S --noconfirm --needed net-tools  # Инструменты настройки для сетей Linux ; https://archlinux.org/packages/core/x86_64/net-tools/ ; http://net-tools.sourceforge.net/ ; 2024-09-30 08:42 UTC
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Группы (audio,games,lp,disk,network,optical,power,scanner,storage,video,rfkill,sys,log,floppy,users,input,wheel),

    2 - Группы (adm + audio,games,lp,disk,network,optical,power,scanner,storage,video,rfkill,sys,log,floppy,users,input,wheel),
...
if [[ $i_groups  == 1 ]]; then
 echo ""
 echo " Добавим пользователя и сразу поместим его в необходимые нам группы "
 echo " Если планируется использовать zsh, то заменим /bin/bash на /bin/zsh "
#  useradd -m -g users -G audio,games,lp,disk,network,optical,power,scanner,storage,video,rfkill,sys,wheel -s /bin/bash $username
useradd -m -g users -G audio,games,lp,disk,network,optical,power,scanner,storage,video,rfkill,sys,log,floppy,users,input,wheel -s /bin/bash $username
# usermod -a -G audio,games,lp,disk,network,optical,power,scanner,storage,video,rfkill,sys,wheel $username
usermod -a -G audio,games,lp,disk,network,optical,power,scanner,storage,video,rfkill,sys,log,floppy,users,input,wheel $username
...
useradd -m -g users -G adm,audio,games,lp,disk,network,optical,power,scanner,storage,video,rfkill,sys,log,floppy,users,input,wheel -s /bin/bash $username
echo ""
echo " Пользователь успешно добавлен в группы и права пользователя "
sleep 01
elif [[ $i_groups  == 3 ]]; then
 echo ""
 echo " Добавим пользователя и сразу поместим его в группу wheel "
 echo " Если планируется использовать zsh, то заменим /bin/bash на /bin/zsh "
...
echo ""
echo " Настройка информация о пользователе пропущена "
sleep 01
fi

clear
echo ""
echo -e "${BLUE}:: ${NC}Устанавливаем Sudo (sudo) — для выполнения команд с правами суперпользователя (root) "
echo -e "${YELLOW}==> Примечание! ${BOLD} *Крайне важно, чтобы файл sudoers был без синтаксических ошибок! Любая ошибка делает sudo неработоспособным. Всегда редактируйте его только с помощью visudo для предотвращения ошибок. ${NC}"
echo -e "${MAGENTA}:: ${BOLD}Sudo — это альтернатива su для выполнения команд с правами суперпользователя (root). В отличие от su, который запускает оболочку с правами root и даёт всем дальнейшим командам root-права, sudo предоставляет временное повышение привилегий для одной команды. Предоставляя привилегии root только при необходимости, sudo снижает вероятность того, что опечатка или ошибка в выполняемой команде произведут в системе разрушительные действия. Этот проект Лицензируется под custom. ${NC}"
echo " Домашняя страница: https://www.sudo.ws/sudo/ ; (hhttps://archlinux.org/packages/core/x86_64/sudo/ ; https://wiki.archlinux.org/title/Sudo). "
echo -e "${BLUE}:: ${NC}Sudo (англ. substitute user do, дословно «подменить пользователя и выполнить») позволяет системному администратору делегировать полномочия, чтобы дать некоторым пользователям (или группе пользователей) возможность запускать некоторые (или все) команды c правами суперпользователя или любого другого пользователя, обеспечивая контроль над командами и их аргументами. "
echo -e "${CYAN}:: ${NC}Совет! Для добавления новых администраторов рекомендуется включить доступ к sudo для группы wheel и добавлять новых пользователей в неё, поскольку по умолчанию Polkit рассматривает членов группы wheel как администраторов. Если пользователь не является членом группы wheel, программы, использующие Polkit, могут запросить аутентификацию с использованием пароля root вместо пароля пользователя. "
echo -e "${YELLOW} Примечание! ${BOLD} *Будьте внимательны! Процесс установки был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. ${NC}"
sleep 03
 echo ""
 echo " Установка утилиты (пакета) Sudo (sudo) "
pacman -S --noconfirm --needed sudo  # Предоставить определенным пользователям возможность запускать некоторые команды от имени root ; https://archlinux.org/packages/core/x86_64/sudo/ ; https://www.sudo.ws/sudo/ ; 2025-06-30 22:05 UTC
...
echo -e "${YELLOW} Примечание! ${BOLD} *Будьте внимательны! Процесс установки Xorg (иксов) НЕ БЫЛ прописан полностью автоматическим, и было принято решение дать возможность пользователю сделать выбор. Вас спросят, устанавливать ли все пакеты, я устанавливал все, просто нажмите Enter. В данной опции выбор всегда остаётся за вами. ${NC}"
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
...
gui_install="xorg-xrandr --noconfirm"  #  Примитивный интерфейс командной строки для расширения RandR
...
clear
echo -e "${MAGENTA}
  <<< Установка графическое окружение (DE) — среда рабочего стола для Archlinux >>> ${NC}"
echo -e "${GREEN}==> Предисловие! ${BOLD} *Среда рабочего стола (DE) — реализация метафоры рабочего стола, состоящая из набора программ, объединённых общим графическим интерфейсом (GUI). Пользователю даётся возможность настраивать графический интерфейс разными путями. Как правило, среды рабочего стола предоставляют для этого готовые и удобные средства. ${NC}"
echo -e "${YELLOW}==> Примечание! ${BOLD} *В сценарии (скрипта) установки представлена часть основных графических окружений рабочего стола, если вы желаете установить что-то чего нет в предложенном списке, например: Cinnamon, Cutefish, Sway WM, Pantheon и т.д. тут уже ручками-дерзайте! Описанием графических окружений заниматься не буду, надеюсь, что почти все пользователи, даже начинающие, за свои первые шаги в Linux-системах успели ознакомиться с ними... *Да ещё, кому нужно установить Budgie Desktop (Budgie DE) (budgie-desktop), он будет представлен далее в сценарии (скрипта), так как был добавлен совсем недавно, *Здесь просто пропустите действие установки (DE), в запросе на установку — Введя на клавиатуре: "0". ${NC}"
sleep 07

clear
echo ""
echo -e "${GREEN}==> ${NC}Ставим DE (графическое окружение) — среда рабочего стола "
echo -e "${YELLOW}==> Предисловие! ${BOLD} *DE (от англ. desktop environment - среда рабочего стола), это обёртка для ядра Linux, предоставляющая основные функции дистрибутива в удобном для конечного пользователя наглядном виде (окна, кнопочки, стрелочки и). ${NC}"
...
echo " Домашняя страница: https://wiki.archlinux.org/title/Desktop_environment "
echo -e "${YELLOW} Примечание! ${BOLD} *Будьте внимательны! Процесс установки был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. ${NC}"
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    ...
    5 - Deepin (Deepin Desktop Environment) — настольный интерфейс и приложения Deepin имеют интуитивно понятный и элегантный дизайн
    ...
    8 - i3 Window Manager — (конфиги стандартные, возможна установка с автовходом)
...
pacman -S --noconfirm --needed plasma  # Устанавливает среду рабочего стола KDE Plasma
# pacman -S --noconfirm --needed plasma-desktop  # Рабочий стол KDE Plasma (Если вам нужна более минималистичная Plasma (с меньшим количеством установленных пакетов и приложений), установите пакет plasma-desktop)
pacman -S --noconfirm --needed konsole  # Эмулятор терминала KDE
pacman -S --noconfirm --needed dolphin  # Файловый менеджер KDE
pacman -S --noconfirm --needed dolphin-plugins  # Дополнительные плагины Dolphin (Предоставляет множество полезных плагинов для Dolphin, включая плагин для монтирования ISO-образов)
pacman -S --noconfirm --needed plasma-wayland-protocols  # Протоколы, специфичные для плазмы, в Wayland
pacman -S --noconfirm --needed kio-admin  # Управляйте файлами от имени администратора, используя протокол admin://KIO. (обеспечивает безопасный способ редактирования файлов от имени root)
pacman -S --noconfirm --needed plasma-x11-session  # Плазменная сессия X11
# pacman -S --noconfirm --needed kwin-x11  # Простой в использовании, но гибкий менеджер окон X Window
pacman -S --noconfirm --needed ark  # архиватор для ( Plasma(kde)- так же можно
pacman -S --noconfirm --needed gwenview  # Быстрый и простой в использовании просмотрщик изображений
pacman -S --noconfirm --needed spectacle  # Утилита для создания скриншотов KDE
pacman -S --noconfirm --needed kmenuedit  # Редактор меню KDE
pacman -S --noconfirm --needed kate # Расширенный текстовый редактор
pacman -S --noconfirm --needed kweather  # Приложение погоды для Plasma Mobile
pacman -S --noconfirm --needed ksystemlog  # Инструмент просмотра системных журналов (System log viewer)
pacman -S --noconfirm --needed partitionmanager  # Утилита KDE, позволяющая управлять дисками, разделами и файловыми системами
pacman -S --noconfirm --needed plasma-systemmonitor  # Интерфейс для мониторинга системных датчиков, информации о технологических процессах и других системных ресурсов
pacman -S --noconfirm --needed htop  # Интерактивный просмотрщик процессов
pacman -S --noconfirm --needed kinfocenter  # Утилита, предоставляющая информацию о компьютерной системе
pacman -S --noconfirm --needed system-config-printer  # Инструмент настройки принтера CUPS и апплет состояния
pacman -S --noconfirm --needed isoimagewriter  # Программа для записи гибридных ISO-файлов на USB-накопители
pacman -S --noconfirm --needed toolbox  # Непривилегированная среда разработки
pacman -S --noconfirm --needed filelight  # Просмотр информации об использовании дискового пространства
pacman -S --noconfirm --needed kcalc  # Калькулятор с поддержкой тригонометрических, логических и статистических вычислений
pacman -S --noconfirm --needed kfind  # Найти файлы/папки
pacman -S --noconfirm --needed kshutdown  # Отключение утилиты
pacman -S --noconfirm --needed sweeper  # Очиститель системы
pacman -S --noconfirm --needed okular  # Просмотрщик документов
pacman -S --noconfirm --needed krusader  # Расширенный файловый менеджер с двумя панелями (в стиле Commander)
pacman -S --noconfirm --needed kcolorchooser  # Выбор цвета
pacman -S --noconfirm --needed kolourpaint  # Программа для рисования (Paint)
pacman -S --noconfirm --needed kgeotag  # Программа геотегирования фотографий
pacman -S --noconfirm --needed kompare  # Инструмент для сравнения файлов в графическом виде
pacman -S --noconfirm --needed akregator  # Программа для чтения RSS-лент от KDE
pacman -S --noconfirm --needed aria2  # Загрузите утилиту, поддерживающую HTTP(S), FTP, BitTorrent и Metalink
pacman -S --noconfirm --needed kget  # Менеджер загрузок
pacman -S --noconfirm --needed libtorrent  # Библиотека BitTorrent с упором на высокую производительность
pacman -S --noconfirm --needed libtorrent-rasterbar  # Эффективная и полнофункциональная реализация библиотеки C++ BitTorrent
pacman -S --noconfirm --needed qbittorrent  # Клиент сети БитТоррент (Обмен файлами по сети БитТоррент)
### Для генерирования эскизов медиафайлов и их предпросмотра в Dolphin или на рабочем столе
pacman -S --noconfirm --needed kdegraphics-thumbnailers  # Инструменты для создания миниатюр для различных форматов графических файлов
pacman -S --noconfirm --needed ffmpegthumbs  # Создатель миниатюр для видеофайлов на основе FFmpeg
### Управление питанием
pacman -S --noconfirm --needed powerdevil  # Управляет настройками энергопотребления корпуса плазменного гриля
# Получения встроенной службы управления энергопотреблением Plasma. Данная служба предлагает дополнительные возможности по оптимизации энергопотребления, регулировке яркости экрана (если поддерживается) и получению информации о состоянии аккумуляторов устройств.
pacman -S --noconfirm --needed power-profiles-daemon  # Обеспечивает возможность обработки профилей энергопотребления по шине D-Bus
# Для интеграции с профилями питания нужна опциональная зависимость power-profiles-daemon.
pacman -S --noconfirm --needed plasma-browser-integration  # Компоненты, необходимые для интеграции браузеров в рабочий стол Plasma
# Начиная с Plasma 5.13 можно интегрировать Firefox или Chrome с Plasma для получения следующих возможностей: управление воспроизведением медиа через трей Plasma, уведомление о загрузках и нахождение открытых вкладок через KRunner. Установите plasma-browser-integration и соответствующее дополнение для браузера. Поддержка Chrome/Chromium уже должна быть включена, для Firefox смотрите Firefox (Русский)#Интеграция с KDE.
pacman -S --noconfirm --needed kamoso  # Программа для записи видео с веб-камеры от сообщества KDE (Используйте веб-камеру для фотографирования или создания видео)
pacman -S --noconfirm --needed skanpage  # Утилита для сканирования изображений и многостраничных документов (Сканирование и сохранение многостраничных документов)
pacman -S --noconfirm --needed kdenlive  # Нелинейный видеоредактор для Linux, использующий видеофреймворк MLT
pacman -S --noconfirm --needed converseen  # Converseen - Пакетное преобразование изображений. Он позволяет конвертировать, изменять размер, поворачивать, отражать и переименовывать сразу множество картинок, а также извлекать изображения из PDF-документов. Программа построена на базе библиотеки ImageMagick, что обеспечивает поддержку более 100 форматов файлов.
pacman -S --noconfirm --needed soundconverter  # SoundConverter - Аудиоконвертер (Конвертер форматов аудио, Преобразование звуковых файлов в другие форматы). Она особенно хорошо подходит для работы с большими библиотеками музыки. Программа написана на Python, а её интерфейс построен на GTK+ и соответствует спецификациям GNOME.
pacman -S --noconfirm --needed shortwave  # Shortwave - Проигрыватель интернет-радио (предоставляет доступ к базе данных с более чем 50 000 радиостанций со всего мира)
pacman -S --noconfirm --needed hypnotix  # Hypnotix — это лёгкий и удобный IPTV-плеер для Linux, разработанный командой Linux Mint. Он позволяет смотреть прямые трансляции телеканалов, а также фильмы и сериалы по запросу.
pacman -S --noconfirm --needed audacious audacious-plugins  # Audacious - Музыкальный проигрыватель (Исходный код Audacious разделен на два пакета. 
pacman -S --noconfirm --needed mpv  # MPV Проигрыватель - Мультимедийный проигрыватель (основанный на MPlayer и mplayer2. Его ключевая особенность — работа через командную строку, но при этом он обладает широкими возможностями настройки и поддержки аппаратного ускорения)
pacman -S --noconfirm --needed smplayer smplayer-skins smplayer-themes  # SMPlayer — медиаплеер с открытым исходным кодом для Linux, который работает на базе MPlayer. Его ключевое преимущество — встроенные кодеки, благодаря которым не нужно отдельно устанавливать пакеты с кодеками для воспроизведения большинства форматов видео и аудио.
pacman -S --noconfirm --needed kid3-qt  # Редактор аудиотегов (Редактирование метаданных звуковых файлов)
pacman -S --noconfirm --needed aegisub  # Aegisub — это бесплатный кроссплатформенный редактор с открытым исходным кодом для создания и редактирования субтитров. Он поддерживает множество форматов, включая собственный Advanced SubStation Alpha (ASS), и предоставляет инструменты для синхронизации текста с аудиодорожкой, стилизации и перевода.
pacman -S --noconfirm --needed mediainfo mediainfo-gui  # MediaInfo - Показывает информацию об аудио и видео файлах
pacman -S --noconfirm --needed okteta  # Okteta - Шестнадцатеричный (HEX) редактор (Просмотр и изменение двоичных файлов). Он разработан в сообществе KDE и является стандартным инструментом для работы с двоичными данными в этой среде
pacman -S --noconfirm --needed foliate  # Foliate - Читалка электронных книг (Читайте электронные книги стильно). Её интерфейс создан с акцентом на минимализм и удобство чтения, с автоматическим скрытием элементов управления для минимизации отвлекающих факторов.
pacman -S --noconfirm --needed chromium  # Веб-браузер, созданный для скорости, простоты и безопасности
pacman -S --noconfirm --needed obs-studio  # OBS Studio - Приложение для потокового вещания и видеозаписи (Свободное и открытое ПО для потокового вещания и видеозаписи)
pacman -S --noconfirm --needed kbackup  # KBackup - Резервное копирование. Приложение для резервного копирования с простым интерфейсом.
pacman -S --noconfirm --needed timeshift  # Timeshift - Программа для восстановления системы (создаёт моментальные снимки (снапшоты) вашей системы и позволяет в любой момент откатить её состояние к предыдущему рабочему)
pacman -S --noconfirm --needed crow-translate  # Переводчик (Простой и легковесный переводчик, который позволяет переводить и озвучивать выделенный текст)
pacman -S --noconfirm --needed keepassxc  # KeePassXC - Менеджер паролей Keepass
pacman -S --noconfirm --needed k3b  # K3b (KDE Burn Baby Burn) - Запись дисков (Программа записи оптических дисков (CD, DVD, Blu-ray))
pacman -S --noconfirm --needed hash-o-matic  # Простой валидатор хешей, позволяющий сравнивать два файла, генерировать контрольную сумму файла и проверять, совпадает ли хеш с хешем файла
pacman -S --noconfirm --needed telegram-desktop  # Клиент мессенджера Telegram
pacman -S --noconfirm --needed seahorse  # Seahorse (в русскоязычной среде иногда называют «Пароли и ключи») (Управление паролями и ключами шифрования). 
pacman -S --noconfirm --needed kleopatra  # Kleopatra - Менеджер сертификатов. Графический инструмент (фронтенд) для работы с криптографическими службами, в первую очередь с GnuPG (GPG). С её помощью удобно управлять ключами шифрования, подписывать и проверять данные, а также шифровать и расшифровывать файлы и даже электронные письма. 
pacman -S --noconfirm --needed thunderbird  # Программа для чтения почты и новостей от mozilla.org
pacman -S --noconfirm --needed thunderbird-i18n-ru  # Русский языковой пакет для Thunderbird
#pacman -S --noconfirm --needed kmail  # Почтовый клиент KDE
pacman -S --noconfirm --needed veracrypt  # VeraCrypt - Менеджер томов VeraCrypt. Создание и монтирование зашифрованных томов VeraCrypt
pacman -S --noconfirm --needed handbrake  # Медиа-транскодер (Перекодирует DVD, Blu-ray и другие носители)
pacman -S --noconfirm --needed mkvtoolnix-gui  # MKVToolNix - Инструменты для создания и обработки файлов Matroska
pacman -S --noconfirm --needed bluefish  # Bluefish Editor - Редактор веб-разработки. Мощный HTML-редактор для опытных веб-дизайнеров и программистов
pacman -S --noconfirm --needed fontforge  # Редактор шрифтов
###############
clear
echo ""
...
clear
 ...
 echo ""
 echo " Установка утилиты (пакета)Xfce + Goodies for Xfce "
pacman -S xfce4 xfce4-goodies --noconfirm
### Group Details - xfce4 (x86_64) - Нетребовательное к ресурсам окружение рабочего стола для UNIX-подобных операционных систем ; https://archlinux.org/groups/x86_64/xfce4/
### Group Details - xfce4-goodies (x86_64) ; Проект Xfce Goodies Project включает дополнительное программное обеспечение и изображения, которые связаны с рабочим столом Xfce , но не являются частью официального выпуска ; https://archlinux.org/groups/x86_64/xfce4-goodies/
...
pacman -S --noconfirm --needed xfce4-genmon-plugin  # Плагин, который отслеживает стандартный вывод настраиваемых программ для панели Xfce4 ; https://archlinux.org/packages/extra/x86_64/xfce4-genmon-plugin/ ; https://docs.xfce.org/panel-plugins/xfce4-genmon-plugin/start ; Группы: xfce4-goodies ; 2025-05-22 05:32 UTC
## mv /usr/share/xsessions/xfce.desktop ~/
### Установка дополнительных утилит (пакетов) для Xfce4 ####
...
clear
 ...
 echo ""
 echo " Установка утилиты (пакета) Gnome "
...
clear
 ...
 echo ""
 echo " Установка утилит (пакетов) LXDE "
...
clear
 ...
 echo ""
 echo " Установка утилит (пакетов) Deepin "
...
clear
 ...
 echo ""
 echo " Установка утилит (пакетов) Mate "
...
echo ""
echo " Установка утилиты (пакета) Lxqt "
...
# pacman -S i3-gaps i3blocks i3status dmenu rofi xfce4-terminal firefox nitrogen picom network-manager-applet blueman bluez bluez-utils gvfs gvfs-mtp pcmanfm --noconfirm
clear
echo ""
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Budgie Desktop (Budgie DE) (budgie-desktop) — Впечатляющая среда рабочего стола?"
echo -e "${YELLOW}==> Примечание! ${BOLD} *Budgie довольно молодая графическая оболочка, поэтому дистрибутивов, которые ее поддерживают «из коробки» относительно мало. Arch Budgie для очень слабого ноутбука. ${NC}"
echo -e "${MAGENTA}:: ${BOLD}Budgie (Budgie DE) — это среда рабочего стола (графическая оболочка), которая была написана с нуля, но использует технологии GNOME, ранее входившая в состав Solus и ставшая независимой в рамках недавно созданной организации Buddies of Budgie в январе 2022 года. Она использует библиотеки GTK+ для виджетов и написана на C и Vala. Начиная с версии Budgie 10, единственный доступный сеанс находится на Xorg . В качестве оконного менеджера используется Budgie Window Manager (BWM), который основан на Mutter. Этот проект Лицензируется под GPL-2.0 . ${NC}"
echo " Домашняя страница: https://github.com/BuddiesOfBudgie/budgie-desktop ; (https://archlinux.org/groups/x86_64/budgie/ ; https://archlinux.org/packages/extra/x86_64/budgie-desktop/ ; https://wiki.archlinux.org/title/Budgie). "
echo -e "${BLUE}:: ${NC}Budgie — это впечатляющая среда рабочего стола (DE), которая предлагает привычную раскладку и уникальный пользовательский интерфейс. Budgie имеет классический интерфейс рабочего стола. В нижней части расположена панель, на которой размещается кнопка для открытия Главного меню (Budgie Menu), иконки для быстрого запуска приложений, область уведомлений и индикаторов, часы, а также кнопка для вызова боковой панели Raven. Тема оформления выполнена в плоском стиле. Элементы темы смотрятся легко и современно. Поддерживается две основных темы оформления — темная в серых тонах и светлая. В нижней части рабочего стола размещается горизонтальная панель на всю ширину экрана. Можно создавать дополнительные панели и менять их расположение (сверху, снизу, справа и слева). Панель поддерживает прозрачность, можно включить и отключить отбрасываемую тень. Поддерживается режим док-панели. В этом режиме панель не растягивается на весь экран, а располагается по центру. "
echo -e "${CYAN}:: ${NC}Для настройки оболочки предусмотрена специальная утилита. Через нее можно настроить внешний вид, рабочий стол, шрифты, поведение окон и панели. Главное меню системы вызывается при клике на кнопку, расположенную в левой части панели. Меню разделено на две колонки. В левой колонке представлены категории, а справа отображаются непосредственно приложения. Сверху меню расположена строка поиска. С ее помощью можно искать установленные приложения. Меню поддерживает компактный режим — в одну колонку. *Raven — это специальное боковое меню, которое вызывается при клике на кнопку, расположенную в левой части панели. Raven представляет собой панель, на которой можно размещать различные апплеты. Панель имеет две вкладки: Апплеты и Уведомления. На вкладке Апплеты, как и следует из ее названия, располагаются Апплеты, а на вкладке Уведомления выводится список системных уведомлений и уведомления программ. Некоторые из апплетов, доступных в Raven: Календарь. Регулятор громкости для всей системы. Регулятор громкости для отдельного приложения. Элементы для управления проигрывателем музыки. Интеграция с VLC, Rhythmbox, Spotify. Отображение обложки музыкального альбома. "
echo -e "${YELLOW} Примечание! ${BOLD} *Будьте внимательны! Процесс установки был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. ${NC}"
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " x_de  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$x_de" =~ [^10] ]]
do
    :
...
sleep 01
 ...
 echo ""
 echo " Установка утилиты (пакета) Budgie DE (budgie-desktop) "
#pacman -Syy  # обновление баз пакмэна (pacman)
...
############ Зависимости ##############
pacman -S --noconfirm --needed budgie-control-center  # Основной интерфейс Budgie для настройки различных аспектов системы ; https://archlinux.org/packages/extra/x86_64/budgie-control-center/ ; https://github.com/BuddiesOfBudgie/budgie-control-center ; Группы: budgie ; 2025-04-30 17:32 UTC
#pacman -S --noconfirm --needed gnome-user-share  # (необязательно) — общий доступ к файлам WebDAV ; Простой в использовании общий доступ к файлам на уровне пользователя для GNOME ; https://archlinux.org/packages/extra/x86_64/gnome-user-share/ ; https://gitlab.gnome.org/GNOME/gnome-user-share ; 2025-06-29 23:28 UTC
#pacman -S --noconfirm --needed power-profiles-daemon  # (необязательно) — поддержка профилей питания ; Обеспечивает возможность управления профилями мощности через D-Bus ; https://archlinux.org/packages/extra/x86_64/power-profiles-daemon/ ; https://gitlab.freedesktop.org/upower/power-profiles-daemon ; 2025-02-18 21:53 UTC
#pacman -S --noconfirm --needed rygel  # (необязательно) — обмен медиафайлами ; UPnP AV MediaServer и MediaRenderer ; https://archlinux.org/packages/extra/x86_64/rygel/ ; https://gitlab.gnome.org/GNOME/rygel ; Группы: gnome ; Обеспечивает: librygel-core-2.8.so=0-64, librygel-db-2.8.so=0-64, librygel-renderer-2.8.so=0-64, librygel-renderer-gst-2.8.so=0-64, librygel-ruih-2.8.so=0-64, librygel-server-2.8.so=0-64 ; 2025-04-30 17:36 UTC
#pacman -S --noconfirm --needed system-config-printer  # (необязательно) — настройки принтера ; Инструмент настройки принтера CUPS и апплет состояния ; https://archlinux.org/packages/extra/x86_64/system-config-printer/ ; https://github.com/OpenPrinting/system-config-printer ; 2024-12-22 13:46 UTC
pacman -S --noconfirm --needed budgie-screensaver  # Заставка для рабочего стола Budgie ; https://archlinux.org/packages/extra/x86_64/budgie-screensaver/ ; https://github.com/BuddiesOfBudgie/budgie-screensaver ; Группы: budgie ; 2024-07-01 02:16 UTC
pacman -S --noconfirm --needed budgie-session  # https://archlinux.org/packages/extra/x86_64/budgie-session/ ; https://github.com/BuddiesOfBudgie/budgie-session ; 2024-06-21 19:20 UTC
#pacman -S --noconfirm --needed network-manager-applet  # Апплет для управления сетевыми подключениями ; https://archlinux.org/packages/extra/x86_64/network-manager-applet/ ; https://gitlab.gnome.org/GNOME/network-manager-applet ; 2024-01-20 01:50 UTC
######### budgie-desktop ###########
pacman -S --noconfirm --needed budgie-desktop  # Знакомая, современная среда рабочего стола ; https://archlinux.org/packages/extra/x86_64/budgie-desktop/ ; https://github.com/BuddiesOfBudgie/budgie-desktop ; Группы: budgie ;  2025-03-25 16:27 UTC
# yay -S budgie-desktop-git  # Знакомая, современная среда рабочего стола — последняя версия git ; https://aur.archlinux.org/packages/budgie-desktop-git?all_deps=1#pkgdeps ; https://aur.archlinux.org/budgie-desktop-git.git (только для чтения, нажмите, чтобы скопировать) ; https://github.com/BuddiesOfBudgie/budgie-desktop ; Группы: budgie ; Обеспечивает: budgie-desktop ; Конфликты: budgie-desktop ; 2025-08-22 18:08 (UTC)
pacman -S --noconfirm --needed budgie-backgrounds  # (необязательно) — набор фонов по умолчанию ; Стандартный набор фоновых рисунков для рабочего стола Budgie ; https://archlinux.org/packages/extra/any/budgie-backgrounds/ ; https://github.com/BuddiesOfBudgie/budgie-backgrounds ; Группы: budgie ; 2024-02-11 13:42 UTC
pacman -S --noconfirm --needed budgie-desktop-view  # (необязательно) — значки рабочего стола ; Реализация значков рабочего стола для Budgie Desktop ; https://archlinux.org/packages/extra/x86_64/budgie-desktop-view/ ; https://github.com/BuddiesOfBudgie/budgie-desktop-view ; Группы: budgie ; 2024-01-20 18:42 UTC
pacman -S --noconfirm --needed cinnamon-translations  # Переводы для Cinnamon и Nemo ; https://archlinux.org/packages/extra/any/cinnamon-translations/ ; https://github.com/linuxmint/cinnamon-translations ; 2025-01-06 19:15 UTC
pacman -S --noconfirm --needed materia-gtk-theme  # темы GTK, рекомендованные разработчиками ; Тема Material Design для сред рабочего стола на базе GNOME/GTK+ ; https://archlinux.org/packages/extra/any/materia-gtk-theme/ ; https://github.com/nana-4/materia-theme ; Обеспечивает: materia-theme ; Заменяет: materia-theme ; Конфликты: materia-theme ; 2024-05-11 20:04 UTC
pacman -S --noconfirm --needed papirus-icon-theme  # Тема значков по умолчанию ; Тема иконок Papirus ; https://archlinux.org/packages/extra/any/papirus-icon-theme/ ; https://github.com/PapirusDevelopmentTeam/papirus-icon-theme ; 2025-05-05 04:30 UTC
### Темы и иконки - https://www.gnome-look.org/browse/ ; Gnome-look.org и pling.com — это сеть сайтов от Opendesktop.org, которые помогают находить темы, обои, иконки и другие полезные материалы в одном месте. Он никоим образом официально не связан ни с одной средой рабочего стола.
pacman -S --noconfirm --needed budgie-extras  # Дополнительные апплеты, разработанные командой Ubuntu Budgie ; Дополнительные компоненты Budgie Desktop от команды Ubuntu Budgie ; https://archlinux.org/packages/extra/x86_64/budgie-extras/ ; https://github.com/UbuntuBudgie/budgie-extras ; 2024-07-24 18:21 UTC
# pacman -S --noconfirm --needed gnome-applets  # (необязательно) ; Небольшие приложения для панели GNOME ; https://archlinux.org/packages/extra/x86_64/gnome-applets/ ; https://wiki.gnome.org/Projects/GnomeApplets ; 2025-04-30 18:56 UTC
############ Зависимости необязательные ##############
pacman -S --noconfirm --needed power-profiles-daemon  # (необязательно) — управление питанием и производительностью с панели ; Обеспечивает возможность управления профилями мощности через D-Bus ; https://archlinux.org/packages/extra/x86_64/power-profiles-daemon/ ; https://gitlab.freedesktop.org/upower/power-profiles-daemon ; 2025-02-18 21:53 UTC
pacman -S --noconfirm --needed switcheroo-control  # (необязательно) — открывает приложения с помощью дополнительного графического процессора ; Служба D-Bus для проверки доступности двух графических процессоров ; https://archlinux.org/packages/extra/x86_64/switcheroo-control/ ; https://gitlab.freedesktop.org/hadess/switcheroo-control ; 2025-09-01 09:43 UTC
# pacman -R mate-terminal --noconfirm  # Эмулятор терминала MATE ; https://archlinux.org/packages/extra/x86_64/mate-terminal/ ; https://mate-desktop.org/ ; Заменяет: mate-terminal-gtk3 ; Конфликты: mate-terminal-gtk3 ; 2024-09-11 08:05 UTC
pacman -S --noconfirm --needed gnome-terminal  #  (необязательно) ; Эмулятор терминала GNOME ; https://archlinux.org/packages/extra/x86_64/gnome-terminal/ ; https://wiki.gnome.org/Apps/Terminal ; 2025-07-20 14:50 UTC
# pacman -S --noconfirm --needed xfce4-terminal  # Современный эмулятор терминала, в первую очередь для среды рабочего стола Xfce ; https://archlinux.org/packages/extra/x86_64/xfce4-terminal/ ; https://docs.xfce.org/apps/terminal/start ; Группы: xfce4 ; 2025-03-26 22:24 UTC
pacman -S --noconfirm --needed leafpad  # Клон блокнота для GTK+ 2.0 ; https://archlinux.org/packages/extra/x86_64/leafpad/ ; http://tarot.freeshell.org/leafpad/ ; 2024-07-24 08:33 UTC
# pacman -S --noconfirm --needed pluma  # Мощный текстовый редактор для MATE ; Pluma — это форк Gedit ; https://archlinux.org/packages/extra/x86_64/pluma/ ; https://mate-desktop.org/ ; https://github.com/mate-desktop/pluma ; Группы: mate-extra ; Заменяет: pluma-gtk3 ; Конфликты: с pluma-gtk3 ; 2025-04-30 17:36 UTC
pacman -S --noconfirm --needed viewnior  # Простая, быстрая и элегантная программа для просмотра изображений ; https://archlinux.org/packages/extra/x86_64/viewnior/ ; https://siyanpanayotov.com/project/viewnior ; 2025-01-14 21:52 UTC
pacman -S --noconfirm --needed gthumb  # Браузер и просмотрщик изображений для рабочего стола GNOME ; https://archlinux.org/packages/extra/x86_64/gthumb/ ; https://wiki.gnome.org/Apps/Gthumb ; 2025-02-10 02:33 UTC
# pacman -S --noconfirm --needed eog  # Eye of Gnome: программа для просмотра и каталогизации изображений ; https://archlinux.org/packages/extra/x86_64/eog/ ; https://wiki.gnome.org/Apps/EyeOfGnome ; 2025-07-26 22:04 UTC
# pacman -S --noconfirm --needed еом  # (необязательно) или Eye of MATE ; Просмотрщик изображений для MATE ; https://archlinux.org/packages/extra/x86_64/eom/ ; https://mate-desktop.org/ ; Заменяет: eom-gtk3 ; Конфликты: с eom-gtk3 ; 2025-04-30 17:32 UTC
pacman -S --noconfirm --needed plank  # Элегантный, простой, чистый док ; https://archlinux.org/packages/extra/x86_64/plank/ ; https://launchpad.net/plank ; 2024-07-13 16:31 UTC
pacman -S --noconfirm --needed xreader  # Просмотрщик документов для файлов PDF и Postscript. Проект X-Apps ; https://github.com/linuxmint/xreader ; https://archlinux.org/packages/extra/x86_64/xreader/ ; https://man.archlinux.org/man/xreader.1.en ; https://wiki.archlinux.org/title/PDF,_PS_and_DjVu ; https://github.com/linuxmint/xreader/blob/master/help/C/xreader.md
# pacman -S --noconfirm --needed atril  # Просмотрщик документов для MATE ; https://archlinux.org/packages/extra/x86_64/atril/ ; https://mate-desktop.org/ ; Группы: mate-extra ; Заменяет: atril-gtk3 ; Конфликты: atril-gtk3 ; 2025-08-06 13:19 UTC
pacman -S --noconfirm --needed gnome-dictionary  # Найдите слова в словарном источнике ; https://archlinux.org/packages/extra/x86_64/gnome-dictionary/ ; https://wiki.gnome.org/Apps/Dictionary ; 2025-07-20 14:15 UTC
pacman -S --noconfirm --needed gnome-power-manager  # Информация и статистика мощности системы ; https://archlinux.org/packages/extra/x86_64/gnome-power-manager/ ; https://gitlab.gnome.org/GNOME/gnome-power-manager ; 2025-04-24 16:59 UTC
########## Файловый менеджер Cinnamon (форк Nautilus) ###############
pacman -S --noconfirm --needed nemo  # Файловый менеджер Cinnamon (форк Nautilus) ; https://archlinux.org/packages/extra/x86_64/nemo/ ; https://github.com/linuxmint/nemo ; 2025-03-02 11:32 UTC
######### Расширение для Nemo (nemo-extensions) ##############
#pacman -S --noconfirm --needed nemo-fileroller  # Расширение архиватора файлов для Nemo ; https://archlinux.org/packages/extra/x86_64/nemo-fileroller/ ; https://github.com/linuxmint/nemo-extensions ; 2025-06-06 14:09 UTC
pacman -S --noconfirm --needed nemo-python  # (привязки Python для Nemo): функциональные ; Привязка Python3 к компонентам Nemo ; https://archlinux.org/packages/extra/x86_64/nemo-python/ ; https://github.com/linuxmint/nemo-extensions ; 2025-06-06 14:09 UTC
#pacman -S --noconfirm --needed nemo-terminal  # функциональный (ответвление от nautilus-terminal-1.0.0) ; Встроенное окно терминала для Nemo ; https://archlinux.org/packages/extra/x86_64/nemo-terminal/ ; https://github.com/linuxmint/nemo-extensions ; 2025-06-06 14:09 UTC
######### Расширение добавить ############# https://github.com/linuxmint/nemo-extensions
#sudo pacman -S --noconfirm --needed nemo-seahorse # функциональный ; Расширение шифрования и подписи PGP для Nemo ; https://archlinux.org/packages/extra/x86_64/nemo-seahorse/ ; https://github.com/linuxmint/nemo-extensions ; 2025-06-06 14:09 UTC
#sudo pacman -S --noconfirm --needed nemo-share  # функциональный ; Расширение Samba для Nemo ; https://archlinux.org/packages/extra/x86_64/nemo-share/ ; https://github.com/linuxmint/nemo-extensions ; 2025-06-06 14:09 UTC
# sudo pacman -S --noconfirm --needed nemo-pastebin  # функциональный ; Расширение Nemo для отправки файлов на Pastebin ; https://archlinux.org/packages/extra/x86_64/nemo-pastebin/ ; https://github.com/linuxmint/nemo-extensions ; 2025-06-06 14:09 UTC
pacman -S --noconfirm --needed nemo-image-converter  # — ответвление от (nautilus-image-converter-0.3.1) ; Расширение Nemo для поворота/изменения размера файлов изображений ; https://archlinux.org/packages/extra/x86_64/nemo-image-converter/ ; https://github.com/linuxmint/nemo-extensions ; 2025-06-06 14:09 UTC
pacman -S --noconfirm --needed nemo-audio-tab  # создан на основе nemo-media-columns и nemo-emblems ; Просмотр информации об аудиотегах на вкладке свойств Nemo ; https://archlinux.org/packages/extra/x86_64/nemo-audio-tab/ ; https://github.com/linuxmint/nemo-extensions ; 2025-06-06 14:09 UTC
############ nemo-dropbox ##########
#yay -S nemo-dropbox --noconfirm  # функциональный ; Dropbox для Linux — расширение Nemo ; https://aur.archlinux.org/packages/nemo-dropbox ; https://aur.archlinux.org/nemo-dropbox.git (только для чтения, нажмите, чтобы скопировать) ; https://github.com/linuxmint/nemo-extensions ; Группы: nemo-extensions ; https://github.com/linuxmint/nemo-extensions/archive/6.4.0.tar.gz ; 2025-01-06 22:57 (UTC)
############ nemo-dropbox ##########
#git clone https://aur.archlinux.org/nemo-dropbox.git  # (только для чтения, нажмите, чтобы скопировать)
#cd nemo-dropbox
# makepkg -fsri
# makepkg -si
...
#rm -Rf nemo-dropbox
############ nemo-compare ##########
#yay -S nemo-compare --noconfirm  # функциональный ; Расширение сравнения контекстного меню для файлового менеджера Nemo ; https://aur.archlinux.org/packages/nemo-compare ; https://aur.archlinux.org/nemo-compare.git (только для чтения, нажмите, чтобы скопировать) ; https://github.com/linuxmint/nemo-extensions ; https://github.com/linuxmint/nemo-extensions/archive/6.4.0.tar.gz ; Группы: nemo-extensions ; 2025-01-06 22:56 (UTC)
############ nemo-compare ##########
#git clone https://aur.archlinux.org/nemo-compare.git  # (только для чтения, нажмите, чтобы скопировать)
#cd nemo-compare
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm    #--не спрашивать каких-либо подтверждений
...
# rm -rf nemo-compare
#rm -Rf nemo-compare
### Установить Nemo как файловый браузер по умолчанию
# https://wiki.archlinux.org/title/Nemo
#xdg-mime default nemo.desktop inode/directory application/x-gnome-saved-search
### Показать/скрыть значки на рабочем столе
#gsettings set org.nemo.desktop show-desktop-icons false
### Изменить эмулятор терминала по умолчанию для Nemo
# gsettings set org.cinnamon.desktop.default-applications.terminal exec имя-терминала
#gsettings set org.cinnamon.desktop.default-applications.terminal exec gnome-terminal
### Чтобы иметь возможность запускать скрипты оболочки из Nemo, обязательно настройте правильный аргумент для предпочитаемого терминального приложения (по умолчанию — -xgnome -terminal ).
# gsettings set org.cinnamon.desktop.default-applications.terminal exec-arg аргумент
### gsettings set org.cinnamon.desktop.default-applications.terminal exec-x gnome -terminal
### Открытие одного файла открывает все файлы
#gsettings set org.nemo.preferences image-viewers-with-external-sort "[]"
### Установить сочетание клавиш для «Открыть в терминале» для Gnome:
# gsettings set org.gnome.desktop.interface can-change-accels true
# Отредактируйте или создайте ~/.gnome2/accels/nemoи добавьте следующую строку (заменив «F4» на нужную комбинацию клавиш):
# ~/.gnome2/accels/nemo
# (gtk_accel_path "<Actions>/DirViewActions/OpenInTerminal" "F4")
# <Alt>, <Primary>, и <Shift> могут использоваться как ключевые модификаторы (например, <Primary><Shift>t).
# gtk_accel_path "<Actions>/DirViewActions/OpenInTerminal" "<Ctrl><Alt>t"
### Nemo Actions (Действия Немо)
# Nemo позволяет пользователю добавлять новые пункты в контекстное меню. Файл /usr/share/nemo/actions/sample.nemo_actionсодержит пример действия Nemo. Каталоги для размещения файлов пользовательских действий:
# /usr/share/nemo/actions/для общесистемных действий
# ~/.local/share/nemo/actions/для действий пользователя
# Файлы действий должны иметь .nemo_action расширение .
### Просмотр текущих настроек сенсорной панели
# gsettings list-recursively org.gnome.desktop.peripherals.touchpad
####################
pacman -S --noconfirm --needed gnome-dictionary  # Найдите слова в словарном источнике ; https://archlinux.org/packages/extra/x86_64/gnome-dictionary/ ; https://wiki.gnome.org/Apps/Dictionary ;  2025-07-20 14:15 UTC
pacman -S --noconfirm --needed gnome-calendar  # Простое и красивое приложение-календарь, идеально подходящее для рабочего стола GNOME ; https://archlinux.org/packages/extra/x86_64/gnome-calendar/ ; https://apps.gnome.org/Calendar ; Группы: gnome ;  Разделенные пакеты: gnome-calendar ; 2025-03-22 14:53 UTC
pacman -S --noconfirm --needed gnome-usage  # Просмотр информации об использовании системных ресурсов ; https://archlinux.org/packages/extra/x86_64/gnome-usage/ ; https://wiki.gnome.org/Apps/Usage ; 2025-04-23 09:38 UTC
### Хороший способ просмотреть информацию об использовании системных ресурсов, таких как память и дисковое пространство. Примечательные особенности: Контролируйте использование процессора, памяти, диска и сети. Поиск приложения, которое использует все ресурсы процессора.Разрешить пользователю принудительно закрывать приложения.
clear
echo ""
echo "  Установка Budgie DE успешно выполнена "
echo ""
echo -e "${GREEN}==> ${NC}Настройка автовхода без DM (Display manager) менеджера входа в Budgie"
echo " Давайте проанализируем действия, которые выполняются. "
echo " 1 — Если вам нужен автовход без DM (Display manager), тогда укажите "1" "
echo " Вы хотите автологин определенного пользователя, автоматический запуск Иксов, запуск окружения (KDE, XFCE, Gnom и т.д.). "
echo " Всё можно сделать без использования DM (например SDDM, LightDM и т.д.), поскольку реализация автозагрузки окружения реализован через startx. "
...
while
    ...
    0 - Нет буду использовать DM (Display manager): " in_budgiewm   # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_budgiewm" =~ [^10] ]]
do
    :
done
if [[ $in_budgiewm  == 0 ]]; then
echo ""
echo " Буду использовать DM (Display manager) "
sleep 01
elif [[ $in_budgiewm  == 1 ]]; then
echo ""
echo " Действия по настройке автовхода без DM (Display manager) "
echo " Поскольку реализация автозагрузки окружения реализована через startx - (иксы), то если Вы установили X.Org Server возможно пакет (xorg-xinit) - уже установлен "
pacman -S xorg-xinit --noconfirm
...
############# Справка ##############
### Просто установите Xscreensaver:
# Затем добавьте его в автозагрузку приложений (щелкните правой кнопкой мыши по рабочему столу > Параметры рабочего стола > Автозапуск > Добавить команду):
# Xscreensaver
# Хранитель экрана и блокировщик для X Window System
# xscreensaver
# или xscreensaver -nosplash
# или xscreensaver -no-splash
# Выйдите из системы и подключитесь снова.
# Настройки Xsreensaver вы найдете в главном меню.
# Спасибо за помощь и указание правильного направления. Я слишком много об этом думал.
# Я сделал, как вы сказали.
# Я выяснил, что нужно добавить
# -nosplash в команду автозапуска.
# Также я отключил заставку с волнистым попугайчиком из автозагрузки.
# Да, или «-no-splash».
# Вы правы: всегда проверяйте варианты!
#  <app name> -h / --help
# budgie-panel --reset --replace &!  # По мере внедрения новых функций может быть полезно сбросить конфигурацию до значений по умолчанию, чтобы убедиться, что всё работает корректно. Чтобы сбросить всё дерево конфигурации.
# Budgie ArchWiki:
...
# https://archlinux.org/groups/x86_64/budgie/
...
echo -e "${YELLOW}==> Примечание! ${BOLD} *DM - Менеджер дисплеев, или Логин менеджер, обычно представляет собой графический пользовательский интерфейс, который отображается в конце процесса загрузки вместо оболочки по умолчанию. ${NC}"
echo -e "${MAGENTA}:: ${BOLD}Существуют различные реализации дисплейных менеджеров, обычно с определенным количеством настроек и тематических функций, доступных для каждого из них. Согласно аннотации ArchWiki рассмотрим список графических менеджеров дисплея, варианты установки DM (Display manager), и их совместимость с различными вариантами DE (средами рабочего стола). ${NC}"
echo " Домашняя страница: https://wiki.archlinux.org/title/Display_manager "
echo -e "${BLUE}:: ${NC}1 — LightDM — Диспетчер дисплеев между рабочими столами, может использовать различные интерфейсы, написанные на любом наборе инструментов, вариант — "1" "
echo " LightDM - идёт как основной DM в Xfce (окружение рабочего стола), совместим с Deepin, и т.д.. Его ключевые особенности: Кросс-десктоп - поддерживает различные настольные технологии, поддерживает различные технологии отображения (X, Mir, Wayland ...), низкое использование памяти и высокая производительность. Поддерживает гостевые сессии, поддерживает удаленный вход (входящий - XDMCP, VNC, исходящий - XDMCP). "
echo -e "${CYAN}:: ${NC}2 — LXDM - Диспетчер отображения LXDE, вариант — "2" "
echo " LXDE — идёт как основной DM в LXDE (окружение рабочего стола), совместим с Xfce, Mate, Deepin, и т.д.. Это легкий диспетчер отображения, пользовательский интерфейс реализован с помощью GTK 2. LXDM не поддерживает протокол XDMCP, альтернатива - LightDM. "
echo -e "${CYAN}:: ${NC}3 — GDM — Диспетчер отображения GNOME, вариант — "3" "
echo " GNOME Display Manager (GDM) — это программа, которая управляет серверами графического дисплея и обрабатывает логины пользователей в графическом режиме. "
echo -e "${CYAN}:: ${NC}4 — SDDM — Диспетчер отображения на основе QML и преемник KDM, вариант — "4" "
echo " SDDM — рекомендуется для KDE Plasma Desktop, и LXQt (окружение рабочего стола). Simple Desktop Display Manager (SDDM) — это диспетчер дисплея (графическая программа входа в систему) для оконных систем X11 и Wayland. KDE выбрала SDDM в качестве преемника KDE Display Manager для KDE Plasma 5. "
echo -e "${CYAN}:: ${NC}5(0) — Если Вам не нужен DM (Display manager), то выбирайте вариант — "0" "
echo -e "${YELLOW}:: ${BOLD}Примечание! *Если Вы при установке i3, сделали выбор без использования DM, то DM не ставим! ${NC}"
echo -e "${YELLOW} Примечание! ${BOLD} *Будьте внимательны! Ориентируйтесь на установленное DE. Процесс установки был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. ${NC}"
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
...
sudo pacman -S --noconfirm --needed lightdm  # Легкий менеджер отображения ; https://archlinux.org/packages/extra/x86_64/lightdm/ ; https://github.com/canonical/lightdm ; 2024-02-11 19:34 UTC
sudo pacman -S --noconfirm --needed lightdm-gtk-greeter  # (необязательно) — приветствие GTK ; GTK + приветствие для LightDM ; https://archlinux.org/packages/extra/x86_64/lightdm-gtk-greeter/ ; https://github.com/Xubuntu/lightdm-gtk-greeter ; 2024-03-05 21:51 UTC
### sudo pacman -S --noconfirm --needed lightdm-slick-greeter  # Элегантный приветственный элемент LightDM ; https://archlinux.org/packages/extra/x86_64/lightdm-slick-greeter/ ; https://github.com/linuxmint/slick-greeter ; 2025-08-13 17:13 UTC
### Основанный на GTK экран приветствия, сосредоточенный на внешнем виде сильнее чем lightdm-gtk-greeter ; является форком lightdm-unity-greeter и экраном по умолчанию в Linux Mint.
sudo pacman -S --noconfirm --needed lightdm-gtk-greeter-settings  # Редактор настроек для LightDM GTK+ Greeter ; https://archlinux.org/packages/extra/any/lightdm-gtk-greeter-settings/ ; https://github.com/xubuntu/lightdm-gtk-greeter-settings ; 2024-12-22 13:31 UTC
sudo pacman -S --noconfirm --needed light-locker  # Простой шкафчик сессий для LightDM ; https://archlinux.org/packages/extra/x86_64/light-locker/ ; https://github.com/the-cavalry/light-locker ; 2023-05-19 17:03 UTC
# pacman -S light-locker --noconfirm  # Простой шкафчик сессий для LightDM
### Приветствие для LightDM #####
sudo pacman -S --noconfirm --needed lightdm-webkit2-greeter  # Приветствие LightDM, использующее WebKit2 для тематизации через HTML/JavaScript ; https://archlinux.org/packages/extra/x86_64/lightdm-webkit2-greeter/ ; https://github.com/antergos/web-greeter ; 2025-04-28 22:15 UTC
sudo pacman -S --noconfirm --needed lightdm-webkit-theme-litarvan  # Современная и полнофункциональная тема Webkit2 LightDM ; https://archlinux.org/packages/extra/any/lightdm-webkit-theme-litarvan/ ; https://github.com/Litarvan/lightdm-webkit-theme-litarvan ; 2023-05-19 16:20 UTC
# sudo pacman -S --noconfirm --needed lightdm-pantheon-greeter  # Экран приветствия из проекта elementary OS ; Приветствие Pantheon для LightDM ; https://archlinux.org/packages/extra/x86_64/lightdm-pantheon-greeter/ ; https://github.com/elementary/greeter ; 2025-03-14 11:19 UTC
...
echo -e "${YELLOW} Примечание! ${BOLD} *Будьте внимательны! Процесс установки был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. Вы можете пропустить этот шаг, если не уверены в правильности выбора. ${NC}"
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
...
### Настройка systemd-timesyncd: https://redos.red-soft.ru/base/redos-7_3/7_3-administation/7_3-timedate/7_3-systemd-timesyncd/
####################################

clear
echo ""
echo -e "${GREEN}==> ${NC}Установить сетевые утилиты Networkmanager?"
echo -e "${BLUE}:: ${NC}'Networkmanager' — сервис для работы интернета. NetworkManager можно установить с пакетом networkmanager, который содержит демон, интерфейс командной строки (nmcli) и интерфейс на основе curses (nmtui). Вместе с собой устанавливает программы (пакеты) для настройки сети. "
...
pacman -S --noconfirm --needed networkmanager  # Диспетчер сетевых подключений и пользовательские приложения ; https://networkmanager.dev/ ; https://archlinux.org/packages/extra/x86_64/networkmanager/ ; Разделенные пакеты: libnm, networkmanager-docs, nm-cloud-setup ; 2025-09-17 18:06 UTC
pacman -S --noconfirm --needed networkmanager-openvpn  # Плагин NetworkManager VPN для OpenVPN ; https://networkmanager.dev/docs/vpn/ ; https://archlinux.org/packages/extra/x86_64/networkmanager-openvpn/ ; Разделенные пакеты: networkmanager-vpn-plugin-openvpn2025-08-26 13:45 UTC
pacman -S --noconfirm --needed network-manager-applet  # Апплет для управления сетевыми подключениями ; https://gitlab.gnome.org/GNOME/network-manager-applet ; https://archlinux.org/packages/extra/x86_64/network-manager-applet/ ; 2024-01-20 01:50 UTC
pacman -S --noconfirm --needed ppp  # Демон, реализующий протокол точка-точка для коммутируемого доступа в сеть ; https://archlinux.org/packages/core/x86_64/ppp/ ; https://www.samba.org/ppp/ ; 2025-01-13 19:40 UTC
pacman -S --noconfirm --needed nm-connection-editor  # Редактор подключений и виджеты графического интерфейса NetworkManager (https://wiki.gnome.org/Projects/NetworkManager/) ; https://gitlab.gnome.org/GNOME/network-manager-applet ; https://archlinux.org/packages/extra/x86_64/nm-connection-editor/ ; Заменяет: libnm-gtk<1.8.18-1 ; Конфликты: libnm-gtk<1.8.18-1 ; 2024-01-20 01:50 UTC
...
pacman -S --noconfirm --needed pptpclient #  Клиент для проприетарного протокола туннелирования точка-точка от Microsoft, PPTP ; http://pptpclient.sourceforge.net/ ; https://archlinux.org/packages/core/x86_64/pptpclient/ ; 2023-01-26 10:03 UTC
pacman -S --noconfirm --needed rp-pppoe  #  Протокол точка-точка Roaring Penguin через клиент Ethernet ; https://dianne.skoll.ca/projects/rp-pppoe ; https://archlinux.org/packages/extra/x86_64/rp-pppoe/ ; 2025-03-22 14:09 UTC
pacman -S --noconfirm --needed xl2tpd  # Реализация L2TP с открытым исходным кодом, поддерживаемая Xelerance Corporation ; https://github.com/xelerance/xl2tpd ; https://archlinux.org/packages/extra/x86_64/xl2tpd/ ; 2025-04-20 13:06 UTC
pacman -S --noconfirm --needed networkmanager-l2tp  # Поддержка L2TP для NetworkManager ; https://archlinux.org/packages/extra/x86_64/networkmanager-l2tp/ ; https://github.com/nm-l2tp/NetworkManager-l2tp ; Заменяет: networkmanager-l2tp-gtk3, networkmanager-l2tp-gtk4 ; Конфликты: networkmanager-l2tp-gtk3, networkmanager-l2tp-gtk4 ; 2025-08-26 13:45 UTC
echo ""
...
pacman -Syy  # обновление баз пакмэна (pacman)
pacman -S --noconfirm --needed ttf-dejavu  # Семейство шрифтов на основе Bitstream Vera Fonts с более широким набором символов ; https://archlinux.org/packages/extra/any/ttf-dejavu/ ; https://dejavu-fonts.github.io/ ; Обеспечивает: ttf-font ; 2024-03-18 09:09 UTC
pacman -S --noconfirm --needed ttf-liberation  # Шрифты Red Hats Liberation ; Семейство шрифтов, нацеленное на метрическую совместимость с Arial, Times New Roman и Courier New ; https://archlinux.org/packages/extra/any/ttf-liberation/ ; https://github.com/liberationfonts/liberation-fonts ; Обеспечивает: ttf-font ; 2024-07-12 23:57 UTC
pacman -S --noconfirm --needed xorg-fonts-encodings  # Файлы кодировки шрифтов X.org  # https://archlinux.org/packages/extra/any/xorg-fonts-encodings/ ; https://gitlab.freedesktop.org/xorg/font/encodings ; Группы: xorg , xorg-fonts ; 2024-03-03 12:28 UTC
pacman -S --noconfirm --needed ttf-anonymous-pro  # Семейство из четырех шрифтов фиксированной ширины, разработанных специально с учетом особенностей кодирования ; https://archlinux.org/packages/extra/any/ttf-anonymous-pro/ ; https://www.marksimonson.com/fonts/view/anonymous-pro ; 2024-07-14 00:37 UTC
pacman -S --noconfirm --needed noto-fonts-emoji  # Шрифт Google Noto Color Emoji ; https://archlinux.org/packages/extra/any/noto-fonts-emoji/ ; https://www.google.com/get/noto/ ; Обеспечивает: fonts-emoji (шрифт эмодзи) ; 2025-09-16 05:57 UTC
pacman -S --noconfirm --needed terminus-font  # Моноширинный растровый шрифт (для X11 и консоли) ; https://archlinux.org/packages/extra/any/terminus-font/ ; http://terminus-font.sourceforge.net/ ; Заменяет: terminus-font-otb ; Конфликты: с terminus-font-otb ; 2025-06-29 23:44 UTC
pacman -S --noconfirm --needed awesome-terminal-fonts  # Шрифты/значки для линий электропередач ; https://archlinux.org/packages/extra/any/awesome-terminal-fonts/ ; https://github.com/gabrielelana/awesome-terminal-fonts ; 2024-07-12 00:03 UTC
pacman -S --noconfirm --needed ttc-iosevka  # Семейство шрифтов, предназначенное для кодирования, использования в терминалах и технической документации ; https://archlinux.org/packages/extra/any/ttc-iosevka/ ; https://typeof.net/Iosevka/ ; Разделенные пакеты: ttc-iosevka-aile , ttc-iosevka-curly , ttc-iosevka-curly-slab , ttc-iosevka-etoile , ttc-iosevka-slab ; 2025-09-08 09:04 UTC
clear
echo ""
...
# pacman -S --noconfirm --needed btrfs-assistant  # Приложение для управления подтомами BTRFS и снимками Snapper ; https://archlinux.org/packages/extra/x86_64/btrfs-assistant/ ; https://gitlab.com/btrfs-assistant/btrfs-assistant ; 2025-09-19 13:52 UTC
...
pacman -S --noconfirm --needed fatresize  # Утилита для изменения размера файловых систем FAT с помощью libparted ; https://archlinux.org/packages/extra/x86_64/fatresize/ ; https://sourceforge.net/projects/fatresize/ ; 2024-07-01 22:08 UTC
pacman -S --noconfirm --needed fuse2  # Библиотека, позволяющая реализовать файловую систему в пользовательской программе ; https://archlinux.org/packages/extra/x86_64/fuse2/ ; https://github.com/libfuse/libfuse ; Обеспечивает: fuse=2.9.9 ; Заменяет: fuse ; Конфликты: fuse ; 2024-07-12 15:36 UTC
pacman -S --noconfirm --needed fuse-exfat  # Модуль FUSE - Утилиты для файловой системы exFAT
###############
echo ""
echo -e "${BLUE}:: ${NC}Установка Инструментов, которые выводят список устройств SCSI / SATA "
pacman -S --noconfirm --needed sg3_utils  # Общие утилиты SCSI ; http://sg.danny.cz/sg/sg3_utils.html ; https://archlinux.org/packages/extra/x86_64/sg3_utils/ ; 2024-07-30 18:50 UTC
...
pacman -S --noconfirm --needed grsync  # GTK + GUI для rsync для синхронизации папок, файлов и создания резервных копий ; https://archlinux.org/packages/extra/x86_64/grsync/ ; http://www.opbyte.it/grsync/ ; 2024-07-12 16:53 UTC
pacman -S --noconfirm --needed cmake  # Кросс-платформенная система сборки с открытым исходным кодом ; https://archlinux.org/packages/extra/x86_64/cmake/ ; https://www.cmake.org/ ; 2025-08-27 22:49 UTC
pacman -S --noconfirm --needed extra-cmake-modules  # Дополнительные модули и скрипты для CMake ; https://archlinux.org/packages/extra/any/extra-cmake-modules/ ; https://community.kde.org/Frameworks ; 2025-09-13 15:00 UTC
pacman -S --noconfirm --needed glibc  # Библиотека GNU C ; https://www.gnu.org/software/libc ; https://archlinux.org/packages/core/x86_64/glibc/ ; 2025-08-14 14:23 UTC
pacman -S --noconfirm --needed glibc-locales  # Готовые локали для библиотеки GNU C ; https://archlinux.org/packages/core/x86_64/glibc-locales/ ; https://www.gnu.org/software/libc ; 2025-08-14 14:23 UTC
pacman -S --noconfirm --needed scons  # (делать) ; Расширяемая утилита сборки на базе Python ; https://archlinux.org/packages/extra/any/scons/ ; https://scons.org/ ; 2025-04-08 07:34 UTC
...
pacman -S --noconfirm --needed ninja  # Небольшая система сборки с упором на скорость ; https://archlinux.org/packages/extra/x86_64/ninja/ ; https://ninja-build.org/ ; 2024-12-22 13:54 UTC
pacman -S --noconfirm --needed perf  # Инструмент аудита производительности ядра Linux ; https://archlinux.org/packages/extra/x86_64/perf/ ; https://www.kernel.org/ ; 2025-08-14 16:51 UTC
...
pacman -S --noconfirm --needed nano nano-syntax-highlighting  # Улучшения подсветки синтаксиса в редакторе Nano (2020.10.10-2) ;https://archlinux.org/packages/extra/any/nano-syntax-highlighting/ ; https://github.com/galenguyer/nano-syntax-highlighting ; 2025-09-17 13:14 UTC
pacman -S --noconfirm --needed bash  # Оболочка GNU Bourne Again ; https://archlinux.org/packages/core/x86_64/bash/ ; https://www.gnu.org/software/bash/bash.html ; 2025-08-02 12:29 UTC
pacman -S --noconfirm --needed bash-completion  # Программируемое завершение для оболочки bash ; https://archlinux.org/packages/extra/any/bash-completion/ ; https://github.com/scop/bash-completion ; 2025-01-16 07:53 UTC
pacman -S --noconfirm --needed ccache  # Кэш компилятора, который ускоряет перекомпиляцию за счет кеширования предыдущих ; https://archlinux.org/packages/extra/x86_64/ccache/ ; https://ccache.dev/ ; 2025-09-18 05:21 UTC
pacman -S --noconfirm --needed squashfs-tools  # Инструменты для squashfs, файловой системы Linux с высокой степенью сжатия, доступной только для чтения ; https://archlinux.org/packages/extra/x86_64/squashfs-tools/ ; https://github.com/plougher/squashfs-tools ; 2025-08-20 21:39 UTC
pacman -S --noconfirm --needed archinstall  # Еще один пошаговый/автоматизированный установщик Arch Linux с изюминкой ; https://github.com/archlinux/archinstall ; https://archlinux.org/packages/extra/any/archinstall/ ; 2025-09-24 22:12 UTC
pacman -S --noconfirm --needed devtools  # Инструменты для сопровождающих Arch Linux пакетов ; https://gitlab.archlinux.org/archlinux/devtools ; https://archlinux.org/packages/extra/any/devtools/ ; 2025-08-29 21:07 UTC
...
pacman -S --noconfirm --needed sed  # Редактор потока GNU ; https://www.gnu.org/software/sed/ ; https://archlinux.org/packages/core/x86_64/sed/ ; 2023-03-05 20:39 UTC
pacman -S --noconfirm --needed hashcat  # Многопоточная расширенная утилита восстановления паролей ; https://hashcat.net/hashcat ; https://archlinux.org/packages/extra/x86_64/hashcat/ ; Обеспечивает: libhashcat.so=7.1.2-64 ; 2025-08-25 14:37 UTC
...
### bat — это улучшенная версия нативной команды cat. Она включает в себя такие функции, как подсветка синтаксиса, интеграция с Git, автоматическое постраничное вывода и многое другое. * Просмотр содержимого файла сначала с помощью cat, а затем с помощью bat.
pacman -S --noconfirm --needed diff-so-fancy  # Красивые дифференциалы с подсветкой и другими функциями ; https://archlinux.org/packages/extra/any/diff-so-fancy/ ; https://github.com/so-fancy/diff-so-fancy ; 2023-12-18 18:50 UTC
### Это инструмент, который преобразует вывод утилиты diff в более понятный для человека вид. Он ускоряет разработку, предоставляя более простой способ сравнения последних изменений. С diff-so-fancy можно сосредоточиться на качестве кода, а не разбираться, что означают все эти + и -.
### Для интеграции diff-so-fancy с git:
# git config --global core.pager "diff-so-fancy | less --tabs=4 -RFX"
# git config --global interact.diffFilter "diff-so-fancy --patch"
# git config --global color.ui true
# Теперь мы можем просто использовать команду git diff для просмотра последних изменений.
pacman -S --noconfirm --needed fzf  # Нечеткий поиск в командной строке ; https://archlinux.org/packages/extra/x86_64/fzf/ ; https://github.com/junegunn/fzf ; 2025-08-31 13:38 UTC
### Это интерактивный фильтр Unix для командной строки, который можно использовать с любым списком: файлами, историей команд, процессами, именами хостов, закладками, коммитами git и т. д. ctrl + t для навигации по файловой системе. ctrl + r для навигации по старым командам.
echo ""
echo -e "${BLUE}:: ${NC}Установка Редактор dconf (пакеты dconf и dconf-editor) "
echo -e " Редактор dconf — общий инструмент для настройки GNOME 3, Unity, MATE, Cinnamon и др. DE "
###  Файл dconf, также называют системным реестром Linux.
### Этот файл двоичный, создается в момент создания профиля нового пользователя.
pacman -S --noconfirm --needed dconf  # требуется для значений, которые хранятся только в DConf + Fallback для GSettings ; Система базы данных конфигурации ; https://archlinux.org/packages/extra/x86_64/dconf/ ; https://wiki.gnome.org/Projects/dconf ; Обеспечивает: libdconf.so=1-64 ; 2024-07-12 03:08 UTC
pacman -S --noconfirm --needed lib32-dconf  # Низкоуровневая система конфигурации (32-бит) ; https://archlinux.org/packages/multilib/x86_64/lib32-dconf/ ; https://live.gnome.org/dconf ; 2024-09-07 11:02 UTC
pacman -S --noconfirm --needed dconf-editor  # Редактор dconf GSettings для GNOME ; https://archlinux.org/packages/extra/x86_64/dconf-editor/ ; https://apps.gnome.org/DconfEditor/ ; Группы: gnome-extra ; 2025-04-16 11:05 UTC
############ shfmt ##############
...
pacman -S --noconfirm --needed bind  # Полная, портативная реализация протокола DNS ; https://www.isc.org/software/bind/ ; https://archlinux.org/packages/extra/x86_64/bind/ ; Обеспечивает: bind-tools, dns-server, dnsutils ; Заменяет: bind-tools, dnsutils, host ; Конфликты: с  bind-tools, dnsutils ; 2025-09-10 16:08 UTC
# dig @1.1.1.1 archlinux.org  # для опроса DNS-серверов
pacman -S --noconfirm --needed ldns  # Библиотека быстрого DNS, поддерживающая последние RFC ; Библиотека быстрого DNS, поддерживающая последние RFC ; https://www.nlnetlabs.nl/projects/ldns/ ; https://archlinux.org/packages/core/x86_64/ldns/ ; Обеспечивает: drill, libldns.so=3-64 ; 2024-09-07 19:04 UTC
# drill archlinux.org | grep "Query time"
#####################
echo ""
echo -e "${BLUE}:: ${NC}Установка пакетов Реализации виртуальной файловой системы для GIO "
pacman -S --noconfirm --needed gvfs  # Реализация виртуальной файловой системы для GIO (Разделенные пакеты: gvfs-afc, gvfs-goa, gvfs-google, gvfs-gphoto2, gvfs-mtp, еще…) ; https://archlinux.org/packages/extra/x86_64/gvfs/ ; https://gitlab.gnome.org/GNOME/gvfs ; Заменяет:    gvfs-afp, gvfs-obexftp ; 2025-09-22 17:12 UTC
pacman -S --noconfirm --needed gvfs-mtp  # Реализация виртуальной файловой системы для GIO - бэкэнд MTP (Android, медиаплеер) ; https://archlinux.org/packages/extra/x86_64/gvfs-mtp/ ; https://gitlab.gnome.org/GNOME/gvfs ; 2025-06-14 16:09 UTC
pacman -S --noconfirm --needed gvfs-afc  # Реализация виртуальной файловой системы для GIO (серверная часть AFC; мобильные устройства Apple) ; https://archlinux.org/packages/extra/x86_64/gvfs-afc/ ; https://gitlab.gnome.org/GNOME/gvfs ; 2025-06-14 16:09 UTC
pacman -S --noconfirm --needed gvfs-goa  # Реализация виртуальной файловой системы для GIO - бэкэнд Gnome Online Accounts (например, OwnCloud) ; https://gitlab.gnome.org/GNOME/gvfs ; https://archlinux.org/packages/extra/x86_64/gvfs-goa/ ; 2025-06-14 16:09 UTC
# pacman -S --noconfirm --needed gvfs-google  # Реализация виртуальной файловой системы для GIO — бэкэнд Google Drive (серверная часть Google Диска ; https://gitlab.gnome.org/GNOME/gvfs ; https://archlinux.org/packages/extra/x86_64/gvfs-google/ ; 2025-06-14 16:09 UTC
pacman -S --noconfirm --needed gvfs-gphoto2  # Реализация виртуальной файловой системы для GIO (бэкэнд gphoto2; камера PTP, медиаплеер MTP) ; https://archlinux.org/packages/extra/x86_64/gvfs-gphoto2/ ; https://gitlab.gnome.org/GNOME/gvfs ; 2025-06-14 16:09 UTC
pacman -S --noconfirm --needed gvfs-nfs  # Реализация виртуальной файловой системы для GIO (серверная часть NFS) ; https://archlinux.org/packages/extra/x86_64/gvfs-nfs/ ; https://gitlab.gnome.org/GNOME/gvfs ; 2025-06-14 16:09 UTC
pacman -S --noconfirm --needed gvfs-smb  # Реализация виртуальной файловой системы для GIO (серверная часть SMB / CIFS; клиент Windows) ; https://archlinux.org/packages/extra/x86_64/gvfs-smb/ ; https://gitlab.gnome.org/GNOME/gvfs ; 2025-06-14 16:09 UTC
...
####### Использование диска #############
pacman -S --noconfirm --needed duf  # Использование диска/Бесплатная утилита ; https://archlinux.org/packages/extra/x86_64/duf/ ; https://github.com/muesli/duf ; 2025-09-09 16:50 UTC
### Использование диска/Бесплатная утилита (Linux, BSD, macOS и Windows) ; Функции: Удобный и красочный вывод. Подстраивается под тему и ширину вашего терминала. Сортируйте результаты в соответствии с вашими потребностями. Группы и фильтры устройств. Может удобно выводить JSON.
echo ""
echo " Установка утилит (пакетов) выполнена "
sleep 01
...
echo ""
echo " Обновление утилит (пакетов) выполнена "
sleep 01
...
echo -e "${YELLOW}==> Примечание! ${NC}*Сейчас Вы можете пропустить установку "AUR". Или вы сможете установить "AUR Helper" уже из установленной системы. "
echo -e "${YELLOW}==> Внимание! ${NC}Во время установки "AUR", Вас попросят ввести (Пароль пользователя) для $username "
echo ""
echo -e "${GREEN}==> ${NC}Установить AUR Helper (yay) или (pikaur)?"
echo -e "${MAGENTA}:: ${NC} AUR - Пользовательский репозиторий, поддерживаемое сообществом хранилище ПО, в который пользователи загружают скрипты для установки программного обеспечения. В AUR — есть практически всё, что можно установить на Linux. В том числе и программы, которые для других дистробутивов пришлось бы собирать из исходников. "
...
sleep 01
 ...
 echo ""
 echo " Установка дополнительных утилит (пакетов) (go) "
...
pacman -Qi go | grep Reason  # Причина установки: Установлен как зависимость для другого пакета
 echo ""
 echo " Установка утилиты (пакета) AUR Helper (yay) "
  cd /home/$username
...
pacman -Syu  # Обновим вашу систему (базу данных пакетов)
 echo ""
 echo " Установка утилиты (пакета) AUR Helper (pikaur) "
  cd /home/$username
...
pacman -Syu  # Обновим вашу систему (базу данных пакетов)
 echo ""
 echo " Установка утилиты (пакета) AUR Helper (yay-bin) "
  cd /home/$username
...
############ Справка ##############
### Опция -u в команде sudo в Linux означает, что нужно выполнить команду от имени другого пользователя, а не только от суперпользователя (root). Это позволяет: запустить процесс от имени специального технического аккаунта; выполнить команду без полного переключения на другого пользователя.
# По умолчанию sudo выполняет команду от имени root, но с помощью -u можно указать и любого другого пользователя системы.
### Синтаксис:
# sudo -u имя_пользователя команда.
# В этой конструкции: имя_пользователя — имя пользователя, от имени которого нужно выполнить команду; команда — команда, которую нужно выполнить.
# Если имя пользователя не указано, sudo по умолчанию выполняет команду от имени
### Пример:
# sudo -u postgres psql — откроет оболочку PostgreSQL от имени пользователя postgres, не выходя из своей сессии.
# sudo -u john ls — запустит команду ls от имени пользователя john.
### Chown -R в команде chown в Linux означает, что изменения будут применяться не только к указанному каталогу, но и ко всем его подкаталогам. Другими словами, рекурсивно изменится владелец всех вложенных директорий и файлов.
#####################################

clear
echo ""
echo -e "${GREEN}==> ${NC}Установить AUR Helper Paru (paru) или (paru-bin) — Помощник для оболочки Pacman?"
echo -e "${YELLOW}==> Примечание! ${BOLD} **Сразу хочу предупредить об одной особенности использования paru. Перед установкой некоторых пакетов он открывает файл с конфигурацией сборки. Я его обычно не просматриваю, так как доверяю помощнику, потому, чтобы закрыть этот файл нужно нажать клавишу Q. ${NC}"
echo -e "${MAGENTA}:: ${BOLD}Paru — это стандартный помощник для оболочки Pacman в AUR с множеством функций и минимальным взаимодействием. AUR или пользовательский репозиторий Archlinux. Для работы с ним необходимы так называемые helpers или помощники. До последнего времени в качестве помощника я использовал yay, но в один из дней я решил попробовать другие помощник и первым среди результатов поиска мне попался paru. На протяжении пары недель активно пользовался им, теперь готов поделиться своими впечатлениями и рассказать о том, и помочь установить этот helper. ${NC}"
...
clear
echo ""
echo -e "${GREEN}==> ${NC}Установим Oh My Zsh - пакет (oh-my-zsh-git) и Powerlevel10k - пакет (zsh-theme-powerlevel10k-git) тема для Zsh, а также несколько плагинов, библиотек и шрифтов из AUR (Arch User Repository)? "
echo -e "${MAGENTA}:: ${BOLD}Zsh — одна из современных командных оболочек UNIX, использующаяся непосредственно как интерактивная оболочка, либо как скриптовый интерпретатор. Zsh не использует readline для ввода команд пользователем. Вместо этого используется собственный редактор ZLE (Zsh Line Editor). Oh My Zsh — это восхитительный, открытый исходный код, поддерживаемый сообществом фреймворк для управления конфигурацией Zsh. фреймворк Oh My Zsh, который вы надеюсь установите, далее позволит настраивать ее и кастомизировать с помощью тем и плагинов. "
echo " Домашняя страница: https://github.com/ohmyzsh/ohmyzsh ; (https://aur.archlinux.org/packages/oh-my-zsh-git). "
echo -e "${BLUE}:: ${NC}Powerlevel10k — это тема для ZSH, гибкая и в то же время простая в плане настройки терминальной темы, которая меняет обычные команды оболочки на красочные команды. Чтобы установить powerlevel10k, нужно установить Oh My Zsh. Оба инструмента имеют открытый исходный код на GitHub. Также можно использовать шрифт nerd, чтобы сделать шрифт темы powerlevel10k более красивым. После обновления powerlevel9k до powerlevel10k настроить тему стало легче. "
echo -e "${YELLOW}=> Примечание! ${BOLD}Обратите внимание, что это изменение не мгновенное, и вам нужно будет выйти из системы и снова войти в нее, чтобы оно вступило в силу. ${NC}"
echo " После этого снова проверьте переменную окружения SHELL, чтобы подтвердить изменение: echo $SHELL . "
...
echo ""
echo " Обновление баз данных пакетов, и системы "
pacman -Syy  # обновление баз пакмэна (pacman)
...
echo " gitstatus.git успешно добавлен в установлен "
 echo ""
 echo " Установка утилит (пакетов) Oh My Zsh! "
##### oh-my-zsh-git ######
...
pacman -S --noconfirm --needed starship  # Подсказка для астронавтов по кросс-оболочке ; https://archlinux.org/packages/extra/x86_64/starship/ ; https://starship.rs/ ; 2025-04-27 20:40 UTC
# curl -sS https://starship.rs/install.sh | sh  # Установить последнюю версию starship ; Чтобы обновить сам Starship, перезапустите указанный выше скрипт. Он заменит текущую версию, не затрагивая конфигурацию Starship.
...
echo ""
echo -e "${GREEN}==> ${NC}Установить Nord-Xfce-Terminal пакет (nord-xfce-terminal) - элегантная цветовая тема терминала XFCE?"
echo -e "${YELLOW}==> Примечание! ${BOLD} *Xfce Terminal — это легкое и простое в использовании приложение-эмулятор терминала со множеством расширенных функций, включая раскрывающиеся списки, вкладки, неограниченную прокрутку, полноцветную настройку, шрифты, прозрачный фон и многое другое. ${NC}"
...
sudo pacman -Syy  # обновление баз пакмэна (pacman)
  cd /home/$username
  git clone https://aur.archlinux.org/nord-xfce-terminal.git  # https://github.com/nordtheme/xfce-terminal.git
...
clear
echo -e "${MAGENTA}
  <<< Установка утилит (пакетов) для оформления дизайна вашего загрузчика GRUB для Archlinux >>> ${NC}"
...
echo ""
echo -e "${GREEN}==> ${NC}Установить Grub2-Theme-Vimix (grub2-theme-vimix) - Тема размытия для grub?"
...
echo " Создать папку для резервного копирования в директории /etc "
mkdir -p /etc/grub-backup
echo ""
...
cp -vf /etc/default/grub /etc/grub-backup/
cp -vf /etc/default/grub.backup0 /etc/grub-backup/
...
cp -vf /boot/grub/grub.cfg /etc/grub-backup/
cp -vf /boot/grub/grub.cfg.backup0 /etc/grub-backup/
...
cp -a /etc/grub.d /etc/grub.d.backup/  # -a - режим резервного копирования, при котором сохраняются все атрибуты, ссылки, а также выполняется резервное копирование папок, аналогично --recursive --preserve=all, --no-dereference;
...
# sudo mv * /etc/grub.d/  # Восстановите записи загрузчика Grub в папке /etc/default/grub.d/
...
clear
echo ""
echo -e "${GREEN}==> ${NC}Установить Grub2-Theme-Breeze (grub2-theme-breeze) - Минималистичная тема для grub?"
...
pacman -Syy  # обновление баз пакмэна (pacman)
pacman -S --noconfirm --needed xf86-input-synaptics  # Драйвер Synaptics для сенсорных панелей ноутбуков ; https://xorg.freedesktop.org/ ; https://archlinux.org/packages/extra/x86_64/xf86-input-synaptics/ ; Обеспечивает: synaptics ; Заменяет: synaptics ; Конфликты: synaptics ; 2025-01-07 14:44 UTC
pacman -S --noconfirm --needed libinput  # Библиотека управления устройствами ввода и обработки событий ; https://gitlab.freedesktop.org/libinput/libinput ; https://archlinux.org/packages/extra/x86_64/libinput/ ; 2025-08-28 14:35 UTC
pacman -S --noconfirm --needed xf86-input-libinput  # Универсальный драйвер ввода для сервера X.Org на основе libinput ; http://xorg.freedesktop.org/ ; https://archlinux.org/packages/extra/x86_64/xf86-input-libinput/ ; Конфликты: X-ABI-XINPUT_VERSION<24, X-ABI-XINPUT_VERSION>=25, xorg-server<1.19.0 ; 2024-10-15 12:49 UTC
pacman -S --noconfirm --needed xorg-xinput  # Небольшой инструмент командной строки для настройки устройств ; https://xorg.freedesktop.org/ ; https://archlinux.org/packages/extra/x86_64/xorg-xinput/ ; 2024-07-14 03:53 UTC
pacman -S --noconfirm --needed evtest  # Монитор событий устройства ввода и инструмент запросов ; https://cgit.freedesktop.org/evtest/ ; https://archlinux.org/packages/extra/x86_64/evtest/ ; 2025-08-27 11:13 UTC
pacman -S --noconfirm --needed xorg-xev  # Распечатать содержимое X событий ; https://gitlab.freedesktop.org/xorg/app/xev ; https://archlinux.org/packages/extra/x86_64/xorg-xev/ ; 2024-03-04 06:34 UTC
pacman -S --noconfirm --needed piper  # Приложение GTK для настройки игровых мышей ; https://github.com/libratbag/piper ; https://archlinux.org/packages/extra/any/piper/ ; 2024-12-22 13:09 UTC
...
sleep 01
 ...
 echo ""
 echo "  Установка утилит (пакетов) Systemd-UI (systemd-ui) "
pacman -Syy  # обновление баз пакмэна (pacman)
...
sleep 01
 ...
 echo ""
 echo " Установка утилит (пакетов) SystemdGenie (systemdgenie) "
pacman -Syy  # обновление баз пакмэна (pacman)
...
pacman -Syy  # обновление баз пакмэна (pacman)
pacman -S --noconfirm --needed man-pages  # Страницы руководства Linux ; https://www.kernel.org/doc/man-pages/ ; https://archlinux.org/packages/core/any/man-pages/ ; 2025-07-23 16:30 UTC
pacman -S --noconfirm --needed man-db  # Утилита для чтения страниц руководства ; создаёт или обновляет кэши index справочных страниц ; https://gitlab.com/man-db/man-db ; https://archlinux.org/packages/core/x86_64/man-db/ ; https://gitlab.com/man-db/man-db ; Обеспечивает: man ; Заменяет: man ; Конфликты: man ; 2025-05-04 08:39 UTC
# sudo systemctl unmask man-db.service && sudo systemctl enable --now man-db.service
pacman -S --noconfirm --needed man-pages-ru  # Русские man-страницы ; https://archlinux.org/packages/extra/any/man-pages-ru/ ; https://salsa.debian.org/manpages-l10n-team/manpages-l10n ; 2025-04-11 19:12 UTC
...
pacman -Syy  # обновление баз пакмэна (pacman)
pacman -S --noconfirm --needed archlinux-keyring  #  Брелок для ключей Arch Linux PGP ; https://archlinux.org/packages/core/any/archlinux-keyring/ (для hkps://hkps.pool.sks-keyservers.net) ; https://gitlab.archlinux.org/archlinux/archlinux-keyring/ ; 2025-08-08 11:00 UTC
pacman -S --noconfirm --needed pkgfile  # Проводник метаданных pacman .files ; https://github.com/falconindy/pkgfile ; https://archlinux.org/packages/extra/x86_64/pkgfile/ ; 2025-03-01 14:15 UTC
# pkgfile -u
...
pacman -Syy  # обновление баз пакмэна (pacman)
pacman -S --noconfirm --needed libldm  # Инструмент и библиотека для управления динамическими дисками Microsoft Windows ; https://github.com/mdbooth/libldm ; https://archlinux.org/packages/extra/x86_64/libldm/ ; Обеспечивает: ldmtool, libldm-1.0.so=0-64 ; Конфликты: с ldmtool ; 2024-07-12 20:41 UTC
###########
...
# Создайте файл конфигурации для настройки устройства zram. Например, чтобы выделить половину оперативной памяти системы под zram и использовать эффективный zstd алгоритм сжатия, выполните:
# cat <<-EOF | sudo tee /etc/systemd/zram-generator.conf
# [zram0]
# zram-size = (ram / 2)
# compression-algorithm = zstd
# EOF
# После сохранения конфигурации перезагрузите систему, чтобы активировать zram.
# По умолчанию zram будет иметь приоритет над существующим разделом подкачки.
# Вы можете использовать команду zramctl для просмотра состояния zram и swaponотображения разделов подкачки (раздел zram будет /dev/zram0).
#[zram0]
#zram-size = ram / 2
#compression-algorithm = lzo-rle
###############
...
pacman -Syy  # обновление баз пакмэна (pacman)
pacman -S --noconfirm --needed numlockx  # Включает клавишу numlock в X11 ; https://github.com/rg3/numlockx ; https://archlinux.org/packages/extra/x86_64/numlockx/ ; 2024-07-13 00:41 UTC
...
echo ""
echo " Установка утилиты (пакета) CKBComp (ckbcomp) "
...
clear
echo -e "${GREEN}
  <<< Итак, вы установили Arch — Поздравляем! >>>
${NC}"
echo -e "${MAGENTA}:: ${BOLD}*Мы знаем, что он считается одним из самых сложных для установки дистрибутивов Linux, вероятно, процесс оказался проще, чем вы ожидали (хотя и затянувшимся из-за установки утилит из AUR, Если таковые были вами установлены)${NC}"
echo -e "${YELLOW} Примечание! ${BOLD} *Установка завершена. Перезагрузите систему. ${NC}"
echo ""
...
echo -e "${YELLOW}==> ...${NC}"
echo -e "${BLUE}:: ${NC}Если вы хотите подключить AUR, установить дополнительный софт (пакеты), установить мои конфиги для DE/XFCE, тогда после перезагрузки и входа в систему выполните команду:"
