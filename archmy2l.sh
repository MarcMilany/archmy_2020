...
echo " Вам нужен пакет из AUR , называемый systemd-numlockontty. Как и большинство пакетов программного обеспечения, доступных через AUR, этот пакет поставляется только в виде исходного кода, который будет встроен в двоичный код на вашей локальной системе с помощью makepkgи PKGBUILD скрипта. "
