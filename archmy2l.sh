...
#pacman -S --noconfirm --needed dnsmasq # Легкий, простой в настройке DNS-пересылатель и DHCP-сервер ; http://www.thekelleys.org.uk/dnsmasq/doc.html ; https://archlinux.org/packages/extra/x86_64/dnsmasq/ ; https://wiki.archlinux.org/title/dnsmasq
# Dnsmasq предоставляет сетевую инфраструктуру для небольших сетей: DNS, DHCP, объявление маршрутизатора и сетевая загрузка. 
#pacman -S --noconfirm --needed openresolv  # Фреймворк управления resolv.conf (resolvconf) ; https://roy.marples.name/projects/openresolv ; https://archlinux.org/packages/extra/any/openresolv/
# pacman -S --noconfirm --needed systemd-resolvconf  # Замена systemd resolvconf (для использования с systemd-resolved) ; https://www.github.com/systemd/systemd ; https://archlinux.org/packages/core/x86_64/systemd-resolvconf/
#pacman -S --noconfirm --needed net-tools  # Инструменты настройки для сетей Linux ; http://net-tools.sourceforge.net/ ; https://archlinux.org/packages/core/x86_64/net-tools/
...
#systemctl start dnsmasq.service   # Then start/enable dnsmasq.service
...
#echo ""
#echo " Добавим resolvconf в автозагрузку  и запустим (resolvconf.service). "
...
# pacman -S --noconfirm --needed dpkg  # Инструменты менеджера пакетов Debian ; https://tracker.debian.org/pkg/dpkg ; https://archlinux.org/packages/extra/x86_64/dpkg/
# dpkg -l resolvconf
