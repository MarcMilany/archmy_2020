...
echo -e "${GREEN}==> ${BOLD}Вот Ваше ядро (это номер версии ядра), включая информацию о сборке: ${NC}"
echo ""
#arch-chroot /mnt uname -r/mnt  # Команда отобразит версию ядра системы, смонтированной в каталоге. Это полезно для проверки версии ядра целевой системы во время установки или восстановления.arch-chroot /mnt uname -r/mnt . Скрипт гарантирует, что основные системные каталоги, такие как , и правильно смонтированы в среде chroot, что позволяет команде работать правильно и возвращать версию ядра целевой системы arch-chroot/dev/proc/sysuname -r
