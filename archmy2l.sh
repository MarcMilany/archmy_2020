...
clear
echo ""
echo -e "${GREEN}==> ${NC}Ставим Драйвера AMD и Intel — для видеокарт "
echo -e "${BLUE}:: ${NC}Сперва определим вашу видеокарту!"
echo -e "${MAGENTA}=> ${BOLD}Вот данные по вашей видеокарте (даже, если Вы работаете на VM): ${NC}"
#echo ""
lspci | grep -e VGA -e 3D
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Установка драйверов AMD...     2 - Установка драйверов Intel...

    0 - Пропустить установку: " x_drivers  # sends right after the keypress; # отправляет сразу после нажатия клавиши
   echo ''
    [[ "$x_drivers" =~ [^120] ]]
do
    :
...
pacman -S --noconfirm --needed xorg plasma  # Устанавливает среду рабочего стола KDE Plasma
