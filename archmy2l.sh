...
pacman -S --noconfirm --needed file-roller  # легковесный архиватор ( для xfce-lxqt-lxde-gnome )
  echo ""
  echo " Установка Parole (parole) — Плеер без наворотов "
####### parole  ##########
pacman -S --noconfirm --needed parole  # Современный медиаплеер на базе фреймворка GStreamer ; https://archlinux.org/packages/extra/x86_64/parole/ ; https://docs.xfce.org/apps/parole/start ; https://github.com/xfce-mirror/parole ; https://docs.xfce.org/apps/parole/plugins ; https://docs.xfce.org/apps/parole/command-line ; Группы: xfce4-goodies ; 2025-05-22 05:32 UTC
######## Parole Plugins ########
pacman -S --noconfirm --needed grilo  # Фреймворк, обеспечивающий доступ к различным источникам мультимедийного контента ; https://archlinux.org/packages/extra/x86_64/grilo/ ; https://wiki.gnome.org/Projects/Grilo ; Обеспечивает: libgrilo-0.3.so=0-64, libgrlnet-0.3.so=0-64, libgrlpls-0.3.so=0-64 ; 2025-06-13 22:55 UTC
pacman -S --noconfirm --needed grilo-plugins  # (необязательно) — плагин Grilo для медиа-браузера ; Коллекция плагинов для фреймворка Grilo ; https://archlinux.org/packages/extra/x86_64/grilo-plugins/ ; https://gitlab.gnome.org/GNOME/grilo-plugins ; 2025-06-25 09:26 UTC
pacman -S --noconfirm --needed gst-libav  # (необязательно) — дополнительные медиакодеки ; Фреймворк мультимедийных графов — плагин libav ; https://archlinux.org/packages/extra/x86_64/gst-libav/ ; https://gstreamer.freedesktop.org/ ; Обеспечивает: gst-ffmpeg=1.26.5-1 ; 2025-08-08 06:45 UTC
pacman -S --noconfirm --needed gst-plugins-bad  # (необязательно) — Дополнительные медиакодеки ; Мультимедийный граф-фреймворк — плохие плагины ; https://archlinux.org/packages/extra/x86_64/gst-plugins-bad/ ; https://gstreamer.freedesktop.org/ ; 2025-08-08 06:45 UTC
pacman -S --noconfirm --needed gst-plugins-ugly  # (необязательно) — Дополнительные медиакодеки ; Фреймворк мультимедийных графов — уродливые плагины ; https://archlinux.org/packages/extra/x86_64/gst-plugins-ugly/ ; https://gstreamer.freedesktop.org/ ; 2025-08-08 06:45 UTC
pacman -S --noconfirm --needed libdmapsharing  # (необязательно) — плагин DAAP Music Sharing ; Библиотека, реализующая семейство протоколов DMAP ; https://archlinux.org/packages/extra/x86_64/libdmapsharing/ ; https://www.flyn.org/projects/libdmapsharing/index.html ; Обеспечивает: libdmapsharing-4.0.so=3-64 ; 2023-07-30 01:24 UTC
echo ""
echo " Установка Galculator (калькулятор) (на основе GTK+)(версия GTK3) "
pacman -S --noconfirm --needed galculator  # Научный калькулятор на основе GTK + (версия GTK3) (Обратные конфликты: galculator-gtk2)
### Galculator - научный калькулятор для Linux. Galculator имеет три режима работы: (простой, научный и paper mode, в котором вычисления можно проводить путем ввода выражения в текстовое окно). Поддерживает десятичную, шестнадцатеричную, восьмеричную и двоичную системы счисления. Также поддерживаются разные угловые меры - градусы, радианы, грады.
#####
  clear
  echo ""
