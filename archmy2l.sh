...
#
echo ""
echo -e "${BLUE}:: ${BOLD}Произведём размонтирование, вне зависимости, используется ли устройство или нет ${NC}"
#echo 'Произведём размонтирование, вне зависимости, используется ли устройство или нет'
# We will perform unmounting, regardless of whether the device is used or not
#clear
echo " umount -R /mnt  # это позволяет заметить любые «занятые» разделы "  
umount -R /mnt    # это позволяет заметить любые «занятые» разделы
