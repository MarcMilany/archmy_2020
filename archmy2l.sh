...
### 
  echo "" 
  echo "#DisableDownloadTimeout" >> /etc/pacman.conf  # Если вы имеете плохое качество соединения или слабый уровень сигнала, то при загрузке пакетов при помощи pacman вы могли сталкиваться с ошибкой превышания лимитов ожидания (таймаутов)
  echo "#XferCommand = /usr/bin/wget --timeout=40 --tries=0 --passive-ftp -c -O %o %u'  # Она задает загрузчик, по умолчанию wget, и параметры к нему" >> /etc/pacman.conf
  echo ""
