...
echo " Вы увидите вывод после команды и наверняка вас попросят обновить микрокод "
echo " Будьте внимательны! Вы можете пропустить это действие, Но лучше не пропускать обновление "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    ...
    9 - Budgie — это впечатляющая среда рабочего стола, которая предлагает уникальный пользовательский интерфейс
...
# pacman -S --noconfirm --needed еом  # (необязательно) ; Просмотрщик изображений для MATE ; https://archlinux.org/packages/extra/x86_64/eom/ ; https://mate-desktop.org/ ; Заменяет: eom-gtk3 ; Конфликты: с eom-gtk3 ; 2025-04-30 17:32 UTC
# yay -S budgie-clipboard-applet  # Менеджер буфера обмена для Budgie ; https://aur.archlinux.org/packages/budgie-clipboard-applet ; https://aur.archlinux.org/budgie-clipboard-applet.git (только для чтения, нажмите, чтобы скопировать) ; https://github.com/prateekmedia/budgie-clipboard-applet ; 2022-08-19 15:55 (UTC)
### budgie-weather-applet  # https://github.com/Dirli/budgie-weather-applet
...
################ Xscreensaver Хранитель экрана и блокировщик для X Window System ###########
# sudo pacman -S --noconfirm --needed i3lock  # Улучшенный блокировщик экрана на основе XCB и PAM ; https://archlinux.org/packages/extra/x86_64/i3lock/ ; https://i3wm.org/i3lock/ ; Группы: i3 ; 2025-07-17 12:08 UTC
# sudo pacman -S --noconfirm --needed xscreensaver  # Хранитель экрана и блокировщик для X Window System ; https://archlinux.org/packages/extra/x86_64/xscreensaver/ ; https://www.jwz.org/xscreensaver/ ;  2025-06-15 12:02 UTC
########## Справка ##########
...
### PipeWire: Разделенные пакеты: alsa-card-profiles , gst-plugin-pipewire , libpipewire , pipewire-alsa , pipewire-audio
######### alsa-lib ##########
...
pacman -S --noconfirm --needed lib32-alsa-lib  # Альтернативная реализация поддержки звука в Linux (32 бит) ; https://archlinux.org/packages/multilib/x86_64/lib32-alsa-lib/ ; https://www.alsa-project.org/ ; Обеспечивает: libasound.so=2-32, libatopology.so=2-32 ; 2025-04-14
############ pipewire ##########
pacman -S --noconfirm --needed pipewire  # Аудио/видео маршрутизатор и процессор с малой задержкой ; https://archlinux.org/packages/extra/x86_64/pipewire/# ; https://pipewire.org/ ; 2025-09-11 13:19 UTC
pacman -S --noconfirm --needed lib32-pipewire  # Аудио/видео маршрутизатор и процессор с малой задержкой - 32 бита ; https://pipewire.org/ ; https://archlinux.org/packages/multilib/x86_64/lib32-pipewire/ ; 24 августа 2024 г., 13:15 UTC
########## PipeWire-Docs — для просмотра документации #########
pacman -S --noconfirm --needed pipewire-docs  # (необязательно) — документация ; Аудио/видео маршрутизатор и процессор с малой задержкой - документация ; https://pipewire.org/ ; https://archlinux.org/packages/extra/x86_64/pipewire-docs/ ; 24 августа 2024 г., 13:15 UTC
########## Клиенты ALSA #########
pacman -S --noconfirm --needed pipewire-alsa  # (необязательно) — конфигурация ALSA ;  Аудио/видео маршрутизатор и процессор с малой задержкой — конфигурация ALSA ; https://pipewire.org/ ; https://archlinux.org/packages/extra/x86_64/pipewire-alsa/ ; 24 августа 2024 г., 13:15 UTC
...
pacman -S --noconfirm --needed pipewire-audio  # (опционально) — поддержка аудио ; Аудио/видео маршрутизатор и процессор с малой задержкой - Поддержка аудио ; https://pipewire.org/ ; https://archlinux.org/packages/extra/x86_64/pipewire-audio/ ; 24 августа 2024 г., 13:15 UTC
############# Клиенты PulseAudio #########
pacman -S --noconfirm --needed pipewire-pulse  # (опционально) — замена PulseAudio ; Аудио/видео маршрутизатор и процессор с малой задержкой — замена PulseAudio ; https://pipewire.org/ ; https://archlinux.org/packages/extra/x86_64/pipewire-pulse/ ; 24 августа 2024 г., 13:15 UTC ; Конфликты: с pulseaudio ; Смотрите Зависимости !
...
pacman -S --noconfirm --needed pipewire-roc  # (опционально) — потоковая передача ROC ; Аудио/видео маршрутизатор и процессор с малой задержкой — поддержка потоковой передачи ROC ; https://archlinux.org/packages/extra/x86_64/pipewire-roc/ ; https://pipewire.org/ ; 2025-07-26 01:14 UTC
########## клиенты ДЖЕК ############
pacman -S --noconfirm --needed pipewire-jack  # (опционально) - замена JACK ; Аудио/видео маршрутизатор и процессор с малой задержкой - замена JACK ; https://pipewire.org/ ; https://archlinux.org/packages/extra/x86_64/pipewire-jack/ ; 24 августа 2024 г., 13:15 UTC ; Конфликты: с jack, jack2, pipewire-jack-client ; Смотрите Зависимости !
...
pacman -S --noconfirm --needed pipewire-session-manager  # (необязательно) — менеджер сеансов ; Менеджер сеансов для PipeWire (поставщик по умолчанию) ; https://archlinux.org/packages/extra/x86_64/pipewire-session-manager/ ; https://pipewire.org/ ; 28 июня 2025 г. 02:23 UTC
...
######## PipeWire GStreamer ###########
pacman -S --noconfirm --needed gst-plugin-pipewire  # (необязательно) — плагин GStreamer ; Мультимедийная графическая структура - плагин pipewire ; https://pipewire.org/ ; https://archlinux.org/packages/extra/x86_64/gst-plugin-pipewire/ ; 24 августа 2024 г., 13:15 UTC
######### EasyEffects (ранее PulseEffects) ##########
pacman -S --noconfirm --needed easyeffects  # Аудиоэффекты для приложений Pipewire ; Заменяет: pulseeffects ; https://github.com/wwmm/easyeffects ; https://archlinux.org/packages/extra/x86_64/easyeffects/ ; Aug. 18, 2024, 3:09 p.m. UTC
###### Подавление шума для голоса ############
pacman -S --noconfirm --needed noise-suppression-for-voice  # Плагин для подавления шума в реальном времени для голоса ; https://github.com/werman/noise-suppression-for-voice ; https://archlinux.org/packages/extra/x86_64/noise-suppression-for-voice/ ; 19 мая 2024 г., 16:58 UTC
################# Зависимости необязательные ############
# pacman -S --noconfirm --needed realtime-privileges  # (необязательно) — привилегии реального времени с модулем rt ; Привилегии в реальном времени для пользователей ; https://archlinux.org/packages/extra/any/realtime-privileges/ ; https://wiki.archlinux.org/index.php/Realtime_process_management ; https://archlinux.org/groups/x86_64/realtime/ ; Группы: pro-audio, realtime ; 2024-07-13 18:28 UTC
...
# pacman -S --noconfirm --needed pipewire-v4l2  # (опционально) - перехватчик V4L2 ; Аудио/видео маршрутизатор и процессор с малой задержкой — перехватчик V4L2 ; https://archlinux.org/packages/extra/x86_64/pipewire-v4l2/ ; https://pipewire.org/ ; 2025-09-11 13:19 UTC
...
# pacman -S --noconfirm --needed pipewire-x11-bell (опционально) - X11 bell ; Аудио/видео маршрутизатор и процессор с малой задержкой — X11 bell ; https://archlinux.org/packages/extra/x86_64/pipewire-x11-bell/ ; https://pipewire.org/ ; 2025-09-11 13:19 UTC
### Модуль x11-bellперехватывает события звонка X11 и использует libcanberra для воспроизведения звука.
##########################
  clear
  echo ""
  echo " Budgie DE успешно установлен "
  echo ""
...
while
    ...
    0 - Нет буду использовать DM (Display manager): " i_budgiewm   # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$i_budgiewm" =~ [^10] ]]
do
    :
done
if [[ $i_budgiewm  == 0 ]]; then
  echo " Буду использовать DM (Display manager) "
elif [[ $i_budgiewm  == 1 ]]; then
  echo ""
  echo " Действия по настройке автовхода без DM (Display manager) "
  echo " Поскольку реализация автозагрузки окружения реализована через startx - (иксы), то если Вы установили X.Org Server возможно пакет (xorg-xinit) - уже установлен "
  pacman -S xorg-xinit --noconfirm
...
# pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
  ...
  echo " Установка дополнительных утилит (пакетов) "
############ Зависимости ##############
pacman -S --noconfirm --needed findutils  # Утилиты GNU для поиска файлов ; https://archlinux.org/packages/core/x86_64/findutils/ ; https://www.gnu.org/software/findutils/ ; 2025-07-04 13:52 UTC
######### plocate #########
pacman -S --noconfirm --needed plocate  # Альтернатива locate, более быстрая и совместимая с базой данных mlocate ; https://archlinux.org/packages/extra/x86_64/plocate/ ; https://plocate.sesse.net/ ; Заменяет: mlocate ; Конфликты: mlocate ; 2024-11-27 18:36 UTC
...
systemctl enable plocate-updatedb.timer  # systemctl правление системой systemd и менеджером служб ; systemctl [ПАРАМЕТРЫ...] КОМАНДА [ЕДИНИЦА...]
...
clear
echo -e "${GREEN}
  <<< Итак, вы установили Arch — Поздравляем! Мы знаем, что он считается одним из самых сложных для установки дистрибутивов Linux, вероятно, процесс оказался проще, чем вы ожидали (хотя и затянувшимся из-за установки утилит из AUR, Если таковые были вами установлены).
