...
# grub-install --target=i386-pc --force --recheck /dev/$x_cfd
  echo "GRUB_DISABLE_OS_PROBER=false" >> /etc/default/grub
# sed -i '/GRUB_DISABLE_OS_PROBER/s/^#//g' /etc/default/grub  # добавить в загрузчик grub другие os
...
### MultiLib (Include= /path/to/config/file) - Этот файл может включать репозитории или общие параметры конфигурации.
  ...
  sed -i '/^Ar/ #DisableDownloadTimeout' /etc/pacman.conf  # Если вы имеете плохое качество соединения или слабый уровень сигнала, то при загрузке пакетов при помощи pacman вы могли сталкиваться с ошибкой превышания лимитов ожидания (таймаутов)
  sed -i '/^#Xf/ #XferCommand = /usr/bin/wget --timeout=40 --tries=0 --passive-ftp -c -O %o %u' /etc/pacman.conf  # Она задает загрузчик, по умолчанию wget, и параметры к нему
  echo ""
...
#echo " Включить и запустить службу рефлектора "
#systemctl enable reflector.service
#systemctl start reflector.service
sleep 1
###################
...
clear
echo ""
echo " Запустить и сделать systemd-timesyncd активным (systemd-timesyncd) "
...
# systemctl status systemd-timesyncd
  ...
  echo " Данная операция может занять продолжительное время "
