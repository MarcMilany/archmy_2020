...
#echo 'GRUB_DISABLE_OS_PROBER=false' >> /etc/default/grub 
  sed -i s/'#GRUB_DISABLE_OS_PROBER=false'/'GRUB_DISABLE_OS_PROBER=false'/g /etc/pacman.conf   # # добавить в загрузчик grub другие os или раскомментить строку GRUB_DISABLE_OS_PROBER  в /etc/default/grub
# sed -i 's/#GRUB_DISABLE_OS_PROBER=false\n/g' /etc/pacman.conf
# sed -i 's/#GRUB_DISABLE_OS_PROBER=false/GRUB_DISABLE_OS_PROBER=false/g' /etc/pacman.conf  
  echo " Загрузчик GRUB установлен на выбранный вами диск (раздел) "
elif [[ $i_grub == 0 ]]; then
  echo ""
...
### Она задает загрузчик, по умолчанию wget, и параметры к нему
  ...
  echo '#XferCommand = /usr/bin/wget --timeout=40 --tries=0 --passive-ftp -c -O %o %u' >> /etc/default/grub   
  echo ""
...
# systemctl enable dhcpcd   # для активации проводных соединений  # systemctl - инструмент для управления службами
 ...
 #systemctl start dhcpcd.service
  ...
  # systemctl stop dhcpcd.service   # Если необходимо настроить статический IP или использовать другие средства настройки сети
...
##################################
echo " Пропишем публичные серверы DNS в /etc/resolvconf.conf "
> /etc/resolvconf.conf
...
#echo ""
#echo " Обновление resolvconf  /etc/resolv.conf "
# resolvconf -u   # обновление resolvconf
# resolvconf -l
# resolvconf -v
### Возможно, какая-то вредоносная программа изменила владельца resolv.confфайла с root на что-то другое. Выполните следующие действия:
### Установите владельца на root с помощью команды chown–
# sudo chown root:root /etc/resolv.conf
# chown root:root /etc/resolv.conf
# Установить разрешение файла 644–
# sudo chmod 644 /etc/resolv.conf
# chmod 644 /etc/resolv.conf
# chmod 0700 /resolv.conf  # 0700 даст владельцу (root) права на чтение, запись и выполнение
# chmod +w /resolv.conf    # Если запись в файл не разрешена, то надо выставить дополнительное право
### Теперь проверьте права доступа и владельца с помощью ls -laкоманды –
...
echo " Сделаем бэкап /etc/resolv.conf "
echo " resolv.conf - Это  основной файл настройки библиотеки распознавателя имен DNS "
#cp /etc/resolv.conf  /etc/resolv.conf.back
cp -v /etc/resolv.conf  /etc/resolv.conf.back  # Для начала сделаем его бэкап
# cp -v /etc/resolv.conf  /etc/resolv.conf.original  # -v или --verbose -Выводить информацию о каждом файле, который обрабатывает команда cp.
echo " Перемещаем и переименовываем исходный файл /etc/resolv.conf.back в /etc/resolvconf/resolv.conf.d/resolv.conf "
#mv /etc/resolv.conf.back  /etc/resolvconf/resolv.conf.d/resolv.conf.back   # Перемещаем и переименовываем исходный файл
mv /etc/resolv.conf.back  /etc/resolvconf/resolv.conf.d/resolv.conf.back_`date +"%d.%m.%y_%H-%M"`   # Перемещаем и переименовываем исходный файл
### echo " Удаление файла /etc/resolv.conf "
### rm /etc/resolv.conf   # rm - Удаление файлов
#################################
echo ""
...
#####################
# Временное решение без необходимости перенастройки в случае изменения IP-адресов DNS:
...
###################
touch /etc/systemd/network/20-ethernet.network   # Создать файл resolv.conf в /etc/resolvconf/resolv.conf.d/
...
# systemd-resolve --status | grep 'DNS Servers' -A2  # Проверьте, что DNS-серверы обновились
...
EOF
######################
echo " Создать ссылку (resolve) в /etc/ "
ln -sf /run/systemd/resolve/stub-resolv.conf /etc/resolv.conf  # Чтобы связать файл с файлом ; https://www.ibm.com/docs/zh/aix/7.1?topic=l-ln-command
#sudo chattr +i /etc/resolv.conf
systemctl enable systemd-resolved
# systemctl status systemd-resolved
...
if [[ $i_systemad == 0 ]]; then
  echo ""
  echo " Установка утилит пропущена "
elif [[ $i_systemad == 1 ]]; then
  echo ""
