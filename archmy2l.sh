...
# pacman -S networkmanager networkmanager-openvpn network-manager-applet ppp --noconfirm  # Диспетчер сетевых подключений и пользовательские приложения; Плагин NetworkManager VPN для OpenVPN; Апплет для управления сетевыми подключениями; Демон, реализующий протокол точка-точка для коммутируемого доступа в сеть.
  ...
  echo ""
  echo -e "${BLUE}:: ${NC}Ставим сетевые утилиты, VPN, Proxy и тд... "
########### VPN #############    
...
############ Proxy ###########
  ...
  pacman -S --noconfirm --needed networkmanager-strongswa  # Плагин Strongswan NetworkManager ; https://wiki.strongswan.org/projects/strongswan/wiki/NetworkManager ; https://archlinux.org/packages/extra/x86_64/networkmanager-strongswan/
  echo ""
...
pacman -Syy  # обновление баз пакмэна (pacman)
pacman -S --noconfirm --needed audit  # Компоненты пользовательского пространства структуры аудита ; https://people.redhat.com/sgrubb/audit ; https://archlinux.org/packages/core/x86_64/audit/
pacman -S --noconfirm --needed arch-audit  # Утилита типа pkg-audit, основанная на данных Arch Security Team ; https://gitlab.com/ilpianista/arch-audit ; https://archlinux.org/packages/extra/x86_64/arch-audit/
pacman -S --noconfirm --needed arch-audit-gtk  # Уведомления об обновлениях безопасности Arch Linux ; https://github.com/kpcyrd/arch-audit-gtk ; https://archlinux.org/packages/extra/x86_64/arch-audit-gtk/
pacman -S --noconfirm --needed python-audit  # Компоненты пользовательского пространства фреймворка аудита — привязки Python ; https://people.redhat.com/sgrubb/audit ; https://archlinux.org/packages/core/x86_64/python-audit/
########## Справка #############
...
###################
#echo ""
#echo " Перезапустим (resolvconf.service и systemd-resolved) "
#systemctl restart systemd-resolved.service
###############################
#echo 'nameserver 1.1.1.1' > /etc/resolv.conf
#echo 'nameserver 8.8.8.8' >> /etc/resolv.conf
...
ls -l etc/X11
ls -l /etc/X11/xorg.conf.d/   # ls — выводит список папок и файлов в текущей директории
sleep 1
echo ""
