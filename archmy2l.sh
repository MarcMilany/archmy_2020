...
echo -e "${BLUE}:: ${NC}GRUB2 — для BIOS-версии, можно установить не только для таблицы разделов MS-DOS, а также для таблицы разделов и GPT — GUID Partition Table (Таблица разделов GUID) — современный стандарт разметки жестких дисков, пришедший на смену более старой системе MBR. *Для этого перед загрузочным расделом BIOS создаём раздел обьёмом от 8 МБ до 50 МБ (в зависимости от ваших возможностей). Далее на этом (созданном) разделе устанавливаем флаг 'grub_bios' , а на разделе самого загрузчика GRUB — для BIOS (/boot), он будет распознавать системой как раздел EFI , установить флаги 'boot' и 'esp'. И вы сможете познать все 'прелести' файловой системы Btrfs (B-tree File System)."
