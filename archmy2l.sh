...
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
#   read -n1 -p  " 1 - Да редактировать, 2 - Просмотреть файл, 0 - Нет пропустить этот шаг: " vm_fstab   # sends right after the keypress; # отправляет сразу после нажатия клавиши
    read -n1 -p  " 
    1 - Да редактировать,    2 - Просмотреть файл,    

    0 - Нет пропустить этот шаг: " vm_fstab   # sends right after the keypress; # отправляет сразу после нажатия клавиши
#echo " Чтобы подтвердить действия ввода, нажмите кнопку 'Ввод' ("Enter") "   
#  read -p " 1 - Да редактировать, 2 - Просмотреть файл, 0 - Нет пропустить этот шаг: " vm_fstab  # To confirm the input actions, click 'Enter' ; # Чтобы подтвердить действия ввода, нажмите кнопку 'Ввод' ("Enter")
    echo ''
    [[ "$vm_fstab" =~ [^10] ]]
do
    :
