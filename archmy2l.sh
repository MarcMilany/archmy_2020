...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Группы (audio,docker,input,kvm,libvirt,games,lp,disk,network,optical,power,scanner,storage,video,rfkill,sys,wheel),

    2 - Группы (adm + audio,docker,input,kvm,libvirt,games,lp,disk,network,optical,power,scanner,storage,video,rfkill,sys,wheel),
...
if [[ $i_groups  == 1 ]]; then
  useradd -m -g users -G audio,docker,input,kvm,libvirt,games,lp,disk,network,optical,power,scanner,storage,video,rfkill,sys,wheel -s /bin/bash $username
  usermod -a -G audio,docker,input,kvm,libvirt,games,lp,disk,network,optical,power,scanner,storage,video,rfkill,sys,wheel $username
  userdbctl groups-of-user $username
  clear
  echo ""
  echo " Пользователь успешно добавлен в группы и права пользователя "
elif [[ $i_groups  == 2 ]]; then
  useradd -m -g users -G adm,audio,docker,input,kvm,libvirt,games,lp,disk,network,optical,power,scanner,storage,video,rfkill,sys,wheel -s /bin/bash $username
  clear
  echo ""
  echo " Пользователь успешно добавлен в группы и права пользователя "
...
####### ПРИМЕЧАНИЕ ###########
### docker — члены группы docker могут запускать команду docker CLI как пользователь без прав root.
### input - Доступ к устройствам ввода.
### kvm - Доступ к виртуальным машинам с помощью KVM .
### libvirt — члены группы libvirt по умолчанию имеют беспарольный доступ к сокету демона RW.
### storage — используется для получения доступа к съемным носителям, таким как жесткие диски USB, флэш-накопители, MP3-плееры; позволяет пользователю монтировать устройства хранения данных.
### video - доступ к устройствам видеозахвата, аппаратное ускорение 2D/3D, кадровый буфер
### wheel — группа администраторов, обычно используемая для предоставления привилегий на выполнение административных действий. Она имеет полный доступ на чтение файлов журналов и право на администрирование принтеров в CUPS . Также может использоваться для предоставления доступа к утилитам sudo и su (ни одна из них не использует её по умолчанию).
###################
echo ""
...
echo " 4 - Установка 'AUR'-'paru' с помощью git clone, PKGBUILD, makepkg - скачивается с сайта 'Arch Linux' (https://aur.archlinux.org/paru.git), собирается и устанавливается, то выбирайте вариант - "4" "
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    ...
    4 - AUR - paru (git clone),    0 - Пропустить установку AUR Helper: " in_aur_help  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_aur_help" =~ [^12340] ]]
do
    :
...
# sudo -u $username  makepkg -fsri --noconfirm
  ...
  echo ""
  echo " Установка AUR Helper - (paru) "
  cd /home/$username
...
# Vifm — это файловый менеджер с интерфейсом curses, который предоставляет среду, похожую на Vim, для управления объектами в файловых системах, расширенную некоторыми полезными идеями из mutt. Если вы используете vi, Vifm дает вам полный контроль над файлами с клавиатуры без необходимости изучать новый набор команд.
  clear
  echo ""
  echo " Установка AUR Helper (paru) завершена "
fi
###
echo ""
...
echo " 3 - Установка обновлений баз данных пакетов, и системы через 'AUR'-'paru', то выбирайте вариант "3" "
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Обновление через - AUR (Yay),   2 - Обновление через - AUR (Pikaur),   3 - Обновление через - AUR (Paru),
