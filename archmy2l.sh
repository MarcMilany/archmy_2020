...
while
# echo " Чтобы подтвердить действия ввода, нажмите кнопку 'Ввод' ("Enter") "
# read -p " 1 - Устанавливаем Xfce + Goodies for Xfce, 2 - Устанавливаем просто Xfce, 0 - Пропустить установку: " x_de  # To confirm the input actions, click 'Enter' ; # Чтобы подтвердить действия ввода, нажмите кнопку 'Ввод' ("Enter")
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p " 
    1 - Устанавливаем Xfce + Goodies for Xfce,    2 - Устанавливаем просто Xfce, 
...
echo -e "${CYAN}:: ${BOLD}Важно! Если Вы НЕ установили Xfce4 (окружения рабочего стола) - рекомендую вариант "0" (для вас действие будет пропущено, и сможете вернутся к этому вопросу ПОСЛЕ установки нужного вам DE). ${NC}"
echo -e "${YELLOW}==> ${NC}Действия выполняются в указанном порядке" 
#echo 'Действия выполняются в указанном порядке'
# Actions are performed in the order listed
echo ""
...
echo -e "${BLUE}:: ${NC}Создание полного набора локализованных пользовательских каталогов по умолчанию (Рабочий стол, Загрузки, Шаблоны, Общедоступные, Документы, Музыка, Изображения, Видео) в пределах "HOME" каталога."
echo -e "${CYAN}:: ${NC}По умолчанию в системе Arch Linux в каталоге "HOME" НЕ создаются папки (Рабочий стол, Загрузки, Шаблоны, Общедоступные, Документы, Музыка, Изображения, Видео). "
...
cat <<EOF >>$(XDG_CONFIG_HOME)/user-dirs.locale
