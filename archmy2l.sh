...
echo -e "${GREEN}==> ${NC}" 
echo " Используйте в имени только буквы латинского алфавита (можно с заглавной буквы)"
echo " Латиница - это английские буквы. Кириллица - русские. "
read -p " => Введите имя компьютера: " hostname
echo -e "${GREEN}==> ${NC}"
echo " Используйте в имени только буквы латинского алфавита (в нижнем регистре (a—z)), и цифры "
read -p " => Введите имя пользователя: " username
#
...
# ============================================================================
read -p " 1 - UTC, 2 - Localtime, 0 - Пропустить настройку: " prog_set
if [[ $prog_set == 1 ]]; then
hwclock --systohc --utc
  echo " hwclock --systohc --utc "
...
echo " passwd root "
echo " Желательно использовать в имени только буквы латинского алфавита ()"
passwd
# --------------------------------------------------------------------------
...
echo " Производители процессоров выпускают обновления стабильности и безопасности для микрокода процессора "
...
echo " Для Arch Linux на съемном носителе добавьте оба файла initrd в настройки загрузчика. Их порядок не имеет значения, если они оба указаны до реального образа initramfs. "
echo -e "${YELLOW}==> ${NC}Вы можете пропустить этот шаг, если не уверены в правильности выбора"
#echo 'Вы можете пропустить этот шаг, если не уверены в правильности выбора'
# You can skip this step if you are not sure of the correct choice
...
# You can skip this step if you are not sure of the correct choice
echo ""
read -p " 1 - Вводим информация о пользователе,  0 - Пропустить этот шаг: " prog_set
if [[ $prog_set == 1 ]]; then
