...
########### DNS-запрос и DHCP-сервер #############
  ...
  pacman -S --noconfirm --needed dnsmasq  # Легкий, простой в настройке сервер пересылки DNS и DHCP-сервер ; http://www.thekelleys.org.uk/dnsmasq/doc.html ; https://archlinux.org/packages/extra/x86_64/dnsmasq/
...
mv /etc/resolv.conf.back  /etc/resolvconf/resolv.conf.d/resolv.conf   # Перемещаем и переименовываем исходный файл
### echo " Удаление файла /etc/resolv.conf "
### rm /etc/resolv.conf   # rm - Удаление файлов
#################################
echo ""
...
echo " gitstatus — это в 10 раз более быстрая альтернатива git statusи git describe. Его основное применение — включение быстрого приглашения git в интерактивных оболочках. "
##### gitstatus-bin ######
...
# makepkg --noconfirm --needed -sic
  ...
  echo " Установка Powerlevel10k как темы для ZSH по умолчанию "
# echo 'source  ~/zsh-theme-powerlevel10k/powerlevel10k.zsh-theme' >>! ~/.zshrc  # После установки убедитесь, что Zsh загружает Powerlevel10k. 
##echo 'source /usr/share/zsh-theme-powerlevel10k/powerlevel10k.zsh-theme' >>! ~/.zshrc 
  echo 'source ~/powerlevel10k/powerlevel10k.zsh-theme' >>~/.zshrc
  echo " Установка Oh My Zsh (командной оболочки) Выполнена! "
  echo " Настроить ZSH для использования шрифта MesloLGS NF по умолчанию "
