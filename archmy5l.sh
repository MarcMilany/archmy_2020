...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Zed - (для редактирования кода)?"
echo -e "${MAGENTA}:: ${BOLD}Zed - это высокопроизводительный многопользовательский редактор кода от создателей Atom и Tree-sitter. Он также имеет открытый исходный код. Редактор кода Zed теперь доступен для Linux. Zed поставляется с различными светлыми и темными темами по умолчанию, а дополнительные темы доступны с помощью расширений. ${NC}"
echo " Домашняя страница: https://zed.dev/ ; (https://archlinux.org/packages/extra/x86_64/zed/ ; https://github.com/zed-industries/zed). "  
echo -e "${MAGENTA}:: ${BOLD}Zed — это редактор кода с большим количеством возможностей, написанный на Rust. Позиционируется как высокопроизводительный редактор с возможностями IDE. Поддерживает многопользовательскую работу с документами. Для рендеринга используется GPU. Над проектом работает команда, которая разработала редактор Atom. ${NC}"
echo " Zed поддерживает работу в Wayland и X11. При запуске программа сама пытается определить подходящую сессию. Для принудительного запуска в режиме X11 можно установить переменную окружения WAYLAND_DISPLAY=’’. " 
echo " Zed сочетает в себе мощь IDE с отзывчивостью легкого редактора для производительности, которую вы можете почувствовать под рукой. Zed эффективно использует каждое ядро ЦП и ваш графический процессор для мгновенного запуска, мгновенной загрузки файлов и реагирования на нажатия клавиш при следующем обновлении экрана. Неумолимая производительность держит вас в потоке и заставляет другие инструменты казаться медленными. " 
echo " Zed поддерживает GitHub Copilot из коробки, и вы можете использовать GPT-4 для генерации или рефакторинга кода, нажав ctrl-enter и введя подсказку на естественном языке. Взаимодействуйте с моделью в разговорном режиме, не переключая контекст на встроенной панели помощника, затем ссылайтесь на свой разговор во время встроенной генерации. Zed поддерживает полное дерево синтаксиса для каждого буфера по мере ввода текста, обеспечивая точное выделение кода, автоматический отступ, просмотр структуры с возможностью поиска и структурный выбор. Zed также говорит на протоколе сервера языка, чтобы обеспечить автодополнение, навигацию по коду, диагностику и рефакторинг. Zed интегрирует полнофункциональный терминал, предоставляя вам доступ к оболочке в том же месте, где вы редактируете код. Используйте встроенный док, чтобы разместить и переключить его в любом месте, или используйте его для удержания других вкладок. "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Да установить,     0 - НЕТ - Пропустить установку: " in_zed  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_zed" =~ [^10] ]]
do
    :
...
echo " Установка утилит (пакетов) пропущена "
  ...
  echo ""
  echo " Установка Zed "
# sudo pacman -S --noconfirm --needed zed # Высокопроизводительный многопользовательский редактор кода от создателей Atom и Tree-sitter ; https://zed.dev/ ; https://archlinux.org/packages/extra/x86_64/zed/ ; https://github.com/zed-industries/zed
# curl https://zed.dev/install.sh | sh
# curl https://zed.dev/install.sh | ZED_CHANNEL=preview sh   # Предлагаем предварительную сборку Zed, которая получает обновления примерно на неделю раньше стабильной.
curl https://zed.dev/install.sh | shCode language: Bash (bash)
### После завершения установки выполните следующие две команды:
...
yay -S visual-studio-code-bin  # https://aur.archlinux.org/packages/visual-studio-code-bin/
...
############# Takao Fonts ###############
# yay -S otf-takao --noconfirm  # Японские контурные шрифты на основе шрифтов IPA (otf-ipafont) ; https://aur.archlinux.org/otf-takao.git (только для чтения, нажмите, чтобы скопировать) ; https://launchpad.net/takao-fonts ; https://aur.archlinux.org/packages/otf-takao
...
### yay -S wps-office-extension-russian-dictionary --noconfirm  # Русский словарь для WPS Office  NO!!!
...
Для установки WPS Office в Ubuntu Linux необходимо скачать deb пакет (имя фида wps-office_10.1.0.5707~a21_amd64.deb) с сайта сообщества: http://wps-community.org/downloads
Для установки дважды щелкните по загруженному файлу и следуйте указаниям установщика.

Также можно выполнить установку deb-пакета из командной строки. Для этого выполните в терминале команду (не забудьте указать правильный путь и имя deb-файла):

sudo dpkg -I ./wps-office_10.1.0.5707~a21_amd64.deb
Запустить приложения из пакета WPS Office можно из главного меню системы из раздела Офис.
