...
# Если вы используете LVM для вашего /boot, вы можете установить GRUB на нескольких физических дисках.
 ...
 grub-install /dev/$x_cfd   #sda sdb sdc sdd # Записываем загрузчик в MBR (Master Boot Record) нашего внутреннего накопителя
...
# Если нужно установить BIOS-версию загрузчика из-под системы, загруженной в режиме UEFI
 grub-install --target=i386-pc /dev/$x_cfd   #sda sdb sdc sdd # Записываем загрузчик в MBR (Master Boot Record) нашего внутреннего накопителя
...
#grub-mkconfig --output=/boot/grub/grub.cfg  # Генерируем файл конфигурации загрузчика
# ------------------------------------------------------------- 
...
# Выполните команду:
pacman -S sudo --noconfirm  # Предоставить определенным пользователям возможность запускать некоторые команды от имени пользователя root
# --------------------------------------------------------
...
# ------------------------------------------------------------
# Разбор утилит для установки:
# wget  - Сетевая утилита для извлечения файлов из Интернета
# git  - Быстрая распределенная система контроля версий
# curl  - Утилита и библиотека для поиска URL
# https://www.archlinux.org/packages/extra/x86_64/wget/
# https://www.archlinux.org/packages/extra/x86_64/git/
...
echo "" 
echo -e "${CYAN}=> ${NC}Удалить кэш ВСЕХ установленных пакетов (высвобождая место на диске)?"
...
echo ""    
pacman -Scc  # Удалит кеш всех пакетов (можно раз в неделю вручную запускать команду)
#pacman --noconfirm -Scc  # --noconfirm  --не спрашивать каких-либо подтверждений
echo "" 
