...
btrfs subvolume create /mnt/@tmp          # @tmp - Основное расположение временных файлов
btrfs subvolume create /mnt/@cache        # @cache - Это кэш пакетного менеджера (pacman)
# btrfs su cr /mnt/@snapshots             # @snapshots – Каталог для хранения снимков.
btrfs subvolume create /mnt/@.snapshots   # @snapshots - Snapper будет хранить здесь ваши снимки BTRFS. Если Snapper не используется, этот раздел не нужен. Содержит снапшоты корня, которые создает snapper.
...
## mount -o ${sub}=/@var_log /dev/$root  /mnt/var/log
# mount -o ${sub}=/@log /dev/$root  /mnt/var/log
