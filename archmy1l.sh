...
# ----------------------------------------------------------------
# netctl - это CLI, сетевой менеджер для настройки и управления сетевыми подключениями через профили. Это нативный проект Arch Linux для настройки сети.
# dhcpcd - это клиент DHCP и DHCPv6. В настоящее время это самый многофункциональный DHCP-клиент с открытым исходным кодом.
# which - Утилита для отображения полного пути команд
# inetutils - Коллекция (Набор) общих сетевых программ
# parted -  утилита для управления таблицами раздел в Unix/Linux ОС. Данная утилита позволяет работать с дисками и разделами ( создавать, удалять, изменять) и легка в понимании. GParted - это интерфейс с графическим интерфейсом.
# rp-pppoe - Точка-точка (Point-to-Point) протокола роутинга Penguin's через Ethernet-клиент. ppp (Paul's PPP Package) — пакет с открытым исходным кодом, который реализует протокол соединения точка-точка (PPP) для систем Linux и Solaris. Пакет предоставляет демон pppd, который может быть использован вместе с xl2tpd, pptpd и netctl. Если подключение к Интернету DSL должно быть установлено с модемом DSL, но без маршрутизатора, это можно установить через ppp с помощью пакета rp-pppoe.
# ----------------------------------------------------------------
