...
ip link  # проверить доступность сетевых адаптеров
sleep 01
echo ""
...
echo ""
echo -e "${BLUE}:: ${NC}Install the DOS File System Utilities"  # Утилиты файловой системы DOS
...
echo ""
echo -e "${YELLOW}==> Примечание! ${BOLD}*Для переключения между русским и английским языком используется сочетание клавиш Ctrl+Shift ${NC}"
sleep 03
clear
######################
...
### Команда, чтобы не вводить 'y'
  echo 'y' | mkfs.ext4 /dev/$root -L root  # Форматировать в Ext4
 echo ""
...
# echo 'y' | mkfs.xfs -f /dev/$root -L root  # Форматировать в XFS Принудительно! Если на нём уже существует существующая файловая система.
...
### Команда, чтобы не вводить 'y'
  echo 'y' | mkfs.f2fs -f /dev/$root -L root  # Форматировать в F2fs
 echo ""
 echo " Проверка файловой системы на ошибки и их автоматическое исправление "
### Fsck (File System Consistency Check) — утилита для Linux и Unix-подобных систем, которая проверяет и (по возможности) чинит файловые системы (ФС). Работает почти со всеми популярными ФС: ext2/3/4, XFS, Btrfs, ReiserFS, JFS и т. д..
### Базовый синтаксис: fsck <options> <filesystem>. В этом примере файловой системой может быть устройство, раздел, точка монтирования и так далее. Некоторые опции: -A — проверить все файловые системы из /etc/fstab; -C — показать прогресс проверки файловой системы; -M — не проверять, если файловая система смонтирована; -N — ничего не выполнять, показать, что проверка завершена успешно.
...
### Команда, чтобы не вводить 'y'
  echo 'y' | mkfs.jfs -f /dev/$root -L root  # Форматировать в JFS
# echo 'y' | mkfs.jfs /dev/$root -L root  # Форматировать в JFS
 echo ""
 echo " Проверка файловой системы на ошибки и их автоматическое исправление "
### Fsck (File System Consistency Check) — утилита для Linux и Unix-подобных систем, которая проверяет и (по возможности) чинит файловые системы (ФС). Работает почти со всеми популярными ФС: ext2/3/4, XFS, Btrfs, ReiserFS, JFS и т. д..
### Базовый синтаксис: fsck <options> <filesystem>. В этом примере файловой системой может быть устройство, раздел, точка монтирования и так далее. Некоторые опции: -A — проверить все файловые системы из /etc/fstab; -C — показать прогресс проверки файловой системы; -M — не проверять, если файловая система смонтирована; -N — ничего не выполнять, показать, что проверка завершена успешно.
...
### Команда, чтобы не вводить 'y'
  echo 'y' | mkfs.f2fs -f /dev/$home -L home  # Форматировать F2fs
  echo ""
  echo " Создания папки home в каталоге /mnt "
# mkdir /mnt/boot
# mkdir /mnt/home
...
### Команда, чтобы не вводить 'y'
  echo 'y' | mkfs.jfs -f /dev/$home -L home  # Форматировать JFS
# echo 'y' | mkfs.jfs /dev/$home -L home  # Форматировать в JFS
  echo ""
  echo " Создания папки home в каталоге /mnt "
# mkdir /mnt/boot
# mkdir /mnt/home
