...
echo -e "${CYAN}:: ${NC}DMI (Desktop Management Interface) - интерфейс (API), позволяющий программному обеспечению собирать данные о характеристиках компьютера. "
echo ""
echo " Установка утилиты (пакета) Dmidecode (dmidecode) "
...
btrfs subvolume create /mnt/@.snapshots    # @snapshots - Snapper будет хранить здесь ваши снимки BTRFS. Если Snapper не используется, этот раздел не нужен.
### Удаление подтома:
...
#sub='rw,relatime,space_cache=v2,autodefrag,nodatacow,subvol'   # для HDD (Твердотельные накопитель, Жёсткий диск, винчестер)
mount -o ${sub}=@ /dev/$root /mnt
# mount -o noatime,commit=120,space_cache=v2,subvol=@ /dev/$root /mnt
### Монтирование в каталоге /mnt для SSD (Solid-State Drive) - Nvme - m2 - usb_flash #######
# mount -o rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,subvol=@ /dev/$root /mnt
...
### Монтирование папок в каталоге /mnt для SSD (Solid-State Drive) - Nvme - m2 - usb_flash #######
# sub='rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,subvol'  # для SSD (Solid-State Drive) - Nvme - m2 - usb_flash
# mount -o rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,subvol=@home  /dev/$root  /mnt/home
# mount -o rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,subvol=@var  /dev/$root  /mnt/var
# mount -o rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,subvol=@log /dev/$root /mnt/var/log
# mount -o rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,subvol=@pkg /dev/$root /mnt/var/cache/pacman/pkg
# mount -o rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,subvol=@.snapshots  /dev/$root /mnt/.snapshots
# mount -o rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,subvol=@tmp /dev/$root /mnt/tmp
# mount -o rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,subvol=@opt /dev/$root /mnt/opt
### Монтирование папок в каталоге /mnt для для HDD (Твердотельные накопитель, Жёсткий диск, винчестер) ####
# sub='rw,relatime,space_cache=v2,autodefrag,nodatacow,subvol'   # для HDD (Твердотельные накопитель, Жёсткий диск, винчестер)
...
# mount -o rw,noatime,compress=zstd:2,space_cache=v2,discard=async,subvol=@log /dev/$root /mnt/var/log
# mount -o rw,noatime,compress=zstd:2,space_cache=v2,discard=async,subvol=@pkg /dev/$root /mnt/var/cache/pacman/pkg
# mount -o rw,noatime,compress=zstd:2,space_cache=v2,discard=async,subvol=@.snapshots /dev/$root /mnt/.snapshots
# mount -o rw,noatime,compress=zstd:2,space_cache=v2,discard=async,subvol=@tmp /dev/$root /mnt/tmp
# mount -o rw,noatime,compress=zstd:2,space_cache=v2,discard=async,subvol=@opt /dev/$root /mnt/opt
################################
...
sleep 01
echo ""
echo -e "${BLUE}:: ${NC}Установить файловую систему Btrfs для HOME- (домашнего раздела)?"
echo " Если таковой был создан при разметке в cfdisk "
echo " Либо HOME был создан вами заранее и готов к дальнейшим действиям "
echo " Или ваш HOME раздел будет находится (расположен) в ROOT ( / корневом ) разделе системы? "
...
mount -o ${sub}=@home /dev/$root /mnt/home
# mount -o subvol=/@home,defaults,rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,commit=120 /dev/$root /mnt/home
# mount -o rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,subvol=/@home  /dev/$root  /mnt/home
####### Монтирование @root #######
...
# mount -o subvol=/@srv,defaults,rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,commit=120 /dev/$root /mnt/srv
...
# mount -o subvol=/@home.snapshots,defaults,rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,commit=120 /dev/$root /mnt/home/.snapshots
####### Монтирование @opt #######
### /opt – используется для установки дополнительных пакетов программного обеспечения
mount -o ${sub}=@opt /dev/$root  /mnt/opt
# mount -o subvol=/@opt,defaults,rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,commit=120 /dev/$root /mnt/opt
####### Монтирование @usr #######
...
# mount -o subvol=/@usr,defaults,rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,commit=120 /dev/$root /mnt/usr
####### Монтирование @local #######
### /usr/local – каталог для пользовательских программ, установленных из исходников.
mount -o ${sub}=@local /dev/$root  /mnt/usr/local
# mount -o subvol=/@local,defaults,rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,commit=120 /dev/$root /mnt/usr/local
####### Монтирование @var #######
### /var – содержит переменные данные, такие как журналы, временные файлы и кэши, которые имеют
mount -o ${sub}=@var /dev/$root /mnt/var
# mount -o subvol=/@var,defaults,rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,commit=120 /dev/$root /mnt/var
####### Монтирование @cache #######
### @cache | /var/cache (выставляю nocow атрибут "chattr +C")
mount -o ${sub}=@cache /dev/$root /mnt/var/cache
...
mount -o ${sub}=@log /dev/$root  /mnt/var/log
# mount -o subvol=/@log,defaults,rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,commit=120 /dev/$root /mnt/var/log
####### Монтирование @pkg #######
mount -o ${sub}=@pkg /dev/$root  /mnt/var/cache/pacman/pkg
# mount -o subvol=/@pkg,defaults,rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,commit=120 /dev/$root /mnt/var/cache/pacman/pkg
...
# mount -o subvol=/@libvirt,defaults,rw,noatime,compress=zstd:2,ssd,space_cache=v2,discard=async,commit=120 /dev/$root /mnt/var/lib/libvirt
...
### mount -o ${sub}=/@abs /dev/$root /mnt/var/cache/pacman/pkg
