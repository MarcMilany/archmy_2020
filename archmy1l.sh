...
pacman -S --noconfirm --needed pacman-contrib  # Добавлены скрипты и инструменты для систем pacman ; https://archlinux.org/packages/extra/x86_64/pacman-contrib/ ; https://gitlab.archlinux.org/pacman/pacman-contrib ; 2025-08-27 00:08 UTC
...
pacman -S --noconfirm --needed util-linux  # Различные системные утилиты для Linux ; https://archlinux.org/packages/core/x86_64/util-linux/ ; https://github.com/util-linux/util-linux ; Обеспечивает: hardlink, rfkill ; Конфликты: с hardlink, rfkill; Заменяет: hardlink, rfkill ; 2025-09-23 12:36 UTC
...
#mkfs.vfat -F32 -n "boot"/dev/$bootd  # -n ИМЯ-ТОМА Задаёт имя тома (метку) файловой системы. Имя тома может содержать до 11 символов. Использование пустой строки, строки, состоящей только из пробелов, или строки «NO NAME» в качестве VOLUME-NAME равносильно отсутствию параметра -n . Значение по умолчанию — без метки.
mkfs.vfat -F32 -n "EFI" /dev/$bootd
# mkfs -t vfat /dev/$bootd  # Для диска формата файловой системы FAT32
 echo ""
 echo " Создания папки boot в каталоге /mnt "
# Используем более закрытые права на каталог
# mkdir -m 700 /mnt/boot/efi  # права доступа (и флаг исполнения для каталога)
# mkdir /mnt/boot  # (cоздаем каталог для загрузчика — если обычный BIOS )
# mkdir -p /mnt/boot  # mkdir -p НЕ выдаст ошибку, если каталог уже существует и его содержимое не изменится.
...
# mount /dev/$bootd /mnt/boot/EFi  # ( для UEFI биоса )
