...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить скрипт, который автоматизирует большую часть того, что называется clean-chroot-manager (https://aur.archlinux.org/packages/clean-chroot-manager) , и предлагается здесь, в AUR?"
echo -e "${MAGENTA}:: ${BOLD}Использование помощника AUR, такого как yay, для сборки пакетов, включая backintime, КРАЙНЕ не рекомендуется. Рекомендуемый метод сборки — использовать чистый chroot. См.: https://wiki.archlinux.org/title/DeveloperWiki:Building_in_a_clean_chroot .. ${NC}"
sudo pacman -Syy  # обновление баз пакмэна (pacman)
