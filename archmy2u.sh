...
#!/bin/bash
apptitle="Arch Linux Fast Install v2.5 UEFI - Version: 2025.06.16.00.40.38 (GPLv3)"
...
EDITOR=nano
###
ARCHMY2U="russian"  # Installer default language (Язык установки по умолчанию)
...
clear
echo ""
echo -e "${GREEN}==> ${NC}Установить (bootloader) загрузчик UEFI-GRUB?"
echo -e "${BLUE}:: ${NC}Установка GRUB2 в процессе установки Arch Linux"
echo " 1 - Установка полноценной версии загрузчика UEFI-GRUB, тогда укажите "1" "
echo " Файлы загрузчика будут установлены в каталог /boot. Код GRUB (boot.img) будет встроен в начальный сектор, а загрузочный образ core.img в просвет перед первым разделом MBR, или boot partition для GPT. "
echo " 2 - Если нужно можно установить rEFInd - UEFI-загрузчик, способный запускать ядра EFISTUB. Это форк более не поддерживаемого rEFIt, и в нем исправлены многие проблемы, касающиеся загрузки UEFI вне Mac-устройств. Он построен вокруг идеи кроссплатформенности и упрощения загрузки нескольких операционных систем, тогда укажите "2" "
echo " rEFInd поставляется с драйверами UEFI, которые реализуют поддержку в режиме "только чтение" ReiserFS (устарел), Ext2, Ext4, Btrfs, ISO-9660 и HFS+. Вдобавок rEFInd имеет доступ к любой файловой системе, к которой имеет доступ сам UEFI, включая FAT (как указано в спецификации UEFI), HFS+ на Mac и ISO-9660 на некоторых системах . (https://wiki.archlinux.org/title/REFInd_(%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9)) "
echo -e "${CYAN} Пример: ${NC}refind-install --usedefault /dev/sdXY
    Где /dev/sdXY - ваш системный раздел EFI (устройство, а не точка его монтирования)."
echo " Совет: По умолчанию refind-install устанавливает лишь драйвер для той файловой системы, где находится ядро. Драйверы дополнительных файловых систем нужно устанавливать вручную копированием из /usr/share/refind/drivers_x64/ в esp/EFI/refind/drivers_x64/, или Вы можете установить все драйвера с опцией --alldrivers. Это полезно для загрузочных USB-устройств. "
echo " После установки файлов rEFInd в ESP убедитесь, что rEFInd создал файл refind_linux.conf, содержащий параметры ядра в том же каталоге, где и Ваше ядро. Этот конфигурационный файл не будет создан, если Вы использовали опцию --usedefault - запустите mkrlconf с правами root, чтобы создать его . "
...
# echo " Использование прелоадера: Выполните refind-install с опцией --preloader /путь/к/прелоадеру "
...
if [[ $dual_boot  == 1 ]]; then
  ...
  pacman -S os-prober mtools fuse --noconfirm  # Утилита для обнаружения других ОС на наборе дисков; Сборник утилит для доступа к дискам MS-DOS;
...
#pacman -S --noconfirm --needed pamixer  # микшер командной строки Pulseaudio, как amixer ; pamixer похож на amixer, но для pulseaudio. Он может управлять уровнями громкости приемников ; Также этот проект может предоставить вам небольшую библиотеку C++ для управления PulseAudio ; https://github.com/cdemoulins/pamixer ; https://archlinux.org/packages/extra/x86_64/pamixer/
...
echo " Создадим файл (скрипт) paccache.timer в /etc/systemd/system/ — система автоматически будет запускать его раз в месяц с помощью таймера systemd . *По сути, файл paccache.timer в /etc/systemd/system/ запустит /usr/lib/systemd/system/paccache.service, который будет запускать paccache ежемесячно и очищать кэш ваших старых и неустановленных пакетов. "
...
echo -e "${MAGENTA}:: ${BOLD}Grub2-Theme-Vimix - Размытая тема для grub2 . ${NC}"
...
echo -e "${MAGENTA}:: ${BOLD}Стандартный каталог Grub для тем — /usr/share/grub/themes/ ${NC}"
echo " pacman проверяет наличие конфликтующих файлов перед установкой, удалите /boot/grub/themes/Archlinux перед обновлением, пока не будет найдено лучшее решение для установки #rm -rf /boot/grub/themes/Archlinux " 
echo -e "${CYAN}:: ${NC}Установка Grub2-Theme-Vimix пакет (grub2-theme-vimix) проходит через сборку из https://github.com/kalax2/grub2-theme-vimix. Сборка пакета проходит через - git clone - эта функция прописана в сценарии скрипта. "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
...
echo " Установка утилит (пакетов) пропущена "
  ...
  echo ""
  echo " Установка Grub2-Theme-Vimix "
sudo pacman -Syy  # обновление баз пакмэна (pacman)  
cd /home/$username
##### Download Themes
