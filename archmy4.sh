...
echo -e "${MAGENTA}:: ${BOLD}Nomacs - это бесплатная программа для просмотра изображений с возможностями обработки фото. Вы можете использовать его для просмотра всех распространенных форматов изображений, включая изображения RAW и psd. ${NC}"
...
echo " Nomacs распространяется бесплатно и имеет открытый исходный код. Программа кроссплатформенная и может работать в Linux, Windows, MacOS. Работает Nomacs очень быстро, изображения грузятся без задержек. Интерфейс очень отзывчивый. "
echo ""
while
...
yay -S --noconfirm --needed nomacs  # для просмотра изображений Qt
echo ""
...
5. Видниор
Viewnior — это элегантный и минималистичный просмотрщик изображений для систем Linux. Он поддерживает различные функции, такие как полноэкранный режим, слайд-шоу, поворот, переворот, обрезка и другие операции с изображениями. Он также имеет настраиваемое действие мыши и простой интерфейс.

Некоторые уникальные особенности Viewnior:

Просмотрщик изображений Viewnior использует модифицированную пользователем библиотеку GtkImageView. Эта библиотека обеспечивает быстрый и плавный просмотр изображений.
Он может просматривать только выбранные изображения. Таким образом, вы можете отфильтровать нежелательные файлы.
Он может автоматически определять среду рабочего стола и устанавливать соответствующее изображение в качестве обоев.
Он может отображать метаданные EXIF ​​и IPTC. Оба содержат настройки камеры, местоположения и информацию о дате.
Вы можете установить Viewnior на свою систему Arch с помощью Pacman:

sudo pacman -S viewnior


После установки откройте изображение, чтобы проверить работу Viewnior:

viewnior ~/Downloads/test_image.jpg


Кроме того, вы можете открыть изображение с помощью Viewnior, непосредственно из файлового менеджера.
...
6. Кочевники
Nomacs — это просмотрщик изображений с открытым исходным кодом с поддержкой множества форматов изображений. Он также поддерживает изображения RAW и PSD. Он предоставляет основные функции для настройки изображений, такие как параметры цвета, яркости, изменения размера и обрезки.

Nomacs предлагает следующие возможности:

Он дает вам дополнительные сведения об изображениях. Эти сведения включают миниатюры, метаданные и гистограммы.
Вы также можете открывать файлы zip или MS Office, содержащие изображения.
Он может синхронизировать несколько экземпляров средства просмотра и накладывать их друг на друга с изменяющейся прозрачностью.
Он может выполнять основные функции редактирования изображений, такие как обрезка, изменение размера, корректировка цвета и псевдоцвета.
Ниже приведена команда установки просмотрщика изображений Nomacs с помощью AUR helper:

ура -S кочевники
Скриншот компьютера Описание создано автоматически

Теперь получите доступ к любому изображению из терминала с помощью средства просмотра изображений Nomacs:

кочевники ~/Downloads/test_image.jpg
Скриншот компьютера Описание создано автоматически

В качестве альтернативы попробуйте воспользоваться средством просмотра изображений Nomacs из файлового менеджера.

Скриншот компьютера Описание создано автоматически

Вы также можете получить пакет AUR Nomacs, напрямую открыв его каталог через терминал и скомпилировав его позже с помощью команды makepkg:

git-клон https://aur.archlinux.org/nomacs.git
...
10. Глаз ГНОМА
Eye of GNOME (EOG) — это просмотрщик изображений для среды рабочего стола GNOME. Он может отображать изображения в различных форматах. Поддерживает файлы PNG, JPEG и TIFF. Также имеет некоторые функции редактирования и печати. ​​EOG также поддерживает плагины для добавления дополнительных функций.

Некоторые уникальные особенности EOG:

EOG может отображать все изображения в папке в виде слайд-шоу или в режиме галереи.
Он может конвертировать изображения из одного формата в другой.
EOG может переименовывать несколько изображений одновременно.
Глаз GNOME может отображать подробную информацию о снимке. Эта информация включает настройки камеры, гистограмму, геолокацию и дату.
Можно установить текущее изображение в качестве фона рабочего стола или отправить его по электронной почте.
EOG также имеет консоль Python, которую можно использовать для запуска скриптов и добавления пользовательских функций.
Чтобы установить EOG на Arch Linux, выполните следующую команду:

sudo pacman -S eog
Скриншот компьютера Описание создано автоматически

После установки проверьте ее, открыв изображение:

eog ~/Загрузки/тестовое_изображение.jpg


Вы можете открыть изображение с помощью EOG непосредственно через файловый менеджер.



Eye of GNOME также доступен в виде пакета AUR. Чтобы получить его, выполните следующую команду в консоли:

ура -С еог-гит
Скриншот компьютерной программы Описание создано автоматически

Кроме того, вы можете загрузить и скомпилировать пакет вручную с помощью следующих команд:

git-клон https://aur.archlinux.org/eog-git.git
...
Написать обзор программы Okular
зарегистрируйтесь, чтобы получить доступ к этим функциям
Okular
Okular — приложение для просмотра документов.

Возможности
Поддерживается большое число форматов: PDF, PS, CHM, DjVu и другие.
Просмотр документов в формате Markdown.
Поддержка добавления аннотаций с форматированием текста аннотации.
Комментирование, выделение текста цветом.
Предпросмотр миниатюр страниц документа.
Добавление закладок.
Просмотр информации о цифровой подписи документа.
Поиск по документу.
Поворот страниц.
Режимы просмотра: Одна страница, Разворот, Непрерывный, Обзор.
Установка
...
Okular - просмотр документов
 ...
 Программа командной строки, конвертирующая документы RTF в другие форматы
