...
sudo pacman -S --noconfirm --needed virtualbox --noconfirm  # Мощная виртуализация x86 как для корпоративного, так и для домашнего использования
#sudo pacman -S --noconfirm --needed virtualbox-host-modules-arch  # для ядра linux - Модули ядра хоста Virtualbox для Arch Kernel
...
yay -S lib32-simplescreenrecorder --noconfirm
...
yay -S skypeforlinux-bin skype-call-recorder vk-messenger viber pidgin-extprefs --noconfirm 
...
yay -S thunar-shares-plugin --noconfirm  # Плагин Thunar для быстрого совместного использования папки с помощью Samba без необходимости root-доступа. 
