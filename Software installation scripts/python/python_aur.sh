...
PYTHON_AUR="russian"  # Installer default language (Язык установки по умолчанию)
...
echo " Установка Python2 из AUR (через - yay) - Собирается ДОЛГО!!!проблема импортирования ключей "
# Вы МОЖЕТЕ в скрипте закомментировать НЕнужные вам пакеты! 
########## Python AUR ########### есть собрался
######### python2 ################ - Собирается ДОЛГО!!!проблема импортирования ключей
...
# rm -Rf python-basiciw
######### python-bencode.py ################ есть собрался
...
######### python-requests-cache ################ есть собрался
...
######## python-future #####
### yay -S python-futur --noconfirm  # Чистая поддержка одного источника для Python 3 и 2
######## python-random2 #####
yay -S python-random2 --noconfirm  # Python 3 совместимый порт случайного модуля Python 2
########## Python2 AUR ###########
