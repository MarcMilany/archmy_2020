...
PYTHON="russian"  # Installer default language (Язык установки по умолчанию)
...
echo " Установка дополнительных пакетов - Python "
##########  IPython ###########
sudo pacman -S --noconfirm --needed ipython  # IPython - Улучшенная интерактивная оболочка Python ; https://ipython.org ; https://archlinux.org/packages/extra/any/ipython/
########## Python ###########
sudo pacman -S --noconfirm --needed python  # Новое поколение языка сценариев высокого уровня Python # возможно присутствует
sudo pacman -S --noconfirm --needed python-audit  # Компоненты пользовательского пространства фреймворка аудита — привязки Python 
...
sudo pacman -S --noconfirm --needed pybind11  # Облегченная библиотека, содержащая только заголовки, которая отображает типы C++ в Python и наоборот
...
sudo pacman -S --noconfirm --needed python-brotli  # Универсальный алгоритм сжатия без потерь — библиотека Python
...
sudo pacman -S --noconfirm --needed python-geoipsets  # Инструмент для генерации диапазонов IP-адресов для конкретных стран для iptables/ipset и nftables
...
sudo pacman -S --noconfirm --needed python-nskeyedunarchiver  # Библиотека для декодирования текстовых или двоичных plist-файлов Apple
...
sudo pacman -S --noconfirm --needed python-pikepdf  # Чтение и запись PDF-файлов с помощью Python на базе qpdf
...
sudo pacman -S --noconfirm --needed python-pymediainfo  # Обертка Python вокруг библиотеки MediaInfo
...
sudo pacman -S --noconfirm --needed python-pytz  # Кроссплатформенная библиотека часовых поясов для Python
...
sudo pacman -S --noconfirm --needed python-reportlab  #  Проверенное надежное решение для создания PDF-файлов
...
sudo pacman -S --noconfirm --needed python-shtab  # Автодополнение оболочки с помощью клавиши Tab для приложений Python CLI
...
sudo pacman -S --noconfirm --needed python-setuptools-scm  # Управляет версиями пакетов Python в метаданных SCM
...
sudo pacman -S --noconfirm --needed python-wheel  # Встроенный формат пакета для Python
sudo pacman -S --noconfirm --needed python-xapp  # Библиотека Python Xapp
sudo pacman -S --noconfirm --needed python-xlib  # Полнофункциональная клиентская библиотека X для программ Python
...
# sudo pacman -S --noconfirm --needed python-pylast  # Интерфейс Python для Last.fm и других API-совместимых веб-сайтов, таких как Libre.fm ; https://github.com/pylast/pylast ; https://archlinux.org/packages/extra/any/python-pylast/  # 
