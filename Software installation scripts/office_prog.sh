...
# Новая версия:
# gsettings set org.gnome.gedit.preferences.encodings candidate-encodings "['UTF-8', 'WINDOWS-1251', 'KOI8-R', 'CURRENT', 'ISO-8859-15', 'UTF-16', 'ISO_8859-1', 'LOCALE']"
# Из Medit:
# UTF-8,LOCALE,KOI8-R,WINDOWS-1251,ISO_8859-15,ISO_8859-1 # original
# UTF-8,LOCALE,KOI8-R,WINDOWS-1251,ISO_8859-15,ISO_8859-1,UTF-16,CURRENT  # repack
...
clear
echo ""
echo -e "${GREEN}==> ${NC}Установить Mousepad (mousepad) - Текстовый редактор?"
#echo -e "${BLUE}:: ${NC}Установить Mousepad (mousepad) - Текстовый редактор?"
#echo 'Установить Mousepad (mousepad) - Текстовый редактор?'
# Install Mousepad (mousepad) - A text editor?
echo -e "${MAGENTA}=> ${BOLD}Mousepad — это простой, графический текстовый редактор, разработанный для использования в терминале в системах Linux. Он известен своей легкостью и высокой производительностью, что делает его популярным выбором среди пользователей, предпочитающих работать в командной строке. Mousepad написанный для Xfce , среды рабочего стола Linux . Программа имеет небольшой размер, похожа на Leafpad, но имеет дополнительные функции, такие как плагины, история поиска и автоматическая перезагрузка. Название Mousepad происходит от мыши в логотипе Xfce. Mousepad имеет простой и интуитивно понятный интерфейс, поддерживает несколько языков и включает встроенную проверку орфографии. Изначально Mousepad разрабатывался с целью создать редактор аналогичный Leafpad, но с возможностью печати документов. Сейчас Leafpad также поддерживает печать. ${NC}"
echo -e "${CYAN}:: ${NC}Основные возможности: Поддерживаются все базовые функции работы с текстом. Преобразование текста (в строчные, заглавные, обратить регистр, с заглавной и так далее). Автоматическая расстановка отступов. Перенос длинных строк. Подсветка синтаксиса. Подсветка текущей строки. Подсветка парных скобок. Настраиваемый режим табуляции. Режим - Только для чтения. Поддержка вкладок. Выбор цветовой схемы. Выбор шрифта текста. Возможность включить/отключить отображение нумерации строк, непечатаемых символов, окончаний строк, панели меню, панели инструментов, строки состояния. Режим - Во весь экран. Подключаемые плагины (например, Проверка орфографии). Печать документов. И другие.  "
echo " Программу разработал программист Эрик Харрисон (Erik Harrison). Исходный код: Open Source (открыт) ; Языки программирования: C ; Библиотеки: GTK ; Лицензия: GNU GPL ; Приложение переведено на русский язык. В целом, Mousepad является универсальным и эффективным текстовым редактором для пользователей Linux, поскольку его легко установить и использовать, а также он предлагает широкий спектр функций, позволяющих легко редактировать текстовые файлы в терминале. "
echo -e "${MAGENTA}=> ${NC}Список программ (пакетов) для установки: - (mousepad)."
echo " Будьте внимательны! Процесс установки, был прописан полностью автоматическим. "
# Be careful! The installation process was fully automatic
echo " Если Вы сомневаетесь в своих действиях, ещё раз обдумайте... "
# If you doubt your actions, think again...
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Да установить,     0 - НЕТ - Пропустить установку: " i_mousepad # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$i_mousepad" =~ [^10] ]]
do
    :
done
if [[ $i_mousepad == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $i_mousepad == 1 ]]; then
  echo ""
  echo " Установка текстового редактора Mousepad (mousepad) "
sudo pacman -S --noconfirm --needed mousepad  # Простой текстовый редактор для Xfce ; https://archlinux.org/packages/extra/x86_64/mousepad/ ; https://docs.xfce.org/apps/mousepad/start ; https://github.com/codebrainz/mousepad ; Группы: xfce4-goodies ; 2025-03-31 19:16 UTC
...
##########
### Используя новый .mousepad класс стиля, я могу получить желаемое поведение с помощью этого фрагмента CSS:
# Он должен работать с Adwaita и Greybird и влияет только на развернутое окно.
# ~/.config/Mousepad/mousepad.css
...
# Kate - ArchWiki (https://wiki.archlinux.org/title/Kate)
# Отключить отслеживание символических ссылок по умолчанию для режима каталога
# https://invent.kde.org/utilities/kate/-/blob/master/addons/project/kateproject.example
# kateproject.example
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Pluma (pluma) — Текстовый редактор?"
echo -e "${YELLOW}==> Примечание! ${BOLD} *Pluma — это форк Gedit. ${NC}"
echo -e "${MAGENTA}:: ${BOLD}Pluma — текстовый редактор, поддерживающий большинство стандартных функций. Он также расширяет эту базовую функциональность другими функциями, которые обычно не встречаются в простых текстовых редакторах. Pluma — графическое приложение, поддерживающее редактирование нескольких текстовых файлов в одном окне (иногда называемом вкладками или MDI ). Этот проект Лицензируется под GPL-2.0 . ${NC}"
echo " Домашняя страница: https://github.com/mate-desktop/pluma ; (https://man.archlinux.org/man/extra/pluma/pluma.1.en ; https://archlinux.org/packages/extra/x86_64/pluma/ ; https://mate-desktop.org/). "
echo -e "${BLUE}:: ${NC}Функции: Pluma полностью поддерживает интернациональный текст благодаря использованию кодировки Unicode UTF -8 в редактируемых файлах. Его основные функции включают подсветку синтаксиса исходного кода, автоматические отступы и поддержку печати (с предварительным просмотром). "
echo -e "${CYAN}:: ${NC} .  "
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Да установить,     0 - НЕТ - Пропустить установку: " in_pluma  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_pluma" =~ [^10] ]]
do
    :
...
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_pluma == 1 ]]; then
  echo ""
  echo " Установка утилиты (пакета) Pluma (pluma) "
#sudo pacman -Syy  # обновление баз пакмэна (pacman)
...
sudo pacman -S --noconfirm --needed pluma  # Мощный текстовый редактор для MATE ; Pluma — это форк Gedit ; https://archlinux.org/packages/extra/x86_64/pluma/ ; https://mate-desktop.org/ ; https://github.com/mate-desktop/pluma ; Группы: mate-extra ; Заменяет: pluma-gtk3 ; Конфликты: с pluma-gtk3 ; 2025-04-30 17:36 UTC
  echo ""
  echo " Посмотрите информацию о версии (pluma) "
pluma --version  # Показать версию приложения
sudo pacman -Q pluma  # Показать версию приложения
sleep 01
echo ""
...
############# Справка ##############
# Pluma (pluma):
# https://linuxcookbook.ru/articles/tekstovyy-redaktor-pluma
# https://translated.turbopages.org/proxy_u/en-ru.ru.3b06175f-68c43eac-15974068-74722d776562/https/en.wikipedia.org/wiki/Pluma_(text_editor)
####################################
...
echo " Установка утилит (пакетов) пропущена "
  ...
  echo ""
  echo " Установка утилиты разработки Geany (geany) "
sudo pacman -S --noconfirm --needed geany geany-plugins
# sudo pacman -S --noconfirm --needed geany  # Быстрая и легкая IDE ; https://archlinux.org/packages/extra/x86_64/geany/ ; https://www.geany.org/ ; 2025-08-05 15:36 UTC
# sudo pacman -S --noconfirm --needed geany-plugins  # Плагины для Geany ; https://archlinux.org/packages/extra/x86_64/geany-plugins/ ; https://plugins.geany.org/ ; 2025-08-05 15:36 UTC
echo ""
...
#sudo pacman -Syy  # обновление баз пакмэна (pacman)
sudo pacman -S --noconfirm --needed bluefish  # Мощный HTML-редактор для опытных веб-дизайнеров и программистов ; http://bluefish.openoffice.nl/ ; https://archlinux.org/packages/extra/x86_64/bluefish/ ; 2025-04-30 17:32 UTC
echo " Установка утилит (пакетов) выполнена "
fi
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить VSCodium (vscodium) - OpenSource Visual Studio Code без телеметрии?"
echo -e "${MAGENTA}:: ${BOLD}Visual Studio Code (VSCode) — это редактор кода, разрабатываемый компанией Microsoft. Код программы доступен в открытом доступе и распространяется по лицензии MIT. ${NC}"
echo " Домашняя страница: https://vscodium.com/ ; (https://github.com/VSCodium/vscodium) "
echo -e "${MAGENTA}:: ${BOLD}Свободные двоичные файлы программного обеспечения с открытым исходным кодом для Visual Studio Code. Это не форк. Это репозиторий скриптов для автоматической сборки репозитория Microsoftvscode в свободно лицензируемые двоичные файлы с конфигурацией по умолчанию, разработанной сообществом. Проект VSCodium создан для того, чтобы вам не пришлось скачивать и собирать исходный код. Этот проект включает в себя специальные скрипты сборки, которые клонируют репозиторий Microsoft vscode, выполняют команды сборки и загружают полученные двоичные файлы в релизы GitHub . Эти двоичные файлы лицензированы по лицензии MIT. Телеметрия отключена. ${NC}"
echo -e "${YELLOW}:: ${NC}Что представляет собой VSCodium и почему он появился? При подготовке готовых сборок программы, компания Microsoft клонирует Open Source репозиторий с исходным кодом Visual Studio Code и добавляет в программу дополнительную функциональность — сбор пользовательских данных (телеметрию), логотип и другие компоненты. В результате, получившееся сборки программы, уже не являются Open Source продуктом, имеют другую лицензию, и в таком виде предлагаются конечному пользователю. Проект VSCodium направлен на создание редактора Visual Studio Code, который не содержит функций телеметрии и других специфических компонентов от Microsoft. При этом используется исходный программный код Visual Studio Code. В результате вы получаете «чистую» сборку программы без «надстроек» от Microsoft и с открытым исходным кодом. То есть, если кратко, то VSCodium — это Open Source версия Visual Studio Code без телеметрии. Не смотря на цели VSCodium, некоторые пользователи скептически относятся к этому продукту. Кто-то считает, что смысла в нем особого нет, так как в исходном VSCode телеметрию можно отключить, и даже если в этом случае что-то и будет пересылаться в Microsoft, то ничего страшного в этом нет. Кто-то относится с подозрением к готовым сборкам самого VSCodium. "
echo " Возможности VSCodium повторяют возможности Visual Studio Code. Возможности программы: Visual Studio Code (VS code) имеет встроенную поддержку языков JavaScript, TypeScript, Node.js, а также большое количество дополнительно подключаемых языков, среди которых: C/C++, C#, Java, Python, PHP, Ruby, Rust, Go, Swift, HTML, XML и другие. VSCodium имеет современный не перегруженный и отзывчивый интерфейс. Вся работа ведется внутри главного окна. Оно разделяется на различные области. Новые окна открываются, как новая область или как всплывающие вспомогательные окна. Для Visual Studio Code существуют различные модули, которые позволяют добавить поддержку других языков программирования, а также повысить функциональность программы. Программа кроссплатформенная, переведена на русский язык. По умолчанию установлена темная тема оформления. Доступно несколько различных тем, от светлых до темных. "
echo -e "${YELLOW}==> Примечание! ${NC}Если вы хотите выполнить сборку из исходного кода самостоятельно, перейдите в репозиторий vscode от Microsoft и следуйте инструкциям . VSCodium создан для того, чтобы упростить получение последней версии VS Code, лицензированного MIT. "
echo -e "${CYAN}:: ${NC}Установка VSCodium (vscodium) и (vscodium-bin), проходит через сборку из исходников. То есть установка производиться с помощью git clone, PKGBUILD, makepkg - скачивается с сайта 'Arch Linux' (https://aur.archlinux.org/vscodium.git), (https://aur.archlinux.org/vscodium-bin.git) - собирается и устанавливается. "
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Установить VSCodium (vscodium),     2 - Установить VSCodium (vscodium-bin),

    0 - НЕТ - Пропустить установку: " in_vscodium  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_vscodium" =~ [^120] ]]
do
    :
done
if [[ $in_vscodium == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_vscodium == 1 ]]; then
  echo ""
  echo " Установка VSCodium (vscodium) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
############### vscodium ###############
yay -S vscodium --noconfirm  # Бесплатные/Libre исполняемые файлы программного обеспечения с открытым исходным кодом VSCode (сборка git из последнего релиза) ; https://aur.archlinux.org/packages/vscodium?all_deps=1#pkgdeps ; https://aur.archlinux.org/vscodium.git (только для чтения, нажмите, чтобы скопировать) ; https://github.com/VSCodium/vscodium.git ; https://github.com/VSCodium/vscodium/releases/download/1.102.14746/VSCodium-1.102.14746-src.tar.gz ; Обеспечивает: codium, vscodium ; Конфликты: codium, vscodium, vscodium-bin, vscodium-git ; 2025-07-16 21:20 (UTC) (UTC)
########### vscodium ############
#git clone https://aur.archlinux.org/vscodium.git  # (только для чтения, нажмите, чтобы скопировать)
#cd vscodium
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm    #--не спрашивать каких-либо подтверждений
...
echo ""
echo " Установка утилит (пакетов) выполнена "
elif [[ $in_vscodium == 2 ]]; then
  echo ""
  echo " Установка VSCodium (vscodium-bin) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
############### vscodium-bin ###############
yay -S vscodium-bin --noconfirm  # Бинарные релизы VS Code без брендинга/телеметрии/лицензирования MS ; https://aur.archlinux.org/packages/vscodium-bin ; https://aur.archlinux.org/vscodium-bin.git (только для чтения, нажмите, чтобы скопировать) ; Обеспечивает: codium, vscode, vscodium ; Конфликты: с vscodium ; https://github.com/VSCodium/vscodium/releases/download/1.102.14746/VSCodium-linux-x64-1.102.14746.tar.gz ; 2025-07-16 19:00 (UTC)
########### vscodium-bin ############
#git clone https://aur.archlinux.org/vscodium-bin.git  # (только для чтения, нажмите, чтобы скопировать)
#cd vscodium-bin
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm    #--не спрашивать каких-либо подтверждений
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить SciTE (scite) - Редактор кода?"
echo -e "${MAGENTA}:: ${BOLD}SciTE — Scintilla based Text Editor — текстовый редактор кода с подсветкой синтаксиса для Linux. SciTE — текстовый редактор на базе SCIntilla. Изначально разработанный для демонстрации возможностей Scintilla (https://www.scintilla.org/) , на котором работает и Notepad++ , он превратился в полезный редактор для широкого круга задач с функциями сборки и запуска программ. SciTE лучше всего подходит для задач с простыми конфигурациями, а также для создания самих SciTE и Scintilla. Основное предназначение Scintilla — редактирование исходных текстов программ. Редактор удобен для программистов тем, что имеет функцию сворачиванием кода, включает в себя подсветку синтаксиса и может открывать сразу несколько текстовых файлов. SciTE позволяет компилировать файлы прямо из интерфейса. Это значит, что результат отображается непосредственно в панели вывода редактора. Поддерживает подсветку синтаксиса для нескольких языков программирования PHP, Perl, C/C++/C#, Pascal, Java, а также HTML, XML, CSS, Java Script, SQL и многих других. В ОС Linux интерфейс SciTE реализован с помощью GTK+ . ${NC}"
echo " Домашняя страница: http://www.scintilla.org/SciTE.html ; (https://www.scintilla.org/SciTEDoc.html ; https://www.scintilla.org/) "
echo -e "${MAGENTA}:: ${BOLD}Редактирование текста в SciTE работает аналогично большинству редакторов Macintosh или Windows, но с добавлением функции автоматического оформления синтаксиса. SciTE может хранить в памяти несколько файлов одновременно, но видимым будет только один из них. Прямоугольные блоки текста в SciTE можно выделять, удерживая клавишу Alt в Windows или клавишу Ctrl в GTK при перемещении мыши по тексту. Клавишу-модификатор, используемую в GTK, можно изменить с помощью свойства rectangle.selection.modifier. ${NC}"
echo -e "${YELLOW}:: ${NC}В SciTE есть две панели: панель редактирования и панель вывода. Панель вывода расположена либо справа от панели редактирования, либо под ней. Изначально она имеет нулевой размер, но её можно увеличить, перетащив разделитель между ней и панелью редактирования. Команда «Параметры» | «Вертикальное разделение» позволяет переместить панель вывода под панель редактирования. "
echo " Особенности реализации отображения текста в SciTE: возможность буферизации графики, чтобы избежать мерцания при прорисовке символов; одновременное использование различных, в том числе и пропорциональных, шрифтов; быстрое изменение размера шрифтов; отображение номеров строк; подсветка синтаксиса для большого количества языков программирования; фолдинг, то есть сворачивание и разворачивание структурных блоков текста (классов, функций, циклов и т. п.). Особенности реализации редактирования: автоматическая установка отступов; подсветка парных или непарных (незакрытых) скобок; автодополнение (автоматическое завершение) используемых в файле имён типов, функций, переменных; всплывающие подсказки о параметрах функций; установка закладок; быстрый поиск по файлу и фиксируемое цветовое выделение выражения под курсором, клавиатурная навигация по выделенным фрагментам; подсветка всех видимых копий слова под курсором; поиск и замена текста с использованием регулярных выражений во всех открытых файлах (вместе с 2 предыдущими пунктами — инструменты упрощения рефакторинга). "
echo -e "${YELLOW}==> Примечание! ${NC}Существует ограничение на количество одновременно открытых файлов: максимальное значение задаётся в настройках и не может превышать 100. SciTE может выполнять команды для компиляции или запуска исходных файлов, при этом выходные данные этих команд направляются в область вывода. "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_scite  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_scite" =~ [^10] ]]
do
    :
...
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_scite == 1 ]]; then
  echo ""
  echo " Установка SciTE (scite) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
############### sniffnet ###############
sudo pacman -S --noconfirm --needed scite  # Редактор с возможностями для создания и запуска программ ; https://archlinux.org/packages/extra/x86_64/scite/ ; https://www.scintilla.org/SciTE.html ; 2025-06-13 22:40 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Codelobster (codelobster) - IDE редактор для разработки PHP/HTML/CSS/JavaScript?"
echo -e "${MAGENTA}:: ${BOLD}Codelobster — это простой IDE -редактор для веб-разработчиков. Он имеет достаточно практичный интерфейс, что позволит Вам быстро разобраться в его настройках. Codelobster IDE работает быстро, благодаря чему можно работать с большими проектами. С его помощью можно редактировать файлы PHP, HTML, CSS, JavaScript и многие другие. ${NC}"
echo " Домашняя страница: https://www.codelobsteride.com/ ; (https://www.scintilla.org/SciTEDoc.html ; https://www.scintilla.org/) "
echo -e "${MAGENTA}:: ${BOLD}CodeLobster IDE — это интеллектуальная бесплатная кроссплатформенная IDE, предназначенная для создания и редактирования PHP, HTML, CSS и JavaScript файлов с поддержкой Drupal CMS, Joomla CMS, Magento CMS, шаблонизаторов Smarty и Twig, библиотеки JQuery, AngularJS, BackboneJS, Laravel, MeteorJS, Phalcon, CodeIgniter, CakePHP, Symfony, Yii и WordPress. Включает в себя отладчик PHP, динамическую справку, расширенное автодополнение, окно Class View для комфортной работы со смешанным кодом и FTP/SFTP-клиент. CodeLobster IDE поддерживает автоматическую установку многих популярных фреймворков, автодополнение для функций, всплывающие подсказки с описаниями методов и справку по API. Это превращает редактор в полнофункциональную IDE. ${NC}"
echo -e "${YELLOW}:: ${NC}Среди отличительных особенностей Codelobster можно отметить выделение синтаксиса и предоставление подсказок для тегов, функций и их параметров. С его помощью Вы сможете редактировать файлы со смешанным содержимым. Например, если Вы вставляете PHP-код в HTML-шаблон, то редактор корректно выделяет как HTML-теги, так и PHP-функции. То же самое относится к коду CSS и JavaScript, который содержится в HTML-файлах. Поддержка 17 языков пользовательского интерфейса, среди которых английский, немецкий, русский, испанский, французский и другие. "
echo " Редактор поддерживает огромный набор плагинов для работы с разными фреймворками, включая Laravel PHP framework, Node.js, WordPress, AngularJS и другие. "
echo -e "${YELLOW}==> Примечание! ${NC}Codelobster — условно-бесплатная интегрированная среда разработки. В бесплатной ("Free") версии доступен редактор с подстветкой синтаксиса и автодополнением, менеджер проектов, а также PHP дебаггер. В платной ("Professional") версии среди прочего дополнительно доступны инструменты для работы с SQL, заявлена поддержка node.js, а также плагины для: AngularJS ; Backbone.js ; Bootstrap ; CakePHP ; CodeIgniter ;Drupal ; Ember.js ; JQuery ; Joomla ; Laravel ; Magento ; Meteor JS ; Phalcon PHP ; Smarty ; Symfony ; VueJS ; WordPress ; Yii . 📌 Попрубуйте активировать Codelobster - предоставленными мной серийными ключами, они закомментированы # в Справке (в сценарии скрипта установки утилиты). "
echo -e "${CYAN}:: ${NC}Установка Codelobster (codelobster), проходит через сборку из исходников. То есть установка производиться с помощью git clone, PKGBUILD, makepkg - скачивается с сайта 'Arch Linux' (https://aur.archlinux.org/packages/codelobster), (https://aur.archlinux.org/codelobster.git) - собирается и устанавливается. "
echo " Будьте внимательны! Процесс установки был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_codelobster  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_codelobster" =~ [^10] ]]
do
    :
done
if [[ $in_codelobster == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_codelobster == 1 ]]; then
  echo ""
  echo " Установка Codelobster (codelobster) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
############### Codelobster ###############
yay -S codelobster --noconfirm  # Бесплатная кроссплатформенная IDE для разработки PHP/HTML/CSS/JavaScript ; https://aur.archlinux.org/packages/codelobster ; https://aur.archlinux.org/codelobster.git (только для чтения, нажмите, чтобы скопировать) ; https://www.codelobsteride.com/ ; Обеспечивает: codelobster-ide ; https://codelobsteride.com/download/codelobsteride-2.6.0_amd64.deb ; 2025-05-14 06:51 (UTC) ; ЛИЦЕНЗИЯ - https://aur.archlinux.org/cgit/aur.git/tree/LICENSE?h=codelobster
### sudo pacman -Fyy
### sudo pacman -F libcomposeplatforminputcontextplugin.so
########### codelobster ############
#git clone https://aur.archlinux.org/codelobster.git  # (только для чтения, нажмите, чтобы скопировать)
#cd codelobster
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm    #--не спрашивать каких-либо подтверждений
...
############ Справка ##############
# CodeLobsterIDE_Key: для версии CodeLobster IDE Professional 2.6.0 (*на Windows активация сработала!)
# Имя:
# CodeLobsterIDE
# CodeLobsterIDE ✅
# Serial:
# ff8622ba581e911a9a8a1b29
# FF86-22BA-581E-911A-9A8A-1B29 ✅
# Имя:
# Alex
# Alex ✅
# Serial:
# 7430646f4afb846d57504911
# 7430-646F-4AFB-846D-5750-4911 ✅
######################################


clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Atom-ng (atom-ng-bin) - Текстовый редактор кода подверженный гиперхакингу?"
echo -e "${MAGENTA}:: ${BOLD}Atom — это многофункциональный текстовый редактор от разработчиков GitHub. Atom — современный, удобный, функциональный редактор кода. Имеет встроенный менеджер пакетов (плагинов) для расширения возможностей. Текстовый редактор — основа инструментария разработчика, но он обычно не работает сам по себе. ${NC}"
echo " Домашняя страница: https://thorium.rocks/atom-ng/ ; (https://github.com/atom ; https://web.archive.org/web/20221210062134/https://atom.io/ ; https://flight-manual-atom-io.github.io/ ; https://flight-manual-atom-io.github.io/getting-started/sections/why-atom/) "
echo -e "${MAGENTA}:: ${BOLD}Вся функциональность Atom построена на концепции отдельных модулей (пакетов). Даже штатные функции реализованы, как отдельные пакеты. Он имеет огромное количество различных расширений, благодаря чему его можно считать полноценной средой разработки. Потому на нем нередко пишут кроссплатформенные десктопные приложения, используя веб-технологии. Большинство плагинов и расширений — это свободное программное обеспечение. То есть они разрабатываются и поддерживаются сообществом. Таким образом, если у Вас есть соответствующие навыки, Вы можете самостоятельно настроить редактор под свои нужды. ${NC}"
echo -e "${YELLOW}:: ${NC}Atom имеет встроенный менеджер пакетов, который позволяет искать и устанавливать новые пакеты прямо внутри программы. Пакетов представлено очень много. Достаточно найти нужный пакет и нажать одну кнопку — Install. Настройка каждого пакета также осуществляется внутри программы, это очень удобно. Вам не нужно прибегать к редактированию каких-либо конфигурационных файлов. Интерфейс Atom выполнен качественно и не перегружен лишними элементами. Слева расположено дерево файлов (проектов). Внизу строка состояния, сверху меню. Над рабочей областью располагаются вкладки открытых файлов.  "
echo " Рабочую область можно разделить на несколько частей, как по горизонтали, так и по вертикали. При этом каждая область имеет свои вкладки с открытыми файлами. Таким образом можно видеть и редактировать сразу несколько файлов. Поддерживаются темы оформления. После установки доступно несколько предустановленных тем, как светлых так и темных. Цвета подобраны очень хорошо и приятны глазу. Новые темы можно установить не выходя из программы через Package Manager. Поддерживается работа с отдельными проектами. Под проектом в Atom понимается любая директория, содержащая файлы и папки. Вы можете добавить несколько проектов и они появятся в левом меню. Каждый проект может иметь свои специфические настройки (например, настройки FTP соединения). "
echo -e "${YELLOW}==> Примечание! ${NC} 📌 Из функциональности стоит отметить (для некоторых требуется установка дополнительных пакетов): Подсветка синтаксиса; Автодополнение кода; Сворачивание блоков кода; Поиск и замена текста в файлах проекта; Быстрый поиск по файлам; Быстрое открытие файлов по имени; Поддержка огромного количества горячих клавиш; Встроенный терминал; Поддержка GIT; Загрузку файлов по FTP; Выбор цвета в палитре (встроенный Color picker). Разработка: Исходный код: Open Source (открыт); Языки программирования: JavaScript; Лицензия: MIT License; Приложение переведено на русский язык. "
echo -e "${CYAN}:: ${NC}Установка Atom-ng (atom-ng-bin), проходит через сборку из исходников. То есть установка производиться с помощью git clone, PKGBUILD, makepkg - скачивается с сайта 'Arch Linux' https://aur.archlinux.org/atom-ng-bin.git), (https://aur.archlinux.org/packages/atom-ng-bin?all_deps=1#pkgdeps) - собирается и устанавливается. "
echo " Будьте внимательны! Процесс установки был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_atomng  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_atomng" =~ [^10] ]]
do
    :
...
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_atomng == 1 ]]; then
  echo ""
  echo " Установка Atom-ng (atom-ng-bin) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
############### atom-ng-bin ###############
yay -S atom-ng-bin --noconfirm  # Текстовый редактор, подверженный гиперхакингу. Поддерживаемая сообществом версия с оптимизацией компилятора ; https://aur.archlinux.org/packages/atom-ng-bin?all_deps=1#pkgdeps ; https://aur.archlinux.org/atom-ng-bin.git (только для чтения, нажмите, чтобы скопировать) ; https://thorium.rocks/atom-ng/ ; Обеспечивает: atom, atom-ng ; Конфликты: atom, atom-ng ; 2024-05-17 12:29 (UTC)
########### atom-ng-bin ############
#git clone https://aur.archlinux.org/atom-ng-bin.git  # (только для чтения, нажмите, чтобы скопировать)
#cd atom-ng-bin
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm    #--не спрашивать каких-либо подтверждений
...
# https://flight-manual-atom-io.github.io/
# https://thorium.rocks/atom-ng/
# https://flight-manual-atom-io.github.io/getting-started/sections/installing-atom/
# https://flight-manual-atom-io.github.io/using-atom/sections/atom-selections/
# https://flight-manual-atom-io.github.io/using-atom/sections/github-package/
# Почему Атом?
# https://flight-manual-atom-io.github.io/getting-started/sections/why-atom/
######################################
...
sudo pacman -S --noconfirm --needed emacs-haskell-mode  # Библиотека, предоставляющая функцию MIME для GNU Emacs ; https://github.com/wanderlust/semi ; https://archlinux.org/packages/extra/any/semi/ ; 13 июля 2024 г., 21:13 UTC
...
clear
echo -e "${MAGENTA}
  <<< Установка утилит (пакетов) Шестнадцатеричный редакторов в Archlinux >>> ${NC}"
# Installing Hexadecimal Editor utilities (packages) in Archlinux
echo -e "{GREEN}==> ${BOLD}Шестнадцатеричный редактор позволяет просматривать/редактировать двоичные данные файла, представленные в виде шестнадцатеричных значений, отсюда и название «шестнадцатеричный редактор». Честно говоря, он нужен не всем. Им пользуется лишь определённая группа пользователей, которым приходится работать с двоичными данными. Чтобы узнать больше о шестнадцатеричных редакторах, начните со страницы в Википедии (https://en.wikipedia.org/wiki/Hex_editor?ref=itsfoss.com). HEX-редакторы онлайн (https://hexed.it/) ${NC}"
#sleep 05
#clear
echo ""
echo -e "${BLUE}:: ${NC}Установить GHex (ghex) - Шестнадцатеричный редактор?"
...
echo " Установка утилит (пакетов) пропущена "
  ...
  echo ""
  echo " Установка GHex (ghex) "
#sudo pacman -Syy  # обновление баз пакмэна (pacman)
sudo pacman -S --noconfirm --needed libadwaita # Строительные блоки для современных адаптивных приложений GNOME ; https://gnome.pages.gitlab.gnome.org/libadwaita/ ; https://archlinux.org/packages/extra/x86_64/libadwaita/ ; 2025-08-02 01:42 UTC
sudo pacman -S --noconfirm --needed ghex  # Простой двоичный редактор для рабочего стола Gnome ; https://wiki.gnome.org/Apps/Ghex ; https://archlinux.org/packages/extra/x86_64/ghex/ ; https://gitlab.gnome.org/GNOME/ghex ; 2025-08-30 10:29 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Okteta (okteta) - Шестнадцатеричный редактор?"
echo -e "${MAGENTA}:: ${BOLD}Okteta — простой (не слишком богатым функционалом) Шестнадцатеричный редактор созданный для рабочего окружения KDE (K Desktop Environment). Он (Hex-редактор) может быть полезен для просмотра и редактирования необработанных данных файлов. Шестнадцатеричный редактор / Hex-редактор (Hex Editor) — приложение для редактирования данных представленных в "сыром виде", как последовательность байтов (так как его "видит" машина). Данные редактором отображаются в виде матрицы, каждая ячейка которой соответствует одному байту, записанному в шестнадцатеричной системе счисления в виде двухзначного числа. ${NC}"
echo " Домашняя страница: https://apps.kde.org/okteta/ ; (https://github.com/KDE/okteta ; https://archlinux.org/packages/extra/x86_64/okteta/). "
echo -e "${MAGENTA}:: ${BOLD}Hex-редактор Okteta имеет интуитивно понятный и частично настраиваемый пользовательский интерфейс (открепляемые и перемещаемые панели инструментов), файл редактором отображается в шестнадцатеричном (десятеричном, восьмеричном) или ASCII формате. Okteta поддерживает редактирование обычных файлов (локальных и удалённых, по различным протоколам http, ftp, fish и других поддерживаемых в системе) и файлов устройств. Одновременно можно открыть любое количество файлов (в вкладках), рабочее пространство можно разделить (вертикально и горизонтально). Заданные для Okteta настройки можно сохранять в "профили", можно добавлять файлы в "закладки", обработка данных быстрая и напоминает работу с обычным текстовым редактором (редактировать данные можно в любом формате). Необходимый для коррекции файла функционал добавляется (отключается) активацией модулей. Помимо базовых возможностей редактирования Okteta предоставляет небольшой набор дополнительных инструментов (декодирование таблиц, отображение статистики, определение контрольных сумм и пр). Данные можно экспортировать в файл исходного формата, текстовой файл и буфер обмена. Okteta имеет бесконечную функцию отмены и возврата сделанных изменений, управление приложением осуществляется с помощью мыши и/или клавиатурных сочетаний (полностью настраиваемых). ${NC}"
echo " Функции: Значения и символы отображаются либо в двух столбцах (традиционное отображение в шестнадцатеричных редакторах), либо в строках со значением над символом. Редактирование и навигация аналогичны текстовому редактору. Настраиваемые представления данных с загружаемыми и сохраняемыми профилями. Инструменты, закрепляемые со всех сторон или плавающие. Числовые коды: шестнадцатеричное, десятичное, восьмеричное, двоичное. Кодировки символов: все 8-битные кодировки, предоставляемые Qt, EBCDIC. Быстрая отрисовка данных на экране. Несколько открытых файлов. Поддержка отмены/повтора действий. Инструмент для анализа и редактирования структур на основе определений структур, создаваемых пользователем. И многое другое... Он выпущен под лицензией: GNU GPL / LGPL / FDL. "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_okteta  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_okteta" =~ [^10] ]]
do
    :
...
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_okteta == 1 ]]; then
  echo ""
  echo " Установка Okteta (okteta) "
sudo pacman -S --noconfirm --needed okteta # Шестнадцатеричный редактор KDE для просмотра и редактирования необработанных данных файлов ; https://archlinux.org/packages/extra/x86_64/okteta/ ; https://apps.kde.org/okteta/ ; https://github.com/KDE/okteta ; 2025-06-06 19:06 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Редактор Bless Hex (bless) - Полнофункциональный Шестнадцатеричный редактор?"
echo -e "${MAGENTA}:: ${BOLD}Bless — это двоичный (шестнадцатеричный) редактор, программа, которая позволяет редактировать файлы как последовательности байтов. Он написан на C# и использует привязки Gtk# для набора инструментов GTK+. Bless Hex Editor - написана автором Alexandros Frantzis. Bless — один из самых популярных шестнадцатеричных редакторов для Linux. Он может легко справляться с редактированием больших файлов, не замедляя работу, поэтому это быстрый шестнадцатеричный редактор. ${NC}"
echo " Домашняя страница: https://github.com/afrantzis/bless ; (https://archlinux.org/packages/extra/any/bless/). Руководство по Bless (шестнадцатеричный редактор) - HackWare (https://hackware.ru/?p=15230). "
echo -e "${MAGENTA}:: ${BOLD}Когда вы запускаете программу Bless, отображается графическое окно. Окно приложения содержит следующие элементы: Строка меню (Menubar): Меню в строке меню содержат все команды, необходимые для работы с файлами в приложении. Панель инструментов (Toolbar): Предоставляет ярлыки для команд, которые наиболее часто используются при работе с файлами в приложении. Просмотр данных (Data View): Окно просмотра данных содержит несколько вкладок, на которых отображаются данные редактируемых файлов. Таблица преобразования (Conversion Table): В таблице преобразования отображаются байты в текущей позиции файла, преобразованные в различные форматы. Статус бар (Statusbar): В строке состояния отображается информация о текущей активности приложения и информация о текущем файле. ${NC}"
echo " Основные характеристики (возможности): Редактирование сырого диска. Эффективное редактирование больших файлов данных.
Множественные (многоуровневые) операции «отмена» и «повтор». Настраиваемые представления данных. Быстрое отображение данных на экране. Много вкладок (несколько вкладок). Операции быстрого поиска и замены. Таблица преобразования данных. Расширенные возможности копирования/вставки. Поддсветка совпавших образцов выделения в файле. Экспорт в текст и html. Поддержка плагинов для расширения функциональности. "
echo " Файлы конфигурации: Bless хранит свои файлы конфигурации в каталоге $XDG_CONFIG_HOME/bless (обычно в $HOME/.config/bless в системах на основе UNIX и C:\Documents and Settings\User Name\Application Data\bless в системах MS Windows) в соответствии со спецификацией XDG. Этот каталог содержит следующие файлы и каталоги: export_patterns: Содержит список сохранённых шаблонов экспорта. history.xml: Сохраняет список недавно открытых файлов. last.session: Сохраняет состояние приложения при последнем закрытии. Сюда входят открытые файлы, смещение файла и смещение курсора в каждом файле, активный файл, размеры окна и т. д.. Preferences.xml: Сохраняет настройки пользователя. layouts/: Этот каталог содержит файлы описания макета для каждого пользователя. Смотрите раздел Файлы макета. "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_bless  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_bless" =~ [^10] ]]
do
    :
...
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_bless == 1 ]]; then
  echo ""
  echo " Установка Bless Hex (bless) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
########## Зависимости ##########
sudo pacman -S --noconfirm --needed gtk-sharp  # Привязки gtk2 для C# ; https://archlinux.org/packages/extra/x86_64/gtk-sharp-2/ ; https://www.mono-project.com/docs/gui/gtksharp/ ; 2020-10-14 15:36 UTC
sudo pacman -S --noconfirm --needed itstool  # Перевод XML с помощью файлов PO с использованием правил набора тегов интернационализации W3C ; https://archlinux.org/packages/extra/any/itstool/ ; https://itstool.org/ ; 2025-02-13 07:36 UTC
########## bless ############
sudo pacman -S --noconfirm --needed bless  # Высококачественный, полнофункциональный шестнадцатеричный редактор ; https://archlinux.org/packages/extra/any/bless/ ; https://github.com/afrantzis/bless ; 2024-09-07 13:39 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
##############
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить WxHexEditor (wxhexeditor) - Шестнадцатеричный и дисковый редактор?"
echo -e "${MAGENTA}:: ${BOLD}wxHexEditor — универсальный шестнадцатеричный редактор с открытым исходным кодом, разработанный для профессионалов и энтузиастов, которым необходимо работать непосредственно с необработанными двоичными данными. Созданный на основе фреймворка wxWidgets и написанный на C++, он обеспечивает полную кроссплатформенную совместимость, без проблем работая в средах Windows, Linux и macOS. Его главное преимущество заключается в способности эффективно обрабатывать очень большие файлы и образы дисков, что делает его подходящим для решения сложных задач, бросающих вызов ограничениям многих других шестнадцатеричных редакторов. Разработчик: Эрдем У. Алтинюрт . Лицензия: GNU General Public License v2.0 . Платформа GUI: wx. Язык интерфейса: English. Операционная система: Linux, MacOSX, Windows 7/Vista/XP . Также, теоретически, совместим с любой системой, которая поддерживает интерфейс wx. ${NC}"
echo " Домашняя страница: http://www.wxhexeditor.org/ ; (http://wxhexeditor.sf.net/home.php ; https://aur.archlinux.org/packages/wxhexeditor ; https://aur.archlinux.org/wxhexeditor.git). О wxHexEditor - https://appmus.com/software/wxhexeditor . "
echo -e "${MAGENTA}:: ${BOLD}Особенности включают в себя: Использует 64-битные файловые дескрипторы (поддерживает файлы или устройства размером до 2^64 байт, что составляет несколько эксабайт, но протестировано пока только 1 петабайтный файл). Он не копирует весь файл в оперативную память. Это обеспечивает высокую скорость и возможность открывать файлы (размерами от нескольких гигабайт до нескольких терабайт, петабайт и эксабайт). Работа с удалением/вставкой байтов в файл более одного раза без создания временного файла. Использование памяти: в настоящее время около ~ 25 Мегабайт при открытии нескольких файлов размером > ~8 ГБ. Может работать с файлами посредством шифрования XOR. Несколько представлений для одновременного отображения нескольких файлов. Поддержка дизассемблирования x86 (через интегрированную библиотеку udis86) позволяет взламывать программы немного быстрее. Имеет красочные теги, делающие обратную разработку проще и интереснее. Копируйте/редактируйте диски, сектора жесткого диска, что может быть полезно для ручного восстановления файлов и разделов. С ее помощью вы можете копировать / редактировать свои диски, сектора жесткого диска.(Полезно для восстановления файлов / разделов вручную). Отформатированные копии! Легко скопировать часть файла в шестнадцатеричном формате для C/C++ source, ASM source, а также поддерживает форматы HTML, phpBB и Wiki-страниц! Индикация сектора на устройствах dDisk, также имеется диалоговое окно «Перейти к сектору». Форматированное копирование! Легко копировать часть файла в формате HEX для исходных текстов на C/C++ и ASM, также поддерживаются форматы страниц HTML, phpBB и Wiki с тегами! Поддерживает работу только в шестнадцатеричном и текстовом редакторах. Также есть возможность отключить смещение области. Поддерживает настраиваемое форматирование и цвета шестнадцатеричной панели. Может открывать ваши устройства в Linux, Windows или MacOSX. Позволяет выполнять операции редактирования памяти процессов Linux. Сравнение двоичных файлов, позволяет объединять близкие результаты. Поддерживает множество кодировок, включая почти все командные коды DOS/Windows/MacOS и многосимвольные наборы, такие как UTF8/16/32, Shift JIS, GBK, EUC_KR… . Режимы адресации: десятичный, шестнадцатеричный, восьмеричный и LBA («Сектор+Смещение») (переключаются друг на друга щелчком правой кнопки мыши на панели «Смещение»). Функция сохранения выбора в виде файла дампа. Функция «Найти некоторые байты» для быстрого поиска следующих значимых байтов в файле/диске. Функции контрольной суммы MD/RIPEMD/SHA/TIGER/HAVAL/CRC/ADLER/GOST/WHRILPOOL/SNEFRU (через интегрированную библиотеку mhash). Поддержка импорта и экспорта ТЭГов из файла. ${NC}"
echo " Пользовательский интерфейс wxHexEditor разработан с учётом функциональности и эффективности. Он поддерживает вкладки для управления несколькими файлами или сеансами работы с дисками одновременно и предлагает такие функции, как редактирование текста по столбцам для структурированной обработки данных. Хотя интерфейс может показаться утилитарным по сравнению с некоторыми современными приложениями, он ориентирован на предоставление мощных низкоуровневых возможностей доступа и управления данными, необходимых для специализированных задач в области разработки программного обеспечения, безопасности и криминалистики данных. "
echo -e "${CYAN}:: ${NC}Установка WxHexEditor (wxhexeditor), а также некоторых зависимостей (gconf ; libgksu ; gksu) проходит через сборку из AUR, через (yay). Если хотите можете собрать пакет через - git clone, PKGBUILD, makepkg - эта функция тоже прописана в сценарии скрипта, НО # закомментирована! "
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Да установить,     0 - НЕТ - Пропустить установку: " in_wxhexeditor  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_wxhexeditor" =~ [^10] ]]
do
    :
done
if [[ $in_wxhexeditor == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_wxhexeditor == 1 ]]; then
  echo ""
  echo " Установка WxHexEditor (wxhexeditor) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
########## Зависимости ##########
sudo pacman -S --noconfirm --needed gcc-libs  # Библиотеки времени выполнения, поставляемые GCC ; https://archlinux.org/packages/core/x86_64/gcc-libs/ ; https://gcc.gnu.org/ ; Обеспечивает: gcc-libs-multilib, libasan.so=8-64, libgfortran.so=5-64, libgo.so=24-64, libgphobos.so=6-64, Подробнее… (https://archlinux.org/packages/core/x86_64/gcc-libs/#) ; Заменяет: gcc-libs-multilib, libgphobos ; 2025-04-30 17:25 UTC
sudo pacman -S --noconfirm --needed glibc  # Библиотека GNU C ; https://archlinux.org/packages/core/x86_64/glibc/ ; https://www.gnu.org/software/libc ; 2025-07-24 21:22 UTC
sudo pacman -S --noconfirm --needed wxwidgets-common  # Общие библиотеки и заголовочные файлы для wxwidgets ; https://archlinux.org/packages/extra/x86_64/wxwidgets-common/ ; https://wxwidgets.org/ ; Заменяет: wxgtk-common ; Конфликты: с wxgtk-common ; 2025-06-07 18:15 UTC
...
########## gksu Зависимости ############
sudo pacman -S --noconfirm --needed xorg-xauth  # Программа настройки авторизации X.Org ; https://archlinux.org/packages/extra/x86_64/xorg-xauth/ ; https://gitlab.freedesktop.org/xorg/app/xauth ; 2025-03-10 13:48 UTC
sudo pacman -S --noconfirm --needed gtk-doc  # Инструмент документирования для API публичной библиотеки ; https://archlinux.org/packages/extra/any/gtk-doc/ ; https://gitlab.gnome.org/GNOME/gtk-doc ; 2025-01-27 23:00 UTC
...
sudo pacman -S --noconfirm --needed libgnome-keyring  # Клиентская библиотека брелока GNOME (устарела) ; https://archlinux.org/packages/extra/x86_64/libgnome-keyring/ ; https://gitlab.gnome.org/Archive/libgnome-keyring ; 2024-02-21 06:41 UTC
...
sudo pacman -S --noconfirm --needed startup-notification  # Библиотека для мониторинга и отображения запуска приложений ; https://archlinux.org/packages/extra/x86_64/startup-notification/ ; https://www.freedesktop.org/wiki/Software/startup-notification/ ; 2023-05-31 06:23 UTC
sudo pacman -S --noconfirm --needed dbus-glib  # Привязки GLib для D-Bus (устарело) ; https://archlinux.org/packages/extra/x86_64/dbus-glib/ ; https://www.freedesktop.org/wiki/Software/dbus/ ; 2025-03-24 15:22 UTC
sudo pacman -S --noconfirm --needed libldap  # Клиентские библиотеки облегченного протокола доступа к каталогам (LDAP) ; https://archlinux.org/packages/core/x86_64/libldap/ ; https://www.openldap.org/ ; 2025-07-16 18:19 UTC
sudo pacman -S --noconfirm --needed libxml2  # Анализатор XML C и набор инструментов ; https://archlinux.org/packages/core/x86_64/libxml2/ ; https://gitlab.gnome.org/GNOME/libxml2/-/wikis/home ; Обеспечивает: libxml2.so=16-64 ; 2025-07-17 02:23 UTC
...
sudo pacman -S --noconfirm --needed glib2-devel  # Базовая библиотека низкого уровня — файлы разработки ; https://archlinux.org/packages/core/x86_64/glib2-devel/ ; https://gitlab.gnome.org/GNOME/glib ; 2025-06-15 18:26 UTC
sudo pacman -S --noconfirm --needed gnome-common  # Общие макросы разработки для GNOME ; https://archlinux.org/packages/extra/any/gnome-common/ ; https://www.gnome.org/ ; 2024-09-19 08:34 UTC
sudo pacman -S --noconfirm --needed gobject-introspection  # Система интроспекции для библиотек на основе GObject ; https://archlinux.org/packages/extra/x86_64/gobject-introspection/ ; https://wiki.gnome.org/Projects/GObjectIntrospection ; 2025-06-05 23:28 UTC
########## gconf ############
...
#rm -Rf gconf
########## libgksu ############
yay -S libgksu --noconfirm  # Библиотека авторизации gksu ; https://aur.archlinux.org/packages/libgksu ; https://aur.archlinux.org/libgksu.git (только для чтения, нажмите, чтобы скопировать) ; http://www.nongnu.org/gksu/index.html ; 24 октября 2021 г. 20:50 (UTC)
########## libgksu ############
...
#rm -Rf libgksu
########## gksu ############
yay -S gksu --noconfirm  # Графический интерфейс для su ; https://aur.archlinux.org/packages/gksu ; https://aur.archlinux.org/gksu.git (только для чтения, нажмите, чтобы скопировать) ; http://www.nongnu.org/gksu/index.html ; 2021-12-11 23:00 (UTC)
########## gksu ############
...
#rm -Rf gksu
########## wxhexeditor ############
yay -S wxhexeditor --noconfirm  # Бесплатный шестнадцатеричный редактор / редактор дисков для Linux, Windows и MacOSX ; https://aur.archlinux.org/packages/wxhexeditor ; https://aur.archlinux.org/wxhexeditor.git (только для чтения, нажмите, чтобы скопировать) ; http://www.wxhexeditor.org/ ; 2025-06-09 04:02 (UTC)
########## wxhexeditor ############
#git clone https://aur.archlinux.org/wxhexeditor.git  # (только для чтения, нажмите, чтобы скопировать)
#cd wxhexeditor
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm    #--не спрашивать каких-либо подтверждений
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить HexEdit (hexedit) (командная строка) - Консольный Шестнадцатеричный редактор?"
echo -e "${MAGENTA}:: ${BOLD}HexEdit (Консоль Hexedit) — это шестнадцатеричный редактор командной строки для Linux, который позволяет просматривать и редактировать двоичные файлы. Программа отображает данные в ASCII и шестнадцатеричном форматах одновременно. Она поддерживает поиск данных по значениям и может показывать вывод со скроллом. Также в Hexedit есть широкий спектр сочетаний клавиш для навигации по шестнадцатеричным данным. Hexedit доступен во всех основных дистрибутивах Linux. ${NC}"
echo " Домашняя страница: https://rigaux.org/hexedit.html ; (https://archlinux.org/packages/extra/x86_64/hexedit/). Руководство по Hexedit (шестнадцатеричный редактор) - https://rigaux.org/hexedit.html)). "
echo -e "${MAGENTA}:: ${BOLD}Hexedit — надёжный и очень простой в использовании редактор двоичных файлов. Этот шестнадцатеричный просмотрщик для Linux обладает множеством функций, таких как быстрый поиск и сравнение, подсветка, EBCDIC, цветовые схемы, автоподбор размера, режимы INS/OVR, закладки и отслеживание изменений. Его функция смены отображения поддерживает более семидесяти свойств, включая даты, числа с плавающей точкой, целые числа и многое другое. При запуске вам нужно будет указать местоположение файла, и он откроет его автоматически. Это мой любимый шестнадцатеричный редактор командной строки для Linux. ${NC}"
echo " Основные характеристики (возможности): Hexedit включает в себя множество функций, а именно: макросы нажатия клавиш, фоновый поиск, редактор дисков и т. д.. Содержит настраиваемые информационные подсказки, калькулятор, шаблон и файлы объемом более четырех гигабайт. Редактор шестнадцатеричного кода Linux предлагает функцию автоматического скрытия и закрепления окон инструментов. Hexedit может просматривать память, диски и файлы по команде пользователя. Эта портативная утилита проста в использовании и может использоваться для проверки оперативной памяти, чтобы открыть BIOS, видеоBIOS и BIOS управления системой. "
echo " Пример использования: чтобы открыть файл sample.bin, нужно ввести команду hexedit sample.bin. Можно менять шестнадцатеричные значения файла и в реальном времени видеть, как они влияют на строку в файле. Для сохранения изменений можно ввести Ctrl+S. С максимальным размером файла в 16 эксабайт (17 179 869 184 гигабайт!) HexEdit может вырезать, копировать, вставлять, вставлять и удалять любые объёмы данных без потери производительности при работе с большими файлами. Благодаря неограниченным многоуровневым возможностям отмены и повтора действий, независимо от размера файла, HexEdit выводит редактирование файлов на новый уровень. "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_hexedit  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_hexedit" =~ [^10] ]]
do
    :
done
if [[ $in_hexedit == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_hexedit == 1 ]]; then
  echo ""
  echo " Установка HexEdit (hexedit) (командная строка) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
########## Зависимости ##########
...
########## hexedit ############
sudo pacman -S --noconfirm --needed hexedit  # Просмотр и редактирование файлов в шестнадцатеричном формате или в формате ASCII ; https://archlinux.org/packages/extra/x86_64/hexedit/ ; https://rigaux.org/hexedit.html ; 2024-04-24 14:42 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
##############
...
sudo pacman -S --noconfirm --needed aspell  # Проверка орфографии, призванная в конечном итоге заменить Ispell ; http://aspell.net/ ; https://archlinux.org/packages/extra/x86_64/aspell/ ; 2024-01-29 23:10 UTC
sudo pacman -S --noconfirm --needed aspell-en  # Английский словарь для aspell ; http://aspell.net/ ; https://archlinux.org/packages/extra/x86_64/aspell-en/ ; 2024-07-11 23:55 UTC
sudo pacman -S --noconfirm --needed aspell-ru  # Русский словарь для aspell ; http://aspell.net/ ; https://archlinux.org/packages/extra/x86_64/aspell-ru/ ; 2024-07-04 09:52 UTC
sudo pacman -S --noconfirm --needed hunspell  # Библиотека и программа для проверки орфографии и морфологического анализатора ; https://github.com/hunspell/hunspell ; https://archlinux.org/packages/extra/x86_64/hunspell/ ; 2024-07-02 19:40 UTC
sudo pacman -S --noconfirm --needed hyphen  # Библиотека для качественной расстановки переносов и выравнивания ; https://hunspell.sf.net/ ; https://archlinux.org/packages/extra/x86_64/hyphen/ ; 2024-07-02 19:57 UTC
...
#sudo pacman -S --noconfirm --needed libmythes  # простой тезаурус ; https://github.com/hunspell/mythes ; https://archlinux.org/packages/extra/x86_64/libmythes/ ; 2024-07-14 14:45 UTC
### это простой тезаурус, использующий структурированную текстовый файл данных и индексный файл с бинарным поиском для поиска слов и фраз и возврата информации о части речи, значениях и синонимах
####### libreoffice-still ############
# sudo pacman -S libreoffice-still libreoffice-still-ru --noconfirm
sudo pacman -S --noconfirm --needed libreoffice-still  # Филиал обслуживания LibreOffice ; https://www.libreoffice.org/ ; https://archlinux.org/packages/extra/x86_64/libreoffice-still/ ; Обеспечивает: libreoffice, libreoffice-en-US ; Конфликты: с libreoffice-fresh ; Обратные конфликты: с libreoffice-fresh ; 2025-09-08 14:29 UTC
sudo pacman -S --noconfirm --needed libreoffice-still-ru  # Пакет русского языка для LibreOffice still ; https://www.documentfoundation.org/ ; https://archlinux.org/packages/extra/any/libreoffice-still-ru/ ; Обеспечивает: libreoffice-ru ; Конфликты: libreoffice-ru ; 2025-09-08 14:29 UTC
########## Дополнительные расширения ####################
sudo pacman -S --noconfirm --needed libreoffice-extension-texmaths  # Редактор формул LaTeX для LibreOffice ; http://roland65.free.fr/texmaths/ ; https://archlinux.org/packages/extra/any/libreoffice-extension-texmaths/ ; 2024-10-26 18:59 UTC
sudo pacman -S --noconfirm --needed libreoffice-extension-writer2latex  # Java-программа и набор расширений LibreOffice для преобразования и работы с LaTeX в LibreOffice ; https://writer2latex.sourceforge.net/ ; https://archlinux.org/packages/extra/any/libreoffice-extension-writer2latex/ ; 2024-07-04 21:03 UTC
sudo pacman -S --noconfirm --needed unoconv  # Конвертер документов на основе Libreoffice ; http://dag.wiee.rs/home-made/unoconv ; https://archlinux.org/packages/extra/any/unoconv/ ; 2025-01-03 08:23 UTC
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить DocBook (docbook-xml) (docbook-xsl) - (для конвертации в HTML, man, PDF, DOCX, ODT, EPUB, FictionBook2 и во многие другие)?"
echo -e "${MAGENTA}:: ${BOLD}DocBook — это семантический язык разметки технической документации. Изначально он был предназначен для написания технических документов, связанных с компьютерным оборудованием и программным обеспечением, но может использоваться и для любой другой документации. Будучи семантическим языком, DocBook позволяет пользователям создавать содержимое документов в нейтральной для представления форме, отражающей логическую структуру содержимого; это содержимое затем может быть опубликовано в различных форматах, включая HTML, XHTML, EPUB, PDF, справочные страницы, веб-справку и HTML-справку, без необходимости внесения изменений в исходный код. Другими словами, документ, написанный в формате DocBook, легко переносится в другие форматы, не требуя переписывания. DocBook это отличным форматом для любой документации… пока вы не попытаетесь её прочитать. На самом деле, файлы .docbook и не предназначены для чтения — они предназначены для конвертации в HTML, man, PDF, DOCX, ODT, EPUB, FictionBook2 и во многие другие. ${NC}"
echo " Домашняя страница: https://docbook.org/ ; (https://www.oasis-open.org/docbook/ ; https://archlinux.org/packages/extra/any/docbook-xml/ ; https://archlinux.org/packages/extra/any/docbook-xsl/). Репозиторий документов технического комитета DocBook (https://www.oasis-open.org/docbook/). DocBook - The Definitive Guide (http://www.docbook.org/tdg/en/html/docbook.html). DocBook XSL – The Complete Guide (http://www.sagehill.net/docbookxsl/index.html). ArchWiki (https://wiki.archlinux.org/title/DocBook). "
echo -e "${MAGENTA}:: ${BOLD}Что такое DocBook? DocBook — это стандарт на основе XML, который используется во многих современных задачах документации. Если вы хотите создать источник документа DocBook, вы пишете файлы XML, которые описывают макет документа, разделение на абзацы и другие атрибуты. Структура файла XML может показаться вам знакомой, если вы уже видели HTML-код раньше. XML имеет тенденцию быть улучшением по сравнению со старой спецификацией HTML и может использоваться для создания полных веб-страниц и других документов разметки. Каковы преимущества DocBook? DocBook — это стандарт OASIS и формат, в котором большинство проектов с открытым исходным кодом хранят свою документацию. Docbook разработан как приложение с открытым исходным кодом. Проект размещён на SourceForge и доступен под лицензией GPL. DocBook доступен как определение типа документа (DTD) и XML-схема (XSD). У проекта есть большое сообщество разработчиков и техподдержки, охватывающее как группы с открытым исходным кодом, так и коммерческие группы. ${NC}"
echo " Наиболее важные причины, по которым в проектах используется DocBook: DocBook — это стандарт. DocBook с открытым исходным кодом. DocBook используется в большинстве крупных проектов. DocBook имеет большое сообщество разработчиков и техподдержки. DocBook также является приложением XML, а технологии XML решают ряд проблем публикации для групп документации, в том числе: Единый исходник. Совместная разработка. Кросс-платформенное редактирование. Многоканальная публикация. Повышение качества и согласованности информации. Расширение функциональности электронного вывода. Отказ от привязки к поставщику. Если вы уже разбираетесь в XML, тогда вы можете начать изучать DocBook. Если вы не понимаете XML, хорошая новость заключается в том, что изучение DocBook поможет вам изучить XML. Ниже приведены две книги, которые необходимо прочитать всем, кто только начинает работать с DocBook. "
echo "  "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_docbook  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_docbook" =~ [^10] ]]
do
    :
done
if [[ $in_docbook == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_docbook == 1 ]]; then
  echo ""
  echo " Установка DocBook (docbook-xml) (docbook-xsl) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
############ Зависимости ###############
sudo pacman -S --noconfirm --needed libxml2  # Анализатор XML C и набор инструментов ; https://archlinux.org/packages/core/x86_64/libxml2/ ; https://gitlab.gnome.org/GNOME/libxml2/-/wikis/home ; Обеспечивает: libxml2.so=16-64 ; 2025-07-17 02:23 UTC
sudo pacman -S --noconfirm --needed libxslt  # Библиотека преобразования таблиц стилей XML ; https://archlinux.org/packages/extra/x86_64/libxslt/ ; https://gitlab.gnome.org/GNOME/libxslt/-/wikis/home ; Обеспечивает: libexslt.so=0-64, libxslt.so=1-64 ; 2025-04-30 17:35 UTC
######### docbook-xml ###############
sudo pacman -S --noconfirm --needed docbook-xml  # Широко используемая схема XML для написания документации и справки ; https://archlinux.org/packages/extra/any/docbook-xml/ ; https://docbook.org/ ; 2025-06-13 16:27 UTC
############## docbook-xsl #################
sudo pacman -S --noconfirm --needed docbook-xsl  # Таблицы стилей XML для преобразований Docbook-xml
######### Дополнения #############
sudo pacman -S --noconfirm --needed itstool  # Перевод XML с помощью файлов PO с использованием правил набора тегов интернационализации W3C ; https://archlinux.org/packages/extra/any/itstool/ ; https://itstool.org/ ; 2025-02-13 07:36 UTC
sudo pacman -S --noconfirm --needed dblatex  # Конвертер DocBook (XML и SGML) в DVI, PDF, PostScript с помощью Latex ; https://archlinux.org/packages/extra/any/dblatex/ ; https://dblatex.sourceforge.net/ ; https://dblatex.sourceforge.net/ ; 2024-12-22 13:04 UTC
######### Pandoc — универсальный конвертер документов #########
sudo pacman -S --noconfirm --needed pandoc  #  Преобразование между форматами документации ; Pandoc может конвертировать документы из множества форматов, включая markdown, reStructuredText, textile, HTML, DocBook, LaTeX и разметку MediaWiki, в множество других форматов, включая markdown, reStructuredText, HTML, LaTeX, ConTeXt, PDF, RTF, DocBook XML, OpenDocument XML, ODT, GNU Texinfo, разметку MediaWiki, страницы руководства groff, слайд-шоу HTML, EPUB, Microsoft Word docx и другие.
sudo pacman -S --noconfirm --needed ruby-pandoc-ruby  # Рубиновая обертка для pandoc ; https://archlinux.org/packages/extra/any/ruby-pandoc-ruby/ ; https://github.com/xwmx/pandoc-ruby ; 2025-04-28 21:02 UTC ; PandocRuby — это оболочка для Pandoc , библиотеки Haskell с инструментами командной строки для преобразования одного формата разметки в другой.
sudo pacman -S --noconfirm --needed groff  # Система форматирования текста GNU troff ; https://archlinux.org/packages/core/x86_64/groff/ ; https://www.gnu.org/software/groff/groff.html ; 2024-08-29 18:19 UTC
sudo pacman -S --noconfirm --needed pandoc-crossref  # Фильтр Pandoc для перекрестных ссылок ; https://archlinux.org/packages/extra/x86_64/pandoc-crossref/ ; https://hackage.haskell.org/package/pandoc-crossref ; Заменяет: haskell-pandoc-crossref ; Конфликты: с haskell-pandoc-crossref ; 2025-07-23 21:14 UTC
# sudo pacman -S --noconfirm --needed pandoc-cli  # Преобразование между форматами документации ; https://archlinux.org/packages/extra/x86_64/pandoc-cli/ ; https://pandoc.org/ ; Обеспечивает: pandoc ; Заменяет: pandoc ; Конфликты: pandoc ; 2025-07-23 21:14 UTC
########## pandoc-bin #########
# yay -S pandoc-bin --noconfirm  # Преобразование между форматами документации ; https://aur.archlinux.org/packages/pandoc-bin ; https://aur.archlinux.org/pandoc-bin.git (только для чтения, нажмите, чтобы скопировать) ; https://pandoc.org/ ; Конфликты: с pandoc-cli ; Обеспечивает: pandoc, pandoc-cli ; 29.05.2025 15:47 (UTC)
########## pandoc-bin #########
#git clone https://aur.archlinux.org/pandoc-bin.git  # (только для чтения, нажмите, чтобы скопировать)
#cd pandoc-bin
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm    #--не спрашивать каких-либо подтверждений
...
#rm -Rf pandoc-bin
################
sudo pacman -S --noconfirm --needed haskell-pandoc  # Преобразование между форматами разметки ; https://archlinux.org/packages/extra/x86_64/haskell-pandoc/ ; https://pandoc.org/ ; 2025-07-23 21:11 UTC
sudo pacman -S --noconfirm --needed haskell-doctemplates  # Шаблоны документов в стиле Pandoc ; https://archlinux.org/packages/extra/x86_64/haskell-doctemplates/ ; https://github.com/jgm/doctemplates ; 2025-07-23 21:09 UTC
sudo pacman -S --noconfirm --needed kdoctools  # Генерация документации из docbook ; https://archlinux.org/packages/extra/x86_64/kdoctools/ ; https://community.kde.org/Frameworks ; 2025-07-11 18:13 UTC
sudo pacman -S --noconfirm --needed poxml  # Переводит файлы DocBook XML с использованием po-файлов gettext ; https://archlinux.org/packages/extra/x86_64/poxml/ ; https://www.kde.org/ ; 2025-07-03 17:07 UTC
sudo pacman -S --noconfirm --needed docbook2x  # Программный пакет, который преобразует документы DocBook в традиционный формат man-страниц Unix и формат GNU Texinfo ; https://archlinux.org/packages/extra/x86_64/docbook2x/ ; http://docbook2x.sourceforge.net/ ; 2024-07-12 04:10 UTC
sudo pacman -S --noconfirm --needed opensp  # Библиотека и набор инструментов для проверки, анализа и обработки документов SGML и XML ; https://archlinux.org/packages/extra/x86_64/opensp/ ; http://openjade.sourceforge.net/ ; 2023-02-26 14:47 UTC
########### docbook-to-man ###########
yay -S docbook-to-man --noconfirm  # Пакетный конвертер из DocBook SGML в макросы nroff/troff man ; https://aur.archlinux.org/packages/docbook-to-man ; https://aur.archlinux.org/docbook-to-man.git (только для чтения, нажмите, чтобы скопировать) ; http://www.oasis-open.org/docbook/tools/dtm/ ; 2021-02-25 00:41 (UTC)
########### docbook-to-man ###########
#git clone https://aur.archlinux.org/docbook-to-man.git  # (только для чтения, нажмите, чтобы скопировать)
#cd docbook-to-man
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm    #--не спрашивать каких-либо подтверждений
...
#pwd    # покажет в какой директории мы находимся
#cd ..   # поднимаемся на уровень выше (выходим из папки сборки)
# rm -rf docbook-to-man
#rm -Rf docbook-to-man
################
sudo pacman -S --noconfirm --needed ghc  # Компилятор Глазго Haskell ; https://archlinux.org/packages/extra/x86_64/ghc/ ; https://www.haskell.org/ghc/ ; Обеспечивает:  haskell-ghc=9.4.8, haskell-haddock=2.27.0, haskell-hp2ps=0.1, haskell-hpc-bin=0.68, haskell-hsc2hs=0.68.8 ; Заменяет: haskell-ghc, haskell-haddock, haskell-hp2ps, haskell-hpc-bin, haskell-hsc2hs ; 2025-04-10 17:45 UTC
sudo pacman -S --noconfirm --needed haskell-haddock-library  # Инструмент для создания документации для библиотек Haskell ; https://archlinux.org/packages/extra/x86_64/haskell-haddock-library/ ; https://www.haskell.org/haddock/ ; 2025-07-23 21:09 UTC
sudo pacman -S --noconfirm --needed happy  # Генератор парсеров для Haskell ; https://archlinux.org/packages/extra/x86_64/happy/ ; https://hackage.haskell.org/package/happy ; 2025-07-14 15:49 UTC
sudo pacman -S --noconfirm --needed alex  # Генератор лексических анализаторов для Haskell ; https://archlinux.org/packages/extra/x86_64/alex/ ; https://hackage.haskell.org/package/alex ; 2025-07-15 10:57 UTC
# sudo pacman -S --noconfirm --needed cabal-install  # Интерфейс командной строки для Cabal и Hackage ; https://archlinux.org/packages/extra/x86_64/cabal-install/ ; https://hackage.haskell.org/package/cabal-install ; 2025-07-23 21:07 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
############## Справка ###########
# Ручная установка pandoc в Arch Linux - http://dreamhunter.me/en/2015/01/07/pandoc-installation-in-arch-linux/ ; https://mictadlo.tumblr.com/post/96445768433/how-to-install-pandoc-on-arch-linux-based-distros
# Какой программой открыть файлы .docbook (DocBook) - https://zalinux.ru/?p=5547
# Конвертируйте Документы Между Различными Форматами. (https://laramatic.com/how-to-install-pandoc-in-debian-ubuntu-arch-kali-fedora-raspbian-and-macos/)
# Как Использовать Pandoc?
# Ниже приведены несколько примеров команд для pandoc, которые вы можете использовать в терминале.
# Конвертировать файл в PDF (выходной формат определяется расширением файла):
# pandoc input.md -o output.pdf
# Преобразование reStructuredText в PDF и DOCX:
...
# Принудительное преобразование для использования определенного формата:
# pandoc input.docx --to markdown_github -o output.md
# Преобразовать в отдельный файл с соответствующими верхними/нижними колонтитулами (для LaTeX, HTML и т. д.):
# pandoc input.md -s -o output.tex
# Список всех поддерживаемых форматов ввода:
# pandoc --list-input-formatsНажмите, чтобы скопировать
# Список всех поддерживаемых форматов вывода:
# pandoc --list-output-formats
# pandoc --help  # Cabal не устанавливает pandoc.1страницу руководства, но вы можете скопировать ее из man/каталога исходного кода /usr/local/share/man/man1/или в любое другое место в вашей системе, где размещаются страницы руководства.
# Затем установите Haskell platform из официальных репозиториев( ссылка ):
# sudo pacman -S ghc cabal-install haddock happy alex
# Затем используйте cabal инструмент для установки pandoc, вы можете cabal-install сначала обновить его, если доступна новая версия (это трудоемкий процесс).
# cabal update
...
echo " Установка утилит (пакетов) пропущена "
  ...
  echo ""
  echo " Установка Xreader (xreader) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Zathura (zathura) - (для просмотра документов PDF, DjVu, PostScript и Comic Book Archive)?"
echo -e "${MAGENTA}:: ${BOLD}Zathura - приложение для просмотра документов в форматах PDF, DjVu, PostScript и Comic Book Archive в Linux. Написан с использованием Poppler и GTK+, имеет минималистичный интерфейс, удобное управление с клавиатуры, поддержку мыши, функцию закладок (автоматическое открытие документа на месте закрытия), автоматическое обновление документов, систему уведомлений, командный буфер, поиск, поддерживает вкладки и просмотр зашифрованных PDF-документов. Не требует библиотек GNOME и KDE. Распространяется под лицензией GNU GPL. Zathura - очень настраиваемый и функциональный источник документов. Он обеспечивает a минималистичный и экономящий пространство интерфейс, а также простое использование, которое в основном Основное внимание уделяется клавиатурному взаимодействию. Хотите только просмотреть документы PDF? Как насчет PostScript или DjVu? Все Вместе? zathura теперь использует систему на основе плагинов для поддержки типы документов, которые позволяют вам выбирать, какие форматы файлов Вы хотите, чтобы ваша версия zathura поддерживала. Это также позволяет использовать разные бэкэнды для одного и того же типа документа: например, мы предоставляем плагин для PDF-документов с использованием либо попплера, либо библиотеки mupdf. ${NC}"
echo " Домашняя страница: https://pwmt.org/projects/zathura/ ; (https://archlinux.org/packages/extra/x86_64/zathura/). "
echo -e "${MAGENTA}:: ${BOLD}Основные особенности: Поддержка SyncTeX: Синхронизация TeXnology под названием SyncTeX является новой функцией позволяет синхронизироваться между входом и выходом, что означает перейти от исходного документа к набору материалу и пороку наоборот. zathura поддерживает эту функцию в обоих направлениях и делает писать документы LaTeX еще веселее! Всякий раз, когда файл, который вы сейчас смотрите на изменения (например: вы в значительной степени работа над документом LaTeX, и вы компилируете его для просмотра изменений), zathura автоматически обнаруживает это и перезаряжает документ без каких-либо Колебания. ${NC}"
echo " Когда вы хотите найти определенную страницу документа позже, вы можете использовать закладки, которые сохраняются с помощью команды :bmark. Вы можете легко переехать в сохраненным закладкам с командой :blist или удалить их с :bdelete. Или вы можете просто использовать быстрые метки. Быстрая марка - это просто страница, назначенная буква или цифра и поэтому могут сохраняться или открываться только с помощью двух ключевых нажатий. Чтобы создать новую быструю метку, просто наведите «м», за которым следует буква или цифра. Для просмотра a сохраненная быстрая марка просто используйте ', а затем ранее использованную букву или номер. "
echo " Zathura позволяет полностью просматривать и перемещаться по Различные документы без использования мыши. Функции для прокрутки или масштабирования отображены на определенные клавиши, а также возможность перейти или открыть ссылки Это указано в документе. Просто нажав клавишу «f» на вашем клавиатура, zathura выделяет все ссылки, отображаемые на текущем экране. По ввод одного из показанных чисел вы можете легко перейти по ссылкам на главы или Откройте их с помощью вашего любимого веб-браузера. Но, конечно... ты все еще можешь Используйте и мышь. Легко настраивается! Вы недовольны ярлыками, которые мы предоставляем? Ты не любишь Цвета? Никаких проблем и никаких забот. Почти все в zathura является легко настраивается с помощью конфигурационного файла. Если вы хотите учиться больше, оформить документацию... и еще много! В дополнение к вышеперечисленным особенностям zathura способен экспортировать изображения и вложения, открывая зашифрованные документы и печатая их. Вы можете искать через документ или просматривать его индекс или проверять документ Метаинформация. Мы предлагаем дополнительный бэкэнд базы данных Sqlite и мы поддерживаем Инструмент с вкладками. "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_zathura  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_zathura" =~ [^10] ]]
do
    :
done
if [[ $in_zathura == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_zathura == 1 ]]; then
  echo ""
  echo " Установка Zathura (zathura) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
sudo pacman -S --noconfirm --needed zathura  # Минималистичный просмотрщик документов ; https://archlinux.org/packages/extra/x86_64/zathura/ ; https://pwmt.org/projects/zathura/ ; 2025-07-06 08:27 UTC
  echo ""
  echo " Установка дополнений Для поддержки форматов DjVu, PDF и PostScript "
sudo pacman -S --noconfirm --needed djvulibre  # Библиотека и утилиты для создания, обработки и просмотра документов DjVu (déjà vu) ; https://archlinux.org/packages/extra/x86_64/djvulibre/ ; https://djvu.sourceforge.net/ ; 2025-07-22 11:22 UTC
sudo pacman -S --noconfirm --needed zathura-djvu  # Поддержка DjVu для Zathura ; https://archlinux.org/packages/extra/x86_64/zathura-djvu/ ; https://pwmt.org/projects/zathura-djvu/ ;  2025-07-06 08:27 UTC
sudo pacman -S --noconfirm --needed zathura-pdf-poppler  # Добавляет поддержку PDF в Zathura с помощью движка Poppler ; https://archlinux.org/packages/extra/x86_64/zathura-pdf-poppler/ ; https://pwmt.org/projects/zathura-pdf-poppler/ ; Обратные конфликты: с zathura-pdf-mupdf ; 2025-07-06 08:27 UTC
sudo pacman -S --noconfirm --needed zathura-ps  # Добавляет поддержку ps в zathura с помощью библиотеки libspectre ; https://archlinux.org/packages/extra/x86_64/zathura-ps/ ; https://pwmt.org/projects/zathura-ps ; 2025-07-06 08:27 UTC
sudo pacman -S --noconfirm --needed zathura-cb  # Добавляет поддержку комиксов в Zathura ; https://archlinux.org/packages/extra/x86_64/zathura-cb/ ; https://pwmt.org/projects/zathura-cb/ ; 2025-07-06 08:27 UTC
### sudo pacman -S --noconfirm --needed zathura-pdf-mupdf  # Поддержка PDF для Zathura (серверная часть MuPDF) (поддерживает PDF, ePub и OpenXPS) ; https://archlinux.org/packages/extra/x86_64/zathura-pdf-mupdf/ ; https://pwmt.org/projects/zathura-pdf-mupdf/ ; Конфликты: с zathura-pdf-poppler ; 2025-07-06 08:27 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
...
sudo pacman -Syy  # обновление баз пакмэна (pacman)
sudo pacman -S --noconfirm --needed python-pikepdf  # Чтение и запись PDF-файлов с помощью Python на базе qpdf. https://github.com/pikepdf/pikepdf ; https://archlinux.org/packages/extra/x86_64/python-pikepdf/ ; 2025-07-17 16:39 UTC
sudo pacman -S --noconfirm --needed img2pdf  # Конвертируйте растровые изображения в PDF без потерь ; https://archlinux.org/packages/extra/any/img2pdf/ ; https://gitlab.mister-muffin.de/josch/img2pdf ; 2025-04-29 21:56 UTC
sudo pacman -S --noconfirm --needed pdfarranger  # Помогает объединять или разделять PDF-документы, а также вращать, обрезать и переставлять страницы. https://github.com/pdfarranger/pdfarranger ; https://archlinux.org/packages/extra/any/pdfarranger/ ; 2025-06-11 21:03 UTC
echo " Установка утилит (пакетов) выполнена "
fi
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить PDF Tricks (pdftricks) — Приложение для небольших манипуляций с PDF-файлами?"
echo -e "${YELLOW}==> Примечание! ${BOLD} *PDF Tricks — минималистичное Vala / GTK (GLib / Granite) графическое приложение для работы с документами формата PDF (Portable Document Format). ${NC}"
echo -e "${MAGENTA}:: ${BOLD}PDF Tricks — Простое и эффективное приложение для небольших манипуляций с PDF-файлами. PDF Tricks прост в использовании и эффективен, поддерживается сжатие PDF файлов (лёгкое, среднее/рекомендованное и максимальное), разделение и объединение PDF фалов, а также преобразование страниц PDF документа в набор JPG (*jpg) / PNG (*.png) изображений, или в текстовой файл (*.txt). PDF Tricks написан на языке Vala. Это бесплатное программное обеспечение с открытым исходным кодом. Этот проект Лицензируется под GPL v3.0 . ${NC}"
echo " Домашняя страница: https://github.com/muriloventuroso/pdftricks ; (https://www.ghostscript.com/ ; https://archlinux.org/packages/extra/x86_64/pdftricks/). "
echo -e "${BLUE}:: ${NC}Особенности включают в себя: Сжать PDF – сжать PDF-файл. Получите то же качество, но с меньшим размером файла. Выберите один из вариантов: Экстремальное сжатие – меньшее качество, высокая степень сжатия. Средняя компрессия – хорошее качество, хорошая компрессия. Рекомендуемое сжатие – хорошее качество, оптимизированное для печати. Меньше сжатия – высокое качество, меньше сжатия. Разделить PDF-файл — разделить PDF-файл по диапазонам страниц, разделить цветные страницы или извлечь все страницы PDF-файла в несколько PDF-файлов. Объединение PDF-файлов — выберите несколько PDF-файлов или изображений и быстро объедините их. Конвертировать PDF – конвертировать PDF-файлы в форматы JPG, PNG и TXT. "
echo -e "${CYAN}:: ${NC}Пользовательский интерфейс PDF Tricks выполнен в соответствии с концепцией Gnome 3 (размещение управляющих элементов приложений на заголовке окна), функционал основан на возможностях ImageMagick и Ghostscript. ImageMagick — набор консольных утилит для создания, редактирования и преобразования изображений большинства существующих графических форматов. Для повышения производительности используется обработка изображений в несколько потоков, поддерживается работа с изображениями любого размера. Ghostscript — набор программного обеспечения позволяющего интерпретировать (анализ, обработка и выполнение) язык описания страниц PostScript и документы PDF, может использоваться как процессор растровых изображений (для растровых компьютерных принтеров) и в качестве универсальной среды программирования. "
echo -e "${YELLOW} Примечание! ${BOLD} *Будьте внимательны! Процесс установки был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. ${NC}"
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_pdftricks  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_pdftricks" =~ [^10] ]]
do
    :
done
if [[ $in_pdftricks == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_pdftricks == 1 ]]; then
  echo ""
  echo " Установка утилиты (пакета) PDF Tricks (pdftricks) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
  echo ""
  echo " Установка дополнительных утилит (пакетов) "
############ Зависимости ##############
sudo pacman -S --noconfirm --needed ghostscript  # (опционально) — поддержка PS/PDF ; Интерпретатор языка PostScript ; https://archlinux.org/packages/extra/x86_64/ghostscript/ ; https://www.ghostscript.com/ ; Заменяет: ghostpcl, ghostxps ; Конфликты: ghostpcl, ghostxps ; 2025-05-12 14:50 UTC
############ pdftricks ##############
sudo pacman -S --noconfirm --needed pdftricks  # Простое и эффективное приложение для небольших манипуляций с PDF-файлами с помощью Ghostscript ; https://github.com/muriloventuroso/pdftricks ; https://archlinux.org/packages/extra/x86_64/pdftricks/ ; 2023-05-19 17:10 UTC
  echo ""
  echo " Посмотрите информацию о версии (pdftricks) "
sudo pacman -Q pdftricks  # Показать версию приложения
sleep 03
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
############# Справка ##############
# PDF Tricks (pdftricks):
# https://github.com/muriloventuroso/pdftricks
# https://www.ghostscript.com/
# https://archlinux.org/packages/extra/x86_64/pdftricks/
#################################
...
#sudo pacman -Syy  # обновление баз пакмэна (pacman)
sudo pacman -S --noconfirm --needed foliate  # Простая и современная GTK-читалка электронных книг ; https://johnfactotum.github.io/foliate/ ; https://archlinux.org/packages/extra/any/foliate/ ; 2025-04-01 08:27 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Sigil (sigil) — Редактор электронных книг EPUB2/EPUB3?"
echo -e "${YELLOW}==> Примечание! ${BOLD} *ePub (Electronic Publication) — открытый формат электронных версий книг, разработанный Международным форумом по цифровым публикациям. Файлы в этом формате имеют расширение .epub. Формат позволяет издателям производить и распространять цифровую публикацию в одном файле, обеспечивая совместимость между программным и аппаратным обеспечением, необходимым для воспроизведения цифровых книг и других публикаций с плавающей вёрсткой. ${NC}"
echo -e "${MAGENTA}:: ${BOLD}Sigil — это бесплатный кроссплатформенный редактор электронных книг с открытым исходным кодом, поддерживающий технологию WYSIWYG (что видишь, то и получишь), использующий Qt6 (и QtWebEngine). Он предназначен для редактирования книг в формате ePub (ePub 2 и ePub 3). Поддерживаются функции форматирования и разметки текста, вставка изображений, гиперссылок, автоматическое создание оглавления и многое другое. С помощью Sigil можно создавать полноценные электронные книги. Sigil имеет удобный и частично настраиваемый пользовательский интерфейс и функциональные возможности типичные для большинства текстовых процессоров и визуальных HTML-редакторов. Создавать и редактировать файлы форматов ePub и FB2 можно в визуальном режиме, в режиме редактора кода и смешанном режиме (область просмотра с веб-инспектором). В отдельных панелях редактор представляет файловую структуру книги и древо таблицы содержания. Программа доступна для Linux, Windows, MacOS X. Этот проект Лицензируется под GPL3. ${NC}"
echo " Домашняя страница: https://sigil-ebook.com/ ; (https://github.com/Sigil-Ebook/Sigil ; https://archlinux.org/packages/extra/x86_64/sigil/). "
echo -e "${BLUE}:: ${NC}Sigil обладает большим количеством возможностей, полностью поддерживает Юникод (UTF8) и спецификации EPUB 2, имеется механизм поиска и замены (работающий со всеми файлами книги сразу), есть инструмент для удаления неиспользуемой графики и стилей. Диалог изменения метаданных позволяет быстро ввести/изменить всю справочную информацию о книге (имя автора, год издания, издательство и прочее). Sigil для новых книг автоматически создаёт все необходимые в стандарте EPUB/EPUB 2 вспомогательные файлы и стандартную структуру внутри архива epub. Так же редактором самостоятельно проводится и архивация всех файлов в zip-архив, со всеми нужными ключами... "
echo -e "${CYAN}:: ${NC}Интерфейс программы довольно простой и не должен вызвать сложностей. Он напоминает html-WYSIWYG редактор. В текст можно вставлять изображения в различных форматах (JPG, PNG, SVG). Поддерживается использование собственных шрифтов, добавление аудио, видео и других объектов. Sigil поддерживает импорт EPUB файлов, то есть можно открывать готовые файлы в формате EPUB и редактировать их. "
echo -e "${YELLOW} Примечание! ${BOLD} *Будьте внимательны! Процесс установки был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. ${NC}"
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Да установить,     0 - НЕТ - Пропустить установку: " in_sigil  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_sigil" =~ [^10] ]]
do
    :
...
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_sigil == 1 ]]; then
  echo ""
  echo " Установка утилиты (пакета) Sigil (sigil) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
  echo ""
  echo " Установка дополнительных утилит (пакетов) "
############ Зависимости ##############
######### Проверка орфографии (английская и русская) ###########
sudo pacman -S --noconfirm --needed aspell  # Проверка орфографии, призванная в конечном итоге заменить Ispell ; http://aspell.net/ ; https://archlinux.org/packages/extra/x86_64/aspell/ ; Обеспечивает: libaspell.so=15-64, libpspell.so=15-64 ; 2024-01-29 23:10 UTC
sudo pacman -S --noconfirm --needed aspell-en  # Английский словарь для aspell ; http://aspell.net/ ; https://archlinux.org/packages/extra/x86_64/aspell-en/ ; 2024-07-11 23:55 UTC
sudo pacman -S --noconfirm --needed aspell-ru  # Русский словарь для aspell ; http://aspell.net/ ; https://archlinux.org/packages/extra/x86_64/aspell-ru/ ; 2024-07-04 09:52 UTC
sudo pacman -S --noconfirm --needed hunspell  # (необязательно) ;  Библиотека и программа для проверки орфографии и морфологического анализатора ; https://github.com/hunspell/hunspell ; https://archlinux.org/packages/extra/x86_64/hunspell/ ; 2024-07-02 19:40 UTC
sudo pacman -S --noconfirm --needed hyphen-en  # (необязательно) — для поддержки переносов английского языка в плагинах ; Правила расстановки переносов в английском ; https://archlinux.org/packages/extra/x86_64/hyphen-en/ ; https://hunspell.sf.net/ ; 2024-07-02 19:57 UTC
sudo pacman -S --noconfirm --needed hunspell-en_us  # (необязательно) — для поддержки английского словаря ; Словари английского языка США Hunspell ; https://archlinux.org/packages/extra/any/hunspell-en_us/ ; http://wordlist.aspell.net/dicts/ ; Обеспечивает: словарь hunspell, hunspell-en_US ; Заменяет: hunspell-en_US<=2020.12.07-1 ; Конфликты: hunspell-en_US ; 2023-07-15 07:38 UTC
sudo pacman -S --noconfirm --needed hyphen  # Библиотека для качественной расстановки переносов и выравнивания ; https://hunspell.sf.net/ ; https://archlinux.org/packages/extra/x86_64/hyphen/ ; 2024-07-02 19:57 UTC
#sudo pacman -S --noconfirm --needed pageedit  # (необязательно) — внешний редактор для замены BookView ; Простой одностраничный визуальный редактор XHTML, созданный на основе BookView от Sigil ; https://archlinux.org/packages/extra/x86_64/pageedit/ ; https://github.com/Sigil-Ebook/PageEdit ; 2025-05-23 11:48 UTC
sudo pacman -S --noconfirm --needed pyside6  # (необязательно) — рекомендуется для плагинов ; Позволяет использовать API Qt6 в приложениях Python ; https://archlinux.org/packages/extra/x86_64/pyside6/ ; https://www.qt.io/ ; Обеспечивает: qt6-python-bindings ; 2025-06-09 11:52 UTC
sudo pacman -S --noconfirm --needed python-chardet  # (необязательно) — рекомендуется для плагинов ; Модуль Python3 для автоматического определения кодировки символов ; https://archlinux.org/packages/extra/any/python-chardet/ ; https://github.com/chardet/chardet ; 2024-12-22 12:57 UTC
sudo pacman -S --noconfirm --needed python-cssselect  # (необязательно) — рекомендуется для плагинов ; Библиотека Python3, которая анализирует селекторы CSS3 и транслирует их в XPath 1.0 ; https://archlinux.org/packages/extra/any/python-cssselect/ ; https://pypi.python.org/pypi/cssselect ; 2024-12-22 13:52 UTC
sudo pacman -S --noconfirm --needed python-html5lib  # (необязательно) — рекомендуется для плагинов ; HTML-парсер/токенизатор Python, основанный на спецификации WHATWG HTML5 ; https://archlinux.org/packages/extra/any/python-html5lib/ ; https://github.com/html5lib ; 2024-12-22 13:52 UTC
sudo pacman -S --noconfirm --needed python-pillow  # (необязательно) — рекомендуется для плагинов ; Форк библиотеки изображений Python (PIL) ; https://archlinux.org/packages/extra/x86_64/python-pillow/ ; https://pillow.readthedocs.io/ ; 2025-07-05 16:46 UTC
#sudo pacman -S --noconfirm --needed python-regex  # (необязательно) — рекомендуется для плагинов ; Альтернативный модуль регулярных выражений Python ; https://archlinux.org/packages/extra/x86_64/python-regex/ ; https://github.com/mrabarnett/mrab-regex ; 2025-08-11 22:05 UTC
sudo pacman -S --noconfirm --needed tk  # (необязательно) - рекомендуется для плагинов ; Набор инструментов для работы с окнами, используемый с tcl ; https://archlinux.org/packages/extra/x86_64/tk/ ; http://tcl.sourceforge.net/ ; 2024-12-25 10:40 UTC
############# sigil ###########
sudo pacman -S --noconfirm --needed sigil  # мультиплатформенный редактор электронных книг EPUB2/EPUB3 ; https://archlinux.org/packages/extra/x86_64/sigil/ ; https://sigil-ebook.com/ ; https://github.com/Sigil-Ebook/Sigil ; 2025-07-18 10:58 UTC
  echo ""
  echo " Посмотрите информацию о версии (sigil) "
sudo pacman -Q sigil  #  Показать версию приложения
sleep 03
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить NFO Viewer (nfoview) - (для просмотра NFO-файлов)?"
echo -e "${MAGENTA}:: ${BOLD}NFO Viewer — это простая программа для просмотра NFO-файлов , представляющих собой ASCII-графику (https://en.wikipedia.org/wiki/Ascii_art) в кодировке CP437. Преимущества NFO Viewer перед текстовым редактором: предустановленные шрифты и кодировка, автоматическое определение размера окна и кликабельные гиперссылки. Приложение для просмотра текста, тщательно разработанное для своей основной задачи: просмотра и представления файлов NFO . ${NC}"
echo " Домашняя страница: https://otsaloma.io/nfoview/ ; (https://github.com/otsaloma/nfoview ; https://archlinux.org/packages/extra/any/nfoview/). "
echo -e "${MAGENTA}:: ${BOLD}NFO Viewer — это «простой просмотрщик NFO-файлов». Он отображает ваши NFO-файлы просто, но эффективно, не заморачиваясь со шрифтами и другими визуальными эффектами. Более того, NFO Viewer бесплатен и имеет открытый исходный код, так что вы можете ознакомиться с исходным кодом, если хотите. Открыв файл NFO, вы можете изменить стиль и цвет шрифта, межстрочный интервал, переключаться между цветовыми схемами или выбрать собственную. NFO Viewer также делает любые URL-адреса в файле NFO кликабельными, что очень удобно. Файлы NFO могут содержать искусно выполненные ASCII-графики, демонстрирующие мастерство их создателя. Файлы NFO, найденные при загрузке через торренты и пиратские копии, также служат своего рода тегом, указывающим остальному сообществу на то, что определённая группа взломала это программное обеспечение. Если вас не интересует ASCII-графика, а интересует только информация из файла, вы можете открыть файл NFO в стандартном текстовом редакторе. Хотя заголовок с замысловатым логотипом не будет обработан, информация о файле должна быть абсолютно корректной. Аналогично, если вы создаете или читаете файлы NFO для Plex, Kodi или другого медиасервиса, вы, вероятно, можете использовать текстовый редактор операционной системы по умолчанию, не столкнувшись при этом с какими-либо проблемами. Файлы NFO по-прежнему очень удобны. ${NC}"
echo " Ключевые особенности: Отображение файлов NFO всех распространенных кодировок (включая CP 437 и Unicode). Исправьте и отобразите множество неисправных кодировок, которые мешают другим зрителям. Вы можете точно настроить цвета текста, фона, блочной графики и многого другого. Интерактивные гиперссылки, в том числе ссылки, занимающие несколько строк. Расширенный режим рендеринга / Классический режим / Режим «Только текст». Базовая поддержка ANSI Art (более 16 цветов). Сглаживание ; Встроенный зум ; Экспорт функциональности. Функции автоматического обновления и просмотра папок. Интегрированное средство обновления. Бесплатно и с открытым исходным кодом. Доступны портативные версии. Версия командной строки для легкого преобразования в PNG, Unicode, HTML, PDF... NFO Viewer доступен для Linux и распространяется как свободное программное обеспечение под лицензией GNU General Public License (GPL). Если NFO Viewer не входит в комплект вашего дистрибутива или вам нужна более новая версия, ознакомьтесь с инструкцией по установке из Flatpak или исходного кода ниже. Стабильные версии доступны на Flathub (https://flathub.org/apps/details/io.otsaloma.nfoview). "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_nfoview  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_nfoview" =~ [^10] ]]
do
    :
done
if [[ $in_nfoview == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_nfoview == 1 ]]; then
  echo ""
  echo " Установка NFO Viewer (nfoview) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
sudo pacman -S --noconfirm --needed nfoview  # Простой просмотрщик файлов NFO в GTK+ 4 ; https://archlinux.org/packages/extra/any/nfoview/ ; https://otsaloma.io/nfoview/ ; https://github.com/otsaloma/nfoview ; 2025-04-13 09:09 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
##################
# Я написал небольшой скрипт bash для правильного просмотра файлов .nfo:
# !/bin/sh
# consolechars -m cp437
# cat $1
# consolechars -m iso01
# просто скопируйте в /usr/bin и введите:
# nfo <dir.file.nfo>
# и все готово,
# У меня есть решение:
# просто выберите шрифт фиксированного размера в aterm, чтобы правильно отображать ASCII.
# Я использую этот шрифт:
# в моем .Xresources:
# aterm*font:-dosemu-vga-medium-r-normal-*-*-160-*-*-p-*-ibm-cp437,
# и пользователь в #linuxger (ircnet) сказал мне, что можно также изменить его на cp437, но я еще не проверял.
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Lokalize (lokalize) - Система автоматизированного перевода?"
echo -e "${MAGENTA}:: ${BOLD}Lokalize — это инструмент локализации для программного обеспечения KDE и другого свободного программного обеспечения с открытым исходным кодом. Это также универсальная система автоматизированного перевода (CAT), с помощью которой можно переводить файлы OpenDocument (*.odt). Translate-Toolkit используется для извлечения текста для перевода из файлов .odt в .xliff и обратного объединения перевода в файл .odt. ${NC}"
echo " Домашняя страница: https://apps.kde.org/lokalize/ ; (https://archlinux.org/packages/extra/x86_64/lokalize/)."
echo -e "${MAGENTA}:: ${BOLD}Программа для локализации приложений, редактор .PO файлов. Lokalize в первую очередь ориентирована на перевод программного обеспечения и также интегрирует внешние инструменты конвертации для перевода офисных документов. Lokalize — это замена KBabel. Этот пакет входит в состав модуля KDE Software Development Kit. ${NC}"
echo " Функции: Поддержка функции памяти переводов (база данных, содержащая набор ранее переведённых сегментов текста); Поддержка глоссария (словарь узкоспециализированных терминов или список часто используемых выражений); Имеется функция синхронизации (объединения) переводов; Поддержка альтернативных версий перевода; Существует возможность добавлять примечания; Поддержка закладок; Есть возможность создавать проекты по переводу; Имеется возможность скрыть панель инструментов и строку состояния в любой вкладке программы, а во вкладке перевода можно убрать/показать различные блоки: сегменты, глоссарий, альтернативные версии и др.; Поддержка работы с программами Pology и LanguageTool; и т.д.. "
echo " Есть возможность настроить внешний вид программы: выбрать семейство шрифта и его размер, цвет фона добавленного и удалённого; Поддержка проверки орфографии; Имеется возможность сменить статус перевода: утверждено, требует проверки и готово; Поддержка поиска и замены в файлах; Поддержка настраиваемых сочетаний (горячих) клавиш. "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_lokalize  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_lokalize" =~ [^10] ]]
do
    :
done
if [[ $in_lokalize == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_lokalize == 1 ]]; then
  echo ""
  echo " Установка Lokalize (lokalize) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
sudo pacman -S --noconfirm --needed hunspell  # Библиотека и программа проверки орфографии и морфологического анализатора ; https://archlinux.org/packages/extra/x86_64/hunspell/ ; https://github.com/hunspell/hunspell ; 2024-07-02 19:40 UTC
sudo pacman -S --noconfirm --needed lokalize  # Система автоматизированного перевода ; https://archlinux.org/packages/extra/x86_64/lokalize/ ; https://apps.kde.org/lokalize/ ; 2025-07-03 17:07 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
