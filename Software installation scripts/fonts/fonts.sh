...
FONTS_PROG_LANG="russian"  # Installer default language (Язык установки по умолчанию)
...
sudo pacman -S --noconfirm --needed wqy-bitmapfont  # Растровый китайский шрифт Song Ti (с засечками) ; https://wenq.org/ ; https://archlinux.org/packages/extra/any/wqy-bitmapfont/
...
# sudo pacman -S --noconfirm --needed ttf-liberation-mono-nerd  # Исправленный шрифт Liberation Mono из библиотеки шрифтов Nerd ; https://archlinux.org/packages/extra/any/ttf-liberation-mono-nerd/ ; https://github.com/ryanoasis/nerd-fonts ; Обеспечивает: ttf-font-nerd ; Заменяет: nerd-fonts-liberation-mono ; Конфликты: nerd-fonts-liberation-mono ; 2025-04-24 18:47 UTC
...
sudo pacman -S --noconfirm --needed ttf-opensans  # Шрифт без засечек, заказанный Google ; https://fonts.google.com/specimen/Open+Sans ; https://archlinux.org/packages/extra/any/ttf-opensans/
sudo pacman -S --noconfirm --needed ttf-firacode-nerd  # Исправленный шрифт Fira (Fura) Код из библиотеки шрифтов для ботаников ; https://github.com/ryanoasis/nerd-fonts ; https://archlinux.org/packages/extra/any/ttf-firacode-nerd/
sudo pacman -S --noconfirm --needed libfontenc  # Библиотека кодирования шрифтов X11 ; https://gitlab.freedesktop.org/xorg/lib/libfontenc ; https://archlinux.org/packages/extra/x86_64/libfontenc/
sudo pacman -S --noconfirm --needed libxfont2  # Библиотека растеризации шрифтов X11 ; https://xorg.freedesktop.org/ ; https://archlinux.org/packages/extra/x86_64/libxfont2/
sudo pacman -S --noconfirm --needed ttf-fira-code  # Моноширинный шрифт с программными лигатурами ; https://github.com/tonsky/FiraCode ; https://archlinux.org/packages/extra/any/ttf-fira-code/
sudo pacman -S --noconfirm --needed ttf-fira-mono  # Моноширинный шрифт Mozilla, разработанный для Firefox OS ; https://github.com/mozilla/Fira ; https://archlinux.org/packages/extra/any/ttf-fira-mono/
sudo pacman -S --noconfirm --needed ttf-ibm-plex  # IBM Plex Mono, Sans и Serif ; https://github.com/IBM/plex ; https://archlinux.org/packages/extra/any/ttf-ibm-plex/
...
sudo pacman -S --noconfirm --needed ttf-agave-nerd  # Исправленный шрифт Agave из библиотеки шрифтов для ботаников ; https://github.com/ryanoasis/nerd-fonts ; https://archlinux.org/packages/extra/any/ttf-agave-nerd/ ; Конфликты: с nerd-fonts-agave ; 9 мая 2024 г., 21:10 UTC
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Birdfont (birdfont) — редактор шрифтов?"
echo -e "${MAGENTA}:: ${BOLD}Birdfont — бесплатный редактор шрифтовна базе Vala с простым и понятным пользовательским интерфейсом, который может создавать векторную графику и экспортировать шрифты SVG, EOT и TTF, OTF. Редактор поддерживает как монохромные, так и цветные шрифты. Он включает в себя базовые инструменты рисования, контрольные точки для кривых и другие функции, такие как импорт текстового изображения и преобразование его в шрифт. ${NC}"
echo " Домашняя страница: https://birdfont.org/ ; (https://github.com/johanmattssonm/birdfont ; https://archlinux.org/packages/extra/x86_64/birdfont/). "
echo -e "${MAGENTA}:: ${BOLD}Основные особенности Birdfont: Возможность импорта изображения и преобразования его в шрифт. Поддержка слоев. Поддержка шрифтов TTF. Резервный шрифт с поддержкой многих языков. Простой и быстрый графический интерфейс. Поисковые запросы в базе данных Unicode. Поддержка файлов SVG, созданных в Illustrator. Настройки импорта и экспорта. Гладить. Круглые линейные колпачки. ${NC}"
echo " У BirdFont есть две системы сборки: одна на основе скрипта Python, которая собирает все двоичные файлы одновременно, и одна система сборки на основе зависимостей, которая использует doit. XML Bird доступен на сайте birdfont.org . Автор: Йохан Маттссон и другие, полное указание авторства смотрите в разделе АВТОРЫ (https://birdfont.org/#contact). License: GNU GPL v3 . "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_birdfont  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_birdfont" =~ [^10] ]]
do
    :
done
if [[ $in_birdfont == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_birdfont == 1 ]]; then
  echo ""
  echo " Установка Birdfont (birdfont) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
sudo pacman -S --noconfirm --needed birdfont  # Редактор шрифтов, который может генерировать шрифты в форматах TTF, EOT и SVG ; https://archlinux.org/packages/extra/x86_64/birdfont/ ; https://birdfont.org/ ; https://github.com/johanmattssonm/birdfont ; 2024-09-16 15:24 UTC
echo ""
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить KCharSelect (kcharselect) — Селектор персонажей?"
echo -e "${YELLOW}==> Примечание! ${BOLD} *Для технического использования показаны различные варианты изображения персонажа. ${NC}"
echo -e "${MAGENTA}:: ${BOLD}KCharSelect — инструмент для выбора специальных символов из всех установленных шрифтов и копирования их в буфер обмена. Вы можете просматривать персонажей по их категориям или быстро находить определенного персонажа, выполнив поиск по его имени. Этот проект Лицензируется под GPL-2.0 или более поздняя версия, LGPL-2.0 или более поздняя версия. ${NC}"
echo " Домашняя страница: https://apps.kde.org/kcharselect/ ; (https://archlinux.org/packages/extra/x86_64/kcharselect/). "
echo -e "${BLUE}:: ${NC}KCharSelect отображает различную информацию о выбранном символе. Она включает не только имя символа Unicode, но и псевдонимы, общие примечания и перекрёстные ссылки на похожие символы. "
echo -e "${CYAN}:: ${NC}KCharSelect также содержит данные Unihan для восточноазиатских символов для отображения английских определений и различных вариантов произношения. "
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_kcharselect  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_kcharselect" =~ [^10] ]]
do
    :
done
if [[ $in_kcharselect == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_kcharselect == 1 ]]; then
  echo ""
  echo " Установка утилиты (пакета) Psensor (kcharselect) "
#sudo pacman -Syy  # обновление баз пакмэна (pacman)
...
############ kcharselect ##############
sudo pacman -S --noconfirm --needed kcharselect  # Селектор персонажей ; https://archlinux.org/packages/extra/x86_64/kcharselect/ ; https://apps.kde.org/kcharselect/ ; Группы: приложения kde , утилиты kde ; 2025-09-11 07:10 UTC
  echo ""
  echo " Посмотрите информацию о версии (kcharselect) "
sudo pacman -Q kcharselect  #  Показать версию приложения
sleep 03
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Пропишем конфиг (fonts.conf) для ситемы Archlinux "
echo " Создать каталог (fontconfig) в домашней директории ~/.config/ "
#mkdir ~/.config/fontconfig   # Создать каталог fontconfig в ~/.config/ ; mkdir опции директория
mkdir -p ~/.config/fontconfig   # Создать каталог fontconfig в ~/.config/ ; -p или --parents ; Создать все директории, которые указаны внутри пути. Если какая-либо директория существует, то предупреждение об этом не выводится.
#mkdir -v ~/.config/fontconfig   # Создать каталог fontconfig в ~/.config/ ; -v или --verbose ; Выводить сообщение о каждой создаваемой директории.
echo " Создать файл fonts.conf в ~/.config/fontconfig "
touch ~/.config/fontconfig/fonts.conf   # Создать файл fonts.conf в ~/.config/
echo " Пропишем конфигурации в fonts.conf "
...
EOF
########
echo " Для начала сделаем его бэкап  ~/.config/fontconfig/fonts.conf "
cp ~/.config/fontconfig/fonts.conf  ~/.config/fontconfig/fonts.conf.back
echo " Просмотреть содержимое файла ~/.config/fontconfig/fonts.conf "
#ls -l ~/.config/fontconfig/fonts.conf   # ls — выводит список папок и файлов в текущей директории
cat ~/.config/fontconfig/fonts.conf  # cat читает данные из файла или стандартного ввода и выводит их на экран
sleep 3
##############
clear
echo -e "${GREEN}
