...
sudo pacman -S --noconfirm --needed firefox  # Автономный веб-браузер от mozilla.org ; https://www.mozilla.org/firefox/ ; https://archlinux.org/packages/extra/x86_64/firefox/ ; 2025-08-27 17:57 UTC
sudo pacman -S --noconfirm --needed firefox-i18n-ru  # Русский языковой пакет для Firefox ; https://www.mozilla.org/firefox/ ; https://archlinux.org/packages/extra/any/firefox-i18n-ru/ ; 2025-08-27 17:57 UTC
sudo pacman -S --noconfirm --needed firefox-spell-ru  # Русский словарь проверки орфографии для Firefox ; https://addons.mozilla.org/firefox/dictionaries/ ; https://archlinux.org/packages/extra-testing/any/firefox-spell-ru/ ; 2024-09-19 08:34 UTC
############ Firefox-addons ############# https://archlinux.org/groups/any/firefox-addons/
sudo pacman -S --noconfirm --needed firefox-ublock-origin  # Надстройка эффективного блокировщика для различных браузеров. Быстрый, мощный и компактный ; https://github.com/gorhill/uBlock ; https://archlinux.org/packages/extra/any/firefox-ublock-origin/ ; Группы: firefox-addons ; 2025-09-08 20:15 UTC
sudo pacman -S --noconfirm --needed firefox-adblock-plus  # Расширение для Firefox, которое блокирует рекламу и баннеры ; https://adblockplus.org/ ; https://archlinux.org/packages/extra/any/firefox-adblock-plus/ ; Группы: firefox-addons ; 2025-03-29 23:12 UTC
sudo pacman -S --noconfirm --needed firefox-dark-reader # Инвертирует яркость веб-страниц и снижает нагрузку на глаза при просмотре веб-страниц ; https://darkreader.org/ ; https://archlinux.org/packages/extra/any/firefox-dark-reader/ ; Группы: firefox-addons ; 2025-07-18 14:46 UTC
sudo pacman -S --noconfirm --needed firefox-noscript  # Расширение для Firefox, которое отключает JavaScript ; https://noscript.net/ ; https://archlinux.org/packages/extra/any/firefox-noscript/ ; Группы: firefox-addons ; 2025-06-09 19:19 UTC
#sudo pacman -S --noconfirm --needed firefox-decentraleyes  # Локальная эмуляция сетей доставки контента ; https://archlinux.org/packages/extra/any/firefox-decentraleyes/ ; https://decentraleyes.org/ ; 2024-12-01 01:43 UTC
...
sudo pacman -S --noconfirm --needed firefox-decentraleyes  # Локальная эмуляция сетей доставки контента ; Защищает вас от отслеживания посредством «бесплатной», централизованной доставки контента ; https://archlinux.org/packages/extra/any/firefox-decentraleyes/ ; https://decentraleyes.org/ ; Группы: firefox-addons ; https://archlinux.org/groups/any/firefox-addons/ ; 2024-12-01 01:43 UTC
...
# Отключите поиск: Выполняйте поиск в частном порядке, используя ваши любимые файлы...
...
# Файлы cookie: Мы используем неидентифицирующие сторонние файлы cookie, чтобы запомнить ваши предпочтения в поиске (язык, страну и поставщика услуг).
# https://search.disconnect.me/
# https://github.com/disconnectme/search
# https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/SDK/Tutorials/Installation
# https://developer.mozilla.org/en-US/search?q=Installation
...
### Decentraleyes - расширение для firefox и не только ; Защищает Вас от отслеживания через «бесплатные», централизованные CDN. Предотвращает отслеживание через множество запросов к CDN (а-ля GHL, MaxCDN, YandexCDN и т.п.), «доставляя» локальные файлы, чтобы не ломать сайты. Дополняет контент-фильтры. https://decentraleyes.org/ ; https://addons.mozilla.org/firefox/addon/decentraleyes/ ; https://chrome.google.com/webstore/detail/decentraleyes/ldpochfccmkkmhdbclfhpagapcfdljkj
...
mkdir ~/SkypeCalls  # # Общая директория, на машине
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
...
echo -e "${MAGENTA}:: ${BOLD}*TeamViewer не осуществляет продажи, обслуживание или поддержку любого рода в странах, которые могут привести к нарушению TeamViewer экспортных положений Европейского союза или Соединенных Штатов, или если это каким-либо образом запрещено в соответствии с Общей внешней политикой и политикой безопасности ЕС или экспортным законодательством США. По понятным юридическим причинам Arch Linux не распространяет Teamviewer. В результате пользователи должны установить программное обеспечение через AUR *. (Также ЖЕЛАТЕЛЬНО! иметь под рукой VPN) ${NC}"
...
############ Справка #############
# Как известно нельзя установить teamviewer в Arch linux, просто скачав его с официального сайта. Его необходимо ставить через AUR (https://aur.archlinux.org/packages/teamviewer). По понятным юридическим причинам Arch Linux не распространяет Teamviewer. В результате пользователи должны установить программное обеспечение через AUR . Для этого сначала откройте терминал и установите необходимые зависимости для сборки Teamviewer. Примечание. В вашем файле /etc/pacman.conf должен быть включен репозиторий с несколькими библиотеками , иначе Teamviewer не создаст нужную версию .
# sudo pacman -S lib32-libpng12 lib32-dbus lib32-libxinerama lib32-libjpeg6-turbo lib32-libxtst
# С установленными зависимостями, пришло время начать процесс сборки. Начните с перехода на страницу Teamviewer AUR и нажатия «загрузить снимок(скачать снимок)». Когда загрузка завершится, используйте команду CD, чтобы перейти в папку «Загрузки».
# cd ~/Downloads
# Внутри загрузок извлеките файл снимка AUR Teamviewer.
# tar -xvzf teamviewer.tar.gz
# Введите папку извлеченного архива моментальных снимков с помощью команды CD.
# cd teamviewer
# Наконец, соберите пакет Arch с помощью команды makepkg.
# makepkg
# Иногда терминал сообщает, что необходимо доставить некоторые зависимости.
# Доустанавливаем их командой: sudo pacman -S qt5-webkit...
# Когда makepkg завершит работу, установите пакет с помощью менеджера пакетов pacman.
# sudo pacman -U teamviewer - *. pkg.tar.xz
# Обязательно включите службу teamviewer через systemd перед использованием программного обеспечения:
# sudo systemctl enable teamviewerd
# sudo systemctl start teamviewerd
#----------------------------
# Обходной путь для startx
# Создать дроп-ин /etc/systemd/system/getty@tty1.service.d/getty@tty1.service-drop-in.conf с:
# [Service]
# Environment=XDG_SESSION_TYPE=x11
#----------------------
# Источник: https://community.teamviewer.com/English/discussion/95696/teamviewer-15-stopped-working-on-debian-buster#M4399
...
# TeamViewer не осуществляет продажи, обслуживание или поддержку любого рода в странах, которые могут привести к нарушению TeamViewer экспортных положений Европейского Союза или США или если это каким-либо образом запрещено в соответствии с Общей внешней политикой и политикой безопасности США. ЕС или с экспортным законодательством США.
...
clear
echo -e "${CYAN}
  <<< Установка программ для замены Skype в GNU / Linux в системе Arch Linux >>> ${NC}"
# Installing Skype replacement programs in GNU/Linux on Arch Linux system
#clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Google-Chat (google-chat-linux) - Неофициальный электронный клиент для настольного компьютера?"
echo -e "${MAGENTA}:: ${BOLD}Google Chat — это платформа для общения и совместной работы, разработанная Google, предназначенная как для делового, так и для общего использования. ${NC}"
echo " Домашняя страница: https://github.com/squalou/google-chat-linux ; (https://aur.archlinux.org/packages/google-chat-linux ; https://aur.archlinux.org/packages/google-chat-linux-bin?O=0). "
echo -e "${MAGENTA}:: ${BOLD}Разбираем альтернативу Skype — сервис Google Hangouts. На первый взгляд, сервисы имеют схожий функционал, но все же есть немало отличий, которые делают Google Hangouts удобным решением для удаленщиков. Google Chat — коммуникационный сервис, разработанный компанией Google. Изначально он был разработан для рабочих групп и бизнес-среды, но затем стал доступен и для обычных пользователей. Он предоставляет личные сообщения между пользователями, групповые беседы и чат-группы, которые позволяют пользователям не только общаться в чате, но и создавать и назначать задания и обмениваться файлами в одном месте. Доступ к сервису можно получить через собственные веб-сайт и приложение, или через веб-сайт и приложение Gmail. Впервые сервис был запущен 9 марта 2017 года под названием Hangouts Chat как одно из двух приложений, заменяющих Google Hangouts, вторым же стало Google Meet. 9 апреля 2020 года сервис был переименован в Google Chat. В начале 2022 года Google отказался от оригинальной версии Hangouts и заменил её на Google Chat. ${NC}"
echo " Что такое Google Hangouts? Google Hangouts — сервис групповых чатов для общения с коллегами по работе, клиентами и друзьями. Используется для обмена сообщениями, звонков и видеовстреч. "
echo " Как использовать Google Hangouts: От встреч до кинопоказов, Google Hangouts позволяет пользователям по всему миру с легкостью общаться в видеочате, сотрудничать и делиться. В приложении Hangouts есть множество функций, поэтому следуйте этому руководству, чтобы начать получать максимальную отдачу от Hangouts. Для звонков в Hangout у вас должно быть подключение к интернету. Вы можете совершать голосовые или видеозвонки. Вы можете сделать снимок экрана с фотографией или сохранить изображение, а затем отправить его на свою электронную почту с помощью Hangouts, а затем открыть электронное письмо на своем компьютере, чтобы сохранить его. К сожалению, вы не можете удалять отдельные сообщения. Однако вы можете удалить всю беседу. "
echo " Основные возможности: Общение с одним или несколькими собеседниками в групповом чате. Ограничение — до 150 участников в чате. В сообщения можно добавлять фотографии, карты, смайлики, стикеры и анимированные картинки. Организация видеовстреч. Достаточно одного нажатия, чтобы из чата перейти в формат видеовстречи. Ограничение — до 10 человек на встречу. Бесплатные звонки другим пользователям Google Hangouts. Звонки на стационарные и мобильные телефоны в разные страны (платно). Google Hangouts поддерживает синхронизацию данных на всех устройствах. Все чаты синхронизируются между компьютером и мобильными устройствами на Android и iOS. "
echo -e "${CYAN}:: ${NC}Установка Google-Chat (google-chat-linux) (google-chat-linux-bin) проходит через сборку из AUR (yay)(https://aur.archlinux.org/google-chat-linux.git ; https://aur.archlinux.org/google-chat-linux-bin.git). Если хотите можете собрать пакет через - git clone, PKGBUILD, makepkg - эта функция тоже прописана в сценарии скрипта, НО # закомментирована! "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Установить Google-Chat (google-chat-linux),  1 - *Установить Google-Chat (google-chat-linux-bin),

    0 - НЕТ - Пропустить установку: " in_hangouts  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_hangouts" =~ [^120] ]]
do
    :
done
if [[ $in_hangouts == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_hangouts == 1 ]]; then
  echo ""
  echo " Установка Google-Chat (google-chat-linux) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
############ Зависимости ############
########## electron29 #########
yay -S electron29 --noconfirm  # Создавайте кроссплатформенные настольные приложения с использованием веб-технологий ; https://aur.archlinux.org/packages/electron29 ; https://aur.archlinux.org/electron29.git (только для чтения, нажмите, чтобы скопировать) ; https://electronjs.org/ ; git+https://github.com/squalou/google-chat-linux#tag=5.29.23-1 ; https://aur.archlinux.org/cgit/aur.git/tree/google-chat-linux.sh?h=google-chat-linux ; 2025-05-05 17:14 (UTC)
########## electron29 #########
#git clone https://aur.archlinux.org/electron29.git  # (только для чтения, нажмите, чтобы скопировать)
#sudo chmod a+w electron29  # Чтобы установить Electron29, вам необходимо изменить права доступа к файлам
#cd electron29
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm   #--не спрашивать каких-либо подтверждений
...
# rm -rf electron29
#rm -Rf electron29
# yay -Rns electron29  # * (Необязательно) Удалите electron29 на Arch с помощью YAY
########### xdg-desktop-portal ##############
sudo pacman -S --noconfirm --needed xdg-desktop-portal  # Порталы интеграции рабочего стола для изолированных приложений ; https://archlinux.org/packages/extra/x86_64/xdg-desktop-portal/ ; https://flatpak.github.io/xdg-desktop-portal/ ; 2025-05-20 18:46 UTC
########### curl ##############
sudo pacman -S --noconfirm --needed curl  # Инструмент командной строки и библиотека для передачи данных с помощью URL-адресов ; https://archlinux.org/packages/core/x86_64/curl/ ; https://curl.se/ ; Обеспечивает: libcurl.so=4-64 ; Заменяет: wcurl ; Конфликты: с wcurl ; 2025-06-04 15:50 UTC
########### gendesk ##############
sudo pacman -S --noconfirm --needed gendesk  # Утилита для генерации файлов рабочего стола путем указания минимума информации ; https://gendesk.roboticoverlords.org/ ; https://archlinux.org/packages/extra/x86_64/gendesk/ ; 2024-05-20 19:13 UTC
########### git ##############
...
########### nvm ##############
sudo pacman -S --noconfirm --needed nvm  # Node Version Manager — простой bash-скрипт для управления несколькими активными версиями node.js ; https://archlinux.org/packages/extra/any/nvm/ ; https://github.com/nvm-sh/nvm ; 2025-04-26 06:41 UTC
########### google-chat-linux ###############
yay -S google-chat-linux --noconfirm  # Неофициальный настольный клиент на базе Electron для Google Chat, Electron не входит в комплект. (Используйте общесистемный Electron) ; https://aur.archlinux.org/packages/google-chat-linux ; https://aur.archlinux.org/google-chat-linux.git (только для чтения, нажмите, чтобы скопировать) ; https://github.com/squalou/google-chat-linux ; Конфликты: с google-chat-linux ; 2025-04-28 03:31 (UTC)
########### google-chat-linux ###############
#git clone https://aur.archlinux.org/google-chat-linux.git   # (только для чтения, нажмите, чтобы скопировать)
#sudo chmod a+w google-chat-linux  # Чтобы установить Google-Chat, вам необходимо изменить права доступа к файлам
...
#pwd    # покажет в какой директории мы находимся
#cd ..   # поднимаемся на уровень выше (выходим из папки сборки)
# rm -rf google-chat-linux
#rm -Rf google-chat-linux
# yay -Rns google-chat-linux  # * (Необязательно) Удалите google-chat-linux на Arch с помощью YAY
echo ""
echo " Установка утилит (пакетов) выполнена "
elif [[ $in_hangouts == 2 ]]; then
  echo ""
  echo " Установка Google-Chat (google-chat-linux-bin) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
############ Зависимости ############
########### xdg-desktop-portal ##############
sudo pacman -S --noconfirm --needed xdg-desktop-portal  # Порталы интеграции рабочего стола для изолированных приложений ; https://archlinux.org/packages/extra/x86_64/xdg-desktop-portal/ ; https://flatpak.github.io/xdg-desktop-portal/ ; 2025-05-20 18:46 UTC
########### binutils ##############
sudo pacman -S --noconfirm --needed binutils  # Набор программ для сборки и обработки двоичных и объектных файлов ; https://archlinux.org/packages/core/x86_64/binutils/ ; https://www.gnu.org/software/binutils/ ; Обеспечивает: libctf.so=0-64, libgprofng.so=0-64, libsframe.so=1-64 ; Заменяет: binutils-multilib ; Конфликты: с binutils-multilib ; 2025-04-30 17:25 UTC
########### nodejs ##############
sudo pacman -S --noconfirm --needed nodejs  # Событийный ввод-вывод для V8 javascript («Текущий» выпуск) ; https://archlinux.org/packages/extra/x86_64/nodejs/ ; https://nodejs.org/ ; 2025-06-10 17:17 UTC
########### npm ##############
sudo pacman -S --noconfirm --needed npm  # Менеджер пакетов JavaScript ; https://archlinux.org/packages/extra/any/npm/ ; https://www.npmjs.com/ ; 2025-06-13 00:34 UTC
########### xz ##############
...
yay -S google-chat-linux-bin --noconfirm  # Неофициальный электронный клиент для настольного компьютера Google Chat, включая Electron ; https://aur.archlinux.org/packages/google-chat-linux-bin?O=0 ; https://aur.archlinux.org/google-chat-linux-bin.git (только для чтения, нажмите, чтобы скопировать) ; https://github.com/squalou/google-chat-linux.git ; Конфликты: с google-chat-linux-electron7-git ; https://github.com/squalou/google-chat-linux/releases/download/5.29.23-1/google-chat-linux_5.29.23-1_amd64.deb ; 2024-02-23 07:29 (UTC)
########### google-chat-linux-bin ###############
#git clone https://aur.archlinux.org/google-chat-linux-bin.git   # (только для чтения, нажмите, чтобы скопировать)
#sudo chmod a+w google-chat-linux-bin  # Чтобы установить Google-Chat, вам необходимо изменить права доступа к файлам
#cd google-chat-linux-bin
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm   #--не спрашивать каких-либо подтверждений
# makepkg -si --skipinteg
#pwd    # покажет в какой директории мы находимся
#cd ..   # поднимаемся на уровень выше (выходим из папки сборки)
# rm -rf google-chat-linux-bin
#rm -Rf google-chat-linux-bin
# yay -Rns google-chat-linux-bin  # * (Необязательно) Удалите google-chat-linux-bin на Arch с помощью YAY
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
########## Справка ##################
# Справка Google Chat - https://support.google.com/chat?hl=ru-ru&ref_topic=6386410&visit_id=638867927680528533-3147357578&rd=1#topic=7649316
# Также Вы можете установить Google Chat Desktop (Snap пакет) - Неофициальное настольное приложение для Google Chat.
# https://snapcraft.io/install/google-chat-desktop/arch
# Это настольное приложение для Google Chat https://chat.google.com/ . Google Chat — это платформа для общения и совместной работы, разработанная Google, предназначенная как для делового, так и для общего использования.
# Сборка основана на репозитории https://github.com/zviryatko/google-chat-electron .
# Включите snaps в Arch Linux и установите google-chat-desktop . Snaps — это приложения, упакованные со всеми их зависимостями для запуска на всех популярных дистрибутивах Linux из одной сборки. Они автоматически обновляются и откатываются изящно.
# Приложения Snap можно найти и установить в Snap Store — магазине приложений с многомиллионной аудиторией. (https://snapcraft.io/store)
# Включить snapd: В Arch Linux snap можно установить из Arch User Repository (AUR). Процесс ручной сборки — это поддерживаемый Arch метод установки для пакетов AUR, и вам понадобятся установленные предварительные требования , прежде чем вы сможете установить любой пакет AUR. Затем вы можете установить snap следующим образом (https://aur.archlinux.org/packages/snapd/) :
# git clone https://aur.archlinux.org/snapd.git
# cd snapd
# makepkg -si
# После установки необходимо включить модуль systemd , который управляет основным сокетом связи Snap:
# sudo systemctl enable --now snapd.socket
# Если в вашей системе включен AppArmor , включите службу, которая загружает профили AppArmor (https://wiki.archlinux.org/title/AppArmor) для снимков:
# sudo systemctl enable --now snapd.apparmor.service
# Чтобы включить поддержку классической привязки, введите следующее для создания символической ссылки между /var/lib/snapd/snapи /snap:
# sudo ln -s /var/lib/snapd/snap /snap
# Либо выйдите из системы и войдите снова, либо перезапустите систему, чтобы убедиться, что пути snap-файлов обновлены правильно.
# Установить google-chat-desktop
# Чтобы установить google-chat-desktop, просто используйте следующую команду:
# git clone https://aur.archlinux.org/snapd.git
# cd snapd
# makepkg -si
# sudo systemctl enable --now snapd.socket
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Webapp Manager (webapp-manager) - Менеджер веб-приложений?"
echo -e "${MAGENTA}:: ${BOLD}Webapp Manager — это инструмент для Linux, который позволяет превращать сайты в самостоятельные настольные приложения. Webapp Manager доступен для разных дистрибутивов Linux, например: Для Debian и Ubuntu — можно скачать официальный файл DEB с официального сайта и установить его с помощью терминала. Для Fedora — приложение доступно в неофициальном репозитории COPR. Для Arch Linux и его производных — приложение доступно в репозитории Arch User Repository (AUR), и вы можете легко установить его с помощью вашего любимого помощника AUR, такого как yay или paru. ${NC}"
echo " Домашняя страница: https://github.com/linuxmint/webapp-manager ; (https://aur.archlinux.org/packages/webapp-manager). "
echo -e "${MAGENTA}:: ${BOLD}Чтобы создать веб-приложение с помощью Webapp Manager, нужно открыть инструмент, ввести URL-адрес сайта, при желании настроить имя и значок, а затем создать веб-приложение. ${NC}"
echo " Часто задаваемые вопросы: Как вернуться назад без кнопок навигации? Щелкните правой кнопкой мыши пустую область веб-страницы, чтобы отобразить контекстное меню. В большинстве браузеров это меню содержит кнопки навигации. Как открывать ссылки в моем основном браузере? Для Firefox все ссылки всегда открываются в WebApp, либо напрямую, либо с помощью новой вкладки. Чтобы открыть ссылку в вашем основном браузере, щелкните правой кнопкой мыши в любом месте, выберите Copy link locationи вставьте ссылку в ваш основной браузер. Chromium и Chrome WebApps открывают внешние ссылки в основном браузере. Как использовать вкладки в Firefox? Нажмите Ctrl+ Tab для переключения между открытыми вкладками. Нажмите Ctrl+ T, чтобы создать новую вкладку. Нажмите Ctrl+ W, чтобы закрыть текущую вкладку. Нажмите Ctrl при нажатии на ссылку, чтобы открыть ее в новой вкладке. Как разрешить открывать новые окна в Firefox вместо новых вкладок? Нажмите и отпустите Alt клавишу, чтобы отобразить главную строку меню. Затем вы можете перейти к Settings пункту Edit меню. Во General вкладке настроек перейдите в Tabs раздел, снимите галочку Open links in tabs instead of new windows. В Privacy & Security разделе перейдите Permissions и создайте исключение для URL-адреса вашего веб-приложения, нажав Exceptions...кнопку рядом с ним, а затем добавив соответствующий URL-адрес в список в следующем всплывающем окне. При использовании этой настройки вместо новых вкладок будут открываться новые окна веб-приложения, например, если вы переходите по ссылкам.  "
echo -e "${CYAN}:: ${NC}Установка Webapp Manager (webapp-manager) или (webapp-manager-git) проходит через сборку из AUR (yay)(https://aur.archlinux.org/webapp-manager.git ; https://aur.archlinux.org/webapp-manager-git.git ). Если хотите можете собрать пакет через - git clone, PKGBUILD, makepkg - эта функция тоже прописана в сценарии скрипта, НО # закомментирована! "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_webapp  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_webapp" =~ [^10] ]]
do
    :
...
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_webapp == 1 ]]; then
  echo ""
  echo " Установка Webapp Manager (webapp-manager) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
############ Зависимости ############
########### dconf ##############
sudo pacman -S --noconfirm --needed dconf  # Конфигурационная система базы данных ; https://archlinux.org/packages/extra/x86_64/dconf/ ; https://wiki.gnome.org/Projects/dconf ; 2024-07-12 03:08 UTC
########### python-beautifulsoup4 ##############
sudo pacman -S --noconfirm --needed python-beautifulsoup4  # HTML/XML-парсер Python, предназначенный для быстрых проектов, таких как анализ экрана ; https://archlinux.org/packages/extra/any/python-beautifulsoup4/ ; https://www.crummy.com/software/BeautifulSoup/ ; 2025-04-12 02:13 UTC
########### python-configobj ##############
sudo pacman -S --noconfirm --needed python-configobj  # Простой считыватель и писатель конфигурационных файлов ; https://archlinux.org/packages/extra/any/python-configobj/ ; https://github.com/DiffSK/configobj ; 2025-01-08 20:08 UTC
########### python-gobject ##############
sudo pacman -S --noconfirm --needed python-gobject  # Привязки Python для GLib/GObject/GIO/GTK ; https://archlinux.org/packages/extra/x86_64/python-gobject/ ; Заменяет: pygobject-devel<=3.36.1-1 ; Конфликты: с pygobject-devel ; https://pygobject.gnome.org/ ; 2025-03-24 03:58 UTC
########### python-pillow ##############
sudo pacman -S --noconfirm --needed python-pillow  # Форк библиотеки обработки изображений Python (PIL) ; https://archlinux.org/packages/extra/x86_64/python-pillow/ ; https://pillow.readthedocs.io/ ; 2025-04-14 07:12 UTC
########### python-setproctitle ##############
sudo pacman -S --noconfirm --needed python-setproctitle  # Позволяет процессу Python изменять название своего процесса ; https://archlinux.org/packages/extra/x86_64/python-setproctitle/ ; https://github.com/dvarrazzo/py-setproctitle ; 2025-03-04 18:34 UTC
########### python-tldextract ##############
sudo pacman -S --noconfirm --needed python-tldextract  # Точно отделите TLD от зарегистрированного домена и поддоменов URL, используя список публичных суффиксов ; https://archlinux.org/packages/extra/any/python-tldextract/ ; https://github.com/john-kurkowski/tldextract ; 2025-04-29 02:41 UTC
########### xapp ##############
sudo pacman -S --noconfirm --needed xapp  # Общая библиотека для проекта X-Apps ; https://archlinux.org/packages/extra/x86_64/xapp/ ; https://github.com/linuxmint/xapp ; Обеспечивает: xapps ; Заменяет: xapps ; Конфликты: с xapps ; 2025-05-23 21:08 UTC
######### webapp-manager #############
yay -S webapp-manager --noconfirm  # Запускайте веб-сайты так, как будто это приложения ; https://aur.archlinux.org/packages/webapp-manager ; https://aur.archlinux.org/webapp-manager.git (только для чтения, нажмите, чтобы скопировать) ; https://github.com/linuxmint/webapp-manager ; https://github.com/linuxmint/webapp-manager/archive/refs/tags/1.4.0.tar.gz ; 2025-01-09 17:43 (UTC)
######### webapp-manager #############
#git clone https://aur.archlinux.org/webapp-manager.git  # (только для чтения, нажмите, чтобы скопировать)
#sudo chmod a+w webapp-manager  # Чтобы установить webapp-manager, вам необходимо изменить права доступа к файлам
#cd webapp-manager
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm   #--не спрашивать каких-либо подтверждений
...
#rm -Rf webapp-manager
# yay -Rns webapp-manager  # * (Необязательно) Удалите webapp-manager на Arch с помощью YAY
########### webapp-manager-git ###########
#yay -S webapp-manager-git  # Управляйте веб-сайтами так, как если бы они были приложениями ; https://aur.archlinux.org/packages/webapp-manager-git ; https://aur.archlinux.org/webapp-manager-git.git (только для чтения, нажмите, чтобы скопировать) ; Конфликты: с  ; git+https://github.com/linuxmint/webapp-manager.git ; 2024-03-10 23:23 (UTC)
########### webapp-manager-git ###########
#git clone https://aur.archlinux.org/webapp-manager-git.git  # (только для чтения, нажмите, чтобы скопировать)
#sudo chmod a+w webapp-manager  # Чтобы установить webapp-manager, вам необходимо изменить права доступа к файлам
#cd webapp-manager
# makepkg -fsri
# makepkg -si
...
########## Справка ##################
# Мессенджер MAX (https://web.max.ru/) в Arch Linux
# Мессенджер MAX в Arch Linux Hyprland - как заставить работать
# https://www.youtube.com/watch?v=E4432hhTNYQ
# https://www.youtube.com/@3y6HuK_/videos
# https://rutube.ru/channel/31019972/
### Команда установки менеджера (в обычных окружениях ярлыки будут доступны где-то в меню):
# yay -S webapp-manager
### Пример моей привязки мессенджера на клавишу
# bind = $mainMod, M, exec, [workspace 9 silent] vivaldi --app="https://web.max.ru/" --class=WebApp-MAX**** --name=WebApp-MAX**** --user-data-dir=$HOME/.local/share/ice/profiles/MAX****
### Для русской раскладки начало привязки такое:
# bind = $mainMod, Cyrillic_softsign, exec, [workspace 9 silent] vivaldi --app="https://web.max.ru/" --class=WebApp-MAX**** --name=WebApp-MAX**** --user-data-dir=$HOME/.local/share/ice/profiles/MAX****
### Правила окна (для моего удобства)
# windowrule = tile, class:WebApp-MAX****
# windowrulev2 = workspace 9 tile,class:^(WebApp-MAX****)$,title:^(MAX)$
### Вместо звёзд цифры смотрите сами, вызвать свойства окна:
# hyprctl clients
### Как установить веб-приложения на Linux
# https://www.linuxfordevices.com/tutorials/linux/install-webapps-on-linux
######################################

clear
echo -e "${CYAN}
  <<< Установка программ для чтения RSS лент (RSS агрегаторы и ридеры) для системы Arch Linux >>> ${NC}"
# Installing RSS feed readers (RSS aggregators and readers) for the Arch Linux system
#clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Liferea (liferea) - RSS-читалка?"
echo -e "${MAGENTA}:: ${BOLD}Liferea – это бесплатная программа для чтения RSS-лент для Linux с открытым исходным кодом. Liferea — Стандартный RSS-агрегатор для Gnome, приложение с нескладным именем (аббревиатура от LInux FEed REAder — читалка лент для Linux), но имеющее мощный и интуитивный набор функций. Имеется способность автоматически определять формат ленты, встраивается в область уведомления, может отображать ссылки в собственном окне (с помощью Mozilla), а также в любом внешнем веб-браузере установленном в системе. Liferea имеет простой, удобный, ясный и локализованный пользовательский интерфейс, позволяющий легко организовывать и просматривать ленты. ${NC}"
echo " Домашняя страница: https://lzone.de/liferea ; (https://archlinux.org/packages/extra/x86_64/liferea/). "
echo -e "${MAGENTA}:: ${BOLD}Имеет встроенный графический браузер, поддерживает чтение статей в автономном режиме и подкасты. Кроме того, в программе предусмотрены корзины новостей для постоянного сохранения заголовков и возможность поиска по папкам. Liferea можно синхронизировать с InoReader, Reedah, TheOldReader и TinyTinyRSS. ${NC}"
echo " Основные возможности: поддержка получения новостей в форматах RSS/RDF, CDF, Atom, OCS; автоматическое определение формата ленты; показ ссылок в собственном окне; настройка интервала обновления для каждой ленты новостей; вывод новых новостей во всплывающем окне. Каналы сортируются по произвольной структуре папок; сообщения можно отмечать, а потом просматривать в отдельной папке «Flagged»; возможно включить показ уведомлений («Heads-up display»); браузер выбирается пользователем (внутренний, либо любой внешний); проигрывает подкасты. Разработка: Исходный код: Open Source (открыт); Языки программирования: C; Java; Python; Библиотеки: GTK; Лицензия: GNU GPL; Приложение переведено на русский язык. На официальном сайте имеется англоязычная справка (https://lzone.de/liferea). "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_liferea  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_liferea" =~ [^10] ]]
do
    :
done
if [[ $in_liferea == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_liferea == 1 ]]; then
  echo ""
  echo " Установка Liferea (liferea) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
sudo pacman -S --noconfirm --needed liferea  # Настольный агрегатор новостей для онлайн-новостных лент и веб-блогов ; https://lzone.de/liferea ; https://archlinux.org/packages/extra/x86_64/liferea/ ; https://github.com/lwindolf/liferea ; 26 августа 2023 г., 7:00 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
########## Справка ##################
# Комбинации клавиш (FAQ из программы)
# Ctrl-U — Обновляет сразу все ваши подписки.
# Ctrl-R — Помечает все элементы выбранной ленты или папки как прочитанные.
# Ctrl-N — Перейти к следующему непрочитанному заголовку.
# Ctrl-M — Переключить непрочитанное состояние выбранного заголовка.
# Ctrl-T — Переключить метку выбранного заголовка.
# Ctrl-O — Воспроизвести следующее вложение заголовка (аудио или видео).
# Ctrl-+ — Увеличить размер шрифта.
# Ctrl-- — Уменьшить размер шрифта.
# Ctrl-0 — Сбросить размер шрифта по умолчанию.
# Space — Пролистайте заголовки.
# В зависимости от ваших предпочтений вам может понадобиться использовать модификатор Ctrl или Alt.
# U — Переместите курсор вверх в списке каналов.
# D — Переместите курсор вниз в списке каналов.
# B — Переместите курсор вверх в списке элементов.
# F — Переместите курсор вниз в списке элементов.
# Del — Удаляет текущий выбранный канал или элемент.
###################

clear
echo ""
echo -e "${BLUE}:: ${NC}Установить NewsFlash GTK (newsflash) - Клиент для чтения RSS?"
echo -e "${MAGENTA}:: ${BOLD}NewsFlash GTK — клиент для чтения RSS-фидов. Поддерживает импорт ленты из различных сервисов. Newsflash — это программа, разработанная для дополнения уже существующей учетной записи веб-ридера RSS. Она сочетает в себе все преимущества веб-сервисов, такие как синхронизация на всех ваших устройствах, со всем, что вы ожидаете от современной программы для настольных компьютеров: уведомления на рабочем столе, быстрый поиск и фильтрация, теги, удобные сочетания клавиш и доступ ко всем вашим статьям в течение всего времени, которое вам нужно. ${NC}"
echo " Домашняя страница: https://apps.gnome.org/NewsFlash/ ; (https://archlinux.org/packages/extra/x86_64/newsflash/). "
echo -e "${MAGENTA}:: ${BOLD}RSS-каналы позволяют вам контролировать то, что вы видите и читаете, а не безликий алгоритм, подающий то, что, по его мнению, вы хотите. Время слона в комнате: NewsFlash (как и другие приложения такого рода) предлагает встроенный скрапер для «извлечения» содержимого постов блога, чтобы вы могли читать их в приложении, без необходимости использования браузера. Это удобно, но есть одна загвоздка. ${NC}"
echo " Возможности: Поддержка сервисов Feedly, FeedBin, Miniflux и Fever. Проверка обновлений фидов в фоновом режиме. Добавление тегов и категорий для фидов. Различные способы сортировки материалов. Добавление статей в Избранное. Уведомления на рабочем столе. Быстрый поиск. Фильтрация. Адаптивный интерфейс. Поддержка горячих клавиш. Темная тема оформления. Разработка: Исходный код: Open Source (открыт); Языки программирования: Rust. "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_newsflash  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_newsflash" =~ [^10] ]]
do
    :
done
if [[ $in_newsflash == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_newsflash == 1 ]]; then
  echo ""
  echo " Установка NewsFlash GTK (newsflash) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
sudo pacman -S --noconfirm --needed newsflash  # Приложение для чтения лент, разработанное в дополнение к уже существующей учетной записи веб-ридера RSS ; https://apps.gnome.org/NewsFlash/ ; https://archlinux.org/packages/extra/x86_64/newsflash/ ; https://gitlab.com/news-flash/news_flash_gtk ; 19 августа 2024 г., 7:24 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Akregator (akregator) - Чтение новостей?"
echo -e "${MAGENTA}:: ${BOLD}Akregator — это программа-агрегатор, которая автоматически собирает новости с сайтов через ленты форматы RSS и Atom. Она помогает следить за новыми записями на сайтах с поддержкой RSS, например, на dot.kde.org и в блогах Planet KDE. Akregator входит в комплект программ Kontact. ${NC}"
echo " Домашняя страница: https://apps.kde.org/akregator/ ; (https://archlinux.org/packages/extra/x86_64/akregator/ ; https://userbase.kde.org/Akregator). "
echo -e "${MAGENTA}:: ${BOLD}Поскольку Akregator проверяет ленты RSS или Atom, вам сначала нужно найти ленту, которую вы собираетесь читать. Это очень просто: откройте ваши любимые сайты и посмотрите, если ли на них ссылки на ленты RSS или Atom. Большинство сайтов, на которых публикуются какого-либо рода новости, имеют такие ссылки. После того как вы получили адрес ленты, чтобы добавить её в Akregator, выберите «Добавить ленту...» из меню «Лента» или нажмите клавишу Insert. Теперь вы можете задать периодичность, с которой Akregator будет проверять наличие новостей в ленте. Периодичность можно настроить как сразу для всех лент, так и для каждой ленты отдельно. Akregator может хранить содержимое лент, на которые вы подписаны, столько времени, сколько вы захотите. Срок хранения можно также настроить и для всех лент сразу (меню Настройка->Настроить Akregator...->Архив), и отдельно для каждой ленты, нажав правой кнопкой мыши на ленте, выбрав «Свойства...» и перейдя на вкладку «Хранение». Для интеграции с Konqueror установите модуль konqueror-plugin-akregator, он позволяет добавлять ленты с сайтов в Akregator одним щелчком мыши. ${NC}"
echo " Возможности / особенности: Простой интерфейс. Простота в использовании. Программа поддерживает чтение Atom и RSS-фидов. Создание папок с фидами. Уведомления о непрочитанных фидах. Импорт и экспорт фидов в формате OPML. Может обрабатывать десятки лент новостей. Может открывать полные версии записей в отдельных вкладках прямо в программе. Архивирование лент, Импорт и экспорт лент. Поддерживает вкладки для просмотра страниц в режиме браузера (через Konqueror). Разрабатывается в рамках проекта: KDE; Исходный код: Open Source (открыт); Языки программирования: C++; Библиотеки: Qt; Приложение переведено на русский язык. "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Да установить,     0 - НЕТ - Пропустить установку: " in_akregator  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_akregator" =~ [^10] ]]
do
    :
done
if [[ $in_akregator == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_akregator == 1 ]]; then
  echo ""
  echo " Установка Akregator (akregator) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
sudo pacman -S --noconfirm --needed akregator  # Программа для чтения новостных лент от KDE ; https://apps.kde.org/akregator/ ; https://archlinux.org/packages/extra/x86_64/akregator/ ; https://userbase.kde.org/Akregator ; July 4, 2024, 4:48 p.m. UTC
# Если при установке пакета возникла ошибка, попробуйте выполнить следующую команду и повторите предыдущие действия:
# sudo pacman -Rs akregator.
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить QuiteRSS (quiterss) - Чтение RSS?"
echo -e "${MAGENTA}:: ${BOLD}QuiteRSS — RSS агрегатор. Программа предназначена для удобного чтения RSS-лент (фидов). QuiteRSS разрабатывается под руководством русскоязычных программистов. Проект основан Алексеем Хохряковым. Приложение переведено на множество языков, включая русский. 21 июня 2017 года было заявлено о начале разработки QuiteRSS версии 2. Программа будет переписана полностью. В предыдущей версии выходят обновления только для исправления критических ошибок. Но за несколько дней работы с программой, у меня никаких проблем не возникало. ${NC}"
echo " Домашняя страница: https://quiterss.org/ ; (https://aur.archlinux.org/packages/quiterss ; https://github.com/QuiteRSS/quiterss2 ; https://aur.archlinux.org/packages/quiterss2-git). "
echo -e "${MAGENTA}:: ${BOLD}Интерфейс QuiteRSS отдаленно напоминает почтовый клиент. Окно разделено на несколько областей — это дерево фидов, обновления в фидах (заголовки новостей, дата, категория и т.д.), непосредственно текст открытой новости и список категорий для выборки фидов. К недостаткам можно отнести перегруженность интерфейса, не хватает легкости. После установки можно добавить RSS-ленту в QuiteRSS. Для этого откройте программу, щёлкните мышкой на крестик и выберите пункт «Ленту». ${NC}"
echo " QuiteRSS обладает широкими возможностями для чтения, каталогизации и управления фидами: Каталогизация фидов по папкам. Поддерживаются вложенные папки. Поддержка вкладок. Во вкладках можно открывать отдельные ленты или просматривать страницы фидов во встроенном браузере. Есть встроенный блокировщик рекламы — AdBlock. Добавление записей в Избранное и назначение тегов. Фильтрация новостей. Просмотр непрочитанных, избранных, удаленных записей. Выборка записей по тегам. Имеется встроенный браузер. Поддерживается открытие страницы фида во внешнем браузере. Возможность делиться новостями. Всплывающие уведомления на рабочем столе о новых записях в фидах. Поддерживается экспорт и импорт лент в формате OPML. Настройка прокси и многое другое. "
echo -e "${CYAN}:: ${NC}Установка QuiteRSS (quiterss) и (quiterss2-git) проходит через сборку из AUR (yay). Если хотите можете собрать пакет через - git clone, PKGBUILD, makepkg - эта функция тоже прописана в сценарии скрипта, НО # закомментирована! "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Установить QuiteRSS (quiterss),     2 - Установить форк QuiteRSS2 (quiterss2-git),

    0 - НЕТ - Пропустить установку: " in_quiterss  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_quiterss" =~ [^120] ]]
do
    :
done
if [[ $in_quiterss == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_quiterss == 1 ]]; then
  echo ""
  echo " Установка QuiteRSS (quiterss) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
########## Зависимости ###########
yay -S qt5-webkit --noconfirm  # Классы для реализации на базе WebKit2 и нового API QML ; https://aur.archlinux.org/qt5-webkit.git (только для чтения, нажмите, чтобы скопировать) ; https://github.com/qtwebkit/qtwebkit ; https://aur.archlinux.org/packages/qt5-webkit?all_deps=1#pkgdeps ; стати, патчи приветствуются здесь https://github.com/FabioLolix/PKGBUILD-AUR_fix/tree/master/q ; https://aur.archlinux.org/cgit/aur.git/tree/icu68.patch?h=qt5-webkit ; 2024-05-28 16:39 (UTC)
...
#rm -Rf qt5-webkit
########### quiterss ##########
yay -S quiterss --noconfirm  # Быстрый и легкий RSS/Atom-ридер, написанный на Qt/С++ ; https://aur.archlinux.org/quiterss.git (только для чтения, нажмите, чтобы скопировать) ; https://quiterss.org/ ; https://aur.archlinux.org/packages/quiterss ;   2023-01-12 23:03 (UTC) ; https://github.com/QuiteRSS/quiterss/archive/0.19.4/quiterss-0.19.4.tar.gz ; https://aur.archlinux.org/cgit/aur.git/tree/install-appdata.patch?h=quiterss
#git clone https://aur.archlinux.org/quiterss.git   # (только для чтения, нажмите, чтобы скопировать)
#cd quiterss
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm   #--не спрашивать каких-либо подтверждений
...
echo ""
echo " Установка утилит (пакетов) выполнена "
elif [[ $in_quiterss == 2 ]]; then
  echo ""
  echo " Установка QuiteRSS2 (quiterss2-git) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
########## quiterss2-git ###########
# Работающий форк QuiteRSS под названием QuiteRSS2 ( Quiterss2-git в AUR), который был перенесен на более новый, все еще поддерживаемый qt5-webengine , и поэтому его по-прежнему можно собрать и запустить.
yay -S quiterss2-git --noconfirm  # Кроссплатформенная программа для чтения новостных лент, написанная на Qt/C++/QML ; https://aur.archlinux.org/quiterss2-git.git (только для чтения, нажмите, чтобы скопировать) ; https://github.com/QuiteRSS/quiterss2 ; https://aur.archlinux.org/packages/quiterss2-git ; Конфликты: с quiterss2 ; 2023-01-16 21:43 (UTC) ; git+https://github.com/QuiteRSS/quiterss2.git
#git clone https://aur.archlinux.org/quiterss2-git.git   # (только для чтения, нажмите, чтобы скопировать)
#cd quiterss2-git
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm   #--не спрашивать каких-либо подтверждений
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить RSSOwlnix (RSSowl) (rssowlnix) - RSS-ридер?"
echo -e "${MAGENTA}:: ${BOLD}RSSowl — это бесплатный, мощный, кроссплатформенный настольный RSS-ридер, работающий на Linux, Windows и macOS. Он помогает вам организовать ваши ленты так, как вам нужно, по разным категориям, а также мгновенно искать и читать ленты с удобством. RSSOwl – кроссплатформенное приложение для подписки и чтения RSS-новостей, написанное на Java. Программа работает во многих операционных системах и требует для работы установленный пакет Java Runtime Environment. ${NC}"
echo " Домашняя страница: http://www.rssowl.org/ ; (https://github.com/Xyrio/RSSOwlnix ; https://aur.archlinux.org/packages/rssowlnix ; https://aur.archlinux.org/packages/rssowlnix-bin). "
echo -e "${MAGENTA}:: ${BOLD}Он позволяет сохранять поиски и использовать их как каналы и поддерживает уведомления. Он также предлагает новостные корзины для хранения новостных записей, которые вы считаете очень важными. RSSowl также поддерживает метки для связывания ключевых слов с новостными записями и многое другое. После первого старта программы может сложиться впечатление, что это какой-то браузер, а не инструмент для работы с новостями RSS. Вы видите домашнюю страницу RSSOwl без каких-либо намеков на список каналов и просмотр списка заголовков текущей новостной ленты. Секрет прост. Необходимо вызывать панель Избранного. Стоит также отметить отсутствие панели инструментов по умолчанию. Она также отдельно включается в настройках. ${NC}"
echo " RSSOwl обладает продвинутым механизмом поиска новостных лент и осуществления на них подписки. С помощью инструмента Поиск Источников вы указываете ключевое слово, на основе которого необходимо найти подходящие RSS-каналы. Программа позволяет создавать языковые предпочтения. По умолчанию используется язык интерфейса. То есть, если вы выбрали во время установки русский язык, то установка галочки напротив предпочтения заставляет программу искать новостные ленты, в первую очередь, на русском языке. Программа содержит подробную справку на английском языке, а также список наиболее часто задаваемых вопросов, представленный в формате отдельной новостной ленты. "
echo -e "${CYAN}:: ${NC}Установка RSSOwlnix (rssowlnix) и (rssowlnix-bin) проходит через сборку из AUR (yay). Если хотите можете собрать пакет через - git clone, PKGBUILD, makepkg - эта функция тоже прописана в сценарии скрипта, НО # закомментирована! "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Установить RSSOwlnix (RSSowl) (rssowlnix),    2 - *Установить RSSOwlnix (RSSowl) (rssowlnix-bin),

    0 - НЕТ - Пропустить установку: " in_rssowlnix  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_rssowlnix" =~ [^120] ]]
do
    :
done
if [[ $in_rssowlnix == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_rssowlnix == 1 ]]; then
  echo ""
  echo " Установка RSSOwlnix (RSSowl) (rssowlnix) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
########## rssowlnix ########
yay -S rssowlnix --noconfirm  # Организуйте, ищите и читайте новостные ленты RSS, RDF и Atom удобным способом ; https://aur.archlinux.org/rssowlnix.git (только для чтения, нажмите, чтобы скопировать) ; https://github.com/Xyrio/RSSOwlnix ; https://aur.archlinux.org/packages/rssowlnix ; git+https://github.com/Xyrio/RSSOwlnix.git#tag=2.8.0 ; 2020-06-23 22:14 (UTC) ;
#git clone https://aur.archlinux.org/rssowlnix.git   # (только для чтения, нажмите, чтобы скопировать)
#cd rssowlnix
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm   #--не спрашивать каких-либо подтверждений
...
echo ""
echo " Установка утилит (пакетов) выполнена "
elif [[ $in_rssowlnix == 2 ]]; then
  echo ""
  echo " Установка RSSOwlnix (RSSowl) (rssowlnix-bin) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
########## rssowlnix-bin ########
yay -S rssowlnix-bin --noconfirm  # RSSOwlnix форк RSSOwl ; https://aur.archlinux.org/rssowlnix-bin.git (только для чтения, нажмите, чтобы скопировать) ; https://github.com/Xyrio/RSSOwlnix ; https://aur.archlinux.org/packages/rssowlnix-bin ; 2021-08-09 10:07 (UTC)
#git clone https://aur.archlinux.org/rssowlnix-bin.git   # (только для чтения, нажмите, чтобы скопировать)
#cd rssowlnix-bin
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm   #--не спрашивать каких-либо подтверждений
...
######## Справка ##########
# Чтобы установить RSSOwl в системе Linux, необходимо загрузить ZIP-файл, извлечь его содержимое, а затем запустить приложение.
# http://www.rssowl.org/
# https://github.com/rssowl/RSSOwl/releases/download/2.2.1/RSSOwl.2.2.1.Linux-x64.zip
# unzip RSSOwl.*.Linux-x64.zip
# cd rssowl/
# ./RSSOwl
##################
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Newsboat (newsboat) - Консольная программа для чтения RSS/Atom-фидов (новостных лент)?"
echo -e "${MAGENTA}:: ${BOLD}Newsboat — это RSS/Atom-ридер для текстовой консоли. RSS и Atom — это ряд широко используемые форматы XML для передачи, публикации и синдицирования статей, например новостей или статьи блога. Это активно поддерживаемый форк Newsbeuter. Терминальные RSS-ридеры обеспечивают скорость и эффективность (плюс меньше отвлекающих факторов в виде встроенных изображений, видео и т. д.); большую конфиденциальность (отсутствие облачных сервисов, регистрирующих то, что вы читаете); и гораздо меньше потребляют ресурсов (текстовые инструменты, как правило, более гибкие). ${NC}"
echo " Домашняя страница: https://newsboat.org/ ; (https://archlinux.org/packages/extra/x86_64/newsboat/ ; https://wiki.archlinux.org/title/Newsboat). "
echo -e "${MAGENTA}:: ${BOLD}Тем не менее, терминальные инструменты подходят не всем, и некоторым людям они кажутся более сложными в использовании, чем традиционные инструменты с графическим интерфейсом.
  Но если не попробуешь, никогда не узнаешь! Настроек немного больше чем в GUI-аналогах. Реально больше. Поддержка 24-цветного терминала. ${NC}"
echo " Использование: Исходный код: Open Source (открыт); Языки программирования: C++; Rust; Лицензия: MIT License. Newsboat не может запуститься без настроенных каналов. Каналы можно настроить в ~/.newsboat/urls. Newsboat можно запустить из командной строки с помощью: newsboat . Чтобы добавить каналы в Newsboat, вы можете просто добавить один URL-адрес канала на строку в файл ~/.newsboat/urls. Например: https://russian.rt.com/rss или http://rss.cnn.com/rss/cnn_topstories.rss "~Главные новости CNN" или  https://lenta.ru/rss - *почитайте https://wiki.archlinux.org/title/Newsboat "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_newsboat  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_newsboat" =~ [^10] ]]
do
    :
done
if [[ $in_newsboat == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_newsboat == 1 ]]; then
  echo ""
  echo " Установка Newsboat (newsboat) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
sudo pacman -S --noconfirm --needed newsboat  # RSS/Atom-ридер для текстовых терминалов ; https://newsboat.org/ ; https://archlinux.org/packages/extra/x86_64/newsboat/ ; 22 июня 2024 г., 12:30 UTC
  echo ""
  echo " Настройка новостных лент в Newsboat (newsboat) "
echo -e "${BLUE}:: ${NC}Пропишем конфиг (urls) для newsboat "
echo " Создать каталог (newsboat) в домашней директории ~/.config/ "
#mkdir ~/.config/newsboat   # Создать каталог newsboat в ~/.config/ ; mkdir опции директория
mkdir -p ~/.config/newsboat   # Создать каталог newsboat в ~/.config/ ; -p или --parents ; Создать все директории, которые указаны внутри пути. Если какая-либо директория существует, то предупреждение об этом не выводится.
#mkdir -v ~/.config/newsboat   # Создать каталог newsboat в ~/.config/ ; -v или --verbose ; Выводить сообщение о каждой создаваемой директории.
#mkdir ~/.newsboat
echo " Создать файл urls в домашней директории ~/.config/newsboat/ "
touch ~/.config/newsboat/urls   # Создать файл urls в .config/newsboat/
#touch ~/.newsboat/urls
echo " Пропишем конфигурации в ~/.config/newsboat/urls "
cat > ~/.config/newsboat/urls << EOF
# WARNING:
https://russian.rt.com/rss "RT на русском"
https://lenta.ru/rss "!Новости Lenta (скрыты)"
EOF
############
echo " Для начала сделаем его бэкап ~/.config/chrome-flags.conf "
cp ~/.config/newsboat/urls  ~/.config/newsboat/urls.back
echo " Просмотреть содержимое файла ~/.config/newsboat/urls "
#ls -l ~/.config/newsboat/urls   # ls — выводит список папок и файлов в текущей директории
cat ~/.config/newsboat/urls  # cat читает данные из файла или стандартного ввода и выводит их на экран
sleep 3
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
...
# https://archlinux.org/packages/extra/x86_64/newsboat/
# https://wiki.archlinux.org/title/Newsboat
# https://manpages.ubuntu.com/manpages/mantic/en/man1/newsboat.1.html
# ~/.config/newsboat/urls
# https://archlinux.org/feeds/news/
# https://lwn.net/headlines/newrss
# https://russian.rt.com/rss
# https://lenta.ru/rss
# http://rss.cnn.com/rss/cnn_topstories.rss "~Главные новости CNN"
# Цветовые схемы: Пакет поставляется с рядом схем, /usr/share/doc/newsboat/contrib/colorschemes/которые можно загрузить с помощью команды include, например:
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Gopeed (gopeed-bin) - Современный менеджер загрузок?"
echo -e "${MAGENTA}:: ${BOLD}Gopeed (полное название — Go Speed) — это высокоскоростной менеджер загрузок, разработанный на Golang и Flutter, который поддерживает загрузку по протоколам (HTTP, BitTorrent, Magnet) и поддерживает все платформы, включая: Windows; Macos; Linux; Android; iOS; Web; Docker; Command Tool... ${NC}"
echo " Домашняя страница: https://gopeed.com/ ; (https://github.com/GopeedLab/gopeed ; https://aur.archlinux.org/packages/gopeed-bin). "
echo -e "${MAGENTA}:: ${BOLD}Функции: (Базовые) - Поддержка HTTP и HTTPS; Поддержка BitTorrent и Magnet; Seed, DHT, PEX, uTP, Webtorrent, Upnp; Обновлять список трекеров автоматически каждый день; Интернационализация; Темный режим. ${NC}"
echo " Помимо базовых функций загрузки, есть много дополнительных функций, которые делают Gopeed более удобным для игры. Открытый HTTP API. Через RESTFul API вы можете легко взаимодействовать с Gopeed, например, вы можете управлять загрузкой Gopeed, приостанавливать ее, удалять и т. д. через API. Веб-версия поддерживает настройку через параметры командной строки или файлы конфигурации. "
echo " Децентрализованное расширение: Вы можете написать расширения, JavaScript которые улучшат возможности загрузки Gopeed, например, загрузку видео с веб-сайта или загрузку музыки с веб-сайта и т.д.. Конечно, у Gopeed также может быть много недостатков, но мы продолжим его совершенствовать. Более того, Gopeed имеет открытый исходный код, вы можете предлагать свои идеи в любое время и в любом месте или напрямую вносить код, чтобы сделать Gopeed лучше. "
echo " Примечание: При развертывании на публичном IP-адресе рекомендуется включить аутентификацию личности, в противном случае возникнут риски безопасности. Конфигурация загрузки по умолчанию: Если вам нужно задать конфигурацию загрузки по умолчанию при первом запуске сервера, вы можете добавить downloadConfigполе в config.json. Подробную информацию о настройке см. в DownloaderStoreConfig . "
echo -e "${CYAN}:: ${NC}Установка Gopeed (gopeed-bin) проходит через сборку из AUR (yay) (https://aur.archlinux.org/packages/gopeed-bin ; https://aur.archlinux.org/gopeed-bin.git). Если хотите можете собрать пакет через - git clone, PKGBUILD, makepkg - эта функция тоже прописана в сценарии скрипта, НО # закомментирована! "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " in_gopeed  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_gopeed" =~ [^10] ]]
do
    :
...
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $in_gopeed == 1 ]]; then
  echo ""
  echo " Установка Gopeed (gopeed-bin) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
########## Зависимости ###########
sudo pacman -S --noconfirm --needed gtk3  # Мультиплатформенный набор графических инструментов на основе GObject ; https://archlinux.org/packages/extra/x86_64/gtk3/ ; https://www.gtk.org/ ; Заменяет: gtk3-print-backends<=3.22.26-1 ; Конфликты: с gtk3-print-backends ; 2025-05-14 00:11 UTC
sudo pacman -S --noconfirm --needed ayatana-ido  # Объекты отображения индикатора Аятана ; https://archlinux.org/packages/extra/x86_64/ayatana-ido/ ; https://github.com/AyatanaIndicators/ayatana-ido ; 2024-08-23 18:05 UTC
sudo pacman -S --noconfirm --needed libayatana-appindicator  # Общая библиотека индикаторов приложений Ayatana ; https://archlinux.org/packages/extra/x86_64/libayatana-appindicator/ ; https://github.com/AyatanaIndicators/libayatana-appindicator ; 2025-03-12 09:19 UTC
sudo pacman -S --noconfirm --needed libayatana-indicator  # Общая библиотека индикаторов Ayatana ; https://archlinux.org/packages/extra/x86_64/libayatana-indicator/ ; https://github.com/AyatanaIndicators/libayatana-indicator ; 2023-10-13 16:27 UTC
sudo pacman -S --noconfirm --needed libdbusmenu-glib  # Библиотека для передачи меню по DBus ; https://archlinux.org/packages/extra/x86_64/libdbusmenu-glib/ ; https://launchpad.net/libdbusmenu ; 28.09.2023 07:27 UTC
######### gopeed-bin ##########
yay -S gopeed-bin --noconfirm  # Высокоскоростной загрузчик, поддерживающий все платформы (готовая версия) ; https://aur.archlinux.org/packages/gopeed-bin ; https://aur.archlinux.org/gopeed-bin.git (только для чтения, нажмите, чтобы скопировать) ; https://gopeed.com/ ; Конфликты: с gopeed ; https://github.com/GopeedLab/gopeed/releases/download/v1.7.1/Gopeed-v1.7.1-linux-amd64.deb ; https://aur.archlinux.org/cgit/aur.git/tree/gopeed.sh?h=gopeed-bin ; https://github.com/GopeedLab/gopeed ; 2025-07-09 01:14 (UTC)
######### gopeed-bin ##########
#git clone https://aur.archlinux.org/gopeed-bin.git  # (только для чтения, нажмите, чтобы скопировать)
#cd gopeed-bin
# makepkg -fsri
# makepkg -si
#makepkg -si --noconfirm    #--не спрашивать каких-либо подтверждений
