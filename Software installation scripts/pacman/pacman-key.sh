...
clear
echo ""
echo -e "${GREEN}==> ${NC}Включить службу-таймер (archlinux-keyring-wkd-sync.timer) для дальнейшего их автоматического обновления?"
...
### Снять маску, если не работает система #########
#  sudo systemctl unmask <unit nev>
#  sudo systemctl unmask archlinux-keyring-wkd-sync.service
#  sudo systemctl unmask archlinux-keyring-wkd-sync.timer
  echo ""
...
#sudo nano archlinux-keyring-wkd-sync.timer
# был но нету etc//etc/systemd/system/
