...
# wget git.io/thunar_setting.sh && sh thunar_setting.sh
# thunar setting
...
# paru -Syu
############ raw-thumbnailer ##########
#yay -S raw-thumbnailer --noconfirm  # Легкий и быстрый инструмент для создания необработанных изображений raw, который необходим для отображения миниатюр raw ; https://aur.archlinux.org/raw-thumbnailer.git (только для чтения, нажмите, чтобы скопировать) ; https://code.google.com/archive/p/raw-thumbnailer/ ; https://aur.archlinux.org/packages/raw-thumbnailer ; Конфликты: с raw-thumbnailer-entry ; 2025-05-15 18:19 (UTC)
############ raw-thumbnailer ##########
git clone https://aur.archlinux.org/raw-thumbnailer.git  #  (только для чтения, нажмите, чтобы скопировать)
cd raw-thumbnailer
#makepkg -fsri
# makepkg -si
...
# rm -rf raw-thumbnailer
rm -Rf raw-thumbnailer
#############
echo ""
echo " Установим мультимедийный фреймворк GStreamer из AUR "
############ gstreamer0.10 ##########
#yay -S gstreamer0.10 --noconfirm  # Мультимедийный фреймворк GStreamer (Если установлен yay - эта команда) ; https://aur.archlinux.org/packages/gstreamer0.10 ; https://aur.archlinux.org/gstreamer0.10.git (только для чтения, нажмите, чтобы скопировать) ; https://gstreamer.freedesktop.org/ ; 2025-03-03 21:12 (UTC)
############ gstreamer0.10 ##########
git clone https://aur.archlinux.org/gstreamer0.10.git  #  (только для чтения, нажмите, чтобы скопировать)
cd gstreamer0.10
#makepkg -fsri
# makepkg -si
makepkg -si --noconfirm   #--не спрашивать каких-либо подтверждений
