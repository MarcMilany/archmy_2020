...
VERSION="v2.5 UEFI Update"
BRANCH="master"
AUTHOR="ordanax_and_poruncov"
LICENSE="GNU General Public License 3.0"
###
ARCHMY1U_LANG="russian"  # Installer default language (Язык установки по умолчанию)
### Start of the script
...
echo -e "${RED}=> ${YELLOW}Обратите Внимание! ${BOLD}Что создавать отдельный загрузочный раздел для разметки диска в обычном BIOS НЕ обязательно, достаточно иметь Root раздел (корневой раздел) и все, (ещё МОЖНО создать Swap file или Swap partiton - свап файл и свап раздел), этого вполне достачно для установки системы под обычным BIOS в таблице разделов MBR/DOS.
  В скрипте установки для файловой системы прописан следующий сценарий: Выбираем нужный диск, теперь запускаем программу для разметки диска (указывая свой Диск). Программа очень простая, Delete - удаляет раздел, New - создает, Write - записывает изменения (прописать yes), Quit - выходит из программы. Создаем два раздела и запоминаем их имя (метку - sda/sdb 1.2.3.4 и т.д.). Для UEFI или EFI в ( mkfs.vfat -F32 ) или ( mkfs.fat -F32 ) .
    ROOT / - Это ВАШ корневой раздел, он будет отформатирован в ( Ext4 ) ;
    BOOT (boot) - Это ВАШ загрузочный раздел (ЕСЛИ таковой был создан при разметке!) будет отформатирован :
     - Для UEFI или EFI в ( mkfs.vfat -F32 ) или ( mkfs.fat -F32 ) ;
    SWAP (linux-swap) - Это ВАШ раздел подкачки (ЕСЛИ таковой был создан при разметке!) ;
    HOME (home) -  Это ВАШ домашний раздел (ЕСЛИ таковой был создан при разметке!), он будет отформатирован в ( Ext4 ) .
...
if [[ $boots == 1 ]]; then
  echo " Чтобы подтвердить действия ввода, нажмите кнопку 'Ввод' ("Enter") "
  read -p " Укажите BOOT раздел (ТОЛЬКО ДЛЯ UEFI )(sda/sdb 1.2.3.4 (sda7 например)): " bootd  # To confirm the input actions, click 'Enter' ; # Чтобы подтвердить действия ввода, нажмите кнопку 'Ввод' ("Enter")
# mkfs.vfat -F32 -O \^64bit /dev/$bootd -L boot  # -O - активировать или деактивировать те или иные возможности файловой системы. Сами возможности мы рассмотрим ниже; 64bit - файловая система сможет занимать место больше чем 2 в 32 степени блоков. При размере блока 4 килобайта, это примерно один терабайт;
  mkfs.vfat -F32  /dev/$bootd -L boot   # или - mkfs.fat -F32  /dev/$bootd -L archboot
  mkdir /mnt/boot
...
if [[ $swap == 1 ]]; then
  ...
  mkswap /dev/$swaps -L swap   # или - mkswap /dev/$swaps -L archswap
  swapon /dev/$swaps
elif [[ $swap == 0 ]]; then
...
echo -e "${MAGENTA}=> ${BOLD}Есть три варианта продолжения установки: ${NC}"
  ...
  (wget git.io/archmy1u ), то выбирайте вариант - пункт "1", (команда работает через wget ....) "
  echo " 2 - Альтернативный вариант для (dhcpcd, wifi), если у Вас бывают проблемы трафика интернета (dhcpcd, wifi), и вы скачали первую часть скрипта через (curl -LO git.io/archmy1u ), то выбирайте вариант - пункт "2", (команда работает через curl ....) "
...
echo -e "${YELLOW}:: ${BOLD}Есть ещё 3(й) способ: команда выполнения как, и во 2-ом варианте через (curl), 
  ...
  (wget git.io/archmy1u или curl -LO git.io/archmy1u ), то выбирайте вариант - пункт "3", (команда работает через curl ...) "
...
if [[ $int == 1 ]]; then
  ...
  wget -P /mnt https://raw.githubusercontent.com/MarcMilany/archmy_2020/master/archmy2u.sh
  chmod +x /mnt/archmy2u.sh
  clear
  echo ""
  ...
  echo " 2 - Вводим команду для продолжения установки "./archmy2u.sh" "
  echo ""
  arch-chroot /mnt
  echo " ############################################### "
  ...
  curl -LO https://raw.githubusercontent.com/MarcMilany/archmy_2020/master/archmy2u.sh
  mv archmy2u.sh /mnt
  chmod +x /mnt/archmy2u.sh
  clear
  echo ""
...
#  arch-chroot /mnt "sh -c \"$(curl -fsSL git.io/archmy2l)\""  # Проверить команду ...
  arch-chroot /mnt sh -c "$(curl -fsSL git.io/archmy2u)"  # sh вызывает программу sh как интерпретатор и флаг -c означает выполнение следующей команды, интерпретируемой этой программой (выполнить команду специально с этой оболочкой вместо bash)
