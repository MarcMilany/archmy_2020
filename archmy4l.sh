...
echo -e "${CYAN}:: ${NC}Установка Ddrescue (ddrescue) проходит из 'Официальных репозиториев Arch Linux' - (Не AUR). Кроме Ddrescue GUI пакета (ddrescue-gui), его установка производиться с помощью git clone, PKGBUILD, makepkg - скачивается с сайта 'Arch Linux' () - собирается и устанавливается. "
echo -e "${CYAN}==> Важно! ${NC}В сценарии (скрипте) представлены несколько вариантов установки: " 
echo " Установка Spotify без дополнений и второй вариант Spotify + дополнения (пакеты) (zenity, ffmpeg-compat-57, xdg-desktop-portal-gtk). "
echo " Будьте внимательны! Процесс установки, после выбранного вами варианта был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. "
# Be careful! The installation process, after the option you selected, was registered fully automatic. In this option, the choice is always yours.
...
Если вы используете функцию ddrescue mapfile, данные спасаются очень эффективно (читаются только необходимые блоки). Также вы можете в любой момент прервать спасательную операцию и возобновить ее позже в тот же момент. Файл карты - важная часть эффективности ddrescue. Используйте его, если не знаете, что делаете.



Автоматическое объединение резервных копий: если у вас есть две или более поврежденных копии файла, компакт-диска и т. Д., И вы запускаете ddrescue на всех из них, по очереди, с одним и тем же выходным файлом, вы, вероятно, получите полную и безошибочную файл. Это потому, что вероятность того, что одна и та же область будет повреждена во всех копиях, мала (если ошибки расположены случайным образом). При использовании файла карты из второй и последующих копий считываются только необходимые блоки.

Ddrescue рекомендует lzip для сжатия резервных копий, потому что формат lzip разработан для долгосрочного архивирования и предоставляет возможности восстановления данных, которые хорошо дополняют возможности ddrescue. (Ddrescue заполняет нечитаемые сектора данными из других копий, а lziprecover исправляет поврежденные сектора данными из других копий). Если причиной повреждения файла является поврежденный носитель, комбинация ddrescue + lziprecover - лучший вариант для восстановления данных из нескольких поврежденных копий. См. Lziprecover-example .

Поскольку ddrescue необходимо читать и писать в случайных местах, он работает только с входными и выходными файлами с возможностью поиска (произвольный доступ).

Если ваша система поддерживает это, ddrescue может использовать прямой доступ к диску для чтения входного файла, минуя кеш ядра.

Ddrescue также имеет «режим заполнения», позволяющий выборочно перезаписывать части выходного файла, который имеет ряд интересных применений, таких как стирание данных, маркировка поврежденных областей или даже, в некоторых случаях, «восстановление» поврежденных секторов.
