...
# yay -S vi-vim-symlink --noconfirm  # Заменить vi на vim ; https://aur.archlinux.org/packages/vi-vim-symlink ; https://aur.archlinux.org/vi-vim-symlink.git (только для чтения, нажмите, чтобы скопировать) ; http://www.vim.org/ ; Конфликты: с vi ; Обеспечивает: vi ;
