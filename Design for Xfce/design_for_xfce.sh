...
yay -S arc-gtk-theme --noconfirm   #  Плоская тема с прозрачными элементами для GTK 3, GTK 2 и Gnome-Shell ; https://github.com/jnsh/arc-theme ; https://aur.archlinux.org/packages/arc-gtk-theme
#### arc-gtk-theme ####
#git clone https://aur.archlinux.org/arc-gtk-theme.git  # (только для чтения, нажмите, чтобы скопировать)
#cd arc-gtk-theme
### makepkg -fsri
### makepkg -si
...
#rm -Rf arc-gtk-theme
echo ""
echo " Установка Arc Theme (arc-solid-gtk-theme) "
yay -S arc-solid-gtk-theme --noconfirm   # Плоская тема для GTK 3, GTK 2 и Gnome-Shell (без прозрачности) https://github.com/jnsh/arc-theme ; https://aur.archlinux.org/packages/arc-solid-gtk-theme
#### arc-solid-gtk-theme ####
#git clone https://aur.archlinux.org/arc-gtk-theme.git  # (только для чтения, нажмите, чтобы скопировать)
#cd arc-solid-gtk-theme
### makepkg -fsri
### makepkg -si
### makepkg -g  # посчитает контрольные суммы пакетов, далее нужно просто эти контрольные суммы заменить в PKGBUILD`е
### sudo mousepad PKGBUILD  # открыть PKGBUILD в редакторе текста
...
yay -S arc-icon-theme --noconfirm   #   Тема значка дуги. Только официальные релизы ; https://github.com/horst3180/arc-icon-theme ; https://aur.archlinux.org/packages/arc-icon-theme
#### arc-icon-theme ####
#git clone https://aur.archlinux.org/arc-icon-theme.git  # (только для чтения, нажмите, чтобы скопировать)
#cd arc-icon-theme
### makepkg -fsri
### makepkg -si
### makepkg -g  # посчитает контрольные суммы пакетов, далее нужно просто эти контрольные суммы заменить в PKGBUILD`е
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Materia GTK Theme (materia-gtk-theme)?"
echo -e "${YELLOW}==> Примечание: ${NC}Поддерживает GTK 2, GTK 3, GTK 4, GNOME Shell, Budgie, Cinnamon, MATE, Unity, Xfce, LightDM, GDM, тему Chrome и т. д. Для правильного отображения темы используйте шрифты со средней насыщенностью (например , Roboto или M+ ). "
echo -e "${MAGENTA}:: ${BOLD}Materia — тема Material Design для сред рабочего стола на базе GNOME/GTK (https://github.com/nana-4/materia-theme) ${NC}"
echo " Поддерживает анимацию ряби для GTK 3 и 4. Доступны 3 варианта цвета и 2 варианта размера. "
echo -e "${RED}==> Требования: ${NC}Поддерживаемые наборы инструментов и рабочие столы: GTK 2 ; GTK 3 >=3.20 ; GTK 4 >=4.0 ; Budgie >=10.4 ; Cinnamon >=3.x ; GNOME Shell >=3.26 ; MATE >=1.14 ; Unity >=7.4 ; Xfce >=4.13 . "
echo " Materia можно настраивать с помощью графического приложения, конструктора тем oomox (https://github.com/themix-project/themix-gui). "
echo -e "${CYAN}=> Справка (пояснение): ${NC}Расширения Chrome: Чтобы улучшить внешний вид Chrome, вы можете установить наши расширения Chrome следующим образом: Откройте /usr/share/themes/Materia<-variant>/chromeпапку в файловом менеджере. Перетащите .crxфайлы на страницу расширений Chrome ( chrome://extensions). "
echo " Будьте внимательны! В данной опции выбор всегда остаётся за вами. "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
...
echo " Установка утилит (пакетов) пропущена "
  ...
  echo ""
  echo " Установка Materia Theme (materia-gtk-theme) "
sudo pacman -S --noconfirm --needed materia-gtk-theme  # Тема Material Design для сред рабочего стола на базе GNOME/GTK+ ; https://archlinux.org/packages/extra/any/materia-gtk-theme/ ; https://github.com/nana-4/materia-theme ; Заменяет: materia-theme ; Обеспечивает: materia-theme ; Конфликты: с materia-theme ; 2024-05-11 20:04 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Deepin GTK Theme (materia-gtk-theme)?"
echo -e "${YELLOW}==> Примечание: ${NC}Поддерживает GTK 2, GTK 3, GTK 4, GNOME Shell, и т.д.. Для правильного отображения темы используйте шрифты со средней насыщенностью (например , Roboto или M+ ). "
echo -e "${MAGENTA}:: ${BOLD}Deepin GTK Theme — Тема Deepin GTK 3 для сред рабочего стола на базе GNOME/GTK. ${NC} "
echo " Поддерживает анимацию ряби для GTK 3 и 4. Доступны 3 варианта цвета и 2 варианта размера. "
echo -e "${RED}==> Требования: ${NC}Поддерживаемые наборы инструментов и рабочие столы: GTK 3 >=3.20 ; GTK 4 >=4.0 ; GNOME Shell >=3.26. "
echo " Тема не работает в XFCE, границы окон взяты из темы Arc. Тема Deepin не распознаётся Cinnamon, поскольку отсутствует папка «gtk-3.20». https://github.com/linuxdeepin/deepin-gtk-theme/issues/20 ; https://askubuntu.com/questions/1365137 . "
echo " Будьте внимательны! В данной опции выбор всегда остаётся за вами. "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - Да установить,     0 - НЕТ - Пропустить установку: " i_deepin  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$i_deepin" =~ [^10] ]]
do
    :
...
echo " Установка утилит (пакетов) пропущена "
  ...
  echo ""
  echo " Установка Deepin GTK Theme (deepin-gtk-theme deepin-icon-theme) "
sudo pacman -S --noconfirm --needed deepin-gtk-theme  # Тема Deepin GTK ; https://archlinux.org/packages/extra/any/deepin-gtk-theme/ ; https://github.com/linuxdeepin/deepin-gtk-theme ; 2024-07-12 04:09 UTC
sudo pacman -S --noconfirm --needed gtk-update-icon-cache  # Обновление кэша иконок GTK ; https://archlinux.org/packages/extra/x86_64/gtk-update-icon-cache/ ; https://www.gtk.org/ ; 2025-06-10 05:22 UTC
sudo pacman -S --noconfirm --needed papirus-icon-theme  # Тема иконок Papirus ; https://archlinux.org/packages/extra/any/papirus-icon-theme/ ; https://github.com/PapirusDevelopmentTeam/papirus-icon-theme ; 2025-05-05 04:30 UTC
sudo pacman -S --noconfirm --needed deepin-icon-theme  # Deepin Иконки ; https://archlinux.org/packages/extra/any/deepin-icon-theme/ ; https://github.com/linuxdeepin/deepin-icon-theme ; Заменяет: deepin-cursor-theme ; Конфликты: с deepin-cursor-theme ; 2025-04-05 19:20 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
...
clear
echo ""
echo -e "${BLUE}:: ${NC}Установить Materia Theme (materia-theme)?"
echo -e "${YELLOW}==> Примечание: ${NC}Поддерживает GTK 2, GTK 3, GTK 4, GNOME Shell, Budgie, Cinnamon, MATE, Unity, Xfce, LightDM, GDM, тему Chrome и т. д. Для правильного отображения темы используйте шрифты со средней насыщенностью (например , Roboto или M+ ). "
echo -e "${MAGENTA}:: ${BOLD}Materia — тема Material Design для сред рабочего стола на базе GNOME/GTK (https://github.com/nana-4/materia-theme) ${NC}"
echo " Поддерживает анимацию ряби для GTK 3 и 4. Доступны 3 варианта цвета и 2 варианта размера. "
echo -e "${RED}==> Требования: ${NC}Поддерживаемые наборы инструментов и рабочие столы: GTK 2 ; GTK 3 >=3.20 ; GTK 4 >=4.0 ; Budgie >=10.4 ; Cinnamon >=3.x ; GNOME Shell >=3.26 ; MATE >=1.14 ; Unity >=7.4 ; Xfce >=4.13 . "
...
echo " Установка утилит (пакетов) пропущена "
  ...
  echo ""
  echo " Установка Materia Theme (materia-theme-git) "
yay -S materia-theme-git --noconfirm  # Пакет git upstream для плоской темы в стиле Material Design для GTK4, GTK3, GTK2, Metacity и GNOME-Shell ; https://aur.archlinux.org/packages/materia-theme-git ; https://aur.archlinux.org/materia-theme-git.git (только для чтения, нажмите, чтобы скопировать) ; https://github.com/nana-4/materia-theme ; Конфликты: с materia-theme ; Обеспечивает: materia-theme ; 09.08.2021 11:34 (UTC)
#### materia-theme-git ####
#git clone https://aur.archlinux.org/materia-theme-git.git   # (только для чтения, нажмите, чтобы скопировать)
#cd materia-theme-git
### makepkg -fsri
### makepkg -si
### makepkg -g  # посчитает контрольные суммы пакетов, далее нужно просто эти контрольные суммы заменить в PKGBUILD`е
### sudo mousepad PKGBUILD  # открыть PKGBUILD в редакторе текста
...
#pwd    # покажет в какой директории мы находимся
#cd ..   # поднимаемся на уровень выше (выходим из папки сборки)
### rm -rf materia-theme-git
#rm -Rf materia-theme-git
########### themix-gui-git ##############
# yay -S themix-gui-git --noconfirm  # Графический интерфейс конструктора тем на основе плагинов для сред (таких как GTK2, GTK3, Cinnamon, GNOME, MATE, Openbox, Xfwm), иконок и палитр терминала ; https://aur.archlinux.org/packages/themix-gui-git ; https://aur.archlinux.org/themix-gui-git.git (только для чтения, нажмите, чтобы скопировать) ; https://github.com/themix-project/oomox ; Обеспечивает: themix-gui ; Конфликты: с oomox, oomox-git, themix-gui ; 28.11.2022 21:11 (UTC)
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
