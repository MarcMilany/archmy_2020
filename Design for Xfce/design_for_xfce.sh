...
echo " Пользоваться ими достаточно удобно как со светлыми, так и с тёмными темами оформления. Своим внешним видом они чем-то похожи на стандартные указатели Windows :). (https://github.com/moka-project/moka-icon-theme) " 
echo "" 
while  
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "      
