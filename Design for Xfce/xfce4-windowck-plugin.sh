...
echo " Домашняя страница: https://docs.xfce.org/panel-plugins/xfce4-windowck-plugin/start ; (https://archlinux.org/packages/extra/x86_64/xfce4-windowck-plugin/). "
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Установить Xfce4-Windowck-plugin (xfce4-windowck-plugin), 2 - Установить Xfce4-Windowck-plugin (xfce4-windowck-plugin-git),

    0 - НЕТ - Пропустить действие: " in_windowck  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$in_windowck" =~ [^120] ]]
do
    :
done
if [[ $in_windowck == 0 ]]; then
echo ""
echo " Установка Docklike Plugin пропущена "
elif [[ $in_windowck == 1 ]]; then
  echo ""
  echo " Установка Xfce4-Windowck-plugin (xfce4-windowck-plugin) "
...
sudo pacman -S --noconfirm --needed xfce4-windowck-plugin  # Плагин панели Xfce для отображения заголовка окна и кнопок ; https://archlinux.org/packages/extra/x86_64/xfce4-windowck-plugin/ ; https://docs.xfce.org/panel-plugins/xfce4-windowck-plugin/start ; 2025-05-22 05:32 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
elif [[ $in_windowck == 2 ]]; then
  #echo ""
