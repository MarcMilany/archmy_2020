...
#yay -Syu
############ raw-thumbnailer ##########
#yay -S raw-thumbnailer --noconfirm  # Легкий и быстрый инструмент для создания необработанных изображений raw, который необходим для отображения миниатюр raw ; https://aur.archlinux.org/raw-thumbnailer.git (только для чтения, нажмите, чтобы скопировать) ; https://code.google.com/archive/p/raw-thumbnailer/ ; https://aur.archlinux.org/packages/raw-thumbnailer
git clone https://aur.archlinux.org/raw-thumbnailer.git
cd raw-thumbnailer
#makepkg -fsri
# makepkg -si
