...
sudo pacman -S zip unzip unrar p7zip zlib zziplib --noconfirm  # Компрессор / архиватор для создания и изменения zip-файлов;  
echo "" 
...
sudo pacman -S snappy --noconfirm  # Библиотека быстрого сжатия и распаковки (на порядок быстрее других)
echo "" 
