...
echo -e "${MAGENTA}=> ${NC}Список программ (пакетов) для установки: - (accountsservice, acpi, acpid, android-tools, android-udev, arch-install-scripts, aspell-en, aspell-ru, autofs, b43-fwcutter, bash-completion, beep, btrfs-progs, busybox, ccache, cmake, cpio, cpupower, crda, davfs2, desktop-file-utils, dhclient, dnsmasq, dosfstools, efibootmgr, efitools, ethtool, f2fs-tools, flex, fortune-mod, fsarchiver, fuse3, glances, gnome-nettool, gnu-netcat, gpm, gptfdisk, gtop, gvfs, gvfs-gphoto2, gvfs-nfs, gvfs-smb, haveged, hddtemp, hidapi, hwinfo, hydra, hyphen-en, id3lib, iftop, inetutils, isomd5sum, jfsutils, lib32-curl, lib32-flex, libfm-gtk2, lksctp-tools, logrotate, lsof, lsb-release, man-db, man-pages, mc, memtest86+, mlocate, mtpfs, ncdu, nfs-utils, nss-mdns, pacman-contrib, patchutils, pciutils, php, poppler-data, powertop, pv, pwgen, python-isomd5sum, python-pip, qt5-translations, reiserfsprogs, ruby, s-nail, sane, scrot, sg3_utils, sdparm, solid, sox, smartmontools, speedtest-cli, syslinux, systemd-ui, termite, termite-terminfo, translate-shell, udiskie, usb_modeswitch, xsel, xterm, xorg-twm, xorg-xkill, yelp, youtube-dl)."
...
echo " Установка дополнительных базовых программ (пакетов) пропущена "
  ...
  echo ""  
  echo " Установка дополнительных базовых программ (пакетов) "
# sudo pacman -S accountsservice acpi acpid android-tools android-udev arch-install-scripts aspell-en aspell-ru autofs b43-fwcutter bash-completion beep btrfs-progs busybox ccache cmake cpio cpupower crda davfs2 desktop-file-utils dhclient dnsmasq dosfstools efibootmgr efitools ethtool f2fs-tools flex fortune-mod fsarchiver fuse3 glances gnome-nettool gnu-netcat gpm gptfdisk gtop gvfs gvfs-gphoto2 gvfs-nfs gvfs-smb haveged hddtemp hidapi hwinfo hydra hyphen-en id3lib iftop inetutils isomd5sum jfsutils lib32-curl lib32-flex libfm-gtk2 lksctp-tools logrotate lsof man-db man-pages mc memtest86+ mlocate mtpfs ncdu nfs-utils nss-mdns patchutils pciutils php poppler-data powertop pv pwgen python-isomd5sum python-pip qt5-translations reiserfsprogs ruby s-nail sane scrot sg3_utils sdparm solid sox smartmontools speedtest-cli syslinux systemd-ui termite termite-terminfo translate-shell udiskie usb_modeswitch xsel xterm xorg-twm xorg-xkill yelp youtube-dl --noconfirm  # light-locker lsb-release python python2 - присутствует  
