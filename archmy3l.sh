...
echo " 1 - Установка NTP Servers (серверы точного времени) - пакет (ntp - Эталонная реализация сетевого протокола времени). "
echo " На сегодняшний день существует множество технологий синхронизации часов, из которых наиболее широкую популярность получила NTP, позволяющая получать точное время посредством локальной сети или сети общего доступа, такой как Интернет. Список общедоступных NTP серверов доступен на сайте http://ntp.org. " # NTP Servers (серверы точного времени) - https://www.ntp-servers.net/
...
#echo 'Установить рекомендованные программы?'
# Install the recommended programs
echo -e "${MAGENTA}=> ${NC}Список утилит (пакетов) для установки: - (gnome-system-monitor, gnome-disk-utility, frei0r-plugins, clonezilla, psensor, copyq, grsync, numlockx, modem-manager-gui, rofi, gsmartcontrol, testdisk, lsof, dmidecode)."
echo " Будьте внимательны! Процесс установки, был прописан полностью автоматическим. " 
# Be careful! The installation process was fully automatic
echo " Если Вы сомневаетесь в своих действиях, ещё раз обдумайте... "
# If you doubt your actions, think again... 
...
sudo pacman -S gnome-system-monitor gnome-disk-utility frei0r-plugins clonezilla psensor copyq grsync numlockx modem-manager-gui rofi gsmartcontrol testdisk lsof dmidecode --noconfirm  
echo ""   
echo " Установка утилит (пакетов) выполнена "
fi
