...
echo -e "${CYAN}:: ${NC}3 - В третьем варианте представлена установка НОВОГО Pacman gui (pamac-all), и (pamac-all-git), через сборку из исходников. То есть установка (pamac-all), и (pamac-all-git) проходит с помощью git clone, PKGBUILD, makepkg - скачивается с сайта 'Arch Linux' (https://aur.archlinux.org/packages/pamac-aur/),(https://aur.archlinux.org/packages/octopi/), собирается и устанавливается. "
