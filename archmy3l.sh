...
Шаг 3. Запустите OpenSnitch

После установки OpenSnitch вы можете запустить его, введя в терминале следующую команду:

opensnitch-ui
Альтернативно вы также можете запустить OpenSnitch из меню «Приложения» в вашей системе.


Запустить службу OpenSnitch
В Debian и Ubuntu демон OpenSnitch запустится автоматически после установки. На всякий случай, если не запустится, запустите службу OpenSnitch вручную.

Вот как вы можете запустить службу OpenSnitch вручную, если она не запускается автоматически после установки или перезагрузки:

Шаг 1. Включите службу OpenSnitch

Сначала включите службу OpenSnitch, выполнив в терминале следующую команду:

sudo systemctl enable --now opensnitch
Эта команда гарантирует, что служба OpenSnitch будет автоматически запускаться при каждой загрузке вашей системы.

Шаг 2. Запустите службу OpenSnitch

Затем вручную запустите службу OpenSnitch с помощью следующей команды:

sudo systemctl start opensnitch
Эта команда немедленно запускает службу OpenSnitch.

Если приведенные выше команды не работают с opensnitch, попробуйте заменить opensnitch на opensnitchd в обеих командах.

После этих шагов служба OpenSnitch должна быть запущена в вашей системе.

Вот и все! Вы успешно установили OpenSnitch в свою систему Linux и запустили его службу. Теперь вы можете начать настраивать его для мониторинга и управления исходящим сетевым трафиком.
...
sudo pacman -S --noconfirm --needed alsa-plugins alsa-firmware alsa-lib  # Дополнительные плагины ALSA; Бинарные файлы прошивки для программ загрузки в alsa-tools и загрузчик прошивок hotplug; Альтернативная реализация поддержки звука Linux
...
sudo pacman -S --noconfirm --needed xorg-xrandr  # Примитивный интерфейс командной строки для расширения RandR ; https://archlinux.org/packages/extra/x86_64/xorg-xrandr/ ; https://xorg.freedesktop.org/ ; 2024-11-08 05:43 UTC
echo " Установка дополнительных драйверов для виртуальной машины "
sudo pacman -S --noconfirm --needed mesa-amber  # Классические драйверы OpenGL (не Gallium3D) ; https://archlinux.org/packages/extra/x86_64/mesa-amber/ ; https://www.mesa3d.org/ ; Обеспечивает: mesa=21.3.9, opengl-driver ; 2024-01-01 01:06 UTC ; Если лагает на старых GPU
...
cho " Для систем, использующих гибридную графику (например, ноутбуки с Intel и NVIDIA), рассмотрите возможность установки nvidia-prime или optimus-manager "
...
#######################
echo " Настройка поддержки Vulkan "
sudo pacman -S --noconfirm --needed vulkan-tools # Инструменты и утилиты Vulkan ; https://archlinux.org/packages/extra/x86_64/vulkan-tools/ ; https://www.vulkan.org/ ; 2025-05-10 14:47 UTC
...
#sudo pacman -S --noconfirm --needed lib32-libva-mesa-driver
