...
echo -e "${MAGENTA}=> ${NC}Список утилит (пакетов) для установки: - (broadcom-wl-dkms, ebtables, ipset, iproute2, iwd, linux-atm, ndisc6, nmap, networkmanager-l2tp, pptpclient, rp-pppoe, wvdial, xl2tpd, traceroute, wavemon, dsniff,wireshark-cli, wireshark-qt, termshark, kdeconnect)."
echo -e "${CYAN}:: ${NC}Вы МОЖЕТЕ в скрипте закомментировать НЕнужные вам пакеты!"
echo " Будьте внимательны! Процесс установки, был прописан полностью автоматическим. "
# Be careful! The installation process was fully automatic
echo " Если Вы сомневаетесь в своих действиях, ещё раз обдумайте... "
...
# sudo pacman -S broadcom-wl-dkms ebtables ipset iwd linux-atm ndisc6 networkmanager-l2tp pptpclient rp-pppoe wvdial xl2tpd
  ...
  # sudo pacman -S --noconfirm --needed   #
  echo ""
  echo " Установка Wireshark (Анализатор/сниффер сетевого трафика и протоколов)"
  sudo pacman -S --noconfirm --needed wireshark-cli  # Анализатор/сниффер сетевого трафика и протоколов — инструменты CLI и файлы данных ; https://www.wireshark.org/ ; https://archlinux.org/packages/extra/x86_64/wireshark-cli/
