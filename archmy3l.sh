...
echo -e "${MAGENTA}:: ${BOLD}Docklike Plugin - это современная минималистичная панель задач в стиле док-станции для Xfce. Используя его на рабочем столе Xfce, вы получи-те "панель задач только значки" с поддержкой прикрепления приложений и груп-пировкой окон. ${NC}"
echo " Этот плагин панели Xfce является отличной альтернативой DockBarX, с меньшим количеством функций и настроек. "
echo -e "${YELLOW}==> Примечание! ${NC}Помимо того, что плагин позволяет запустить/закрыть окно приложения, минимизиро-вать в один клик на панели, он также может управлять параметрами открытых окон из значка. Используйте Ctrl, чтобы изменить порядок приложений или получить доступ к панели настроек (щелкнув правой кнопкой мыши)." 
