...
sudo pacman -S projectm-pulseaudio --noconfirm  # Музыкальный визуализатор (projectM PulseAudio Visualization), использующий ускоренный трехмерный рендеринг на основе итеративного изображения (pulseaudio) https://github.com/projectM-visualizer/projectm
clear
echo ""
...
sudo pacman -S zip unzip unrar p7zip --noconfirm  # Компрессор / архиватор для создания и изменения zip-файлов; Для извлечения и просмотра файлов в архивах .zip; Программа распаковки RAR; Файловый архиватор из командной строки с высокой степенью сжатия.
sudo pacman -S lzop --noconfirm  # Компрессор файлов с использованием lzo lib
sudo pacman -S zlib zziplib --noconfirm  # Библиотека сжатия, реализующая метод сжатия deflate, найденный в gzip и PKZIP; Легкая библиотека, которая предлагает возможность легко извлекать данные из файлов, заархивированных в один zip-файл.
echo ""
...
echo -e "${MAGENTA}=> ${NC}Список программ (пакетов) для установки: - (accountsservice, acpi, acpid, android-tools, android-udev, arch-install-scripts, aspell-en, aspell-ru, autofs, b43-fwcutter, bash-completion, beep, bind, btrfs-progs, busybox, c-ares, ccache, cpio, cpupower, crda, davfs2, desktop-file-utils, dhclient, dmraid, dmidecode, dnsmasq, dosfstools, efibootmgr, efitools, ethtool, f2fs-tools, flex, foremost, fortune-mod, fsarchiver, fwupd, fuse3, glances, gnome-nettool, gnu-netcat, gpm, gptfdisk, gtop, gvfs, gvfs-gphoto2, gvfs-nfs, gvfs-smb, haveged, hddtemp, hdparm, hidapi, hwdetect, hwinfo, hydra, hyphen-en, id3lib, iftop, inetutils, isomd5sum, jfsutils, lib32-curl, lib32-flex, libfm-gtk2, lksctp-tools, logrotate, lsof, lsb-release, lvm2, man-db, man-pages, mc, memtest86+, mlocate, mtpfs, ncdu, nfs-utils, nmon, nss-mdns, pacman-contrib, patchutils, pciutils, pcurses, php, polkit, poppler-data, powertop, pv, pwgen, python-isomd5sum, python-pip, qt5-translations, reiserfsprogs, ruby, s-nail, sane, scrot, sg3_utils, sdparm, sof-firmware, solid, sox, smartmontools, speedtest-cli, squashfs-tools, syslinux, systemd-ui, termite, termite-terminfo, translate-shell, udiskie, udisks2, usbutils, usb_modeswitch, wimlib, wipe, xdg-utils, xfsprogs, xsel, xterm, xorg-twm, xorg-xkill, yelp, yt-dlp,picom_X11,kvantum)."
...
sudo pacman -S foremost --noconfirm  # Консольная программа для восстановления файлов на основе их верхних и нижних колонтитулов и внутренних структур данных (http://foremost.sourceforge.net/)
...
# sudo pacman -S grub-btrfs --noconfirm  # Включите снимки btrfs в параметры загрузки GRUB
...
sudo pacman -S sdparm --noconfirm  # Утилита, аналогичная hdparm, но для устройств SCSI (http://sg.danny.cz/sg/sdparm.html)
...
sudo pacman -S wipe --noconfirm  # Утилита для безопасной очистки файлов (http://wipe.sourceforge.net/)
...
sudo pacman -S picom --noconfirm  # Легкий композитор для X11 https://archlinux.org/packages/extra/x86_64/picom/
sudo pacman -S kvantum --noconfirm  # Механизм тем на основе SVG для Qt6 (включая инструмент настройки и дополнительные темы) https://archlinux.org/packages/extra/x86_64/kvantum/
...
sudo pacman -S htop iotop --noconfirm  # интерактивный просмотрщик запущенных процессов; просмотр процессов ввода-вывода по использованию жесткого диска
...
sudo pacman -S smplayer smplayer-skins smplayer-themes --noconfirm  # Медиаплеер со встроенными кодеками, который может воспроизводить практически все видео и аудио форматы; Скины для SMPlayer; Темы для SMPlayer; Приложение, позволяющее просматривать, искать и воспроизводить видео на YouTube.
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
###
...
echo " Установка утилит (пакетов) пропущена "
  ...
  echo ""
  echo " Установка текстового редактора (gedit) "
sudo pacman -S leafpad --noconfirm  # Клон блокнота для GTK+ 2.0. https://archlinux.org/packages/extra/x86_64/leafpad/
echo ""
...
# sudo pacman -S broadcom-wl-dkms ebtables ipset iwd linux-atm ndisc6 networkmanager-l2tp pptpclient rp-pppoe wvdial xl2tpd --noconfirm
  ...
  sudo pacman -S ipset --noconfirm  # Инструмент администрирования наборов IP (Помечен как устаревший 5 июня 2024 г.)
...
# sudo pacman -S  --noconfirm  #
  ...
  sudo pacman -S dsniff --noconfirm  # Сборник инструментов для сетевого аудита и тестирования на проникновение (https://www.monkey.org/~dugsong/dsniff/) Помечен как устаревший 24 октября 2023 г.
  echo ""
  echo " Установка утилит (пакетов) выполнена "
fi
###########
...
##### timeshift ######
sudo pacman -S timeshift --noconfirm  # Утилита восстановления системы для Linux. https://archlinux.org/packages/extra/x86_64/timeshift/
echo ""
