...
echo -e "${MAGENTA}:: ${BOLD}Docklike Plugin - это современная минималистичная панель задач в стиле док-станции для Xfce. Используя его на рабочем столе Xfce, Вы получите "панель задач - только значки" с поддержкой прикрепления приложений и груп-пировкой окон. ${NC}"
