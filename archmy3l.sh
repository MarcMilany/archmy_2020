...
sudo pacman -S --noconfirm --needed wireplumber  # Реализация менеджера сеансов/политик для PipeWire ; https://archlinux.org/packages/extra/x86_64/wireplumber/ ; https://pipewire.pages.freedesktop.org/wireplumber/ ; Обеспечивает: pipewire-session-manager ; Конфликты: с pipewire-media-session ; Обратные конфликты: с pipewire-media-session ; 2025-05-22 05:55 UTC
sudo pacman -S --noconfirm --needed pipewire-session-manager  # Менеджер сеансов для PipeWire (поставщик по умолчанию) ; https://archlinux.org/packages/extra/x86_64/pipewire-session-manager/ ; https://pipewire.org/ ; 28 июня 2025 г. 02:23 UTC
...
echo " Установка утилит (пакетов) пропущена "
  ...
  echo ""
  echo " Установка Double Commander (Qt5) "
...
#echo 'Установить рекомендованные программы?'
# Install the recommended programs
echo -e "${MAGENTA}=> ${NC}Список утилит (пакетов) для установки: - (edk2-ovmf gnome-system-monitor, gnome-disk-utility, gpart, frei0r-plugins, fuseiso, clonezilla, crypto++, ddrescue, psensor, copyq, rsync, grsync, numlockx, modem-manager-gui, ranger, pacmanlogviewer, rofi, gsmartcontrol, testdisk, dmidecode, putty, w3m)."
echo -e "${CYAN}:: ${NC}Вы МОЖЕТЕ в скрипте закомментировать НЕнужные вам пакеты! (😃)"
echo " Будьте внимательны! Процесс установки, был прописан полностью автоматическим. "
# Be careful! The installation process was fully automatic
echo " Если Вы сомневаетесь в своих действиях, ещё раз обдумайте... "
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
    1 - *Да установить,     0 - НЕТ - Пропустить установку: " i_sniffnet  # sends right after the keypress; # отправляет сразу после нажатия клавиши
    echo ''
    [[ "$i_sniffnet" =~ [^10] ]]
do
    :
done
if [[ $i_sniffnet == 0 ]]; then
echo ""
echo " Установка утилит (пакетов) пропущена "
elif [[ $i_sniffnet == 1 ]]; then
  echo ""
  echo " Установка Sniffnet (sniffnet) "
  sudo pacman -Syy  # обновление баз пакмэна (pacman)
# sudo pacman -Syyu  # --noconfirm --needed  # -y – обновить списки пакетов из репозиториев ; -u – обновить пакеты ; Ключ -Syyu является наиболее часто используемой опцией и служит для обновления системы и всех установленных пакетов
############### sniffnet ###############
  sudo pacman -S --noconfirm --needed sniffnet  # Приложение для комфортного мониторинга сетевого трафика ; https://archlinux.org/packages/extra/x86_64/sniffnet/ ; https://github.com/GyulyVGC/sniffnet ; https://itshaman.ru/articles/1827/kak-kontrolirovat-internet-trafik-s-pomoshchyu-sniffnet-v-linux ; 28 июня 2025 г. 10:12 UTC
  echo ""
  echo " Установка утилит (пакетов) выполнена "
fi
########### Справка ############
...
sudo pacman -S --noconfirm --needed vlc-plugin-avahi  # Мультиплатформенный проигрыватель MPEG, VCD/DVD и DivX — плагин mDNS/DNS-SD ; https://archlinux.org/packages/extra/x86_64/vlc-plugin-avahi/ ; https://www.videolan.org/vlc/ ; 2025-07-08 20:26 UTC
############# Справка ####################
