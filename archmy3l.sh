...
echo -e "${MAGENTA}=> ${NC}Список утилит (пакетов) для установки: - (#ebtables, ipset ,iproute2, traceroute, nmap, vulscan, wavemon, dsniff, wvdial, broadcom-wl-dkms, linux-atm)."
echo -e "${CYAN}:: ${NC}Вы МОЖЕТЕ в скрипте закомментировать НЕнужные вам пакеты!"
echo " Будьте внимательны! Процесс установки, был прописан полностью автоматическим. "
# Be careful! The installation process was fully automatic
echo " Если Вы сомневаетесь в своих действиях, ещё раз обдумайте... "
...
echo -e "${CYAN}:: Wireshark — анализатор трафика (сниффер) для Linux, поддерживает более сотни сетевых протоколов. Анализ, учет и сбор трафика в режиме реального времени. Он доступен во всех основных настольных операционных системах, таких как Windows, Linux, macOS, BSD и других. "
...
echo -e "${CYAN}:: KDE Connect — Добавляет связь между KDE и вашим смартфоном. "
...
clear
echo -e "${CYAN}
  <<< Установка Сетевых утилит - Avahi, SSH, Samba и т.д. для системы Arch Linux >>> ${NC}"
##### SSH (client) ###
echo ""
...
pacman -S --noconfirm --needed --noprogressbar --quiet avahi  # Обнаружение служб для Linux с использованием mDNS/DNS-SD (совместимо с Bonjour) ; https://github.com/avahi/avahi ; https://archlinux.org/packages/extra/x86_64/avahi/
...
echo -e "${CYAN}:: Его цель - Реализовать доступ клиентских терминалов к папкам, принтерам и дискам про протоколу SMB/CIFS. ${NC}"
