...
echo -e "${MAGENTA}:: ${BOLD}Impression — это приложение для создания загрузочных дисков (записи образов) на Linux. Позволяет загружать ISO-образы операционных систем и записывать их на USB-накопители. ${NC}"
echo " Домашняя страница: https://apps.gnome.org/Impression/ ; (https://archlinux.org/packages/extra/x86_64/impression/). "
echo -e "${MAGENTA}:: ${BOLD}Impression - минималистичная программа для создания загрузочных носителей. Он позволяет с лёгкостью записать образ на диск. Нет никаких дополнительных или лишних функций. Только возможность загрузки образа дистрибутива Linux из списка из сети для последующей записи на накопитель. С легкостью записывайте образы на свои диски. Выберите образ, вставьте диск, и все готово! Impression — полезный инструмент как для заядлых любителей дистрибутивов, так и для обычных пользователей компьютеров. ${NC}"
echo " Linux предлагает гамму небольших утилит с открытым исходным кодом, которые выполняют функции от обыденных до замечательных. По нашему мнению, именно широта этих инструментов помогает сделать Linux привлекательной операционной системой. Impression — это бесплатное программное обеспечение с открытым исходным кодом. Информация о программе: Язык интерфейса: Русский ; Разработчик: Khaleel Al-Adhami ; Лицензия: GPL v3 ; Сайт программы: apps.gnome.org/ru/Impression . "
echo ""
while
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p "
...
echo " Установка утилит (пакетов) пропущена "
  ...
  echo ""
  echo " Установка Impression (impression) "
sudo pacman -Syy  # обновление баз пакмэна (pacman)
sudo pacman -S --noconfirm --needed impression  # Приложение для создания загрузочных дисков из образов дисков ; https://apps.gnome.org/Impression/ ; https://archlinux.org/packages/extra/x86_64/impression/ ; 2025-04-25 19:45 UTC
echo ""
echo " Установка утилит (пакетов) выполнена "
fi
########
