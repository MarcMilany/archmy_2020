...
echo -e "${MAGENTA}=> ${NC}Список программ (пакетов) для установки: - (accountsservice, acpi, acpid, android-tools, android-udev, arch-install-scripts, aspell-en, aspell-ru, autofs, b43-fwcutter, bash-completion, btrfs-progs, busybox, ccache, cmake, cpio, cpupower, crda, davfs2, desktop-file-utils, dhclient, dnsmasq, dosfstools, efibootmgr, ethtool, f2fs-tools, flex, fortune-mod, fsarchiver, fuse3, glances, gnome-nettool, gnu-netcat, gpm, gptfdisk, gtop, gvfs, gvfs-gphoto2, gvfs-nfs, gvfs-smb, haveged, hddtemp, hidapi, hydra, hyphen-en, id3lib, iftop, inetutils, isomd5sum, jfsutils, lib32-curl, lib32-flex, libfm-gtk2, lksctp-tools, logrotate, lsof, lsb-release, man-db, man-pages, mc, memtest86+, mlocate, mtpfs, ncdu, nfs-utils, nss-mdns, pacman-contrib, patchutils, pciutils, php, poppler-data, powertop, pv, pwgen, python-isomd5sum, python-pip, qt5-translations, reiserfsprogs, ruby, s-nail, sane, scrot, sox, smartmontools, speedtest-cli, syslinux, termite, termite-terminfo, translate-shell, udiskie, usb_modeswitch, xsel, xterm, xorg-twm, xorg-xkill, yelp, youtube-dl)."
...
echo " Установка дополнительных базовых программ (пакетов) пропущена "
  ...
  echo ""  
  echo " Установка дополнительных базовых программ (пакетов) "
# sudo pacman -S accountsservice acpi acpid android-tools android-udev arch-install-scripts aspell-en aspell-ru autofs b43-fwcutter bash-completion btrfs-progs busybox ccache cmake cpio cpupower crda davfs2 desktop-file-utils dhclient dnsmasq dosfstools efibootmgr ethtool f2fs-tools flex fortune-mod fsarchiver fuse3 glances gnome-nettool gnu-netcat gpm gptfdisk gtop gvfs gvfs-gphoto2 gvfs-nfs gvfs-smb haveged hddtemp hidapi hydra hyphen-en id3lib iftop inetutils isomd5sum jfsutils lib32-curl lib32-flex libfm-gtk2 lksctp-tools logrotate lsof man-db man-pages mc memtest86+ mlocate mtpfs ncdu nfs-utils nss-mdns patchutils pciutils php poppler-data powertop pv pwgen python-isomd5sum python-pip qt5-translations reiserfsprogs ruby s-nail sane scrot sox smartmontools speedtest-cli syslinux termite termite-terminfo translate-shell udiskie usb_modeswitch xsel xterm xorg-twm xorg-xkill yelp youtube-dl --noconfirm  # light-locker lsb-release python python2 - присутствует  
...
# sudo pacman -S gnome-system-monitor gnome-disk-utility gnome-multi-writer frei0r-plugins clonezilla psensor copyq rsync grsync numlockx modem-manager-gui rofi gsmartcontrol ranger testdisk lsof dmidecode qemu w3m --noconfirm  
