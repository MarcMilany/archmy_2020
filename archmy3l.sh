...
echo " 1 - Установка свежего списка зеркал со страницы Pacman Mirrorlist Generator Russia от (2021-03-09). "
...
echo " Действия ввода, выполняется сразу после нажатия клавиши "
    read -n1 -p " 
    1 - Установка свежего списка от (2021-03-09),     2 - Обновление зеркал с помощью (reflector) 
...
echo -e "${MAGENTA}:: ${BOLD}Docklike - это современная минималистичная панель задач в стиле док-станции для Xfce. Используя его на рабочем столе Xfce, вы получи-те "панель задач только значки" с поддержкой прикрепления приложений и груп-пировкой окон. ${NC}"
echo " Этот плагин панели Xfce является отличной альтернативой DockBarX, с меньшим количеством функций и настроек. "
echo -e "${YELLOW}==> Примечание! ${NC}Помимо того, что плагин позволяет запустить/закрыть окно приложения, минимизиро-вать в один клик на панели, он также может управлять параметрами открытых окон из значка." 
echo " Как отмечалось в начале статьи, плагин обладает небольшим количеством функций и настроек. "


echo -e "${CYAN}:: ${NC}Установка Docklike Plugin (xfce4-docklike-plugin-git) проходит через сборку из исходников. То есть установка производиться с помощью git clone, PKGBUILD, makepkg - скачивается с сайта 'Arch Linux' (https://aur.archlinux.org/packages/xfce4-docklike-plugin-git/) - собирается и устанавливается. "
echo " Будьте внимательны! Процесс установки, после выбранного вами варианта был прописан полностью автоматическим. В данной опции выбор всегда остаётся за вами. "
# Be careful! The installation process, after the option you selected, was registered fully automatic. In this option, the choice is always yours.
echo " Если Вы сомневаетесь в своих действиях, ещё раз обдумайте... "
# If you doubt your actions, think again...  
